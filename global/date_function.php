...
// fungsi untuk mengubah tanggal dari format MM/DD/YYYY ke format YYYY-DD-MM
  ...
  function getNextDateNextMonth($startDate,$duration = 1) {
    ...
    if (validStandardDate($startDate)) {
      ...
      if ($bulan == 0) $bulan = 12;
      if ($bulan == 12) $tahun = $tahun - 1;
      if ($tanggal > $arrDaysOfWeek[$bulan]) $tanggal = $arrDaysOfWeek[$bulan];
  ...
  function extractDate($strDate) {
    ...
    if (isset($arrTgl[1])) $arrResult['month'] = (int)$arrTgl[1];
    if (isset($arrTgl[2])) $arrResult['day'] = (int)$arrTgl[2];

    if (isset($arrTmp[1])) {
      // extrat jam
      $arrWkt = explode(":", $arrTmp[1]);
      $arrResult['hour'] = (int)$arrWkt[0];
      if (isset($arrWkt[1])) $arrResult['minute'] = (int)$arrWkt[1];
      if (isset($arrWkt[2])) $arrResult['second'] = (int)$arrWkt[2];
    }

    $arrResult['integer'] = @mktime($arrResult['hour'], $arrResult['minute'], $arrResult['second'], $arrResult['month'], $arrResult['day'], $arrResult['year']);
...
// ----------------------------------- BAGIAN WAKTU --------------------
  ...
  function getIntervalHour($tsAwal,$tsAkhir) {
    ...
    $intHasil = 0;
    if ($tsAkhir == "" || $tsAwal == "") return 0;
    //cek apakah awal lebih besar dari akhir, dianggap melewati tengah malam
    if (substr($tsAwal, 2, 1) == ":") list($jam,$menit) = explode(":",$tsAwal);
    else
    {
    ...
    if (substr($tsAkhir, 2, 1) == ":") list($jam,$menit) = explode(":",$tsAkhir);
    else
    {
  ...
  function minuteToTime($intMin, $bolEmpty = true) {
    ...
    {
      if ($bolEmpty && $intMin == 0) $strResult = "";
      else
      {
...
function lastday($month, $year) {
 ...
 function getDefaultSalaryPeriode(&$strStart, &$strFinish, $strCurrent = "", $isForSeverance = false)
 {
    ...
    $strStart = (is_numeric(getSetting("salary_date_from"))) ? date("Y-m-d", mktime(10,0,0, $dtNow['mon'], getSetting("salary_date_from"),date("Y"))) : date("Y")."-".date("m")."-01";
  ...
  // Fungsi merubah format tanggal dari template 30/07/12 menjadi 2012-07-30
  function timestampDate2Date($strTs){
    if (strlen($strTs) != 8) {
      return $strTs; // error, langsug dibalikin
    } else {
      //Tambahkan angka 20 pada tahun
      $strYears= substr($strTs,6,2);
      $strResult = "20".$strYears."-".substr($strTs,3,2)."-".substr($strTs,0,2);
      return $strResult;
    }
  }

  //Fungsi memishakan tanggal bila formatny menggunakan satu digit ex: 12/31/2021 (d/m/yyyy) jadikan ke format (yyyy-mm-dd)
  function timeStamp2SingleDate($date){
    //Pecah file berdasarkan garis miring
    $arrDateExplode=explode("/",$date);
    // Jika bulan dan tanggal satu digit maka tambhkan NOL
    if ((strlen($arrDateExplode[0]))==1) $arrDateExplode[0]='0'.$arrDateExplode[0];
    if ((strlen($arrDateExplode[1]))==1) $arrDateExplode[1]='0'.$arrDateExplode[1];

    $strResult = $arrDateExplode[2]."-".$arrDateExplode[1]."-".$arrDateExplode[0];
    return $strResult;

  }
  function getNextDateNextMonthNew($startDate = null,$duration = 1) {
      global $arrDaysOfWeek;
      $lastDate = null;
      if (!empty($startDate) && validStandardDate($startDate)){
          $lastDate = $startDate; //default
          list($tahun,$bulan,$tanggal) = explode("-",$startDate);
          $lastDate = date('Y-m-d', mktime(0,0,0,(int)$bulan + (int)$duration, (int)$tanggal, (int)$tahun));
      }
      return $lastDate;
  } // getNextDate
?>
