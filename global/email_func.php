  ...
  if(!empty($candidate))
  {
    foreach($candidate as $group => $id)
    {
      ...
      if(in_array($employee_branch, $arrDataBranch) || empty($arrDataBranch))
      {
        ...
        if(in_array($employee_management, $arrDataManagement) || empty($employee_management))
        {
          ...
          if(in_array($employee_division, $arrDataDivision) || empty($employee_division))
          {
            ...
            if(in_array($employee_department, $arrDataDepartment) || empty($employee_department))
            {
              ...
              if(in_array($employee_section, $arrDataSection) || empty($employee_section))
              {
                ...
                if(in_array($employee_sub_section, $arrDataSubSection) || empty($employee_sub_section))
...
function getEmployeeEmailById($id = null)
{
  global $db;
  $empEmail = null;
  if (!empty($id)){
 ...
 * @return send email
 */
function sendMail($subject,$body,$cc,$altBody)
{
  global $db;
  global $employeeID;
  global $type;
  ...
  $mail       = new PHPMailer;
  $sender     = 'admin.hrd'; // Sender Email Address

  //$mail->SMTPDebug = 3;                               // Enable verbose debug output
  $strGroupID = getDataEmail($type,$module_permission,$employeeID);
  $to = getEmailTo($strGroupID);
  $mail->isSMTP();                                      // Set mailer to use SMTP
  $mail->Host       = 'smtp.adyawinsa.com'; // Specify main and backup SMTP servers
  $mail->SMTPAuth   = true; // Enable SMTP authentication
  $mail->Username   = $sender; // SMTP username
  $mail->Password   = 'adminhrd201708'; // SMTP password
...
function sendDirectMail($subject,$body,$cc = null,$altBody,$to)
{
  global $db;
  global $employeeID;
  global $type;
  ...
  $mail       = new PHPMailer;
  $sender     = 'admin.hrd@adyawinsa.com'; // Sender Email Address
  ...
  $mail->SMTPAuth   = true; // Enable SMTP authentication
  $mail->Username   = $sender; // SMTP username
  $mail->Password   = 'adminhrd201610'; // SMTP password
  $mail->SMTPSecure = 'tls'; // Enable TLS encryption, `ssl` also accepted
  $mail->Port       = 587; // TCP port to connect to
