<?
  ...
  */
  ...
  include_once('../classes/hrd/hrd_sub_section.php');
//  $tblCompany = new cHrdCompany();
//  $arrCompany = $tblCompany->findAll("", "division_code, division_name", "", null, 1, "division_code");
  $tblSubSection = new cHrdSubSection();
  $arrSubSection = $tblSubSection->findAll("", "sub_section_code, sub_section_name", "", null, 1, "sub_section_code");
  ...
  function wrapRow($strText1 = "", $strText2 = "", $strValue = "", $bolNumber = false, $bolComment = false, $bolNowrap = true)
  {
  ...
  // fungsi untuk menampilkan baris per baris dalam kolom dalam formulir disposisi / slip
  function wrapRowBorderLeftRight($strText1 = "", $strText2 = "", $strValue = "", $bolNumber = false, $bolComment = false, $bolNowrap = true)
  {
    $strNowrap = ($bolNowrap) ? "nowrap" : "";
    $strColspan     = ($bolComment) ? "colspan=3" : "";
    $strAttribute   = ($bolNumber) ? "align=\"right\"" : "";
    $strResult      = "  <tr style = 'border-left-width : thin; border-left-color: black; border-left-style : solid; border-right-width : thin; border-right-color: black; border-right-style : solid;'><td ".$strColspan."  $strNowrap valign='top'>$strText1&nbsp;</td>";
    $strResult     .= (!$bolComment) ? "  <td  $strNowrap valign='top'>&nbsp;" .$strText2."</td>" : "";
    $strResult     .= (!$bolComment) ? " <td  $strNowrap ".$strAttribute.">&nbsp;" .$strValue."</td></tr>" : "";
    return $strResult;
  }
  ...
  function wrapRowSize($strText1 = "", $strText2 = "", $strValue = "",  $width = 1, $bolRight = false, $bolNumber = false, $bolComment = false)
  {
    $strColspan     = ($bolComment) ? "colspan=3" : "";
    $strAttribute   = ($bolNumber) ? "align=\"right\"" : "";
  ...
  function formatDate($params)
  {
    if(!is_numeric($params) && !is_string($params) && $params != "")
      ...
      $value = $params;
    return pgDateFormat($value, "Y-m-d");
  }
  function formatTime($params)
  {
    if(!is_numeric($params) && !is_string($params) && $params != "")
      ...
      $value = $params;    return minuteToTime($value, true);
  }
  ...
  function printCompanyName($params)
  {
    ...
    if ($value != "") $strSQL .= "WHERE id = '$value'";
    $resDb = $db->execute($strSQL);
    if ($rowDb = $db->fetchrow($resDb))
  ...
  function getSubSectionName($params)
  {
    ...
    if(!is_string($params))
      ...
      $value = $params;
    $strCode = $value;
    return (isset($arrSubSection[$strCode])) ? $arrSubSection[$strCode]['sub_section_name'] : $strCode;
  }
...
function getFunctionalName($params)
  {
  ...
  function getBranchName($params)
  {
    ...
    if(!is_string($params))
      ...
      $value = $params;
    $strCode = $value;
    return (isset($arrBranch[$strCode])) ? $arrBranch[$strCode] : $strCode;
  }
  ...
  function printEmployeeStatus2($params)
  {
    extract($params);
    global $ARRAY_EMPLOYEE_STATUS;
    return getWords($ARRAY_EMPLOYEE_STATUS[$params]);
  }

  function printYesNo($params)
  {
    extract($params);
    if ($params == 1)
      return getWords("Yes");
  ...
  function printLoanCategory($params)
  {
    extract($params);
    global $ARRAY_LOAN_CATEGORY;
    return getWords($ARRAY_LOAN_CATEGORY[$params]);
  }
  ...
  function printGlobalEditLink($params)
  {
    ...
    if (!isset($record['status'])) $record['status'] = REQUEST_STATUS_NEW;
	if (!isset($record['baseFilename'])) $baseFilename = getEditFile( basename($_SERVER['PHP_SELF']));
	else $baseFilename = $record['baseFilename'];
    return ($record['status'] != REQUEST_STATUS_NEW && $record['status'] != REQUEST_STATUS_CHECKED) ? "<font color='lightgrey'><strike>edit</strike></font>" : "<a href=\"".$baseFilename."?dataID=".$record['id']."\">" .getWords('edit'). "</a>";
  }
  ...
  function printGlobalEditLinkOT($params)
  {
    extract($params);
    if (!isset($record['status'])) $record['status'] = REQUEST_STATUS_NEW;
    return  "<a href=\"".getEditFile( basename($_SERVER['PHP_SELF']))."?dataID=".$record['id']."\">" .getWords('edit'). "</a>";
  }
