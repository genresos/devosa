<?
  ...
  */
  ...
  function getEmployeeIsAllDay($db, $idEmployee) {
    $strResult = "";
    $strSQL = "SELECT is_all_day FROM hrd_employee as te LEFT JOIN hrd_position as tp ON te.position_code = tp.position_code
                WHERE te.id = $idEmployee";
    $resDb = $db->execute($strSQL);
    if ($rowDb = $db->fetchrow($resDb)) {
      $strResult = $rowDb['is_all_day'];
    }
    return $strResult;

  }

  function getEmployeeIsSatIn($db, $idEmployee) {
    $strResult = "";
    $strSQL = "SELECT is_sat_in FROM hrd_employee as te LEFT JOIN hrd_position as tp ON te.position_code = tp.position_code
                WHERE te.id = $idEmployee";
    $resDb = $db->execute($strSQL);
    if ($rowDb = $db->fetchrow($resDb)) {
      $strResult = $rowDb['is_sat_in'];
    }
    return $strResult;
  ...
  function getEmployeeManagerID($db, $strIDEmployee = "", $strEmployeeID = "")
  {
    ...
    if ($strIDEmployee === "" && $strEmployeeID === "") return $intResult;
    ...
    if ($rowDb = $db->fetchrow($resDb))
    {
      ...
      if ($rowDb['position_code'] == $strDH)
        // dept head, dirinya sendiri
        $intResult = $rowDb['id'];
  ...
  function updateEmployeeCareerData($db, $strDataID, $strIDEmployee) {
    ...
    if ($strIDEmployee != "") {
      ...
      if ($rowDb = $db->fetchrow($resDb)) {
        ...
        $strContractDate = $rowDb['contract_from'];
        ...
        $strFunctional = $rowDb['functional_code'];
				$strBranchPenugasan = $rowDb['branch_penugasan_code'];
        $strBranchContract  = $rowDb['branch_code'];
        $strCostCenter = $rowDb['branch_cost_center_code'];
        ...
        $strSQL  = "SELECT status_new, status_old, status_date_from, status_date_thru FROM hrd_employee_mutation_status AS t1
                    ...
                    WHERE id_mutation = '$strDataID' AND status >= " .REQUEST_STATUS_APPROVED_2." ";
        ...
        if ($rowTmp = $db->fetchrow($resTmp)) {
          ...
          {
            $strContractDate = $rowTmp['status_date_from'];
...
//        }
        ...
        $resTmp = $db->execute($strSQL);
        if ($rowTmp = $db->fetchrow($resTmp)) {
        	if (!empty($rowTmp['position_new'])){ 
	          $strPosition = $rowTmp['position_new'];
	        }
	        if (!empty($rowTmp['grade_new'])){
	          $strGrade = $rowTmp['grade_new'];
	        }
	        if (!empty($rowTmp['functional_new'])){
	          $strFunctional = $rowTmp['functional_new'];
	        }
        }
        ...
        $resTmp = $db->execute($strSQL);
        if ($rowTmp = $db->fetchrow($resTmp)) {
        	if (!empty($rowTmp['company_new'])){
          	$strCompany = $rowTmp['company_new'];
          }
          if (!empty($rowTmp['management_new'])){
          	$strManagement = $rowTmp['management_new'];
          }
          if (!empty($rowTmp['division_new'])){
          	$strDivision = $rowTmp['division_new'];
          }
          if (!empty($rowTmp['department_new'])){
          	$strDepartment = $rowTmp['department_new'];
          }
          if (!empty($rowTmp['section_new'])){
          	$strSection = $rowTmp['section_new'];
          }
          if (!empty($rowTmp['sub_section_new'])){
          	$strSubSection = $rowTmp['sub_section_new'];
          }
        }

        $strSQL  = "SELECT t1.* FROM hrd_employee_mutation_id AS t1
                    LEFT JOIN hrd_employee_mutation AS t2 ON t2.id = t1.id_mutation
                    WHERE id_mutation = '$strDataID' AND status >= " .REQUEST_STATUS_APPROVED_2." ";
        $resTmp = $db->execute($strSQL);
        if ($rowTmp = $db->fetchrow($resTmp)) {
        	if (!empty($rowTmp['id_employee_new'])){
          	$strNewNIK = $rowTmp['id_employee_new'];
          }
        }
        $strSQL  = "SELECT t1.* FROM hrd_employee_mutation_branch AS t1
                    LEFT JOIN hrd_employee_mutation AS t2 ON t2.id = t1.id_mutation
                    WHERE id_mutation = '$strDataID' AND status >= " .REQUEST_STATUS_APPROVED_2." ";
        $resTmp = $db->execute($strSQL);
        if ($rowTmp = $db->fetchrow($resTmp)) {
        	if (!empty($rowTmp['branch_penugasan_new'])){
          	$strBranchPenugasan = $rowTmp['branch_penugasan_new'];
          }
          if (!empty($rowTmp['branch_contract_new'])){
          	$strBranchContract  = $rowTmp['branch_contract_new'];
          }
        }
        $strSQL  = "SELECT t1.* FROM hrd_employee_mutation_cost_center AS t1
                    LEFT JOIN hrd_employee_mutation AS t2 ON t2.id = t1.id_mutation
                    WHERE id_mutation = '$strDataID' AND status >= " .REQUEST_STATUS_APPROVED_2." ";
        $resTmp = $db->execute($strSQL);
        if ($rowTmp = $db->fetchrow($resTmp)) {
        	if (!empty($rowTmp['cost_center_new'])){
          	$strCostCenter = $rowTmp['cost_center_new'];
          }
        }

        // falidasikan tanggal
        //$strJoinDate = ($strJoinDate == "") ? "NULL" : "'$strJoinDate'";
        $strContractDate = ($strContractDate == "") ? "NULL" : "'$strContractDate'";
        ...
        if (!is_numeric($strStatus)) $strStatus = $rowDb['employee_status'];

        // update data employee
        $strSQL  = "UPDATE hrd_employee SET ";
        $strSQL .= "due_date = $strDueDate, contract_from = $strContractDate, active = $strActive, ";
        $strSQL .= "permanent_date = $strPermanentDate, resign_date = $strResignDate, ";
        $strSQL .= "employee_status = '$strStatus', position_code = '$strPosition', functional_code = '$strFunctional', ";
        ...
        $strSQL .= "id_company = '$strCompany', branch_penugasan_code = '$strBranchPenugasan', branch_code = '$strBranchContract', branch_cost_center_code = '$strCostCenter',";
        ...
        // Ambil Salary Set terbaru terkait company id employee
        // Auto get salary set di non aktifkan, user set secara manual
        /*
        $strSQL = "SELECT id_company FROM hrd_employee WHERE id='$strIDEmployee'";  
			  $resDb = $db->execute($strSQL);
				$rowDb = $db->fetchrow($resDb);
				$strIDCompany = $rowDb['id_company'];
				$strSQL1 ="SELECT id FROM hrd_basic_salary_set WHERE id_company='$strIDCompany' ORDER BY start_date DESC LIMIT 1";
				$res1 = $db->execute($strSQL1);
				$row1 = $db->fetchrow($res1);
				$setID = $row1['id'];
				*/
        $strSQL = "SELECT id_salary_set,allowance_type_id,allowance_type_code,old_value,new_value,id_salary_set_old FROM ";
        $strSQL .= "hrd_employee_mutation_salary t1 LEFT JOIN hrd_employee_mutation t2 ON ";
        $strSQL .= "t1.id_mutation = t2.id WHERE t1.id_mutation = '$strDataID' AND ";
        $strSQL .= "t2.status >= " .REQUEST_STATUS_APPROVED_2." ";
        $resDb = $db->execute($strSQL);
        $idSalarySetOld = 0;
        $strUpdate = "";
        while ($rowDb = $db->fetchrow($resDb)){
        	$setID = $rowDb['id_salary_set'];
        	if (isset($rowDb['allowance_type_code'])/* && $rowDb['new_value'] >= 0*/ && !empty($setID)){
        		$strUpdate .= "DELETE FROM hrd_employee_allowance WHERE id_employee = '$strIDEmployee' AND allowance_code = '".$rowDb['allowance_type_code']."';";	
        		$strUpdate .= "INSERT INTO hrd_employee_allowance (created, id_employee, allowance_code, amount, id_salary_set) ";
            $strUpdate .= "VALUES(now(),'$strIDEmployee','".$rowDb['allowance_type_code']."','".$rowDb['new_value']."', '".$setID."');";
            $db->execute($strUpdate);
            $idSalarySetOld = $rowDb['id_salary_set_old'];
        	}
        }
        if (!empty($strUpdate) && !empty($idSalarySetOld) && $idSalarySetOld != $setID){
	        $sqlInsertEmpBasicSalary = "DELETE FROM hrd_employee_basic_salary WHERE id_employee = '$strIDEmployee' AND id_salary_set='$idSalarySetOld';";
	        $sqlInsertEmpBasicSalary .= "INSERT INTO hrd_employee_basic_salary (id_employee, created_by, id_salary_set)";
			  	$sqlInsertEmpBasicSalary .= " VALUES ('$strIDEmployee','".$_SESSION['sessionUserID']."','$setID');";
  ...
  function saveEmployeeLeaveHistory($db, $strIDEmployee, $intYear = "", $strJoinDate = "", $strEmpStatus = 1) {
    ...
    if ($strIDEmployee === "") return $arrResult;

    if ($intYear === "" || !is_numeric($intYear)) $intYear = date("Y");

    if ($strJoinDate == "") { // cari joinDate jika tidak disertakan
      $arrTmp = getEmployeeInfoByID($db, $strIDEmployee, "join_date, employee_id, EXTRACT(year FROM AGE('$strDate', join_date)) AS kerja ");
  ...
  function getEmployeeLeaveQuota($db, $strIDEmployee, $strDate = "") {
    ...
    if ($strIDEmployee == "") return $arrResult;
    if ($strDate == "") $strDate = date("Y-m-d"); // default

    $dtNow = getdate();

    // cari data employee
    $arrEmp = getEmployeeInfoByID($db, $strIDEmployee, "join_date, employee_status, employee_id, EXTRACT(year FROM AGE('$strDate', join_date)) AS kerja ");
    if (!isset($arrEmp['join_date'])) return $arrResult; // gak ketemu datanya
    if ($arrEmp['join_date'] == "") return $arrResult; // gak ada data

    if ($arrEmp['kerja'] == 0) return $arrResult;
    else {
      ...
      if (!$bolPrev) {
        ...
        $arrLeave = saveEmployeeLeaveHistory($db, $strIDEmployee, $intYearPrev, $arrEmp['join_date'], $arrEmp['employee_status']);
      ...
      if (!$bolCurr) {
        ...
        $arrLeave = saveEmployeeLeaveHistory($db, $strIDEmployee, $intYear, $arrEmp['join_date'], $arrEmp['employee_status']);
  ...
  function getEmployeeLeaveAnnualByYear($db, $strYear, $strSection = "", $strIDEmployee = "") {
    ...
    $strSQL  = "SELECT id_employee, leave_duration FROM hrd_absence AS t1 LEFT JOIN hrd_employee AS t2 ON t1.id_employee = t2.id
                WHERE t1.status >= '" .REQUEST_STATUS_APPROVED_2. "' ";
    if ($strSection != "") {
  ...
  function getEmployeeLeaveRemain($db, $strIDEmployee) {
    ...
    $intResult = 0;
    if ($strIDEmployee === "") return $intResult;
  ...
  function getEmployeeMedicalQuota($db, $strIDEmployee = "", $strEmployeeCompany = "", $strYear = "", $strClaimID = "")
  {
    ...
    if ($strClaimID != "") $strSQL .= "AND t1.id_master <> '$strClaimID' ";
    ...
    if (!isset($fltClaim)) $fltClaim = 0;
    if (!isset($fltQuota)) $fltQuota = 0;
    ...
    foreach($arrMedicalType AS $strType => $arrDetail)
    {
      ...
      {
        if (!isset($arrMedicalClaim[$strType][$strCode])) $arrMedicalClaim[$strType][$strCode] = 0;
...
function getTimeDiff($db, $strIDEmployee)
{
  include_once("../classes/hrd/hrd_branch.php");
    ...
    $arrResult = $tblBranch->find(array("branch_code" => $arrEmp['branch_code']), "local_time_difference", "", null, 1, "id");
