...
*/
  ...
  function getData($db, &$intRows, $strKriteria = "", $strOrder = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= "  <td align=right><input type=hidden name=detailGrade$intRows value=\"" .(float)$rowDb['grade_allowance']. "\" disabled>" .standardFormat($rowDb['grade_allowance']). "&nbsp;</td>";
      $strResult .= "  <td align=right><input type=hidden name=detailTransport$intRows value=\"" .(float)$rowDb['transport_allowance']. "\" disabled>" .standardFormat($rowDb['transport_allowance']). "&nbsp;</td>";
      $strResult .= "  <td align=right><input type=hidden name=detailMeal$intRows value=\"" .(float)$rowDb['meal_allowance']. "\" disabled>" .standardFormat($rowDb['meal_allowance']). "&nbsp;</td>";
      $strResult .= "  <td align=right><input type=hidden name=detailVehicle$intRows value=\"" .(float)$rowDb['vehicle_allowance']. "\" disabled>" .standardFormat($rowDb['vehicle_allowance']). "&nbsp;</td>";
  ...
  function saveData($db, &$strError) {
    ...
    (isset($_REQUEST['dataGrade']) && is_numeric($_REQUEST['dataGrade'])) ? $strDataGrade = $_REQUEST['dataGrade'] : $strDataGrade = "0";
    (isset($_REQUEST['dataTransport']) && is_numeric($_REQUEST['dataTransport'])) ? $strDataTransport = $_REQUEST['dataTransport'] : $strDataTransport = "0";
    (isset($_REQUEST['dataMeal']) && is_numeric($_REQUEST['dataMeal'])) ? $strDataMeal = $_REQUEST['dataMeal'] : $strDataMeal = "0";
    (isset($_REQUEST['dataVehicle']) && is_numeric($_REQUEST['dataVehicle'])) ? $strDataVehicle = $_REQUEST['dataVehicle'] : $strDataVehicle = "0";
