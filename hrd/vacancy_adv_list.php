...
$f->addInput(getWords("date to"), "date_thru", date("Y-m-d"), [], "date", true, true, true);
...
$myDataGrid->addColumn(
    new DataGrid_Column((getWords('note')), "note", [], [], false, false, "", "", "string", true, 12)
);
if (!($bolPrint || $bolExcel)) {
    if ($bolCanEdit) {
        $myDataGrid->addColumn(
...
if ($f->getValue('date_from') != '') {
    $strCriteria .= " AND advertisement_date >= '" . $f->getValue('date_from') . "' ";
    $strCriteriaC .= " AND job_reference_date >= '" . $f->getValue('date_from') . "' ";
}
if ($f->getValue('date_thru') != '') {
    $strCriteria .= " AND advertisement_date <= '" . $f->getValue('date_thru') . "' ";
    $strCriteriaC .= " AND job_reference_date <= '" . $f->getValue('date_thru') . "' ";
}
...
$strSQLE = 'SELECT
            ...
            FROM
                hrd_candidate
            WHERE
                NOT (job_reference_date IS NULL) ' . $strCriteriaC;
...
$strSql = 'SELECT
                jba."id",
                jba.advertisement_date,
                jba.ref_type,
                jba.reference,
                jba.due_date,
                jbd.id_advertisement,
                jbd.position_name,
                jbd.note
            FROM
                "public".hrd_job_advertisement AS jba
            INNER JOIN "public".hrd_job_advertisement_detail AS jbd ON jba."id" = jbd.id_advertisement
            WHERE  
                1 = 1  ' . $strCriteria;
$countSql = 'SELECT
                    "count" (*) AS total
...
$myDataGrid->totalData = $myDataGrid->getTotalData($db, $countSql);
