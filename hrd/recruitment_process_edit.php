<?php
  ...
  $dataPrivilege = getDataPrivileges("recruitment_process_list.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove);
  ...
  $arrInterviewType = array(); // mencatat jenis-jenis proses rekrutmen yang merupakan interview
  $arrTestDriverType = array(); // mencatat jenis-jenis test drive
  //----------------------------------------------------------------------
  ...
  function getRecruitmentProcessList($strName, $strDefault = "", $strAction = "")
  {
    ...
    $strResult = "<select name='$strName' id='$strName' $strAction>";
  ...
  function getRecruitmentProcessDetail($intRows, $arrInfo, $strDataID)
  {
    ...
    $strDefaultSubject = ($intRows == 1 && count($arrInfo) == 0 && count($arrTmp = array_values($arrProcessType)) > 0) ? $arrTmp[0] : "";
    $strLetterButton = "";
    if (count($arrInfo) == 0)
    {
      $strProcess   = "";
    ...
    else
    {
      ...
      if (isset($arrType[$strProcess])) $type = $arrType[$strProcess]; 
    ...
    $strDataDetail .= "
      <tr id='detailProcess$intRows' $strStyle>
        <td>
          <fieldset><legend><b>Process #$intRows</b></legend>
          <table width=100% cellspacing=0 cellpadding=0>
            <tr valign=top>
              <td width='120' >&nbsp;<strong>" .getWords('process'). "
                <input type=hidden name=detailID$intRows value=\"$strProcessID\">
                </strong></td>
              <td width='10' >&nbsp;:</td>
              <td width='320' >&nbsp;" .getRecruitmentProcessList("detailProcessName$intRows", $strProcess,"onChange=\"document.getElementById('detailSubject$intRows').value = this.value;\" ")."</td>
              <td width='10' >&nbsp;</td>
              <td width='120' >&nbsp;<strong>" .getWords('subject'). "</strong></td>
              <td width='10' >&nbsp;:</td>
              <td><input type=text size=50 maxlength=100 name='detailSubject$intRows' id='detailSubject$intRows' style=\"width:120px\" value=\"$strSubject\"></td>
            </tr>
            <tr valign=top>
              <td>&nbsp;<strong>" .getWords('schedule date'). "</strong></td>
              <td>&nbsp;:</td>
              <td>&nbsp;<input type=text name=detailSchedule$intRows id=detailSchedule$intRows size=15 maxlength=10 value=\"$strSchedule\">
                <input type=button name=btnSchedule$intRows id=btnSchedule$intRows value='..'>
              </td>
              <td>&nbsp;</td>
              <td>&nbsp;<strong>" .getWords('place'). "</strong></td>
              <td>&nbsp;:</td>
              <td><input type=text size=30 maxlength=50 name=detailPlace$intRows style=\"width:120px\" value=\"$strPlace\"></td>
            </tr>
            <tr valign=top>
              <td>&nbsp;<strong>" .getWords('process date'). "</strong></td>
              <td>&nbsp;:</td>
              <td>&nbsp;<input type=text name=detailDate$intRows id=detailDate$intRows size=15 maxlength=10 value=\"$strDate\">
                <input type=button name=btnDate$intRows id=btnDate$intRows value='..'>
              <td>&nbsp;</td>
              <td>&nbsp;<strong>" .getWords('PIC'). "</strong></td>
              <td>&nbsp;:</td>
              <td><input type=text size=30 maxlength=50 name=detailPIC$intRows style=\"width:120px\" value=\"$strPIC\"></td>
            </tr>
            <tr valign=top>
              <td nowrap>&nbsp;<strong>" .getWords('result'). "</strong></td>
              <td>&nbsp;:</td>
              <td>&nbsp;" .getRecruitmentResultList("detailResult$intRows",$strResult,""," style=\"width:120px\" onChange=\"document.getElementByID('dataResult').value = this.value;\"")."
                <br /> $strLetterButton
              </td>
              <td>&nbsp;</td>
              <td nowrap>&nbsp;<strong>" .getWords('note'). "</strong></td>
              <td>&nbsp;:</td>
              <td><textarea id='detailNote$intRows' name='detailNote$intRows' style='width:250px' rows='3'>$strNote</textarea></td>
            </tr>
          </table></fieldset>
        </td>
      </tr>
  ...
  function getData($db, &$arrData, $strDataID = "", $strDataCandidateID = "")
  {
    ...
    else if ($strDataCandidateID != "")
    { // belum ada proses, ambil dta kandidat aja
...
*/
    ...
    {
      getRecruitmentProcessDetail($i, ((isset($arrDetail[$i])) ? $arrDetail[$i] : array()), $strDataID);
    }
    // footer
    ...
    $strDataDetail .= "
      <tr valign=top>
        <td>&nbsp;
          <input type='button' name='btnAdd' id='btnAdd' value=\"".getWords('add more process'). "\" onClick='showMore()'>
...
*/
    ...
    if (isset($arrLetter['x']))
    {
      foreach ($arrLetter['x'] AS $x => $arrDb) {
        $strNo  = $arrDb['no'] ."/". $arrDb['code'] ."/". $arrDb['letter_code'] ."/". $arrDb['year_code'];
        if ($strInputLetter != "") $strInputLetter .= "<br>";
  ...
  function getDataLetters($db, $strDataID)
  {
    ...
    while ($rowDb = $db->fetchrow($resDb))
    {
      ...
      {
        if (isset($arrResult[$rowDb['idProcessDetail']])) $arrResult[$rowDb['idProcessDetail']][] = $rowDb;
        else $arrResult[$rowDb['idProcessDetail']][0] = $rowDb;
      } else {
        if (isset($arrResult['x'])) $arrResult['x'][] = $rowDb;
        else $arrResult['x'][0] = $rowDb;
      }
    }
  ...
  function saveData($db, &$strDataID, &$strError)
  {
    ...
    if ($strDataID == "")
    {
      $data = array("id_candidate" => $strDataCandidateID,
    ...
    else
    {
      $data = array("id_candidate" => $strDataCandidateID,
                    ...
                    "note" => $strDataNote,
                    "id_recruitment_need" => $intIDRecruitmentNeed);
      if ($tbl->update(array("id" => $strDataID), $data))
      {
    ...
    if ($strDataID != "")
    {
      ...
      for ($i = 1; $i <= $intTotal;$i++)
      {
        ...
        if ($strID == "")
        { // baru
          if ($data['process_name'] != "") $tblDetail->insert($data);
        }
        else
        {
          if ($data['process_name'] == "")
            $tblDetail->delete(array("id" => $strID), $data);
          else
            $tblDetail->update(array("id" => $strID), $data);
        }
      }
  ...
  if ($db->connect())
  {
    ...
    $strInputInvitationDate  = "<input type=text name=dataInvitationDate id=dataInvitationDate size=15 maxlength=10 value=\"" .$arrData['invitation_date']. "\" >";
    ...
    $strInputInvitationMethod = getComboFromArray($arrInvitationMethod, "dataInvitationMethod", $arrData['invitation_method']);
    ...
    $strInputResult = getRecruitmentResultList("dataResult",$arrData['result'],""," style=\"width:$intDefaultWidthPx\" onChange=\"onResultChange()\" id=\"dataResult\" ");
