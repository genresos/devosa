<?php
  ...
  $dataPrivilege = getDataPrivileges("training_request_edit.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove);
  ...
  ($bolPrint) ? $strMainTemplate = getTemplate("training_evaluation_edit_print.html") : $strTemplateFile = getTemplate("training_evaluation_edit.html");
...
//	  echo $strSQL; die();
    ...
    while ($rowDb = $db->fetchrow($resDb))
    {
      ...
      $int1++;
      $strAnswer = ($bolReadonly) ? $rowDb['answer']."&nbsp;" : getPointList($arrPoint, "detailAnswer$int1", $rowDb['answer'], $rowDb['question_type']);
    ...
    if ($intRows == 0)
    {
      ...
      while ($rowDb = $db->fetchrow($resDb))
      {
        ...
        $int1++;
        $strAnswer = ($bolReadonly) ? "&nbsp;" : getPointList($arrPoint, "detailAnswer$int1", null, $rowDb['question_type']);
      ...
      $GLOBALS['strBtnPrint'] .= "<button type=submit name=btnPrint onClick=\"document.formInput.target='_blank'\">" .getWords("print")."</button>";
    }
  ...
  function getPointList($arr, $strName, $strDefault = "", $intQT)
  {
    ...
    foreach ($arr AS $id => $arrValue) {
     if ($strDefault == $arrValue['note']) {
      ...
      if ($intQT == $arrValue['type'])$strResult .= "<option value='" .$arrValue['note']."' $strSelected>" .$arrValue['note']."</option>\n";
    }
...
// echo $strResult;
  ...
  function saveData($db, &$strError) {
    ...
    if ($strRequestID === "" || $strEmployeeID === "") return false;
...
//		echo $strSQL;die();
    ...
    if ($intQuestion > 0) $fltPoint = ($fltPoint/$intQuestion);
  ...
  if ($db->connect()) {
    ...
    else
    {
      ...
      if ($rowDb = $db->fetchrow($resDb)) {
        ...
        $arrData['dataDate'] = pgDateFormat($rowDb['training_date'], "d-M-y") ."  -- ".pgDateFormat($rowDb['training_date_thru'], "d-M-y");
        ...
        if (isMe($rowDb['participant'])) $bolReadonly = false;
    ...
    if ($bolCanView) {
      if (($_SESSION['sessionUserRole'] == ROLE_SUPER || $_SESSION['sessionUserRole'] == ROLE_ADMIN) && $bolPrint) $bolReadonly = false;

      getData($db, $strDataID, $strRequestID, $arrData['dataIdEmployee'], $bolReadonly);

      if (!$bolReadonly)
        $strButtons .= "<button type='submit' name='btnSave' onClick=\"document.formInput.target = ''\">" .getWords("save")."</button> ";
