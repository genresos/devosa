<?php
  ...
  $dataPrivilege = getDataPrivileges("data_department_tree.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  ...
  function getData($db, &$intRows, $strKriteria = "", $strOrder = "") {
    ...
    //$strSQL .= "WHERE management_code like '".printCompanyCode($_SESSION['sessionCAR'])."%' ";
...
//add 28-11-2012 by adnan
    ...
    if($companyID == 3)
    {
	$strSQL .= "WHERE management_code NOT LIKE '".getCompanyCode()."%' $strKriteria ORDER BY $strOrder management_code ";
   }
...
else
{
  ...
  function getDataTree($db)
  {
    ...
    $arrMan = $tblMan->findAll("management_code LIKE '".printCompanyCode($_SESSION['sessionCAR'])."%'", null, "management_code");
    ...
    foreach ($arrMan as $rowMan)
    {
      ...
      $management[$rowMan['management_code']] = $myTreeview->addNode(
                                              new TreeNode(
                                                ...
                                                printLink($rowMan['management_code'].'-'.$rowMan['management_name'],
      ...
      foreach ($arrDiv as $rowDiv)
      {
        ...
        $division[$rowDiv['division_code']] = $myTreeview->addNode(
                                                new TreeNode(
                                                  ...
                                                  printLink($rowDiv['division_code'].'-'.$rowDiv['division_name'],
                                                    $rowDiv['management_code'],
                                                    $rowDiv['division_code']),
                                                  true), $management[$rowDiv['management_code']]);
        //find department
        ...
        foreach ($arrDept as $rowDb)
        {
          $dept[$rowDb['department_code']] = $myTreeview->addNode(
                                              new TreeNode(
                                                ...
                                                printLink($rowDb['department_code'].'-'.$rowDb['department_name'],
                                                  $rowDb['management_code'],
                                                  $rowDb['division_code'],
                                                  $rowDb['department_code']),
                                                true), $division[$rowDb['division_code']]);
          // cari data section
          ...
          foreach ($arrSect as $rowSec)
          {
            $section[$rowSec['section_code']] = $myTreeview->addNode(
                                                  new TreeNode(
                                                    ...
                                                    printLink($rowSec['section_code'].'-'.$rowSec['section_name'],
                                                      $rowSec['management_code'],
                                                      $rowSec['division_code'],
                                                      $rowSec['department_code'],
                                                      $rowSec['section_code']),
                                                    true), $dept[$rowSec['department_code']]);
            $arrSubSect = $tblSubSect->findAllByDepartmentCodeAndSectionCode($rowDb['department_code'], $rowSec['section_code'], null, "sub_section_code");
            foreach ($arrSubSect as $rowSub)
              $myTreeview->addNode(
                new TreeNode(
                  $rowSub['sub_section_code'].'-'.$rowSub['sub_section_name'],
                  printLink($rowSub['sub_section_code'].'-'.$rowSub['sub_section_name'],
                  $rowSub['management_code'],
                  $rowSub['division_code'],
                  $rowSub['department_code'],
                  $rowSub['section_code'],
                  $rowSub['sub_section_code']),
                true), $section[$rowSub['section_code']]);
          } // end cari data section
        }
      }
    }
  ...
  function getDataGrid($myDataGrid, $db)
  {
    ...
    // $myDataGrid->addColumn(new DataGrid_Column(getWords("ID employee"), "id", array('width' => 100), array('align' => 'center')));
    $myDataGrid->addColumn(new DataGrid_Column(getwords("n i k"), "employee_id", array('width' => 100), array('align' => 'center')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("name"), "employee_name", null, array('nowrap' => '')));
   // $myDataGrid->addColumn(new DataGrid_Column("Band", "grade_code", array('width' => 30), array('align' => 'center')));
    ...
    if ($strCriteria == "") $strCriteria = " 1 = 1";
  ...
  else
  {
    ...
    if ($strMessages != "") $strClass = "style=\"width:$strDateWidth%\" class=\"bgOK\" ";
