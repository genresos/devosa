<?php
    ...
    while ($row = $db->fetchrow($res))
    {
      ...
      $strResult .= "
        <tr>
          ...
          <td align=center nowrap><input type=button name='btnShowDetail$intRow' id='btnShowDetail$intRow' value='" .getWords('show detail')."' onclick=\"goViewDetail('" .$row['employee_id']."')\"></td>
        </tr>
      ";
    }
  ...
  function getDataDetail($db, $strDataDateFrom, $strDataDateThru, &$intRows, $strKriteria = "", $strOrder = "")
  {
    ...
    while ($row = $db->fetchrow($res))
    {
      ...
      $strDueDate = genTextDate("DueDate".$intRow, $row['dayoff_duedate'], (($bolIsEmployee) ? "readonly" : ""), (($bolIsEmployee) ? "disabled" : ""));//form_object.php
      $strBtn = ($row['dayoff'] == 1 && !$bolIsEmployee) ? "<input type=button name='btnShowDetail$intRow' id='btnShowDetail$intRow' value='" .getWords('change due date')."' onclick=\"goChangeDue('" .$intRow."')\">" : "&nbsp;";
      ...
      if ($strTakenDate == "" && isset($arrTaken[$row['overtime_date']])) $strTakenDate = $arrTaken[$row['overtime_date']];
      
      $strTakenStatus = ($row['dayoff'] == 1) ? getWords("no") : getWords("yes");
      if ($strTakenDate != "" && $row['dayoff'] == 0) $strTakenStatus .= " (" .$strTakenDate. ") ";
  ...
  if ($db->connect()) 
  {
    ...
    (isset($_REQUEST['dataDateThru'])) ? $strDataDateThru = $_REQUEST['dataDateThru'] : $strDataDateThru = getNextDate(getNextDateNextMonth(date("Y-m-d"),0));
