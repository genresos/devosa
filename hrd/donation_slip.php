<?php
  ...
  function getData($db)
  {
    ...
    if ($db->connect())
    {
      ...
      $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id", array('width' => '30'), array('align'=>'center', 'nowrap' => '')), false /*bolDisableSelfStatusChange*/);

      $myDataGrid->addColumnNumbering(new DataGrid_Column("No", "", array('width'=>'30'), array('nowrap'=>'')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("form code"), "form_code", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("created"), "created", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("date from"), "event_date_from", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("date thru"), "event_date_thru", "", array('nowrap' => '')));
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("donation type"), "name", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("name"), "relation_name", "", array('nowrap' => '')));

      $myDataGrid->addColumn(new DataGrid_Column(getWords("relation"), "relation_type", "", array('nowrap' => '')));
  ...
  function getSlip()
  {
    ...
    foreach ($myDataGrid->checkboxes as $strDataID)
    {
      ...
      */
      ...
      //assign header
      $dataDonation = $tblDonation->findAll("id = '$strDataID'", "id_employee, form_code, event_date_from || ' to ' || event_date_thru AS event_date, donation_code, relation_name || ' - ' || relation_type AS relation, note, amount", "", null, 1);
      $strSlipTopic = $dataDonationType[$dataDonation[0]['donation_code']]['name']; 
      //ambil data approval => ambil nama karyawan
      $dataApproval = $tblDonation->findAll("id = '$strDataID'", "checked_by, checked_time, approved_by, approved_time, now()::timestamp without time zone as printed_time, '".$_SESSION['sessionUserID']."' as printed_by", "", null, 1);
      if (count($dataApproval) > 0)
      {
        $dataApproval[0]['checked_by']  = ($dataApproval[0]['checked_by'] != "") ? getUserName($db, $dataApproval[0]['checked_by'])." (".$dataApproval[0]['checked_time'].")" : "";
        $dataApproval[0]['approved_by']  = ($dataApproval[0]['approved_by'] != "") ? getUserName($db, $dataApproval[0]['approved_by'])." (".$dataApproval[0]['approved_time'].")" : "";
        $dataApproval[0]['printed_by']  = ($dataApproval[0]['printed_by'] != "") ? getUserName($db, $dataApproval[0]['printed_by'])." (".$dataApproval[0]['printed_time'].")" : "";
      ...
      $dataEmployee = getEmployeeInfoByID($db, $strIDEmployee, "employee_id || ' - ' || employee_name AS employee_data, division_name as division, company_code, grade_code as grade, branch_code");
      $dataEmployeeAccount = getEmployeeInfoByID($db, $strIDEmployee, "bank2_account as bank_account, bank2_account_name as account_name, bank_name as bank");
      ...
      foreach ($dataDonation[0]  AS $strField => $strValue)
      {
        if ($strField != "id_employee" && $strField != "form_code" )
        {
          if ($strField == "note" )
            ...
            $strSlipContent2 .= wrapRow(getWords(str_replace("_", " ", $strField)), ":", standardFormat($strValue), false, false, false);
          else
  ...
  if ($db->connect()) 
  {
    ...
    $f->addSelect(getWords("donation type"), "dataDonationType", getDataListDonationType("", true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);
