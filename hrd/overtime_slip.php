<?php
  ...
  $strWordsTripAllowanceQuota = getWords("overtime trip disposition form");
...
function printAddAllowanceLink($params)
  {
    extract($params);
    return "<a href=\"slip_untuk_overtime.php?dataID=".$record['id_application']."&cash=t\">" .getWords('get slip'). "</a>";
  }
  ...
  function getData($db)
  {
    ...
    if ($strDataDateFrom != "" && $strDataDateThru != "") {
      $strKriteria .= "AND overtime_date BETWEEN '$strDataDateFrom' AND '$strDataDateThru' ";
    }
	...
	$myDataGrid->addColumn(new DataGrid_Column(getWords("Aksi"), "", array("rowspan" => 2, "width" => "60"), array('align' => 'center', 'nowrap' => ''), false, false, "", "printAddAllowanceLink()", "", true/*show in excel*/));
...
//$myDataGrid->addColumn(new DataGrid_Column(getWords("IDnya"), "id", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumnNumbering(new DataGrid_Column("No", "", array("rowspan" => 2,'width'=>'30'), array('nowrap'=>'')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("Overtime Date"), "overtime_date", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getwords("n i k"), "employee_id", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("Name"), "employee_name", array("rowspan" => 2), array('nowrap' => '')));
      //$myDataGrid->addColumn(new DataGrid_Column(getWords("Salary"), "salary_month", array("rowspan" => 2), array('nowrap' => '')));
    ...
    WHERE t1.status = '3' AND 1=1 $strKriteria";
  ...
  function getSlip()
  {
    ...
    foreach ($myDataGrid->checkboxes as $strDataID)
    {
      ...
      $dataEmployee2 = getEmployeeInfoByID($db, $strIDEmployee, "division_name as division, branch_code");
      ...
      $tbsPage->LoadTemplate("templates/overtime_slip_template.html") ;
 ...
 $f->caption = strtoupper("");
    $f->addSelect(getWords(""), "dataTripType", getDataListTripType($strDataTripTypeID), array("style" => "display:none;width:$strDefaultWidthPx"), "", false);  
