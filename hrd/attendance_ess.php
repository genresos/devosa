<?php
  ...
  function saveData($db, &$strError, $strDataDate, $strIDEmployee) {
    ...
    if ($bolDataIn && $objToday->strAttendanceStart == "")
    {
      ...
      writeLog(ACTIVITY_ADD, MODULE_PAYROLL, $objAttendanceClass->arrEmployee[$strIDEmployee]['employee_id']."-".$objToday->strAttendanceDate."-". $objToday->strAttendanceStart );
    ...
    if ($bolDataOut && $objToday->strAttendanceFinish == "")
    {
      ...
      writeLog(ACTIVITY_ADD, MODULE_PAYROLL, $objAttendanceClass->arrEmployee[$strIDEmployee]['employee_id']."-".(($objToday->bolYesterday) ? getNextDate($objToday->strAttendanceDate, -1) : $objToday->strAttendanceDate) ."-". $objToday->strAttendanceStart );
  ...
  if ($db->connect()) {
    ...
    if (isset($_REQUEST['btnSave'])) {
      if ($bolCanEdit) {
        ...
        else writeLog(ACTIVITY_EDIT, MODULE_EMPLOYEE, $arrUserInfo['employee_name'], 0);
      }
    } 
    if ($bolCanView) {
