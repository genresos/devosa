<?php
  ...
  //include_once(getTemplate("words.inc"));
   ...
   header ('Content-Disposition: attachment; filename="attendance_slip.doc"');  
   echo "<style>@page Section1 {margin:.5in .5in .5in .5in;}";
   echo "body{font-family: Verdana, Arial, Helvetica, sans-serif;font-size:10px;}";
   echo "td,th {font-family: Verdana, Arial, Helvetica, sans-serif;font-size: 10px;}";
   echo "\n";
  ...
  if ($db->connect()) {
    ...
    $strSQL .= "substr(attendance_finish,0,6) as time_out, holiday, is_overtime, ";
    ...
    while ($rowDb = $db->fetchrow($resDb))
    {
		if ($rowDb['is_overtime'] == "t") 
		{
			$rowDb['morning_overtime'] = 0;
			$rowDb['overtime'] = 0;
		}
    ...
    foreach ($arrEmp as $strIDEmployee => $arrEmployee)
    {
       ...
       while(dateCompare($strTempDate, $strDateThru) <= 0)
       {
          ...
          if (isset($arrAtt[$strIDEmployee][$strTempDate]))
          {
             ...
             $intOTEarly     += $arrDetail['morning_overtime'];
             $intOTAfternoon += ($arrDetail['overtime'] - $arrDetail['morning_overtime']);
             $intOTTotal     += $arrDetail['overtime'];
			 ...
			 $fltEarlyOT  = standardFormat(($arrDetail['morning_overtime']/60), false, 2);
			 $fltAftOT    = standardFormat((($arrDetail['overtime'] - $arrDetail['morning_overtime'])/60), false, 2);
			 $fltTotalOT  = standardFormat(($arrDetail['overtime']/60), false, 2);
             $$strColumn .= "<td $strStyleTd>&nbsp;". $fltEarlyOT."</td>";
             $$strColumn .= "<td $strStyleTd>&nbsp;". $fltAftOT."</td>";
             $$strColumn .= "<td $strStyleTd>&nbsp;".$fltTotalOT."</td>";
          }
          else
          {
             ...
             $$strColumn .= $strNaN.$strNaN.$strNaN.$strNaN.$strNaN.$strNaN;
          }
          $$strColumn .="</tr>";
       ...
       $tbsPage->LoadTemplate(getTemplate("attendance_slip.html"));
       $tbsPage->noErr = false;
    ...
    echo "<body> <div class=Section1>"; echo "</div></body> ";
  }
