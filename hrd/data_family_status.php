  ...
  if ($db->connect())
  {
    ...
    if ($bolCanEdit)
    {
      ...
      $f->addInput(getWords("code"), "dataCode", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);
      	...
      	$f->addInput(getWords("code"), "dataCode", "", array("size" => 30, "maxlength" => 31, "readOnly" => true), "string", true, true, true);
      }
			// $f->addSelect(getWords("marital status"), "positi", getDataListPositionGroup(), array(), "numeric", true, true, true);
      $f->addSelect(getWords("marital status"), "dataMaritalStatus", getDataListMaritalStatus(), array(), "string", true, true, true);
      $f->addInput(getWords("number of children"), "dataChildren", 0, array("size" => 3, "maxlength" => 3), "integer", true, true, true);
      $f->addInput(getWords("tax reduction"), "dataTaxReduction", 0, array("size" => 10, "maxlength" => 10), "numeric", true, true, true);
      ...
      $f->addTextArea(getWords("note"), "dataNote", "", array("cols" => 48, "rows" => 2, "maxlength" => 127), "string", false, true, true);

      $f->addSubmit("btnSave", getWords("save"), "", true, true, "", "", "saveData()");
      $f->addButton("btnAdd", getWords("add new"), array("onClick" => "javascript:myClient.editData('0|family_status|3');"));
    ...
    $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "family_status_code", array('width' => '30'), array('align'=>'center', 'nowrap' => '')));
    $myDataGrid->addColumnNumbering(new DataGrid_Column(getWords("no."), "", array('width'=>'30'), array('nowrap'=>'')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("code"), "family_status_code", array('width' => '70'),array('nowrap' => '')));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("children"), "children", array('width' => '80'), array('align' => 'center')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("tax reduction"), "tax_reduction", array('width' => '100'), array('align' => 'right'), true, true, "", "formatNumber()"));

    for ($i = 1; $i <= MAX_ALLOWANCE_SET; $i++)
      $myDataGrid->addColumn(new DataGrid_Column(getSetting($strSet.$i."_allowance_name"), $ARRAY_ALLOWANCE_SET[$strSet]['field_name'].$i, array('width' => '100'), array('align' => 'right'), true, true, "", "formatNumber()"));
...
//--------------------------------------------------------------------------------
  ...
  function printEditLink($params)
  {
    ...
    return "
      ...
      <input type=hidden name='detailMaritalStatus$counter' id='detailMaritalStatus$counter' value='".$record['marital_status']."' />
  ...
  function printMaritalStatus($params)
  {
    ...
    if ($record['marital_status'] == "" )
      ...
      return getWords($ARRAY_MARITAL_STATUS[$record['marital_status']]);
  }
...
function saveData()
  {
    ...
    $data = array("family_status_code" => $strDataCode,
									"marital_status" => intval($f->getValue('dataMaritalStatus')),
    ...
    {
      $bolSuccess = $dataHrdFamilyStatus->update(/*pk*/"family_status_code='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    if ($bolSuccess)
    {
