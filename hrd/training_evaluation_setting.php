<?php
  ...
  if ($strDataType == "") $strDataType = getDataFromCookie("question_type");
  else setDataToCookie("question_type", $strDataType);
...
$db = new CdbClass;
  ...
  if($strAction=="desc") goSortOrder($db, $strDataType, false);
  else if ($strAction=="asc") goSortOrder($db, $strDataType, true);
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addSelect(getWords("question type"), "question_type", $arrModule, array("onChange" => "javascript:myClient.doRefreshMenu()"), "string", true, true, true);
    $f->addInput(getWords("question category"), "category", "", array("size" => 50), "string", false, true, true);
    $f->addInput(getWords("question"), "question", "", array("size" => 50), "string", true, true, true);
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("question type"), "question_type", array('width' => ''), array('nowrap' => 'nowrap'), false, false, "", ""));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("question category"), "category", array('width' => ''), array('nowrap' => 'nowrap'), false, false, "", ""));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("question"), "question", array('width' => '600'), array('nowrap' => 'nowrap'), false, false, "", ""));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("edit"), "", array('width' => '40'), array('nowrap' => 'nowrap','align' => 'center'), true, false, "", "printEditLink()"));
...
//    $intWeight = (float)$f->getValue("weight");
    ...
    if ($strDataID == "") 
    {
      ...
      $strSQL  = "
        INSERT INTO hrd_training_evaluation_question
          ...
          VALUES('".$f->getValue('category')."', '".$f->getValue('question_type')."', '".$f->getValue('question')."')";
    }
    else 
    {    
      $strSQL  = "
    ...
    if (executeSaveSQL($strSQL, getWords("menu"), $f->message)) $bolOK = true;
    else $bolOK = false;
    ...
    foreach ($arrModule AS $idx => $val)
    {
      if ($default == "" || $default == "0")
      {
        if ($isFirst)
        {
          $isFirst = false;
          $default = $idx;
          $arrResult[] = array("value"=>$idx, "text"=>$val, "selected" => true);
        }
        else
          $arrResult[] = array("value"=>$idx, "text"=>$val, "selected" => false);
      }
      else if ($default == $idx)
      {
        $arrResult[] = array("value"=>$idx, "text"=>$val, "selected" => true);
