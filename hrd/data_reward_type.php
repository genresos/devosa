<?php
  ...
  include_once('../classes/hrd/hrd_reward_type.php');
  ...
  $strWordsRewardTypeData = getWords("reward type data");
  ...
  $strWordsNOTE = getWords("note");
  $strWordsNAME = getWords("name");
  $strWordsLISTOFREWARDTYPE = getWords("list of reward type");
  //----------------------------------------------------------------------
  ...
  function getDataRows($rowDb, $intRows) {
    ...
    $strResult .= "  <td align=right><input type=hidden name=detailAmount$intRows value=\"" .$rowDb['amount']. "\" disabled>" .$rowDb['amount']. "&nbsp;</td>";
    $strResult .= "  <td align=right><input type=hidden name=detailName$intRows value=\"" .$rowDb['name']. "\" disabled>" .$rowDb['name']. "&nbsp;</td>";
  ...
  function getData($db, &$intRows, $strKriteria = "", $strOrder = "") {
    ...
    $strSQL  = "SELECT * FROM hrd_reward_type ";
  ...
  function saveData($db, &$strError) {
    ...
    $strError = "";
    $strFields = "created, modified_by, created_by, code, \"amount\", note";
    ...
    if ($strDataCode == "") {
      ...
      if (isDataExists($db,"hrd_reward_type","code",$strDataCode,$strKriteria)) {
    ...
    if ($strDataID == "") {
      ...
      $strSQL  = "INSERT INTO hrd_reward_type (created,created_by,modified_by, ";
      $strSQL .= "\"code\",\"amount\", note, name) ";
      $strSQL .= "VALUES(now(),'" .$_SESSION['sessionUserID']. "','" .$_SESSION['sessionUserID']. "', ";
      $strSQL .= "'$strDataCode', '$strDataAmount', '$strDataNote', '$strDataName') ";
      ...
      $strSQL  = "UPDATE hrd_reward_type ";
      $strSQL .= "SET modified_by = '" .$_SESSION['sessionUserID']. "', \"code\" = '$strDataCode', ";

      $strSQL .= "\"amount\"= '$strDataAmount', note = '$strDataNote' , name = '$strDataName'";
  ...
  function deleteData($db) {
    ...
    foreach ($_REQUEST as $strIndex => $strValue) {
      if (substr($strIndex,0,5) == 'chkID') {
        ...
        $strSQL = "DELETE FROM hrd_reward_type WHERE id = '$strValue'; ";
        $resExec = $db->execute($strSQL);
        $i++;
      }
    }
