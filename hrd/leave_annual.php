<?php
  ...
  $strEmpID =  getwords("n i k");
  // $strIDEmp = getWords("id employee");
  ...
  function getData($db, &$intRows, $strKriteria = "", $intPage = 1, $bolLimit = true, $strOrder = "") {
    ...
    if(isset($tempInfo[$rowDb['id']][$strPrevYear]['additional_quota']) && $tempInfo[$rowDb['id']][$strPrevYear]['additional_quota']!="") $intLeaveAdditionalQuotaPrev = $tempInfo[$rowDb['id']][$strPrevYear]['additional_quota'];
	  if(isset($tempInfo[$rowDb['id']][$strCurrYear]['additional_quota']) && $tempInfo[$rowDb['id']][$strCurrYear]['additional_quota']!="") $intLeaveAdditionalQuotaCurr = $tempInfo[$rowDb['id']][$strCurrYear]['additional_quota'];
	  if(isset($tempInfo[$rowDb['id']][$strPrevYear]['additional']) && $tempInfo[$rowDb['id']][$strPrevYear]['additional']!="") $intLeaveAdditionalPrev = $tempInfo[$rowDb['id']][$strPrevYear]['additional'];
	  if(isset($tempInfo[$rowDb['id']][$strCurrYear]['additional']) && $tempInfo[$rowDb['id']][$strCurrYear]['additional']!="") $intLeaveAdditionalCurr = $tempInfo[$rowDb['id']][$strCurrYear]['additional'];
      ...
      else
      {
        ...
        {
          // uddin: remain hanya untuk annual leave
          //$arrCuti['prev']['remain'] -= $intLeaveAdditionalPrev;
          //$arrCuti['prev']['remain'] += $intLeaveAdditionalQuotaPrev;
      ...
      $strLeaveQuotaPrev = ($bolPrint || !$bolCanEdit) ? "$intLeaveQuotaPrev &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailQuotaPrev$intRows value=\"$intLeaveQuotaPrev\" $strDisabledPrev $strReadonlyPrev>";
      $strLeaveQuotaCurr = ($bolPrint || !$bolCanEdit) ? "$intLeaveQuotaCurr &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailQuotaCurr$intRows value=\"$intLeaveQuotaCurr\">";
      $strLeaveTakenPrev = ($bolPrint || !$bolCanEdit) ? "$intLeaveTakenPrev &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailTakenPrev$intRows value=\"$intLeaveTakenPrev\" $strDisabledPrev $strReadonlyPrev>";
      $strLeaveTakenCurr = ($bolPrint || !$bolCanEdit) ? "$intLeaveTakenCurr &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailTakenCurr$intRows value=\"$intLeaveTakenCurr\">";
      ...
      $strLeaveAdditionalQuotaPrev = ($bolPrint || !$bolCanEdit) ? "$intLeaveAdditionalQuotaPrev &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailAdditionalQuotaPrev$intRows value=\"$intLeaveAdditionalQuotaPrev\">";
      $strLeaveAdditionalQuotaCurr = ($bolPrint || !$bolCanEdit) ? "$intLeaveAdditionalQuotaCurr &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailAdditionalQuotaCurr$intRows value=\"$intLeaveAdditionalQuotaCurr\">";
      $strLeaveAdditionalPrev = ($bolPrint || !$bolCanEdit) ? "$intLeaveAdditionalPrev &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailAdditionalPrev$intRows value=\"$intLeaveAdditionalPrev\">";
      $strLeaveAdditionalCurr = ($bolPrint || !$bolCanEdit) ? "$intLeaveAdditionalCurr &nbsp;" : "<input type=text size=5 maxlength=10 class=numeric name=detailAdditionalCurr$intRows value=\"$intLeaveAdditionalCurr\">";
  ...
  function resetData($db)
  {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strID = $rowDb['id'];
        $strThisYear =  date("Y");
        $strStartCurr = $objLeave->getStartPeriod($strThisYear, $rowDb['join_date']); // ambil periode awal untuk tahun sekarang
        // bandingkan, apakah sekarang sudah melewati masa join date
        if ($strStartCurr > $objLeave->strDate) $strThisYear--;
  ...
  function importData($db) {
    ...
    if (is_uploaded_file($_FILES["fileData"]['tmp_name'])) {
      ...
      $arrForm = array(); // mencatat daftar form yang sudah ada, indexnya adalah form number
  ...
  if ($db->connect()) {
    ...
    scopeData($strFilteremployee_id, $strFilterSubsection, $strFilterSection, $strFilterDepartment, $strFilterDivision, $_SESSION['sessionUserRole'], $arrUserInfo, $strDataBranch);
    ...
    if (isset($_REQUEST['btnShowAll']) || isset($_REQUEST['btnPrintAll'])) {
      ...
      if ($strDataBranch != ""){
      	$strKriteria .= "AND he.branch_code = '$strDataBranch' ";
      }
...
/*    if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) {
    ...
    $strInputFilteremployee_id = "<input type=text name=filteremployee_id id=filteremployee_id size=$intDefaultWidth value=\"$strFilteremployee_id\" style=\"width:$intDefaultWidthPx\"width:$intDefaultWidthPx\">";
    ...
    $strInputFilterCompany = getCompanyList($db, "dataCompany",$strDataCompany, $strEmptyOption2, $strKriteria2, "style=\"width:$intDefaultWidthPx\" ");
