<?
  ...
  function saveData($db, $strDataID, &$strError) {
    ...
    // format : flag = 0
    $arrData = array(); // daftar evaluatin target yang terakhir
    $strSQL  = "SELECT * FROM hrd_employee_evaluation_target ";
      ...
      if ($strID == "") {
        ...
        if ($strGoal == "") {
          ...
          if (isset($arrData[$strID]) && $bolMakeNew) {
            ...
            if ($bolSame) $bolSame = ($strTarget == $arrData[$strID]['target']);
            if ($bolSame) $bolSame = ($strNote == $arrData[$strID]['note']);
            if ($bolSame) $bolSame = ($strWeight == $arrData[$strID]['weight']);
            if ($bolSame) $bolSame = ($strPoint == $arrData[$strID]['point']);
            if ($bolSame) $bolSame = ($strAchievement == $arrData[$strID]['achievement']);
      ...
      if ($rowDb = $db->fetchrow($resDb)) {
        ...
        if ($rowDb['status'] < 2) { //belum diapprove
          if ($arrUserInfo['id_employee'] == $rowDb['id_employee']) { // yang login karyawan
            ...
            $strButton .= " &nbsp; <input type=submit name=btnApprove value=\"" .getWords('approve')."\" $strApproveConfirm>";
        ...
        if ($_SESSION['sessionUserRole'] == ROLE_ADMIN) {
          ...
          $strButton .= " &nbsp; <input type=submit name=btnCancelApprove value=\"" .getWords('cancel approval')."\" onClick = \"return confirmCancel();\">";
        }

        $strEvaluationMenu = getEvaluationMenu($strDataID,0,0);
        if ($bolCanView) {
          $strDataDetail = ($bolPrint) ? getDataPrint($db,$intTotalData, $strKriteria,"",$bolDataReadonly) : getData($db,$intTotalData, $strKriteria,"",$bolDataReadonly); // ambil listnya

          // tambah button untuk print
          $strButton .= "&nbsp;<input type=button name='btnPrint' value=\"" .getWords("print")."\" onClick=\"window.open('evaluation_target.php?btnPrint=Print&dataID=$strDataID')\"> ";

          // tampilkan keterangan tentang status hasil, untuk pritn aja
          if ($bolPrint)
          {

            if ($strTotal > 90) $strNoteKriteria = $strTargetA;
            else if ($strTotal > 80) $strNoteKriteria = $strTargetB;
            else if ($strTotal > 70) $strNoteKriteria = $strTargetC;
            else if ($strTotal > 59) $strNoteKriteria = $strTargetD;
            else $strNoteKriteria = $strTargetE;
