<?
  ...
  function getData($db, $strDateFrom, $strDateThru, &$intRows, $strKriteria = "", $strOptionShow = "",$intPage = 1, $bolLimit = true, $strOrder = "") {
    global $words;
  ...
  function saveData($db, &$strError) {
    ...
    for($i=1; $i <= $intTotal; $i++) {
      ...
      if ($stremployee_id != "")
      {
        ...
        $bolOK = true;
        if (!is_numeric($intAtt)) $bolOK = false;
        else if (!is_numeric($intLate)) $bolOK = false;
        else if (!is_numeric($intEarly)) $bolOK = false;
        else if (!is_numeric($intOT1)) $bolOK = false;
        else if (!is_numeric($intOT2)) $bolOK = false;
        else if (!is_numeric($intOT3)) $bolOK = false;
        else if (!is_numeric($intOT4)) $bolOK = false;
  ...
  if ($db->connect()) {
    ...
    if ($bolCanView && ($_SESSION['sessionUserRole'] == ROLE_SUPERVISOR || $_SESSION['sessionUserRole'] == ROLE_ADMIN)) {
      if (validStandardDate($strDataDateFrom) && validStandardDate($strDataDateThru) && $bolShow) {
        ...
        $strDataDetail = getData($db,$strDataDateFrom, $strDataDateThru, $intTotalData, $strKriteria, $strDataView, $intCurrPage);
    ...
    $strButtons .= " <button name='btnWork' type=button onClick =\"useWorkingDays()\">".getWords("use working days")."</button>";
    ...
    if ($strDataDateFrom != $strDataDateThru) $strInfo .= " - ". strtoupper(pgDateFormat($strDataDateThru, "d M Y"));
