<?
  ...
  if (!isset($_SESSION['sessionUserID'])) {
    header("location:login.php?dataPage=candidate_edit_education.php");
    exit();
  }

  $bolCanView = getUserPermission("candidate_edit_education.php", $bolCanEdit, $bolCanDelete, $strError);

  $strTemplateFile = getTemplate("candidate_edit_education.html");
  //---- INISIALISASI ----------------------------------------------------
  $strDataDetail = "";
  $strCandidateID = "";
  $strCandidateName = "";
  ...
  function getData($db, &$intRows, $strKriteria = "", $strOrder = "") {
    ...
    $strSQL  = "SELECT * FROM \"hrdCandidateEducation\" ";
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= "  <td nowrap class='tableHeader'>&nbsp;" .$words['education level']."</td>";
      $strResult .= "  <td nowrap><input type=hidden name=detailID$intRows value=\"" .$rowDb['id']. "\">:&nbsp;</td>";
      $strResult .= "  <td>&nbsp;" .getEducationList($db,"detailCode$intRows", $rowDb['education_level_code']). "</td>";
      $strResult .= "</tr>\n";

      $strResult .= "<tr valign=top>\n";
      $strResult .= "  <td nowrap class='tableHeader'>&nbsp;" .$words['institution']."</td>";
      $strResult .= "  <td nowrap>:&nbsp;</td>";
      $strResult .= "  <td>&nbsp;<input type=text size=$intDefaultWidth maxlength=50 name=detailInstitution$intRows value=\"" .$rowDb['institution']. "\" style=\"width:$intDefaultWidthPx\"></td>";
      $strResult .= "</tr>\n";
      ...
      $strResult .= "<tr valign=top>\n";
      $strResult .= "  <td nowrap class='tableHeader'>&nbsp;" .$words['faculty']."</td>";
      $strResult .= "  <td nowrap>:&nbsp;</td>";
      $strResult .= "  <td>&nbsp;<input type=text size=$intDefaultWidth maxlength=50 name=detailFaculty$intRows value=\"" .$rowDb['faculty']. "\" style=\"width:$intDefaultWidthPx\"></td>";
      $strResult .= "</tr>\n";
    ...
    for ($i = 1; $i <= $intAdd; $i++) {
      ...
      $strResult .= "  <td nowrap class='tableHeader'>&nbsp;" .$words['education level']."</td>";
      $strResult .= "  <td nowrap>:&nbsp;</td>";
      $strResult .= "  <td>&nbsp;" .getEducationList($db,"detailCode$intRows","", "", "", $strDisabled). "</td>";
      $strResult .= "</tr>\n";
    ...
    for ($i = 1; $i <= $intMax; $i++) {
      ...
      if ($strID == "") {
        if ($strCode != "") { // insert new data
          $strSQL  = "INSERT INTO \"hrdCandidateEducation\" (created,modified_by, created_by, ";
          $strSQL .= "id_candidate, education_level_code, institution, note, ";
          $strSQL .= "location, faculty, ";
          ...
          $strSQL .= "'$strDataID', '$strCode', '$strInstitution', '$strNote', ";
          $strSQL .= "'$strLocation', '$strFaculty', ";
        ...
        if ($strCode == "") {
          // delete data
          $strSQL  = "DELETE FROM \"hrdCandidateEducation\" WHERE id = '$strID' ";
          $resDb = $db->execute($strSQL);
          writeLog(ACTIVITY_DELETE, MODULE_PAYROLL,"$strDataID",0);
        } else {
          // update data
          $strSQL  = "UPDATE \"hrdCandidateEducation\" SET modified_by = '" .$_SESSION['sessionUserID']. "', ";
          $strSQL .= "created = now(), education_level_code = '$strCode', ";
