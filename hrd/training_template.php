<?php
  ...
  if($db->connect())
  {
    ...
    if ($bolCanEdit)
    {
      ...
      $f->addSelect(getWords("grade"), "dataGradeCode", getDataListSalaryGrade(), array("style" => "width:270"));

      $f->addSelect(getWords("training type"), "dataIdType", getDataListTrainingCategoryType("", false), array("style" => "width:270"));

      $f->addSubmit("btnSave", getWords("save"), array("onClick" => "javascript:myClient.confirmSave();"), true, true, "", "", "saveData()");
      $f->addButton("btnAdd", getWords("add new"), array("onClick" => "javascript:myClient.addData();"), true, true, "", "", "");
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("grade"), "grade_code", "", array('width' => '50',  'valign' => 'top', 'nowrap' => '')));
    foreach($arrCategory AS $strIdCategory => $arrDetail)
    {
      $myDataGrid->addColumn(new DataGrid_Column(getWords($arrDetail['training_category']), "category_".$strIdCategory, "", array('valign' => 'top', 'nowrap' => ''), false, false, "", "", "", false));
      $myDataGrid->addColumn(new DataGrid_Column(getWords($arrDetail['training_category']), "category_".$strIdCategory."_excel", array("style" => "display:none"), array("style" => "display:none"), false, false, "", "", "", true, 15));
    }
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    if($bolSave)
    {
      ...
      $data = array("grade_code" => $f->getValue('dataGradeCode'),
      ...
      {
        $bolSuccess = $dataHrdTrainingTemplate->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
      }
    }
