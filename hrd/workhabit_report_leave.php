<?php
  ...
  function getData($strKriteria){
	...
	$codeSearch = "leave_day";
	//getmaxleaveday
	$strSQL = "SELECT MAX($codeSearch) AS max FROM hrd_salary_detail WHERE id_salary_master = $strDate";
	...
	for($i = 0; $i<($maxLeave+1)/$strDataInterval; $i++){
		...
		$strExecuteKriteria = " AND $codeSearch BETWEEN $minRange AND $maxRange ";
	...
	$strExecuteKriteria = " AND $codeSearch IS NULL ";
	$strExecuteSQL = $strSQL . $strKriteria . $strExecuteKriteria;
	$numOfEmployee = $db->execute($strExecuteSQL);
	$numOfEmployee = $db->fetchrow($numOfEmployee);
	$numOfEmployee = $numOfEmployee["count"];
	if ($numOfEmployee == 0) continue;
	$dataSet->addPoint(new Point("No $strWordsTitle Data", $numOfEmployee));
	
	$strResultInTable .= "
	<tr>
		<td>No $strWordsTitle Data</td>
		<td align=right>$numOfEmployee</td>
	</tr>
	";
	$strDataTotal += $numOfEmployee;
