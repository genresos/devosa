<?
  ...
  function importData($db, &$intTotal) {
    ...
    if (is_uploaded_file($_FILES["fileData"]['tmp_name'])) {
      ...
      $arrStatus = array(
        "anak" => array(4, 1), // kode status, gender
        "anaka" => array(4, 1),
        "istri" => array(2, 0),
        "iatri" => array(2, 0),
        "karyawan" => array(-1, 1),
      ...
      for ($i=2;$i <= $intRows; $i++) {
        ...
        if (!is_numeric($strApprovedCost)) $strApprovedCost = 0;
        if (!is_numeric($strCost)) $strCost = 0;
        ...
        // cek jenis perawatan
        if (($tmp = strpos($strType, "jalan")) !== false) $intType = 0; // rawat jalan
        else if (($tmp = strpos($strType, "inap")) !== false) $intType = 2;
        else if (($tmp = strpos($strType, "frame")) !== false) $intType = 1;
        else if (($tmp = strpos($strType, "glasses")) !== false) $intType = 1;
        else if (($tmp = strpos($strType, "lens")) !== false) $intType = 1;
        else $intType = 0; //d efault rawat jalan
        ...
        $strClaimDate = substr($strClaimDate,4,4) ."-". substr($strClaimDate,2,2) ."-". substr($strClaimDate,0,2);
