<?php
  ...
  foreach($arrTripType AS $strTripID => $arrTripDetail)
  {
    ...
    foreach($ARRAY_CURRENCY AS $strCurrencyNo => $strCurrencyCode)
    {
         ...
         $arrTripCost[$strTripID][$strCurrencyCode] = array();
  ...
  function getData($db)
  {
    ...
    if ($db->connect() && $strDataTripTypeID != "")
    {
      ...
      $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id", array("rowspan" => 2, 'width' => '30'), array('align'=>'center', 'nowrap' => '')), false /*bolDisableSelfStatusChange*/);

      $myDataGrid->addColumnNumbering(new DataGrid_Column("No", "", array("rowspan" => 2,'width'=>'30'), array('nowrap'=>'')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("form code"), "form_code", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("created"), "created", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("date from"), "date_from", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("date thru"), "date_thru", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("employee id"), "employee_id", array("rowspan" => 2), array('nowrap' => '')));
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("destination"), "destination", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("purpose"), "purpose", array("rowspan" => 2), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("task detail"), "task", array("rowspan" => 2,'width' => '250'), ""));
      ...
      foreach ($ARRAY_CURRENCY as $strCurrencyNo => $strCurrencyCode)
      {
        if (count($arrTripCost[$strDataTripTypeID][$strCurrencyCode]) > 0)
        {
          ...
          foreach($arrTripCost[$strDataTripTypeID][$strCurrencyCode] AS $strCostID)
          {
            ...
            if(count($arrTripCost[$strDataTripTypeID][$strCurrencyCode])>1){
               $myDataGrid->addColumn(new DataGrid_Column(getWords($arrTripCostType[$strCurrencyCode][$strCostID]['trip_cost_type_name'])." ".$strCurrencyCode, "trip_cost_".$strCostID, array('width' => '75'), array('align' => 'right'), false, false, "", "formatNumber()", "numeric", true, 15));
              }else{
                $myDataGrid->addColumn(new DataGrid_Column(getWords("Trip Cost ".$arrTripCostType[$strCurrencyCode][$strCostID]['trip_cost_type_name'])." ".$strCurrencyCode, "trip_cost_".$strCostID, array('width' => '75'), array('align' => 'right'), false, false, "", "formatNumber()", "numeric", true, 15));  
              }
          }
          $myDataGrid->addColumn(new DataGrid_Column(getWords("total"), "total_cost_".$strCurrencyCode, array("rowspan" => 2,'width' => '75'), array('align' => 'right'), false, false, "", "formatNumber()"));
        }
      }
      $myDataGrid->addColumn(new DataGrid_Column(getWords("status"), "status", array("rowspan" => 2), "", false, false, "","printRequestStatus()"));
      ...
      if ($myDataGrid->sortName == "division_name") $myDataGrid->sortName = "t2.division_code,department_name";  
      ...
      {
        $arrTripDetail = $tblTripDetail->findAll("id_trip = ".$arrDetail['id'], "id_trip_cost_type, amount", "", null, 1, "id_trip_cost_type");
  ...
  function getSlip()
  {
    ...
    foreach ($myDataGrid->checkboxes as $strDataID)
    {
      ...
      $dataTrip1 = $tblTrip->findAll("id = '$strDataID'", "date_from || ' until ' || date_thru AS trip_date", "", null, 1);
      ...
      $dataApproval1 = $tblTrip->findAll("id = '$strDataID'", "created_by, created, now()::timestamp without time zone as printed_time, '".$_SESSION['sessionUserID']."' as printed_by", "", null, 1);
      if (count($dataApproval1) > 0)
      {
        $dataApproval1[0]['created_by']  = ($dataApproval1[0]['created_by'] != "") ? getUserName($db, $dataApproval1[0]['created_by'])." (".substr($dataApproval1[0]['created'],0,16).")" : "";
        $dataApproval1[0]['printed_by']  = ($dataApproval1[0]['printed_by'] != "") ? getUserName($db, $dataApproval1[0]['printed_by'])." (".substr($dataApproval1[0]['printed_time'],0,16).")" : "";
      ...
      $dataApproval2 = $tblTrip->findAll("id = '$strDataID'", "checked_by, checked_time, approved_by, approved_time", "", null, 1);
      if (count($dataApproval2) > 0)
      {
        
        $dataApproval2[0]['approved_by']  = ($dataApproval2[0]['approved_by'] != "") ? getUserName($db, $dataApproval2[0]['approved_by'])." (".substr($dataApproval2[0]['approved_time'],0,16).")" : "";
		$dataApproval2[0]['checked_by']  = ($dataApproval2[0]['checked_by'] != "") ? getUserName($db, $dataApproval2[0]['checked_by'])." (".substr($dataApproval2[0]['checked_time'],0,16).")" : "";
      ...
      {
        $dataTripDetail[$strCurrencyCode] = $tblTripDetail->findAll("id_trip = '$strDataID' AND currency = '$strCurrencyCode'", "trip_cost_type, amount, currency");
      }
      // ambil ID employee
      $strIDEmployee = $dataTrip[0]['id_employee'];
      $dataEmployee1 = getEmployeeInfoByID($db, $strIDEmployee, "employee_id ||' - '|| employee_name as employee, grade_code as grade");
      $dataEmployee2 = getEmployeeInfoByID($db, $strIDEmployee, "t0.division_code as division");

      $dataEmployeeAccount1 = getEmployeeInfoByID($db, $strIDEmployee, "bank2_account as bank_account, bank2_account_name as account_name");
      ...
      {
          $strSlipContent2c .= "<tr><td width=80 valign=top>".ucWords($strField)."</td><td valign=top>:</td><td colspan=4 height=30 valign=top>".$strValue."</td></tr>";
      }
      foreach ($dataTripDetail AS $strCurrencyCode => $arrCost)
      {
        if(count($arrCost) > 0)
        {
          ...
          {
            $strSlipContent3 .= wrapRowSize(getWords($arrValue['trip_cost_type']), ": (".$strCurrencyCode.")" , standardFormat($arrValue['amount']), "80",true, true);
  ...
  if ($db->connect()) 
  {
    ...
    $f->addSelect(getWords("trip type"), "dataTripType", getDataListTripType($strDataTripTypeID), array("style" => "width:$strDefaultWidthPx"), "", false);
    $f->addInput(getWords("date from"), "dataDateFrom", date("Y-m")."-01", array("style" => "width:$strDateWidth"), "date", false, true, true);
    $f->addInput(getWords("date thru"), "dataDateThru", date("Y-m-d"), array("style" => "width:$strDateWidth"), "date", false, true, true);
    ...
    $f->addSelect(getWords("destination"), "dataDestination", getDataListDestination("", true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);
