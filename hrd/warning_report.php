<?php
  ...
  function showDataDetail($db) 
  {
    ...
    if ($strDataGrade != "") $strKriteriaDept .= "AND emp.salary_grade_code = '$strDataGrade' ";
    if ($strDataCompany != "") $strKriteriaDept .= "AND emp.id_company = '$strDataCompany' ";
    ...
    while ($rowDb = $db->fetchrow($resDb)) 
    {
      ...
      if (isset($arrEmp[$rowDb['department_code']][$rowDb['id_employee']]))
      {
        if ($rowDb['letter_no'] != "")
        {
          if ($arrEmp[$rowDb['department_code']][$rowDb['id_employee']]['letter_no'] != "") $arrEmp[$rowDb['department_code']][$rowDb['id_employee']]['letter_no'] .= ", ";
    ...
    $strResult .= "    <th rowspan=2>" .getwords("n i k")."</th>\n";
    ...
    {
      $strResult .= "  <tr><td colspan=" .(count($arrType) + 9)."><b>" .(($name == "") ? $code : $name)."</b></td></tr>\n"; // nama department
  ...
  function showDataDepartment($db) 
  {
    ...
    $strKriteriaDept = "";
    $strKriteria = "";
    if ($strDataDepartment != "") $strKriteriaDept .= "AND department_code = '$strDataDepartment' ";
    $strKriteria .= $strKriteriaDept;
    if ($strDataCompany != "") $strKriteria .= "AND id_company = '$strDataCompany' ";
    if ($strDataGrade != "")   $strKriteria .= "AND salary_grade_code = '$strDataGrade' ";
  ...
  function showDataBand($db) 
  {
    ...
    $strKriteriaBand = "";
    $strKriteria = "";
    if ($strDataGrade != "")   $strKriteriaBand .= "AND salary_grade_code = '$strDataGrade' ";
  ...
  function showDataReason($db) 
  {
    ...
    {      
      ...
      if ($intTotal == 0) $fltPercent = 0;
      else $fltPercent = (($intWarning/$intTotal) * 100);
    ...
    $strResult .= "    <td align='center'><b>" .(($intTotal == 0) ? standardFormat(0) : standardFormat(100))."</b>&nbsp;</td>\n";
    $strResult .= "  </tr>\n";
  ...
  if ($db->connect()) {
    ...
    if ($bolCanView) {
      ...
      if ($bolShow) {
        ...
        if (isset($_REQUEST['btnExcel'])) {
            // ambil data CSS-nya
          if (file_exists("../css/excel.css")) $strStyle = "../css/excel.css";
          $strPrintCss = "";
        $strPrintInit = "";
    ...
    $strInputDivision = getDivisionList($db,"dataDivision",$strDataDivision, $strEmptyOption, $strTmpKriteria, "style=\"width:$intDefaultWidthPx\" $strDisabled");
    /*
    ...
    */
    ...
    $arrTmp = array(getWords("detail"), getWords("summary by department"), getWords("summary by band"), getWords("summary by reason"));
    ...
    $strInputGrade = getSalaryGradeList($db,"dataGrade",$strDataGrade, $strEmptyOption, "", "style=\"width:$intDefaultWidthPx\" $strDisabled");
