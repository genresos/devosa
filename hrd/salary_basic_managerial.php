<?php
  ...
  function getData($db, &$intRows, $strIDSalarySet = "", $strIDSalarySetSource = "", $strKriteria = "", $intPage = 1, $strDefault = "", $strDefault2 = "", $strDefault3 = "", $bolLimit = true) {
    global $strDisable;
    global $words;
...
//    echo $strIDSalarySet;
    ...
    while ($rowTmp = $db->fetchRow($resTmp))
    {
      ...
      {
        getDefaultFix($db, "basic_salary", $arrFix, "AND t1.id = ".$rowTmp['id_employee_key']."", $strIDSalarySetSource);
    ...
    if ($strDefault != "") getDefaultFix($db, $strDefault, $arrFix, "");
    else if ($strDefault2 != "")
    {
  ...
  function getRecValue ($arrParam, $arrOpr, $strEmp)
  {
    ...
    {
      if(is_numeric($arrParam)) return $arrParam;
      else return ($arrTempVal[$strEmp][$arrParam] == null) ? 0 : $arrTempVal[$strEmp][$arrParam];
    }
    else
    {
      ...
      else
      {
        ...
        return computeArithmatic(getRecValue($arrParam2, $arrOpr, $strEmp), getRecValue($strSlice, $arrOpr, $strEmp), $arrOpr[count($arrParam2)-1]);
      }
    }
  }
  ...
  function getDefaultFix ($db, $strDefault, &$arrFix, $strCondition = "", $strIDSource = "")
  {
    ...
    else
    {
      ...
      $strIndex = substr($strDefault, -11, 1); //dikurangi string "xxxi_allowance" 11 karakter dari belakang, sebanya 1 karakter
      ...
      while ($rowTmp = $db->fetchrow($resTmp))
      {
        $arrFix[$rowTmp['t1_id']][$strDefault] = is_numeric($rowTmp[$ARRAY_ALLOWANCE_SET[$strKey]['field_name'].$strIndex]) ? $rowTmp[$ARRAY_ALLOWANCE_SET[$strKey]['field_name'].$strIndex] : 0;
      }
    }
  }
       ...
       //  echo $_REQUEST['dataAllowance2_'.$i]."|";
        ...
        for ($j = 1; $j<=$intColumn;$j++) {
          ...
          if ($strKode != "") {
            ...
            if (!is_numeric($fltAllowance)) $fltAllowance = 0;
  ...
  function getAllowanceData($db) {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      if (count($arrTemp) > 1)
      {
         $strDataColumn .= str_replace(array("_"," "),"<br>", strtoupper($rowDb['name']))."<br>";
      }
      else
      {
...
//    $strDisable  = "";
   ...
   $strKriteria .= $strKriteriaCompany;
    ...
    {
      $arrSetSource[$keySet] = $arrSet['start_date']." - ". printCompanyName($arrSet['id_company']) ." - ".$arrSet['note'];
    }
    ...
    if ($bolCanView) {
      if ($bolShow)
      {
        ...
        if (isset($_REQUEST['dataDefault']))
           $strDataDetail = getData($db, $intTotalData, $strIDSalarySet, $strIDSalarySetSource, $strKriteria, $intCurrPage, $_REQUEST['dataDefault'], $_REQUEST['dataDefault2'], $_REQUEST['dataDefault3']);
        else
           $strDataDetail = getData($db, $intTotalData, $strIDSalarySet, $strIDSalarySetSource, $strKriteria, $intCurrPage);
      } else {
        $strDataDetail = "";
      }
    } else {
    ...
    $strInputCurrency = getComboFromArray($ARRAY_CURRENCY, "dataCurrency", $strDataCurrency, $strEmptyOption, " style=\"width:$strDefaultWidthPx\"");
