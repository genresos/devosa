...
// Fungsi untuk generate data question type
  function getQuestionTypeList($db,$id,$value)
  {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= "<option value=\"" .$rowDb['id']. "\" $strSelect>" .getQuestionName($db,$rowDb['id']). "</option>\n";
    }
...
function getRewardTypeList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
  ...
  function getPositionList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      ($strCode == $default) ? $strSelect = "selected" : $strSelect = "";
      $strResult .= "<option value=\"$strCode\" $strSelect>$strCode - $strName - $strNote</option>\n";
    }
	...
	function getCategoryList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false, $strDomain = "", $strType = "") {
    $strResult  = "";
...
function getMasaIkatanDinas($varname, $default = "", $extra = "", $action = "") {
    $strResult = "";
...
$strSQL  = "SELECT * FROM hrd_training_type $criteria ORDER BY \"training_type\" ";
  ...
  // fungsi untuk generate data cost center, valuenya = code
  // function getCostCenterList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
  //   $strResult  = "";
    ...
    // $strResult .= $extra;
    //
    // $strSQL  = "SELECT * FROM hrd_cost_center $criteria ORDER BY code ";
    // $resDb = $db->execute($strSQL);
    // while ($rowDb = $db->fetchrow($resDb)) {
    //   $strCode = $rowDb['code'];
    //   $strName = $rowDb['name'];
  ...
  function getCostCenterList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
    ...
    $strSQL  = "SELECT * FROM hrd_cost_center $criteria ORDER BY name ASC ";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb)) {
      $strCode = $rowDb['code'];
      $strName = $rowDb['name'];
 ...
 // mengenerate combo list untuk institution  training
  ...
  // mengenerate combo list untuk instructor  training
   function getTrainingInstructorList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false)
  {
    $strResult  = "";
    $strHidden = "";
    if (!$listonly) {
  ...
  function getSalaryDateList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
    ...
    $strResult .= $extra;
    $strSQL  = "SELECT mst.*, comp.company_name FROM hrd_salary_master mst LEFT JOIN hrd_company comp ON mst.id_company=comp.id
    ...
    while ($rowDb = $db->fetchrow($resDb))
    {
      ...
      ($rowDb['id'] == $default) ? $strSelect = "selected" : $strSelect = "";
      $strResult .= "<option value=\"".$rowDb['id']."\" $strSelect>".$strName." - ".$rowDb['company_name']."</option>\n";
    }
  ...
  function getCompetencyTrainingList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
    global $strDataCompany;
    $strResult  = "";
    //$criteria .= ($strDataCompany == "") ? "" : "AND management_code LIKE '%". printCompanyCode($strDataCompany)."%'";
  ...
  function getDomainTrainingList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
    global $strDataCompany;
    $strResult  = "";
    //$criteria .= ($strDataCompany == "") ? "" : "AND management_code LIKE '%". printCompanyCode($strDataCompany)."%'";
  ...
  function getTypeTrainingList($db, $varname, $default = "", $extra = "", $criteria = "", $action = "", $listonly = false) {
    ...
    $strResult="";
		if ($default != "") {
		  ...
		  $resDb = $db->execute($strSQL);
		  $rowDb = $db->fetchrow($resDb);
		  $strResult = "<input type=text name=\"$varname\" $action value='".$rowDb['topic']. " [" .$rowDb['department_code']."] "."'>";
    ...
    $strResult = "<input type=text $action>";
		if ($default != "") {
		  ...
		  $resDb = $db->execute($strSQL);
		  $rowDb = $db->fetchrow($resDb);
		  $strResult = "<input type=$type name=\"$varname\" $action value='".$rowDb['department_code']. " [" .$rowDb['department_name']."] "."'>";
		}
    return $strResult;
    }
    ...
    $strResult = "<input type=text $action>";
		if ($default != "") {
		  ...
		  $resDb = $db->execute($strSQL);
		  $rowDb = $db->fetchrow($resDb);
		  $strResult = "<input type=$type name=\"$varname\" $action value='".$rowDb['division_code']. " [" .$rowDb['division_name']."] "."'>";
		}
    return $strResult;
    }
    ...
    $strResult = "<input type=text $action>";
		if ($default != "") {
		  ...
		  $resDb = $db->execute($strSQL);
		  $rowDb = $db->fetchrow($resDb);
		  $strResult = "<input type=$type name=\"$varname\" $action value='".$rowDb['section_code']. " [" .$rowDb['section_name']."] "."'>";
		}
    return $strResult;
    }
    ...
    $strResult = "<input type=text $action>";
		if ($default != "") {
		  ...
		  $resDb = $db->execute($strSQL);
		  $rowDb = $db->fetchrow($resDb);
		  $strResult = "<input type=$type name=\"$varname\" $action value='".$rowDb['sub_section_code']. " [" .$rowDb['sub_section_name']."] "."'>";
		}
    return $strResult;
    }

	function getDataTrainingInstitution($db, $varname, $default = "", $type = "", $criteria = "", $action = "", $listonly = false) {
    $strResult = "<input type=text $action>";
		if ($default != "") {
		  $strSQL  = "SELECT * FROM hrd_training_vendor WHERE id='$default' $criteria ";
		  $resDb = $db->execute($strSQL);
		  $rowDb = $db->fetchrow($resDb);
		  $strResult = "<input type=$type name=\"$varname\" $action value='".$rowDb['name_vendor']."'>";
		}
    return $strResult;
    }

	function getDataTrainingInstructor($db, $varname, $default = "", $type = "", $criteria = "", $action = "", $listonly = false) {
    $strResult = "<input type=text $action>";
		if ($default != "") {
		  $strSQL  = "SELECT * FROM hrd_training_instructor WHERE id='$default' $criteria ";
		  $resDb = $db->execute($strSQL);
		  $rowDb = $db->fetchrow($resDb);
		  $strResult = "<input type=$type name=\"$varname\" $action value='".$rowDb['name_instructor']."'>";
		}
    return $strResult;
    }
  ...
  function getDataCostCenterByCode($db, $varname, $default = "", $type = "", $criteria = "", $action = "", $listonly = false) {
