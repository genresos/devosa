<?php
  ...
  {
    $arrResult = array(
      "status" => false, "id" => "", "from" => "", "thru" => ""
    );
    if ($strID == "" || $strDateFrom == "" || $strDateThru == "") return $arrResult;
    ...
    if ($strIDException != "") $strSQL .= "AND id <> '$strIDException' ";
  ...
  function isTrainingExists($db, $strID, $strDateFrom, $strDateThru, $strIDException = "")
  {
    ...
    if ($strIDException != "") $strSQL .= "AND t1.id <> '$strIDException' ";
    $resDb = $db->execute($strSQL);
    if ($rowDb = $db->fetchrow($resDb))
    {
      $arrResult['status'] = true;
  ...
  function recheckAttendanceData($db, $strDateFrom, $strDateThru) {
    ...
    if (!validStandardDate($strDateFrom) || !validStandardDate($strDateThru)) return false;
  ...
  {
    if ($strDateFrom == "") return false;
    if (!validStandardDate($strDateFrom)) return false;
  ...
  {
    if ($strIDEmployee == "" || $strDate == "") return false;
    if (!validStandardDate($strDate)) return false;
...
*/
  ...
  function updateEmployeeLeave($db, $strIDEmployee, $intLeave = 0, $fltLeaveWeight = 1)
  {
    ...
    if ($intDuration > 0 ) // nambah pemakaian cuti, ngurangi jatah cuti
    {
      ...
      $arrCuti_sorted = array(); //untuk disortir
      $bolGrand = $bolPrev = true;
      if($arrCuti['prev']['year'] == "") $bolPrev = false;
      if($arrCuti['grand']['year'] == "") $bolGrand = false;

      if($bolPrev) $arrCuti_sorted[] = $arrCuti['prev'];
      if($bolGrand) $arrCuti_sorted[] = $arrCuti['grand'];
...
//      //while($intRemain > 0){
  ...
  function getOutOfficeInfo($db, $strDateFrom, $strDateThru, $strIDEmployee = "", $bolAbsOnly = false) {
    ...
    if (!validStandardDate($strDateFrom) || !validStandardDate($strDateThru)) return $arrResult;
    ...
    $strSQL .= "AND t2.is_leave = FALSE AND t3.status >= " .REQUEST_STATUS_APPROVED_2." ";
    if ($strIDEmployee != "") $strSQL .= "AND t1.id_employee = '$strIDEmployee' ";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb))
    {
    ...
    $strSQL .= "LEFT JOIN hrd_absence AS t3 ON t1.id_absence = t3.id ";
    $strSQL .= "WHERE (absence_date BETWEEN '$strDateFrom' AND '$strDateThru' OR absence_date = '$strDateFrom') ";
    $strSQL .= "AND t2.is_leave = TRUE  AND t3.status >= " .REQUEST_STATUS_APPROVED_2." ";
    if ($strIDEmployee != "") $strSQL .= "AND t1.id_employee = '$strIDEmployee' ";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb))
    {
    ...
    // $strSQL .= "AND t1.status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    ...
    $strSQL .= "AND t1.status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    if ($strIDEmployee != "") $strSQL .= "AND t2.id_employee = '$strIDEmployee' ";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb))
    {
    ...
    $strSQL .= "    OR (date_thru = DATE '$strDateFrom') ";
    $strSQL .= "    OR (date_thru = DATE '$strDateThru')) ";
    $strSQL .= "AND status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    if ($strIDEmployee != "") $strSQL .= "AND id_employee = '$strIDEmployee' ";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb))
    {
      // cari awal dan akhirnya
      $strStart = (pgDateFormat($strDateFrom, "Ymd") < pgDateFormat($rowDb['date_from'], "Ymd")) ? pgDateFormat($rowDb['date_from'], "Y-m-d") : pgDateFormat($strDateFrom, "Y-m-d");
      $strFinish = (pgDateFormat($strDateThru, "Ymd") < pgDateFormat($rowDb['date_thru'], "Ymd")) ? pgDateFormat($strDateThru, "Y-m-d") : pgDateFormat($rowDb['date_thru'], "Y-m-d");
  ...
  function getShiftScheduleByDate($db, $strDate, $strSection = "", $strSubSection = "", $strEmployee = "")
  {
    ...
    if ($strSection != "" || $strSubSection != "")
    {
      ...
      if ($strSection != "") $strKriteria .= " AND \"section_code\" = '$strSection' ";
      if ($strSubSection != "") $strKriteria .= " AND \"sub_section_code\" = '$strSubSection' ";
      $strKriteria .= ") ";
    }
  ...
  function getWorkSchedule($db, $strDate = "", $strIDEmployee = "", $strCriteria = "")
  {
    ...
    if ($strIDEmployee != ""){
      $strSQLAdded = "AND (";
      if (isset($tempEmployee['employee_id']) && !empty($tempEmployee['employee_id'])){
        $strSQLAdded .= "(table_name = 'Employee' and link_code = '".$tempEmployee['employee_id']."') ";
      }
      if (isset($tempEmployee['sub_section_code']) && !empty($tempEmployee['sub_section_code'])){
        if ($strSQLAdded == "AND ("){
          $strSQLAdded .= "(table_name = 'SubSection' and link_code = '".$tempEmployee['sub_section_code']."') ";
        }else{
          $strSQLAdded .= "OR (table_name = 'SubSection' and link_code = '".$tempEmployee['sub_section_code']."') ";
        }
      }
      if (isset($tempEmployee['section_code']) && !empty($tempEmployee['section_code'])){
        if ($strSQLAdded == "AND ("){
          $strSQLAdded .= "(table_name = 'Section' and link_code = '".$tempEmployee['section_code']."') ";
        }else{
          $strSQLAdded .= "OR (table_name = 'Section' and link_code = '".$tempEmployee['section_code']."') ";
        }
      }
      if (isset($tempEmployee['department_code']) && !empty($tempEmployee['department_code'])){
        if ($strSQLAdded == "AND ("){
          $strSQLAdded .= "(table_name = 'Department' and link_code = '".$tempEmployee['department_code']."') ";
        }else{
          $strSQLAdded .= "OR (table_name = 'Department' and link_code = '".$tempEmployee['department_code']."') ";
        }

      }
      if (isset($tempEmployee['division_code']) && !empty($tempEmployee['division_code'])){
        if ($strSQLAdded == "AND ("){
          $strSQLAdded .= "(table_name = 'Division' and link_code = '".$tempEmployee['division_code']."') ";
        }else{
          $strSQLAdded .= "OR (table_name = 'Division' and link_code = '".$tempEmployee['division_code']."') ";
        }
      }
      if ($strSQLAdded == "AND ("){
        $strSQLAdded = "";
    ...
    if (strlen($strStartTime) > 5) $strStartTime = substr($strStartTime, 0, 5);
    $strFinishTime = getSetting("finish_time");
    if (strlen($strFinishTime) > 5) $strFinishTime = substr($strFinishTime, 0, 5);
    //jumat
    $isDayOff = 'f';
    if ($intDay == 5)
    {
  ...
  function getEmployeeAttendance($db, $strFrom, $strThru, $intTipe = 0, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "AND (t1.attendance_start is not null OR t1.attendance_finish is not null) AND is_absence != 't' ";
    ...
    $addSQL = "SELECT COUNT(t1.id_employee) AS duration, t1.id_employee
                 FROM hrd_absence_detail AS t1
                 LEFT JOIN hrd_absence_type AS t2 ON t1.absence_type = t2.code
                 LEFT JOIN hrd_absence AS t3 ON t3.id = t1.id_absence
                 WHERE t2.is_leave = TRUE AND treated_as_attend = TRUE AND t1.absence_date between '$strFrom' and '$strThru'
                 AND t3.status >= 2
                 GROUP BY t1.id_employee ";

    $resAddDb = $db->execute($addSQL);
    while ($rowAddDb = $db->fetchrow($resAddDb))
    {
      $strID = $rowAddDb['id_employee'];
      $arrResult[$strID]['additional_total'] = $rowAddDb['duration'];

      $arrResult[$strID]['total'] += $arrResult[$strID]['additional_total']; // total kehadiran
    }
  ...
  function getEmployeeAbsence2($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "AND (status >= '" .REQUEST_STATUS_APPROVED_2."' OR status = '" .REQUEST_STATUS_DENIED. "') ";
    if ($strSection != "") {
      $strSQL .= "AND (t2.section_code = '$strSection' OR t2.sub_section_code = '$strSection') ";
    }
    if ($strEmployee != "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      // cari total waktu
      (pgDateFormat($strFrom, "Ymd") < pgDateFormat($rowDb['date_from'], "Ymd")) ? $strDateStart = $rowDb['date_from'] : $strDateStart = $strFrom;
      (pgDateFormat($strThru, "Ymd") < pgDateFormat($rowDb['date_thru'], "Ymd")) ? $strDateFinish = $strThru : $strDateFinish = $rowDb['date_thru'];
  ...
  function getEmployeeAbsence($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "WHERE 1=1 AND t4.status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    if ($strSection != "") {
      $strSQL .= "AND (t2.section_code = '$strSection' OR t2.sub_section_code = '$strSection') ";
    }
    if ($strEmployee != "") {
  ...
  function getEmployeeLeave($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      if(!isset($arrResult[$rowDb['id_employee']][$rowDb['absence_type']])) $arrResult[$rowDb['id_employee']][$rowDb['absence_type']] = 0;
      if(!isset($arrResult[$rowDb['id_employee']]['total'])) $arrResult[$rowDb['id_employee']]['total'] = 0;
  ...
  function getEmployeeLeaveMonthly($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "AND leave_type_code = 2 AND status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    if ($strSection != "") {
      $strSQL .= "AND (t2.section_code = '$strSection' OR t2.sub_section_code = '$strSection') ";
    }
    if ($strEmployee != "") {
  ...
  function getEmployeeLeaveMaternity($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL  = "SELECT t1.* FROM hrd_leave AS t1, hrd_employee AS t2 ";
    $strSQL .= "WHERE t1.id_employee = t2.id AND t2.gender = 0 ";
    $strSQL .= "AND leave_type_code = 1 AND status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    if ($strSection != "") {
      $strSQL .= "AND (t2.section_code = '$strSection' OR t2.sub_section_code = '$strSection') ";
    }
    if ($strEmployee != "") {
  ...
  function getEmployeeLeaveAnnual($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL  = "SELECT t1.* FROM hrd_leave AS t1, hrd_employee AS t2 ";
    $strSQL .= "WHERE t1.id_employee = t2.id AND t1.leave_type_code = 0 AND status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    if ($strSection != "") {
      $strSQL .= "AND (t2.section_code = '$strSection' OR t2.sub_section_code = '$strSection') ";
    }
    if ($strEmployee != "") {
  ...
  function getEmployeeOvertimeApplication($db, $strFrom, $strThru, $intTipe = 0, $strOutdated = "FALSE", $strIDEmployee = "", $strKriteria = "")
  {
    $arrResult = array();
  ...
  function getEmployeeOvertime($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "ON t1.id_employee = t2.id ";
    $strSQL .= "WHERE t1.status >= '" .REQUEST_STATUS_APPROVED_2."' ";
  ...
  function getAutoShiftOT($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "WHERE t3.status >= '" .REQUEST_STATUS_APPROVED_2."' ";
  ...
  {
     $arrOTMax    = array(1 => 1, 2 => 7, 3 => 1, 4 => 15);
  ...
  function getAbsenceInfoByEmployee($db,$strDate, $strID) {
    $arrResult = array("tipe" => "", "code" => ""); // tipe = 0=abs, 1=leave
    ...
    if ($rowDb = $db->fetchrow($resDb)) {
      ...
      // cari cuti
      $strSQL  = "SELECT absence_type_code FROM hrd_absence ";
      $strSQL .= "WHERE id_employee = '$strID' AND status >= '" .REQUEST_STATUS_APPROVED_2."' ";
      $strSQL .= "AND '$strDate' BETWEEN date_from AND date_thru ";
      $resDb = $db->execute($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
  ...
  function getEmployeeTrip($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "WHERE t1.id_employee = t2.id AND status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    if ($strSection != "") {
      $strSQL .= "AND (t2.section_code = '$strSection' OR t2.sub_section_code = '$strSection') ";
    }
    if ($strEmployee != "") {
  ...
  function getEmployeeTraining($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "WHERE t3.id_employee = t2.id AND t1.status >= '" .REQUEST_STATUS_APPROVED_2."' ";
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      // cari total waktu
      (pgDateFormat($strFrom, "Ymd") < pgDateFormat($rowDb['training_date'], "Ymd")) ? $strDateStart = $rowDb['training_date'] : $strDateStart = $strFrom;
      (pgDateFormat($strThru, "Ymd") < pgDateFormat($rowDb['training_date_thru'], "Ymd")) ? $strDateFinish = $strThru : $strDateFinish = $rowDb['training_date_thru'];
  ...
  function syncShiftAttendance($db, $strDateFrom, $strDateThru, $strKriteria)
  {
     ...
     // cari info Absence
     $arrAbsence = array();
     $strSQL  = "SELECT id_employee, absence_date ";
     $strSQL .= "FROM hrd_absence_detail AS t0 ";
     $strSQL .= "LEFT JOIN hrd_employee AS t1 ON t0.id_employee = t1.id ";
     $strSQL .= "WHERE absence_date BETWEEN '$strDateFrom' AND '$strDateThru' $strKriteria";
     $resS = $db->execute($strSQL);
     while ($rowS = $db->fetchrow($resS))
     {
        $arrAbsence[$rowS['absence_date']][$rowS['id_employee']] = $rowS;
     }
        ...
        foreach($arrAttendance as $strDate => $arrAttDate)
        {
           ...
           foreach($arrAttDate as $strID => $arrAttRow)
           {
              $arrAttRow = $arrAttendance[$strDate][$strID];
							$arrSchedule = getWorkSchedule($db, $strDate, $strID);
              ...
              {
                 $strShiftCode        = "";
                 if (!isset($arrSchedule[$strID])){
                   $strNormalStart      = $strDefaultStart;
                   $strNormalFinish     = ($bolFriday)  ? $strFridayFinish : $strDefaultFinish;
                 }else{
                   $strNormalStart      = isset($arrSchedule[$strID]['start_time']) && !empty($arrSchedule[$strID]['start_time']) ? $arrSchedule[$strID]['start_time'] : $strDefaultStart;
                   $strNormalFinish     = isset($arrSchedule[$strID]['finish_time']) && !empty($arrSchedule[$strID]['finish_time']) ? $arrSchedule[$strID]['finish_time'] : $strDefaultFinish;
                 }
                 $intHoliday          = ($bolHoliday) ? 1 : 0;
              }
              $bolAbsence              = (isset($arrAbsence[$strDate][$strID]) && count($arrAbsence[$strDate][$strID]) > 0) ? "t" : "f";
              ...
              if ($strNormalStart == "") continue;
              $arrOT               = calculateOvertime($db, $strDate, $strNormalStart, $strNormalFinish, $strAttendanceStart, $strAttendanceFinish);
              ...
              $strSQL .= "holiday= '" .$intHoliday. "', is_absence = '".$bolAbsence."' ";
  ...
  function checkInvalidAttendance($db, $strDateFrom, $strDateThru, $strKriteria = "")
  {
     ...
     while ($rowDb = $db->fetchrow($resDb))
     {
        ...
        if (getIntervalHour($strNormalStart, $strAttendanceFinish) < 240 && $strNormalStart != $strAttendanceFinish  && getIntervalHour($strNormalStart, $strNormalFinish) > 240)
        {
  ...
  function checkBlankAttendance($db, $strDateFrom, $strDateThru, $strKriteria = "")
  {
     ...
     while (dateCompare($strCurDate, $strDateThru) <= 0)
     {
        ...
        while ($rowDb = $db->fetchrow($resDb))
        {
           ...
           {
              $tblAbsence->insert(array("id_employee" => $rowDb['id'], "date_from" => $strCurDate, "date_thru" => $strCurDate, "absence_type_code" => "A", "note" => "generated by system", "status" => REQUEST_STATUS_NEW));
              $tblAbsenceDetail->insert(array("id_absence" => $tblAbsence->getLastInsertId(), "id_employee" => $rowDb['id'], "absence_date" => $strCurDate, "absence_type" => "A"));
           }
        }
...
//	-- CHECK EARLY --
        ...
        if((getMinutes($rowS['attendance_finish'])>=getMinutes($rowS['normal_finish']))&&(getMinutes($rowS['attendance_finish'])<getMinutes($rowS['normal_start'])) && (getMinutes($rowS['attendance_finish'])>getMinutes($rowS['attendance_start'])) ){
            ...
            $strSQL  = "UPDATE hrd_attendance SET early_duration = FLOOR(Extract(EPOCH FROM(normal_finish-attendance_finish)::INTERVAL)/60)";
            $strSQL .= " WHERE attendance_date = '".$rowS['attendance_date']."' ";
            $strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee WHERE id='".$rowS['id_employee']."') ";
            $res = $db->execute($strSQL);

        }
		  else if((getMinutes($rowS['attendance_finish'])<getMinutes('24:00:00')) && (getMinutes($rowS['attendance_finish'])>=getMinutes($rowS['attendance_start'])) &&  (getMinutes($rowS['attendance_finish'])!=getMinutes($rowS['normal_finish']))){

			  if (getMinutes($rowS['attendance_finish'])<(getMinutes($rowS['normal_finish'])+1440)){
				$strSQL  = "UPDATE hrd_attendance SET early_duration = FLOOR(Extract(EPOCH FROM(normal_finish-attendance_finish+'86400')::INTERVAL)/60)";
				$strSQL .= " WHERE attendance_date = '".$rowS['attendance_date']."' ";
				$strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee WHERE id='".$rowS['id_employee']."') ";
				$res = $db->execute($strSQL);
			  }
		  ...
		  else if((getMinutes($rowS['attendance_finish'])>=getMinutes($rowS['normal_finish']))||(strtotime($rowS['attendance_finish'])<strtotime($rowS['normal_start']))){
	    $strSQL  = "UPDATE hrd_attendance SET early_duration = 0 ";
            $strSQL .= "WHERE attendance_date = '".$rowS['attendance_date']."' ";
            $strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee WHERE id='".$rowS['id_employee']."') ";
            $res = $db->execute($strSQL);
	    	...
	    	$strSQL  = "UPDATE hrd_attendance SET early_duration = 0 ";
            	...
            	$strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee WHERE id='".$rowS['id_employee']."') ";
            	$res = $db->execute($strSQL);
            } else if(strtotime($rowS['attendance_finish'])<strtotime($rowS['normal_finish']) && getMinutes($rowS['attendance_finish'])>=getMinutes($rowS['attendance_start']) ){
            	$strSQL  = "UPDATE hrd_attendance SET early_duration = FLOOR(Extract(EPOCH FROM(normal_finish-attendance_finish)::INTERVAL)/60)";
            	$strSQL .= " WHERE attendance_date = '".$rowS['attendance_date']."' ";
            	$strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee WHERE id='".$rowS['id_employee']."') ";
            	$res = $db->execute($strSQL);
...
//		 --CHECK LATE--
	if ($rowS['normal_start'] > $rowS['normal_finish']){ //overnight
		...
		if(getMinutes($rowS['attendance_start'])<=getMinutes($rowS['normal_start']) && getMinutes($rowS['attendance_start']) > getMinutes($rowS['normal_finish'])){
			...
			if ((getMinutes($rowS['attendance_start'])+1440)>getMinutes($rowS['normal_start'])){
				$strSQL  = "UPDATE hrd_attendance SET late_duration = FLOOR(Extract(EPOCH FROM('86400' - (normal_start-attendance_start))::INTERVAL)/60)";
				$strSQL .= " WHERE attendance_date = '".$rowS['attendance_date']."' ";
				$strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee WHERE id='".$rowS['id_employee']."') ";
				$res = $db->execute($strSQL);
			}
	...
	else
	{
		 if(strtotime($rowS['attendance_start'])<=strtotime($rowS['normal_start'])){
			...
			$strSQL  = "UPDATE hrd_attendance SET late_duration = FLOOR(Extract(EPOCH FROM(attendance_start-normal_start)::INTERVAL)/60) ";
				$strSQL .= "WHERE attendance_date = '".$rowS['attendance_date']."' ";
				$strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee WHERE id='".$rowS['id_employee']."') ";
				$res = $db->execute($strSQL);
		   }
