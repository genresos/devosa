<?php
  ...
  $strWordsDriverLicenseAValid = getWords ("driving license A valid");
  $strWordsDriverLicenseBValid = getWords ("driving license B valid");
  $strWordsDriverLicenseCValid = getWords ("driving license C valid");
  $strWordsNationality = getWords ("nationality");
  $strWordsPassport = getWords ("passport");
  $strWordsPassportValid = getWords ("passport valid");
  ...
  $strWordsJamsostekNo =  getWords ("b p j s ketenagakerjaan");
  ...
  $strWordsBPJSNo = getWords ("B P J S kesehatan");
  ...
  $strWordsIDCardValid = getWords("K T P Valid");
...
*/
  ...
  function getEmployeeEducation($db, $strDataID = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strPeriode1 = "";
      if ($rowDb['day_from'] != "") $strPeriode1 .= $rowDb['day_from']." ";
      if ($rowDb['month_from'] != "") $strPeriode1 .= getBulanSingkat($rowDb['month_from'])." ";
      if ($rowDb['year_from'] != "") $strPeriode1 .= $rowDb['year_from'];

      $strPeriode2 = "";
      if ($rowDb['day_thru'] != "") $strPeriode2 .= $rowDb['day_thru']." ";
      if ($rowDb['month_thru'] != "") $strPeriode2 .= getBulanSingkat($rowDb['month_thru'])." ";
      if ($rowDb['year_thru'] != "") $strPeriode2 .= $rowDb['year_thru'];
    ...
    if ($intRows == 0) {
      $strResult .= "<tr align=center><td colspan=9>--</td></tr>\n";
  ...
  function getEmployeeTraining($db, $strDataID = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      if ( $bolPrint ) {
        ...
        $strResult .= "  <td>&nbsp;" .$rowDb['note']. "</td>\n";
        $strResult .= "  <td>&nbsp;" .$rowDb['masa_ikatan_dinas']. "</td>\n";
        ...
        $strResult .= "  <td nowrap>&nbsp;" .$strPeriode. "</td>\n";
        $strResult .= "  <td nowrap align=right>" .standardFormat($rowDb['cost'], true, 0). "&nbsp;</td>\n";
        $strResult .= "  <td nowrap align=right>&nbsp;" .$rowDb['note']. "</td>\n";
        $strResult .= "  <td nowrap align=right>&nbsp;" .$rowDb['masa_ikatan_dinas']. "</td>\n";
      }
      $strResult .= " </tr>\n";
    }
  ...
  function getEmployeeWarning($db, $strDataID = "") {
    ...
    $strSQL .= "WHERE id_employee = '$strDataID' AND status = 6 ORDER BY warning_date";
    $resDb = $db->execute($strSQL);
...
function getEmployeeReward($db, $strDataID = "") {
    ...
    // cari daftar surat peringatan
    $intRows = 0;
    $strSQL  = "SELECT * FROM hrd_employee_reward ";
    $strSQL .= "WHERE id_employee = '$strDataID' AND status = 6 ORDER BY reward_date";
    $resDb = $db->execute($strSQL);
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= " <tr valign=top>\n";
      $strResult .= "  <td nowrap align=right>$intRows&nbsp;</td>\n";
      $strResult .= "  <td nowrap>&nbsp;" .pgDateFormat($rowDb['reward_date'], "d-M-y"). "</td>\n";
      $strResult .= "  <td nowrap>&nbsp;" .$rowDb['reward_code']. "</td>\n";
      $strResult .= "  <td nowrap>&nbsp;" .$rowDb['reward_amount']. "</td>\n";
//      $strResult .= "  <td nowrap>&nbsp;" .$rowDb['due_date']. "</td>\n";
      $strResult .= "  <td>&nbsp;" .$rowDb['note']. "</td>\n";
      $strResult .= " </tr>\n";
      }
...
function getEmployeeMutationDepartment($db, $strDataID = "") {
  ...
  function getEmployeeMutationBranch($db, $strDataID = "") {
      global $arrCompanyName;
      $strResult = "";
      if ($strDataID === "") return "";
      ...
      $strSQL  = "SELECT * FROM hrd_employee_mutation_branch as t0 LEFT JOIN hrd_employee_mutation as t1 on t1.id = t0.id_mutation ";
      $strSQL .= "WHERE t1.id_employee = '$strDataID' AND t1.status = 6";
      $resDb = $db->execute($strSQL);
      ...
      while ($rowDb = $db->fetchrow($resDb)) {
          ...
          $strResult .= " <tr valign=top>\n";
          $strResult .= "  <td nowrap align=right>$intRows&nbsp;</td>\n";
          $strResult .= "  <td nowrap>&nbsp;".getBranchName($rowDb['branch_contract_old'])."</td>\n";
          $strResult .= "  <td nowrap>&nbsp;".getBranchName($rowDb['branch_penugasan_old'])."</td>\n";
          $strResult .= "  <td nowrap>&nbsp;".getBranchName($rowDb['branch_contract_new'])."</td>\n";
          $strResult .= "  <td nowrap>&nbsp;".getBranchName($rowDb['branch_penugasan_new'])."</td>\n";
          $strResult .= "  <td nowrap>&nbsp;".pgDateFormat($rowDb['branch_new_date'], "d-M-y"). "</td>\n";
          $strResult .= " </tr>\n";
      }
...
//      $strResult .= "  <td nowrap>&nbsp;" .$rowDb['note']. "</td>\n";
  ...
  if ($db->connect()) {
    ...
    $strInputEmployeeID2      = $arrData['dataEmployeeID2'];
    ...
    $strInputMotherName             = $arrData['dataMotherName']."";
    ...
    $strInputIDCardValid      = $arrData['dataIDCardValid']."";
    $strInputLicenseA         = $arrData['dataLicenseA']."";
    $strInputLicenseB         = $arrData['dataLicenseB']."";
    $strInputLicenseC         = $arrData['dataLicenseC']."";
    $strDataLicenseAValid     = $arrData['dataLicenseAValid']."";
    $strDataLicenseBValid     = $arrData['dataLicenseBValid']."";
    $strDataLicenseCValid     = $arrData['dataLicenseCValid']."";
    $strInputNationality      = $arrData['dataNationality']."";
    $strInputPassport         = $arrData['dataPassport']."";
    $strDataPassportValid     = $arrData['dataPassportValid']."";
    $strInputJamsostek        = $arrData['dataJamsostekNo']."";
    $strInputJamsostekRegis   = $arrData['dataJamsostekRegis']."";
    ...
    $strInputNPWPRegis        = $arrData['dataNPWPRegis']."";
    ...
    $strInputContractFrom     = pgDateFormat($arrData['dataContractFrom']."","d-M-Y");
    ...
    $strInputBPJSNo           = $arrData['dataBPJSNo']."";
    $strInputBPJSRegis        = $arrData['dataBPJSRegis']."" ;
    ...
    $strInputResignReason = $arrData['dataResignReason']."";
    $strInputECStatus = $arrData['dataECStatus']."";
    /*if ($_SESSION['sessionUserRole'] == ROLE_SUPER || $arrData['dataEmployeeID'] == $arrUserInfo['employee_id'])
      ...
      $strInputBranchPenugasan = ($strInputBranchPenugasan == "") ? "" : $arrBranch[$strInputBranchPenugasan];
    ...
    $strDataReward        = getEmployeeReward($db, $strDataID);
    ...
    $strDataMutationBranch = getEmployeeMutationBranch($db, $strDataID);
