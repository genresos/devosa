...
class cDataGrid2 extends cDataGrid
  ...
  function getData($db)
  {
    ...
    if ($arrData['dataDivision']!= "") {
      $strKriteria .= "AND t3.division_code = '".$arrData['dataDivision']."'";
    }
    if ($arrData['dataDepartment']!= "") {
      $strKriteria .= "AND t3.department_code = '".$arrData['dataDepartment']."'";
    }
    if ($arrData['dataSection']!= "") {
      $strKriteria .= "AND t3.section_code = '".$arrData['dataSection']."'";
    }
    if ($arrData['dataSubSection']!= "") {
      $strKriteria .= "AND t3.sub_section_code = '".$arrData['dataSubSection']."'";
    }
    ...
    if ($db->connect())
    {
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("evaluator id"), "evaluator_id",  "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("evaluator name"), "evaluator_name",  "", array('nowrap' => '')));
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("evaluation date"), "evaluation_date",  "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("period from"), "evaluation_period_from",  "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("period thru"), "evaluation_period_thru",  "", array('nowrap' => '')));
		$myDataGrid->addColumn(new DataGrid_Column(getWords("expected total score"), "expected_total","", array('align' => 'right', 'nowrap' => ''), false, false, "",""));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("actual total score"), "total_score","", array('align' => 'right', 'nowrap' => ''), false, false, "",""));
$myDataGrid->addColumn(new DataGrid_Column(getWords("overall result"), "total_result","", array('align' => 'right', 'nowrap' => ''), false, false, "",""));
		$myDataGrid->addColumn(new DataGrid_Column(getWords("remarks"), "remarks",  "", array('width' => '100', 'valign' => 'top', 'nowrap' => '')));
      ...
      generateRoleButtons(($dataPrivilege['edit'] == 't'), ($dataPrivilege['delete'] == 't'), ($dataPrivilege['check'] == 't'), ($dataPrivilege['approve'] == 't'), true, true, $myDataGrid);
      ...
      $strSQLCOUNT  = "SELECT COUNT(*) AS total FROM hrd_evaluation_master as t1 
                       ...
                       LEFT JOIN hrd_division AS t3a ON t2.division_code = t3a.division_code 
                       LEFT JOIN hrd_department AS t3b ON t2.department_code = t3b.department_code 
                       LEFT JOIN (select id, employee_id, employee_name, division_code, department_code, section_code, sub_section_code from hrd_employee) as t3 ON t1.id_evaluator = t3.id WHERE 1=1 "; 
      $strSQL       = "SELECT t1.id,t1.status, t1.evaluation_date, t1.total_score, t1.total_result, t1.expected_total,t1.evaluation_period_from, t1.evaluation_period_thru, 
                       t2.employee_id, t2.employee_name, t2a.division_name, t2b.department_name, 
                       t3.id as id_employee, t3.employee_id as evaluator_id , t3.employee_name as evaluator_name, 
					   CASE WHEN (total_result >=1 AND total_result < 2) THEN 'Very Poor'::TEXT
		       WHEN (total_result >=2 AND total_result < 3) THEN 'Poor'::TEXT
		       WHEN (total_result >=3 AND total_result < 4) THEN 'Good'::TEXT
		       WHEN (total_result >=4 AND total_result < 4.5) THEN 'Very Good'::TEXT
		       WHEN (total_result >=4.5 AND total_result <= 5) THEN 'Excellent'::TEXT END as remarks
                       FROM hrd_evaluation_master AS t1 
                       LEFT JOIN hrd_employee AS t2 ON t1.id_employee = t2.id 
  ...
  if ($bolCanView)
  {
    ...
    $f->addInput(getWords("date from"), "dataDateFrom", getInitialValue("DateFrom", date("Y-")."01-01"),  array("style" => "width:$strDateWidth"), "date", false, true, true);
    $f->addInput(getWords("date thru"), "dataDateThru", getInitialValue("DateThru", date("Y-m-d")),  array("style" => "width:$strDateWidth"), "date", false, true, true);
    $f->addInputAutoComplete(getWords("employee"), "dataEmployee", getDataEmployee(getInitialValue("Employee", null, $strDataEmployee)), array("style" => "width:$strDateWidth", $strEmpReadonly => $strEmpReadonly), "string", false);

    $f->addLabelAutoComplete("", "dataEmployee", "");
    $f->addInputAutoComplete(getWords("evaluator"), "dataEvaluator", getDataEmployee(getInitialValue("Evaluator")), array("style" => "width:$strDateWidth"), "string", false);
    $f->addLabelAutoComplete("", "dataEvaluator", "");
    $f->addSelect(getWords("request status"), "dataRequestStatus", getDataListRequestStatus(getInitialValue("RequestStatus"), true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);  

    $f->addSelect(getWords("branch"), "dataBranch", getDataListBranch(getInitialValue("Branch"), true), array("style" => "width:$strDefaultWidthPx"), "", false, false);  

    $f->addSelect(getWords("level"), "dataPosition", getDataListPosition(getInitialValue("Position"), true), array("style" => "width:$strDefaultWidthPx"), "", false, false);  
    $f->addSelect(getWords("grade"), "dataGrade", getDataListSalaryGrade(getInitialValue("Grade"), true), array("style" => "width:$strDefaultWidthPx"), "", false, false);  
    $f->addSelect(getWords("status"), "dataStatus", getDataListEmployeeStatus(getInitialValue("EmployeeStatus"), true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false, false);  
...
function changeStatus($db, $intStatus) {
    ...
    foreach ($_REQUEST as $strIndex => $strValue) 
    {
      ...
      {
        $strSQLx = "SELECT status
                    FROM hrd_evaluation_master
                    WHERE id = '$strValue' ";
//		  echo $strSQLx;die();
        ...
        { 
          ...
          {
            $strSQL .= "UPDATE hrd_evaluation_master SET $strUpdate status = '$intStatus'  ";
