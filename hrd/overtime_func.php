<?php
  ...
  */
  ...
  {
     $arrResult = array("total" => 0, "l1" => 0, "l2" => 0, "l3" => 0, "l4" =>0, "l5" => 0, "morning" => 0, "late" => 0, "early" => 0);
...
function calculateOvertimeByOTSchedule($db, $strDate, $strOvertimeStart, $strOvertimeFinish, $intEarlyOvertime = 0, $strOvertimeType = "", $intTipe = "", $bolBreak = true, $breakFlag = 0, $breakLinkID = "")
{
   ...
   $arrResult = array("total" => 0, "l1" => 0, "l2" => 0, "l3" => 0, "l4" =>0, "l5" => 0);
   $bol5WorkDay = false;
   $intWDay = getWDay($strDate);
...
//echo $fltMeal;
  ...
  function syncOvertimeApplication($db, $strDateFrom, $strDateThru, $strIDEmployee = "", $strKriteria = "")
  {
     ...
     $strSQL .= "WHERE t1.overtime_date BETWEEN '$strDateFrom' AND '$strDateThru' $strKriteria";
     if ($strIDEmployee != "")
     {
  ...
  function generateOTAttendanceSQL($strIDEmployee, $strDate, $arrAttRow, $arrOTRow, $bolAutoOT = false)
  {
    ...
    */
    ...
    else
    {
      ...
      else
      {
         ...
         {
           if (timeCompare($arrAttRow['attendance_finish'], $arrOTRow['start_plan']) <= 0 && timeCompare($arrAttRow['attendance_finish'], $arrOTRow['finish_plan']) > 0 && timeCompare($arrAttRow['attendance_start'], $arrAttRow['attendance_finish']) <= 0) 
           {
             $strOvertimeStart = $strOvertimeFinish = "";}
           else 
           {
    ...
    */
    ...
    {
      $arrOT = array('l1' => 0, 'l2' => 0, 'l3' => 0, 'l4' => 0, 'l5' => 0, 'total' => 0, 'is_overtime' => 'f'); 
		...
		*/
		if ($bolHoliday){
			...
			if ($bolIsShift){
				...
				*/
				if (strtotime($arrAttRow['attendance_start']) <= strtotime($shiftTypeData['start_time']) && strtotime($arrAttRow['attendance_finish']) >= strtotime($shiftTypeData['finish_time'])){
