<?php
  ...
  if (isset($_REQUEST['changeLanguageTo']))  $_SESSION['sessionLanguage'] = $_REQUEST['changeLanguageTo'];
  if (!DEFINED('CONFIGURATION_LOADED')) include_once("../global/configuration.php");
  DEFINE("VALID_APPLICATION", 1);
	...
	if (isset($_SESSION['sessionBasePath']))
      $basePath =  $_SESSION['sessionBasePath'];
    else
      $basePath =  "";
  ...
  */
  ...
  //Tidak jadi di-comment karena selain memakai include, memakai bbrp function dalam global.php
  ...
  $isCandidate = true;//($_SESSION['sessionGroupRole'] == ROLE_CANDIDATE);
  ...
  if(isset($_REQUEST['MrfId']) && isset($_REQUEST['MrfPos'])){
	  $arrData['MRF'] = $_REQUEST['MrfId'];
	  $tempMrfId = $arrData['MRF'];
	  $strTempSQL = "select request_number from hrd_recruitment_need where id = $tempMrfId";
	  $tempMrfId = $db->fetchrow($db->execute($strTempSQL));
	  $tempMrfId = $tempMrfId['request_number'];
	  $arrData['position'] = $_REQUEST['MrfPos'];
  }else{
	  $tempCandId = $_REQUEST['dataID'];
	  $strTempSQL = "select t2.id, t2.request_number from hrd_candidate AS t1 
	  LEFT JOIN hrd_recruitment_need AS t2 ON t1.id_recruitment_need = t2.id
	  WHERE t1.id = $tempCandId";
	  $tempMrfId = $db->fetchrow($db->execute($strTempSQL));
	  $arrData['MRF'] = $tempMrfId['id'];
	  $tempMrfId = $tempMrfId['request_number'];
  }
 ...
 // }
  ...
  //{
	...
	$f->addLabel(getWords("MRF No."), "", $tempMrfId);
	$f->addHidden("id_recruitment_need", $arrData['MRF']);
    $f->addLabel(getWords("position applied"), "", $arrData['position']);//, array("size"=> 50), "string", true, true, true, "", "", true, array("width" => 120));
	$f->addHidden("position", $arrData['position']);//, array("size"=> 50), "string", true, true, true, "", "", true, array("width" => 120));
  //}
  ...
  */
  ...
  $f->addFieldSet(getWords("educational information"), 2);
  ...
  */
  ...
  if (!isset($dataPrivilege['icon_file'])) $dataPrivilege['icon_file'] = 'blank.png';
  $pageIcon = "../images/icons/".$dataPrivilege['icon_file'];

  $strTemplateFile = "templates/candidate_edit_published.html";
  //candidate user
 //if ($isCandidate)
 // {
    //------------------------------------------------
...
//            </tr>";
  ...
  function saveData()
  {
    ...
    if (saveDataCandidate())
    {
      ...
      header("location: candidate_edit_published.php?dataID=".$f->getValue('dataID')."&message=".$f->message);
      exit();
    }
    return false;
  }
  ...
  {
	$exitLink = "candidate_open_exit.php";
