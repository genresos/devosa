<?php
  ...
  */
  ...
  {
    var $data;
    var $strID;       // id employee
    var $strCode; // NIK (employee_id)
    var $strName;
    var $arrDetail; // data-data yang lebih detail, dengan index sama dengan nama column di database
    var $arrDetailException; // data-data yang lebih detail, yang bukan bagian dari tabel hrd_employee - untuk keperluan copyEmployee
    ...
    {
      $this->arrDetail = array(); // kosongkan dulu, reset
      if (trim($strFields) == "") $strFields = "*";
      if ($strFields == "*") $strFields = "hrd_employee.* ";
    ...
    {
      if (isset($this->arrDetail[$strField])) return $this->arrDetail[$strField];
      else return "";
    }
    ...
    {
      $bolOK = false;
      if ($strOldID == "" || $strNewID == "" || $strNewCode == "") return false;
      
      if (count($this->arrDetail) == 0) $this->loadDataById($strOldID);
      $strFields = "";
  ...
  */
  class clsEmployees
  {
    ...
    var $arrCode;     // daftar nik -> id, untuk memudahkan pencarian id karyawan berdasarkan NIK
    ...
    */
    function loadData($strFields = "*", $strCriteria = "", $arrCriteria = array())
    {
      $this->arrEmployee = array(); // kosongkan dulu, reset
      if (trim($strFields) == "") $strFields = "*";
      if (isset($arrCriteria) && count($arrCriteria) > 0)
      {
        ...
        if ($strTmp != "") $strCriteria .= " AND id IN ($strTmp) ";
      }
      $strSQL = "
    ...
    */
    ...
    {
      if (isset($this->arrEmployee[$strID][$strField])) return $this->arrEmployee[$strID][$strField];
      else return "";
    }
    ...
    */
    function getTransitionEmployee($strDateFrom = "", $strDateTo = "", $strCriteria = "")
    {
      ...
      if ($strDateFrom == "") $strDateFrom = date("Y-m-d");
      if ($strDateTo == "") $strDateTo = date("Y-m-d");
      $arrResult = array();
