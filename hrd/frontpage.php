  ...
  */
  ...
  function getDataContract($db) {
    ...
    if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) {
      ...
      if ($arrUserInfo['isDeptHead']) $strSQL .= "AND department_code = '".$arrUserInfo['department_code']."' ";
      else if ($arrUserInfo['isGroupHead']) $strSQL .= "AND section_code = '".$arrUserInfo['section_code']."' ";
      else $strSQL .= "AND 1=2 "; // kosongkan isi
    }
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= "  <td align=center nowrap><a href=\"mutation_edit.php?btnRenew=Renew&$strLinkPar\">" .getWords("renew")." $strConfirm</a>&nbsp;</td>";
      $strResult .= "</tr>\n";
    }
  ...
  function getAlertActivity($db) {
    ...
    if ($arrUserInfo['id_employee'] != "") {
      ...
      while ($rowDb = $db->fetchrow($resDb)) {
        ...
        $strTeks = "Attendance Request: <a href=\"attendance_edit_employee.php?dataID=" .$rowDb['id']."\">". pgDateFormat($rowDb['attendance_date'], "d-M-y"). "</a> &nbsp; &raquo; <strong>";
      ...
      while ($rowDb = $db->fetchrow($resDb)) {
        ...
        $strTeks = "Training Participation : ". pgDateFormat($rowDb['training_date'], "d-M-y"). " &raquo; ";//<strong>". getWords($ARRAY_REQUEST_STATUS[$rowDb['status']])."</strong>";
        $strResult .= " <tr valign=top >\n";
        $strResult .= "  <td align=left nowrap>&nbsp;$strTeks </td>\n";
        $strResult .= " </tr>\n";
      }
      ...
      $resDb = $db->execute($strSQL);
      while ($rowDb = $db->fetchrow($resDb)) {
        $strTeks = "Training Need Evaluation : <a href=\"training_evaluation_edit.php?dataID=" .$rowDb['id']."\">". pgDateFormat($rowDb['training_date'], "d-M-y"). "</a> &raquo; ";//<strong>". getWords($ARRAY_REQUEST_STATUS[$rowDb['status']])."</strong>";
        $strResult .= " <tr valign=top >\n";
        $strResult .= "  <td align=left nowrap>&nbsp;$strTeks </td>\n";
        $strResult .= " </tr>\n";
      }
