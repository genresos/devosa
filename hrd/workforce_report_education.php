...
function getData($strKriteria)
{
  ...
  for ($i = 0; $i < $numCode; $i++) {
    ...
    if (!isset($arrEduCode[$i]) || $arrEduCode[$i] === '') {
      $arrEduCode[$i] = 'Empty Education';
    }
    if (isset($strDataPoints) && $strDataPoints !== '') {
      $strDataPoints .= ',';
    }
    $strDataPoints .= '{y: '.$numOfEmployee.', label: "'.$arrEduCode[$i].'"}';
  }
  $strGraphic = '<script type="text/javascript">
                   ...
                   {
                    title:{
                     text: "Education Graphical Report"
                    },
                    axisY: {
                     title: "Amount",
                     interval: 20
...
$strPageTitle = 'Education Graphical Report';
