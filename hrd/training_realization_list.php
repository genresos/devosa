<?php
  ...
  $f->addInput(getWords("training date"). " ".getWords("from"), "date_from", date("Y-m-d"), array(), "date", false, true, true);
  $f->addInput(getWords("date to"), "date_thru", date("Y-m-d"), array(), "date", false, true, true);  
  $f->addInput(getWords("employee id"), "employee_id", "", array(), "string", false, true, true);
  $f->addLabel(getWords(""),"employee_name", "", array(), "string");
  $f->addSelect(getWords("company"), "id_company", getDataListCompany(null, true, $emptyData), array("style" => "width:$intPxWidth"), "string", false, true, true);
  $f->addSelect(getWords("division"), "division_code", getDataListDivision(null, true, $emptyData, $objUP->genFilterDivision()), array("style" => "width:$intPxWidth"), "string", false, true, true);
  $f->addSelect(getWords("department"), "department_code", getDataListDepartment(null, true, $emptyData, $objUP->genFilterDepartment()), array("style" => "width:$intPxWidth"), "string", false, true, true);
  $f->addSelect(getWords("area"), "id_wilayah", getDataListWilayah(null, true, $emptyData), array("style" => "width:$intPxWidth"), "string", false, true, true);
...
/*
  ...
  */
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("request date"), "request_date", array("nowrap" => "nowrap"), array("align" => "center"), true, true, "", "", "date", true, 12));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("employee id"), "employee_id", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("name"), "employee_name", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("department"), "department_name", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(strtoupper(getWords("no.")), "request_number", array(), array('nowrap' => 'nowrap'), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("requested by"), "employee_name", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("training topic"), "training_topic", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "printTopic()", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("training type"), "training_type", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("category"), "training_category", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("expected result"), "result", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("institution"), "name_vendor", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("instructor"), "trainer", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("place"), "address", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("training cost"), "p_cost", array("nowrap" => "nowrap"), array("nowrap" => "nowrap", "align" => "right"), true, true, "", "printNumeric()", "integer", true, 10));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("other cost"), "p_cost_other", array("nowrap" => "nowrap"), array("nowrap" => "nowrap", "align" => "right"), true, true, "", "printNumeric()", "integer", true, 10));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("training date"), "date_details", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), false, false, "", "", "string", true, 12));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("training realization"), "realization", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), false, false, "", "", "string", true, 12));
  ...
  if (!($bolPrint || $bolExcel))
  {
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("realization"), "", array("rowspan" => 2), array('align' => 'center', 'nowrap' => 'nowrap'), false, false, "","printRealizationLink()", "string", false));
  ...
  */
  ...
  if ($f->getValue("date_from") != "" && $f->getValue("date_thru") != '')
    $strCriteria .= " AND id IN (SELECT DISTINCT id_request FROM hrd_training_request_detailtime 
                        WHERE trainingdate BETWEEN '" .$f->getValue("date_from"). "' AND '" .$f->getValue("date_thru"). "') ";
...
//--------------------------------------------------------------------------------
  ...
  {
    extract($params);
    return "<a href=\"training_realization_edit.php?dataRequestID=" .$record['id']."&dataEmployeeID=" .$record['id_employee_participant']. "\">" .getWords('edit realization'). "</a>";
  }
  ...
  function printTopic($params)
  {
    ...
    if ($bolPrint || $bolExcel) return $value;
    else return "
  ...
  function printLink($params)
  {
    ...
    $strResult = "<a href=\"javascript:openWindowDialog('training_request_print.php?dataID=" .$record['id']. "');\">" .getWords("print")."</a>";
...
// tambahan info
    ...
    foreach ($myDataGrid->checkboxes as $strValue)
    {
      ...
      if (!$objUP->isManagerHR())
        $strSQL .= "AND status <>  ".REQUEST_STATUS_APPROVED; // yang udah apprve gak boleh diedit
      if ($objUP->isRoleSupervisor())
      { // supervisor tidak boleh approve data yang dientry oleh dia sendiri
