<?php
  ...
  */
  ...
  function getDataMutation($db, $strDataDateFrom, $strDataDateThru, &$intRows, $strKriteria = "", $strOrder = "") 
  {
    ...
    $arrInfoDetail = array(); // terkait perubahan data 
    ...
    while ($rowDb = $db->fetchrow($resDb)) 
    {
      ...
      if ($rowDb['status'] >= REQUEST_STATUS_APPROVED) $strClass = "";
      ...
      if (isset($arrInfoDetail[$rowDb['id']]))
      {
        foreach($arrInfoDetail[$rowDb['id']] AS $x => $arrDb)
        {
          ...
          {
            if ($strChanges != "") $strChanges .= "<br />\n";
            $strChanges .= "Perpanjangan Kontrak : ". ((isset($ARRAY_EMPLOYEE_STATUS[$arrDb['status_new']])) ? getWords($ARRAY_EMPLOYEE_STATUS[$arrDb['status_new']]) : ""). " &nbsp;";
            if ($arrDb['contract_month'] > 0) $strChanges .= " Periode : ". $arrDb['contract_month']. " bulan &nbsp;";
          ...
          else if ($arrDb['mutation_type'] == MUT_CONTRACT_NEW)
          {
            ...
            if ($arrDb['contract_month'] > 0) $strChanges .= " Periode : ". $arrDb['contract_month']. " bulan &nbsp;";
            $strChanges .= "Mulai : " . pgDateFormat($arrDb['status_date_from'], "d-M-Y")." &nbsp;";
            $strNewNIK = ($arrDb['new_employee_id'] == "") ? "[<a class='bgError' href='mutation_edit.php?dataID=" .$rowDb['id']."'>" .getWords('edit new ID')."</a>]" : $arrDb['new_employee_id'];
          ...
          else if ($arrDb['mutation_type'] == MUT_ROTATION)
          {
            ...
            */
            if ($arrDb['position_new'] != "") $strC .= " ".$arrDb['position_new']."  ";
            if ($strC != "") $strChanges .= "<br />\n $strC ";
          }
          else if ($arrDb['mutation_type'] == MUT_DIVISION)
          {
            $strChangesType = ""; 
            if ($rowDb['new_position_type'] == 1) $strChangesType = " (Rangkap) ";
            else if ($rowDb['new_position_type'] == 2) $strChangesType = " (Berhenti) ";
            ...
            if ($arrDb['division_new'] != "") $strC .= " ".$arrDb['division_new']." (DIV) ";
            if ($arrDb['department_new'] != "") $strC .= " ".$arrDb['department_new']." (DEPT) ";
            if ($arrDb['section_new'] != "") $strC .= " ".$arrDb['section_new']." (UNIT) ";
            if ($arrDb['subsection_new'] != "") $strC .= " ".$arrDb['subsection_new']." (SECT) ";
            if ($strC != "") $strChanges .= "<br />\n $strC ";
            if ($arrDb['company_new'] != "") $strChanges .= "<br />\n ". ((isset($arrCompany[$arrDb['company_new']])) ? $arrCompany[$arrDb['company_new']] : "");
            if ($arrDb['wilayah_new'] != "") $strChanges .= " - ". ((isset($arrWilayah[$arrDb['wilayah_new']])) ? $arrWilayah[$arrDb['wilayah_new']] : "");
          }
          else if ($arrDb['mutation_type'] == MUT_PROMOTION)
          {
            $strChangesType = "Promosi Menjadi ".$arrDb['position_new']; 
            if ($rowDb['new_position_type'] == 1) $strChangesType = "Rangkap Jabatan ".$arrDb['position_new'];
            else if ($rowDb['new_position_type'] == 2) $strChangesType = "Berhenti Sebagai ".$arrDb['position_old'];
            
            if ($strChanges != "") $strChanges .= "<br />\n";
            
            $strChanges .= $strChangesType; //"Promosi Menjadi " . $arrDb['position_new'];
            if ($arrDb['grade_new'] != "") $strChanges .= ", Grade ".$arrDb['grade_new'].", ";
          ...
          else if ($arrDb['mutation_type'] == MUT_ACTING)
          {
            ...
            if ($rowDb['new_position_type'] == 1) $strChangesType = "Rangkap Jabatan Sementara ".$arrDb['position_new'];
            else if ($rowDb['new_position_type'] == 2) $strChangesType = "Berhenti Sebagai ".$arrDb['position_old'];
          ...
          {
            if ($strPersonal != "") $strPersonal .= "<br />\n";
            $strPersonal .= "* Ganti Alamat : " . nl2br($arrDb['note']);
            if ($arrDb['note2'] != "") $strPersonal .= " ".$arrDb['note2'];
          }
          ...
          else if ($arrDb['mutation_type'] == MUT_MARRIED)
          {
            ...
            if ($arrDb['note'] != "") $strPersonal .= " Dengan ".$arrDb['note'];
          }
          ...
          else if ($arrDb['mutation_type'] == MUT_BANK_ACCOUNT)
          {
            ...
            if ($arrDb['note'] != "") $strPersonal .= " No. ".$arrDb['note'];
          }
          else if ($arrDb['mutation_type'] == MUT_ALLOWANCE)
          {
            if ($strPersonal != "") $strPersonal .= "<br />\n";
            if ($arrDb['note2'] == 1) $strPersonal .= "* Perubahan Tunjangan ".$arrDb['note'];
            else if ($arrDb['note2'] == 2) $strPersonal .= "* Pencabutan Tunjangan ".$arrDb['note'];
            else $strPersonal .= "* Penambahan Tunjangan ".$arrDb['note'];
