<?php
  ...
  function saveData($db, &$strError) {
    ...
    if ($rowDb = $db->fetchrow($resDb)) {
      ...
      if ($strID != "" && $strDataDate != "") {
        ...
        $arrLembur = calculateOvertime($db, $strDataDate, $strNormalStart, $strNormalFinish, $strAttendanceStart, $strAttendanceFinish);
  ...
  if ($db->connect()) {
    ...
    $strInputNormalStart = "<input type=input name=dataNormalStart id=dataNormalStart size=5 maxlength=5 value=\"" .substr($arrData['normal_start'],0,5). "\" readonly>";
    $strInputNormalFinish = "<input type=input name=dataNormalFinish id=dataNormalFinish size=5 maxlength=5 value=\"" .substr($arrData['normal_finish'],0,5). "\" readonly>";
    $strInputActualStart = "<input type=input name=dataAttendanceStart id=dataAttendanceStart size=5 maxlength=5 value=\"" .substr($arrData['actual_start'],0,5). "\" readonly $strLateMark>";
    $strInputActualFinish = "<input type=input name=dataAttendanceFinish id=dataAttendanceFinish size=5 maxlength=5 value=\"" .substr($arrData['actual_finish'],0,5). "\" readonly> $strLateText";
    $strInputChangeStart = "<input type=input name=dataChangeStart id=dataChangeStart size=5 maxlength=5 value=\"" .substr($arrData['change_start'],0,5). "\" class='time-empty' $strDisabledRevision>";
    $strInputChangeFinish = "<input type=input name=dataChangeFinish id=dataChangeFinish size=5 maxlength=5 value=\"" .substr($arrData['change_finish'],0,5). "\" class='time-empty' $strDisabledRevision>";
