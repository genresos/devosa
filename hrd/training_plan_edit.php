...
//
	...
	$resDb = $db->execute($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
		...
		if (empty($arrData['doc'])) {
		  ...
		  if($strDataDoc != "" && $bolCanEdit) $strFileOption .= "<input name=\"btnDeleteDoc\" type=\"button\" id=\"btnDelete\" value=\"$strWordsDeleteFile\" onClick=\"deleteFile($strDataID);\"></td>";
  ...
  function getTrainingParticipant($db, $strDataID = "") {
	...
	//$strAction = "onfocus = \"alert('test');\"";
        ...
        if ($strTargetElements != "") $strTargetElements .= ", ";
...
//adnan
  ...
  function saveData($db, &$strError) {
    ...
    if (!is_numeric($strDataParticipant)) $strDataParticipant = 1;
    if (!is_numeric($strDataDuration)) $strDataDuration = 1;
      ...
      if ($strDataID == "") {
	   ...
	   $resExec = $db->execute($strSQL);
		...
		if (is_uploaded_file($_FILES["dataTrainingSyllabus"]['tmp_name'])) {
		  ...
		  $strNamaFile = $strID."_".strtolower(substr($_FILES["dataTrainingSyllabus"]['name'],0, -(strlen($arrNamaFile[count($arrNamaFile) -1])+1)));
...
//DELETE FILE - untuk syllabus
  ...
  if ($db->connect()) {
	...
	if($arrData['dataCreator']){$employeeCode=getEmployeeCode($db,$arrData['dataCreator']); $employeeName=getEmployeeName($db,$arrData['dataCreator']);  }
   ...
   $strLabelEmployee= "<strong id=employee_name>". $employeeName."</strong>";
	...
	$strInputInstitution = getTrainingInstitutionList($db,"dataInstitution", $arrData['dataInstitution'], "","", "style=\"width:$strDefaultWidthPx\"");
	$strInputInstructor = getTrainingInstructorList($db,"dataInstructor", $arrData['dataInstructor'], "","", "style=\"width:$strDefaultWidthPx\" ");
    ...
    else $strDeptKriteria = "";
	
	$strInputTrainingSyllabus = "<input name=\"dataTrainingSyllabus\" type=\"file\" id=\"dataTrainingSyllabus\" size=\"42\" >";
	$strInputDepartment = getDepartmentList($db,"dataDepartment",$arrData['dataDepartment'], "", "$strDeptKriteria", " style=\"width:$strDefaultWidthPx \" $strReadonly");
    $strInputDivision    = getDivisionList($db, "dataDivision", $arrData['dataDivision'], $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" ". $ARRAY_DISABLE_GROUP['division']);
    $strInputSection        = getSectionList($db,"dataSection", $arrData['dataSection'], $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" ". $ARRAY_DISABLE_GROUP['section']);
    $strInputSubSection     = getSubSectionList($db,"dataSubSection", $arrData['dataSubSection'], $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" ". $ARRAY_DISABLE_GROUP['sub_section']);
	$strInputCompetencyTraining = getCompetencyTrainingList($db, "dataCompetency", $arrData['competency'], "", "", "style=\"width:$strDefaultWidthPx\" id=dataCompetency");
	$strInputDomainTraining = getDomainTrainingList($db, "dataDomain", $arrData['domain'], "", "", "style=\"width:$strDefaultWidthPx\" id=dataDomain");
	$strInputTypeTraining = getTypeTrainingList($db, "dataType", $arrData['dataType'], "", "", "style=\"width:$strDefaultWidthPx\" id=dataType");
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 die();
    ...
    $strInputYear = getYearList("dataYear",$arrData['dataYear'], "", " style=\"width:$strDefaultWidthPx \" $strReadonly");
    $strInputMonth = getMonthList("dataMonth",$arrData['dataMonth'], "", " style=\"width:$strDefaultWidthPx \" $strReadonly");
