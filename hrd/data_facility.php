<?php
  ...
  include_once('../classes/hrd/hrd_facility.php');
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("name"), "name", array('width' => '150'), array('nowrap' => '')));
  ...
  $strSQLCOUNT  = "SELECT COUNT(*) AS total FROM hrd_facility ";
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    $dataHrdFacility = new cHrdFacility();
    $data = array(
    ...
    {
      // data baru
      $bolSuccess = $dataHrdFacility->insert($data);
      $intDataID = $dataHrdFacility->getLastInsertId();
    } 
    else 
    {
      $bolSuccess = $dataHrdFacility->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
      $intDataID = $f->getValue('dataID');
    }
    if ($bolSuccess) $f->setValue('dataID', $intDataID);
    
    $f->message = $dataHrdFacility->strMessage;
  } // saveData
  ...
  function deleteData() 
  {
    ...
    $dataHrdFacility = new cHrdFacility();    
    $dataHrdFacility->deleteMultiple($arrKeys);
    
    $myDataGrid->message = $dataHrdFacility->strMessage;
  } //deleteData
