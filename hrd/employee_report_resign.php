<?
  ...
  //---- INISIALISASI ----------------------------------------------------
  $strWordsListOfResignedEmployee   = getWords("list of resigned employee");
...
//    echo $strCriteria;
    ...
    if ($isExport)
    {
      ...
      $myDataGrid->strFileNameXLS = "resigned_employee_list.xls";
      $myDataGrid->strTitle1 = getWords("list of resigned employee");
    }
    if (!$bolPrint)
    {
      $myDataGrid->addButtonExportExcel("Export Excel", "resigned_employee_list.xls", getWords("list of resigned employee"));
    }
...
*/
  ...
  if ($db->connect()) {
    ...
    $strKriteria .= "AND (resign_date BETWEEN '$strDate' AND '$strDateThru') ";
...
//    $strKriteria .= " OR (due_date BETWEEN '$strDate' AND '$strDateThru'))";
    if ($bolCanView) 
    {
      ...
      else
      {
        ...
        $myDataGrid->caption = getWords("list of resigned employee");
      }
