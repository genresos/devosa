<?php
  ...
  */
  class clsBonusCalculation
  {
    ...
    var $strDataID;     // data ID dari master bonus
    var $strDataDate;   // tanggal perhitungan bonus
    var $strDataTaxDate;// tanggal perhitungan pajak,jika ada
    var $intStatus;     // status perhitungan bonus
    
    var $arrCompany;    // array daftar company, atau entitas yang mendapat hak bonus, karena gak semua
    var $arrEmployee;   // array data karyawan
    var $arrDivision;   // array data divisi
    var $fltShare;      // nilai share service
    ...
    */
    ...
    {
      $arrResult = array();
      if ($strDate == "") return $arrResult;
      
      $arrMut = array(); // menyimpan info mutasi karyawan
      ...
      while ($row = $this->data->fetchrow($res))
      {
        if (isset($arrMut[$row['id_employee']])) 
        { // bandingkan dulu tanggalnya, ambil yang paling lama
    ...
    */
    function saveData()
    {
      ...
      if ($intShare > 0 && $fltTotalShare <> 0) $fltShare = ($fltTotalShare / $intShare);
      ...
      if ($bolOK) $this->data->execute("commit");
    ...
    */
    function setFinish()
    {
      if ($this->strDataID != "")
      {
        ...
        ";
        $resExec = $this->data->execute($strSQL);
        if ($resExec != false) $this->intStatus = $intStatus;
      }
    }
