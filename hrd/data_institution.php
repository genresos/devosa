<?php
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("institution code"), "dataCode", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);  
    $f->addInput(getWords("institution name"), "dataName", "", array("size" => 30, "maxlength" => 63), "string", true, true, true);  
    $f->addInput(getWords("address"), "dataAddress", "", array("size" => 30, "maxlength" => 127), "string", true, true, true);  
    $f->addInput(getWords("phone"), "dataPhone", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);  
    $f->addInput(getWords("contact person"), "dataPIC", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);
	$f->addInput(getWords("subject"), "subject", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);	
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("institution code"), "institution_code", array('width' => '150'),array('nowrap' => '')));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    $data = array("institution_code" => $f->getValue('dataCode'),
    ...
    {
      $bolSuccess = $dataHrdInstitution->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    if ($bolSuccess)
    {
      if ($isNew)
