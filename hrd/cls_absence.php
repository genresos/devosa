<?php
  ...
  */
  class clsAbsenceReport
  {
    ...
    var $strStartDate; // tanggal awal
    var $strFinishDate; // tanggal akhir
    ...
    var $strIDEmployee; // id dari karyawan, jika ada atau khusus untuk 1 karyawan saja
    var $strEmployeeFilter; // khusus untuk filter pengambilan data karyawan
                            // format query: "AND xxxx"
    var $arrData = array(); // data disimpan dalam array saja
                            // index adalah id_employee
    var $arrAbsenceType = array();
    var $arrShiftType   = array();
    var $arrShiftEmployee = array(); // daftar karyawan yang memiliki jadwal shift
    var $objWork;   // objek untuk fasilitas perhitungan waktu kerja - cls_worktime.php
    var $objShift;  // objek untuk ambil info terkait shift - cls_shift.php
    ...
    function clsAbsenceReport($db, $strStartDate, $strFinishDate, $strIDEmployee = "", $strEmployeeFilter = "")
    {
      ...
      if ($this->strIDEmployee != "") $this->strEmployeeFilter .= "AND id = '" .$this->strIDEmployee."' ";
      $this->arrData = array();
      ...
      $this->intTotalWorkDay = $this->objWork->getTotalWorkDay($db,$strStartDate, $strFinishDate);
    ...
    */
    ...
    {
      if (isset($this->arrData[$strIDEmployee])) unset($this->arrData[$strIDEmployee]);
      $this->arrData[$strIDEmployee] = array(
        ...
        "total_tl"  => 0, // total kehadiran (termasuk di hari libur)
        ...
        "absence_detail"    => array(), // total absence dikelompokkan berdasar jenis absensi
        "shift_detail"      => array(), // total shift dikelompokkan berdasar jenis absensi
      );
      ...
      else $this->arrData[$strIDEmployee]['total_workday'] = $this->intTotalWorkDay;
    ...
    */
    function generateAttendanceReport()
    {
      ...
      $strSQL .= "SUM(CASE WHEN (late_duration > " .LATE_TOLERANCE." AND not_late = 'f' AND holiday = 0 AND is_absence = 'f') THEN 1 ELSE 0 END) AS total_late, "; // cari total keterlambatan, dalam hari
      ...
      $strSQL .= "SUM(CASE WHEN (not_late = 't' AND holiday = 1 AND is_absence = 'f') THEN 0 ELSE t1.late_duration END) AS total_late_min, "; // total telat, dalam menit
      ...
      while ($rowDb = $this->data->fetchrow($resDb))
      {
        ...
        if (!isset($this->arrData[$strID])) $this->initData($strID);
        ...
        $this->arrData[$strID]['total_late']      = $rowDb['total_late'];
        ...
        if (isset($this->arrData[$strID]['shift_detail'])) $this->arrData[$strID]['total_shift'] = array_sum($this->arrData[$strID]['shift_detail']);
      }
      ...
      $addSQL = "SELECT COUNT(t1.id_employee) AS duration, t1.id_employee
                 ...
                 WHERE t2.is_leave = TRUE AND treated_as_attend = TRUE AND t1.absence_date between '".$this->strStartDate."' and '".$this->strFinishDate."'
                 AND t3.status >= 2
                 GROUP BY t1.id_employee ";

     $resAddDb = $this->data->execute($addSQL);
     $addAttendance = 0;
     while ($rowAddDb = $this->data->fetchrow($resAddDb))
     {
         $strID = $rowAddDb['id_employee'];
         if (!isset($this->arrData[$strID])) $this->initData($strID);
         $this->arrData[$strID]['total_tl'] = $rowAddDb['duration'];

         $this->arrData[$strID]['total_attendance'] += $this->arrData[$strID]['total_tl']; // total kehadiran
     }
    ...
    */
    function generatePartialAbsenceReport()
    {
      ...
      while ($rowDb = $this->data->fetchrow($resDb))
      {
        ...
        if ($rowDb['late'] > 0 )
        {
          $intLate[$strID] = ($rowDb['late'] < LATE_TOLERANCE) ? 0 : round(ceil($rowDb['late']/30)*30)/60;//$rowDb['late'] / 60;
          if (isset($arrAbsencePartial[$rowDb['attendance_date']][$strID][PARTIAL_ABSENCE_LATE]) && is_numeric($arrAbsencePartial[$rowDb['attendance_date']][$strID][PARTIAL_ABSENCE_LATE]['approved_duration']))
        ...
        if ($rowDb['early'] > 0 )
        {
          $intEarly[$strID] = ($rowDb['early'] < LATE_TOLERANCE) ? 0 : round(ceil($rowDb['early']/30)*30)/60;//$rowDb['early'] / 60 ;
          if (isset($arrAbsencePartial[$rowDb['attendance_date']][$strID][PARTIAL_ABSENCE_EARLY]) && is_numeric($arrAbsencePartial[$rowDb['attendance_date']][$strID][PARTIAL_ABSENCE_EARLY]['approved_duration']))
          ...
          if ($intEarly > 0)
          {
            ...
            if ($intLate == 0) $this->arrData[$strID]['total_late_early']++;
          }
        }
        else
          $intEarly[$strID] = 0;
    ...
    */
    ...
    function getDataShiftHour($strIDEmployee)
    {
      ...
      if (isset($this->arrData[$strIDEmployee]['shift_detail']))
      {
        foreach ($this->arrData[$strIDEmployee]['shift_detail'] AS $strCode => $intTotal)
        {
          $fltHour =   getIntervalHour($this->arrShiftType[$strCode]['start_time'], $this->arrShiftType[$strCode]['finish_time']);
