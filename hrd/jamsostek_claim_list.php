<?php
  ...
  function getData($db)
  {
    ...
    if ($db->connect())
    {
      ...
      $myDataGrid->caption = getWords(strtoupper(vsprintf(getWords("list of %s"), getWords("jamsostek claim") . " " . getwords("form"). " KK2")));
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("created"), "date_request", "", array('nowrap' => '')));
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("position"), "position_code",  "", array('nowrap' => ''), false, false, "","getPositionName()"));
      if ($bolCanEdit)
      ...
      $strSQLCOUNT  = "SELECT COUNT(*) AS total FROM hrd_jamsostek_claim_kk2 AS t1 LEFT JOIN hrd_employee AS t2 ON t1.id_employee = t2.id ";
  ...
  function deleteData() 
  {
    ...
    {
      $strSQL = "DELETE FROM hrd_jamsostek_claim_kk2 WHERE id = $strValue";
  ...
  if ($db->connect()) 
  {
    ...
    $f->addSelect(getWords("position"), "dataPosition", getDataListPosition($_getInitialValue("Position"), true), array("style" => "width:$strDefaultWidthPx"), "", false);  
