    ...
    foreach ($arrTableName as $strTableName => $strPK)
    {
       ...
       foreach ($arrTempData as $arrTempRow) 
       {
          foreach ($arrTempRow as $strHeader => $arrContent)
             if (!(is_numeric(substr($strHeader,0,1)))) $arrTableHeader[$strTableName][$i++] = array("text" => $strHeader, "width" => "", "type" => ""); 
          break;
       }
       ...
       foreach ($arrTempData as $arrRow)
       {
          ...
          foreach ($arrRow as $strHeader => $strContent)
             if (!(is_numeric($strHeader))) $arrTableContent[$strTableName][$i][$j++] = $strContent;
          $i++;
       }
    }
  ...
  function importData($db, $flExcelImport)
  {
     ...
     if (is_uploaded_file($flExcelImport['tmp_name']))
     {
        if (substr($flExcelImport['name'],strlen($flExcelImport['name'])-3,3) == "xls")
        {
           ...
           if (file_exists($strNewFile)) unlink($strNewFile);
                 ...
                 for ($intRow = 2; $intRow <= count($arrCell)+1 ; $intRow++)
                 {
                    ...
                    for ($i = 2; $i < 20; $i++)
                    {
                     //  if ($i == 7) echo $arrCell[$intRow][7]."|";
                       if (!isset($arrCell[$intRow][$i])) $arrCell[$intRow][$i] = "NULL";
                    ...
                    if ($arrCell[$intRow][5] != "NULL") $arrCell[$intRow][5] = excelSerialDateToString($arrCell[$intRow][5]);
                    if ($arrCell[$intRow][8] != "NULL") $arrCell[$intRow][8] = excelSerialDateToString($arrCell[$intRow][8]);
                    if ($arrCell[$intRow][9] != "NULL") $arrCell[$intRow][9] = excelSerialDateToString($arrCell[$intRow][9]);
                    if ($arrCell[$intRow][10] != "NULL") $arrCell[$intRow][10] = excelSerialDateToString($arrCell[$intRow][10]);
                    if ($arrCell[$intRow][12] != "NULL") $arrCell[$intRow][12] = excelSerialDateToString($arrCell[$intRow][12]);
