<?php
  ...
  include_once '../global/email_func.php';
	...
	//sendDirectMail('Test','Test','','','heri500@gmail.com');
  ...
  function saveData($db, &$strError)
  {
    ...
    $strApprovedOvertimeId = (isset($_REQUEST['dataApprovedOvertime'])) ? $_REQUEST['dataApprovedOvertime']  : null;

    if (isset($strApprovedOvertimeId) && $strApprovedOvertimeId != '') {
      $strSQL = "SELECT date_from_overtime, date_thru_overtime FROM hrd_salary_master WHERE id = $strApprovedOvertimeId AND is_overtime_only = TRUE;";
      $res = $db->execute($strSQL);
      if ($row = $db->fetchrow($res)) {
        $strDataOvertimeDateFrom = $row['date_from_overtime'];
        $strDataOvertimeDateThru = $row['date_thru_overtime'];
      }
    }
    ...
    $objSalary = new clsSalaryCalculation($db, "", $bolIrregular, $strDataDate, array("id_company" => $strDataCompany),  $strDataDateFrom, $strDataDateThru, $strDataSalaryDateFrom, $strDataSalaryDateThru, false,$strApprovedOvertimeId);
  ...
  function getDataGrid($db, $strCriteria, $bolLimit = true, $isFullView = false)
  {
    ...
    $strCriteria.= "AND (is_managerial is false or is_managerial is null) AND (is_overtime_only is false OR is_overtime_only is null)";
