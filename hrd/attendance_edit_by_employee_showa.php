<?php
  ...
  $dataPrivilege = getDataPrivileges("attendance_edit_by_employee.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove);
  ...
  function getData($db, $strDataDateFrom, $strDataDateThru, $strDataEmployee, &$intRows, $strKriteria = "", $intPage = 1, $bolLimit = false) {
  ...
  function saveData($db, &$strError) {
    ...
    for($i=1; $i <= $intTotal; $i++)
    {
      ...
      if ($strDataDate != "")
      {
        ...
        $varTemp = "";
        if($strNormalStart == "24:00" && (int)substr($strNormalStart, 0,2) > (int)substr($strAttendanceStart, 0,2) && (int)substr($strAttendanceStart, 0,2) < (int)substr($strNormalFinish, 0,2))
        {
            $strNormalStart = "00:00";
            $varTemp = "true";
        }
        
        $arrLembur = calculateOvertime($db, $strDataDate, $strNormalStart, $strNormalFinish, $strAttendanceStart, $strAttendanceFinish, "", "", $bolHoliday, true);
  ...
  if ($db->connect()) {
    ...
    if ($bolCanView) {
      if (validStandardDate($strDataDateFrom) && validStandardDate($strDataDateThru) && ($strDataEmployee != "") && isset($_REQUEST['btnShow'])) {
        // tampilkan hanya jika ada permintaan dan data tanggalnya tepat
        $strDataDetail = getData($db, $strDataDateFrom, $strDataDateThru, $strDataEmployee, $intTotalData, $strKriteria, $strDataView, $intCurrPage);
    ...
    $arrViewOption = array("","overtime", "late", "early leave");
    ...
    if ($strDataEmployee != "")
    {
      $rowDb = getEmployeeInfoByCode($db, $strDataEmployee,"employee_id, employee_name, position_code, division_code, department_code, section_code, sub_section_code");
