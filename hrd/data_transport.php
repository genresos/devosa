<?
  ...
  function getData($db, &$intRows, $strKriteria = "", $strOrder = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= "  <td align=right><input type=hidden name=detailAllowance$intRows value=\"" .(float)$rowDb['overtimeAllowance']. "\" disabled>" .standardFormat($rowDb['overtimeAllowance']). "&nbsp;</td>";
  ...
  function saveData($db, &$strError) {
    ...
    if ($strDataID != "") {
      ...
      for ($i = 1; $i <= $intMaxBus;$i++) {
        ...
        if ($strID == "") { //add new
          if ($strCode != "") {
            if (!is_numeric($strSeat)) { $strSeat = 0; };
