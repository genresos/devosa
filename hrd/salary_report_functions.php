<?php
  ...
  function getDataReport($db, &$intRows, $strKriteria = "", $intPage = 1, $bolLimit = true, $strOrder = "") 
  {
    ...
    if ($strComponent == "basicSalary") $strFields = "basicSalary";
    else if ($strComponent != "")
    {
      ...
      if (count($arrTmp) > 1)
      {
        ...
        else if ($arrTmp[0] == 1) $intComponentType = 1; // allowance
        else if ($arrTmp[0] == 2) $intComponentType = 2; // deduction
...
//     $strResult .= "</table>\n";
  ...
  {
    for($i=2; $i<=3; $i++) $GLOBALS['sTahun'.$i] = '';
    for($i=0; $i<=6; $i++) $GLOBALS['sNo'.$i] = '';
    for($i=0; $i<=14; $i++) $GLOBALS['sNPWP_No'.$i] = '';
    for($i=0; $i<=22; $i++) $GLOBALS['sNPWP_Nama'.$i] = '';
    for($i=0; $i<=22; $i++) $GLOBALS['sNama'.$i] = '';
    for($i=0; $i<=14; $i++) $GLOBALS['sNPWP_Emp'.$i] = '';
    for($i=0; $i<=45; $i++) $GLOBALS['strAlamat'.$i] = '';
    for($i=1; $i<=4; $i++) $GLOBALS['sBulan'.$i] = '';
    for($i=1; $i<=24; $i++) $GLOBALS['sGaji'.$i] = 0;
    for($i=0; $i<=1; $i++) $GLOBALS['sBulan_24a_'.$i] = '';
    for($i=0; $i<=3; $i++) $GLOBALS['sTahun_24a_'.$i] = '';
    for($i=0; $i<=1; $i++) $GLOBALS['sBulan_24b_'.$i] = '';
    for($i=0; $i<=3; $i++) $GLOBALS['sTahun_24b_'.$i] = '';
    for($i=0; $i<=1; $i++) $GLOBALS['sTgl'.$i] = '';
    for($i=0; $i<=1; $i++) $GLOBALS['sBln'.$i] = '';
    for($i=0; $i<=3; $i++) $GLOBALS['sThn'.$i] = '';
    for($i=0; $i<=19; $i++) $GLOBALS['sPerusahaan'.$i] = '';
    for($i=0; $i<=14; $i++) $GLOBALS['sNPWP_Co'.$i] = '';
    ...
    for($i=0; $i<=1; $i++) $GLOBALS['sTgl'.$i] = $strTgl[$i];
    for($i=0; $i<=1; $i++) $GLOBALS['sBln'.$i] = $strBln[$i];
    for($i=0; $i<=3; $i++) $GLOBALS['sThn'.$i] = $strThn[$i];
  ...
  function generateForm1721A1($db, $strYear, $strEmployeeID)
  {
    ...
    if (strlen($strYear) == 4)
    {  
      for($i=2; $i<=3; $i++) $GLOBALS['sTahun'.$i] = $strYear[$i];
    }
    ...
    {
      foreach($row AS $key => $val) $row[$key] = strtoupper($val);
      $strIDEmployee = $row['idEmployee'];
    
      // no urut, sementara dianggap nik saja
      $strNo = addPrevZero($row['employeeID'], 7);
      for($i=0; $i<=6; $i++) $GLOBALS['sNo'.$i] = $strNo[$i];
      
      $intLen = strlen($row['employeeName']);
      for($i=0; $i<$intLen; $i++) $GLOBALS['sNama'.$i] = $row['employeeName'][$i];
      ...
      for($i=0; $i<$intLen; $i++) $GLOBALS['strAlamat'.$i] = $row['primaryAddress'][$i];
    ...
    if ($strIDEmployee != "")
    {
      ...
      while ($row = $db->fetchrow($res))
      {
        ...
        if ($GLOBALS['sGaji10'] > $fltMaxPosition) $GLOBALS['sGaji10'] = $fltMaxPosition;
        else
        {
          // hanya hitung jika poin 10 belum mencapai maksimal
          $GLOBALS['sGaji11'] = $GLOBALS['sGaji8'] * TAX_POS_RATE;
          if ($GLOBALS['sGaji11'] > $fltMaxPosition) $GLOBALS['sGaji11'] = $fltMaxPosition;
        }
      ...
      for($i=1; $i<=24; $i++) 
        if ($GLOBALS['sGaji'.$i] != "") $GLOBALS['sGaji'.$i] = standardFormat($GLOBALS['sGaji'.$i]);
    }
