<?php
 ...
 // $tblTrainingCategoryType = new cHrdTrainingType();
  ...
  {
    $dataTrainingType = $tblTrainingType->findAll("id_category = '".$strIDCategory."'", "id, code, name", "", null, 1, "id");
  ...
  if($strDataID != "") 
  {
    ...
    $arrTrainingType = $tblTrainingType->findAll("category = '".$arrData['dataTrainingCategory']."'", "", "", null, 1, "id");
  }
  else
    $arrData = getDataByID("");
  ...
  // ------------------------------------------------------------------------------------------------------------------------------
    if ($bolCanEdit)
    {
      ...
      $f->addInputAutoComplete(getWords("employee ID"), "dataEmployee", getDataEmployee($strDataEmployee), "style='width:350px' ". $strEmpReadonly, "string", true);
      $f->addLabelAutoComplete("", "dataEmployee", "");
      $f->addLiteral("", "", "");

      $f->addSelect(getWords("training category"), "dataTrainingCategory", getDataListTrainingCategory($arrData['training_category']), array("onChange" => "switchTrainingType(this.value, '".$arrData['id_training_type']."');", "style" => "width:350px"), "", true, true, true, "", "", true, null);
      foreach($dataTrainingCategory as $strIDCategory => $arrCategoryDetail)
      {
        ...
        $f->addSelect("", "dataTempTrainingType_".$strIDCategory, getDataListTrainingType($arrData['id_training_type'], false, "", "id_category = '".$strIDCategory."'"), array("class" => "classTraining", "onChange" => "document.formInput.submit()",  "style" => "width:350px"), "", false, $bolEnable, true);
      }
      $f->addHidden("dataTrainingType", getPostValue("dataTempTrainingType_".getPostValue("dataTrainingCategory")));


      $f->addInput(getWords("date from"), "dataDateFrom", $arrData['date_from'], array("style" => "width:$strDateWidth"), "date", true, true, true);
      $f->addInput(getWords("date thru"), "dataDateThru", $arrData['date_thru'], array("style" => "width:$strDateWidth"), "date", true, true, true);

      $f->addInput(getWords("topic"), "dataTrainingTopic", $arrData['topic'], array("size" => 79, "maxlength" => 255), "string", true, true, true);  
      $f->addInput(getWords("purpose"), "dataPurpose", $arrData['purpose'], array("size" => 79, "maxlength" => 255), "string", false, true, true);  
      $f->addInput(getWords("expected result"), "dataExpectedResult", $arrData['expected_result'], array("size" => 79, "maxlength" => 255), "string", false, true, true);  
      $f->addInput(getWords("place"), "dataPlace", $arrData['place'], array("size" => 79, "maxlength" => 255), "string", false, true, true);  
      $f->addInput(getWords("institution"), "dataInstitution", $arrData['institution'], array("size" => 79, "maxlength" => 255), "string", false, true, true);  
      $f->addInput(getWords("trainer/instructor"), "dataTrainer", $arrData['trainer'], array("size" => 79, "maxlength" => 255), "string", true, true, true);  
      $f->addInput(getWords("cost"), "dataCost", $arrData['cost'], array("size" => 79, "maxlength" => 255), "integer", false, true, true);  
      $f->addTextArea(getWords("note"), "dataNote", $arrData['note'], array("cols"=>76, "rows"=>2), "string", false, true, true);
      $f->addLiteral("","","");
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    if ($db->connect())
    {
      ...
      $tblTrainingRequest = new cHrdTrainingRequest();
      $data = array("id_employee" => getPostValue('dataIDEmployee'),
