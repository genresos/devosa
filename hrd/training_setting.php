...
//  include_once('../global.php');
  ...
  function getData($db) 
  {
	...
	$strTmpText .= "  <td>&nbsp;</td>\n";
    $strTmpText .= " </tr>\n";
    $arrKey = array("[#TRNAME]", "[#STYLE]", "[#ID]", "[#NAME]", "[#VALUE]", "[#NAME1]", "[#VALUE1]", "[#NAME2]", "[#TYPE]");

    $arrData['point'] .= " <tr class=tableHeader>\n";
    $arrData['point'] .= "  <td>&nbsp;</td>\n";
    $arrData['point'] .= "  <td nowrap><input type='text' name='[#NAME]' id='[#NAME]' value=\"" .strtoupper(getWords("answer"))."\" size=40 disabled style=\"color:black\" >";
    $arrData['point'] .= "<input type='text' name='[#NAME1]' id='[#NAME1]' value=\"" .strtoupper(getWords("weight"))."\" size=15 disabled style=\"color:black\" ><input type='text' name='[#NAME2]' id='[#NAME2]' value=\"" .strtoupper(getWords("question type"))."\" size=20 disabled style=\"color:black\" ></td>\n";
	$arrData['point'] .= "  <td>&nbsp;</td>\n";
    $arrData['point'] .= " </tr>\n";
    ...
    while ($rowDb = $db->fetchrow($resDb))
    {
      ...
      $arrValue = array($strTRName, "", $strID, $strName, $strValue, $strName1, $strValue1, $strName2, $strType );
    ...
    for ($i = 0; $i < $intAdd; $i++) 
    {
	  ...
	  $strName2 = "detailPointType$int1";
      $strType = getQuestionTypeList($db,$strName2,$rowDb['type']);
      $arrValue = array($strTRName, "style=\"display:none\"", $strID, $strName, $strValue, $strName1, $strValue1, $strName2, $strType);
      $arrData['point'] .= str_replace($arrKey, $arrValue, $strTmpText);
  ...
  function saveData($db, &$strError) 
  {
    ...
    for ($i = 1; $i <= $intTotalPoint; $i++)
    {
      ...
      if (!is_numeric($strWeight)) $strWeight = 0;
