<?php
  ...
  $dataPrivilege = getDataPrivileges("candidate_edit.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  ...
  if ($pregnant_month != "" && $pregnant_month > 0) $pregnant_month .= " bulan";
  ...
  function getData($strDataID)
  {
    ...
    if ($strDataID != "")
    {
      if ($rowDb = $tblCandidate->findById($strDataID))
      {
        ...
        {
          $arrTmp = $tblRel->find(array("code" => $rowDb['religion_code']), "name");
          if (isset($arrTmp['name'])) $rowDb['religion_code'] = $arrTmp['name'];
        }
        return $rowDb;
      }
    }
    ...
    foreach($arrMainFamily as $mainFamily)
    {
      ...
      if (isset($arrResult[$mainFamily['id']]))
      {
        ...
        if ($row['id_gender'] == FEMALE)
          ...
          $row['id_gender'] = "";
        $arrBirth = array();
        if ($row['birthplace']) $arrBirth[] = $row['birthplace'];
        if ($row['dob']) $arrBirth[] = $row['dob'];
...
<table class=language border=0 cellpadding=0 cellspacing=0>
 <tr>
  ...
  <th colspan=5>MENULIS</th>
    ...
    if ($strDataID != "")
    {
      ...
      {
        $arrDefault = array(1 => "1", "2", "3", "4", "5");
        foreach ($arrResult as $row)
        {
          $counter++;
          $arrInfo = array( // menyimpan info text untuk keterangan
            "L" => $arrDefault,
            "R" => $arrDefault,
            "S" => $arrDefault,
            "W" => $arrDefault
          ...
          for($i = 1; $i <= 5; $i++)
          {
            ...
            if ($i == 5) $$strClassL = "class=right"; else $$strClassL = "";
            $strClassR  = "strClassR".$i;
            if ($i == 5) $$strClassR = "class=right"; else $$strClassR = "";
            $strClassS  = "strClassS".$i;
            if ($i == 5) $$strClassS = "class=right"; else $$strClassS = "";
            $strClassW  = "strClassW".$i;
            if ($i == 5) $$strClassW = "class=right"; else $$strClassW = "";
          }
 ...
 <tr>
  ...
  <td $strClassW5>" .$arrInfo['W'][5]. "</td>
    ...
    if ($intSelisihRowCounter > 0) $intSelisihRowCounter = 0;
 ...
 <tr>
  ...
  function getQuestion($strDataID)
  {
    ...
    for($i = 1; $i <=2 ; $i++)
    {
      ...
      if ($arrData[$i]['is_yes'] == 't') $checkedY = $checked;
      else if ($arrData[$i]['is_yes'] == 'f') $checkedN = $checked;
 ...
 <tr height=18>
  ...
  <td class=xl32>&nbsp;</td>
    ...
    for($i = 16; $i <=17 ; $i++)
    {
      ...
      if (!isset($arrData[$i]['answer1'])) $arrData[$i]['answer1'] = "";
      if (is_numeric($arrData[$i]['answer1'])) $arrData[$i]['answer1'] = number_format($arrData[$i]['answer1']);
      if (!isset($arrData[$i]['answer2'])) $arrData[$i]['answer2'] = "";
      $strResult .= "
 <tr height=16 style='height:12.0pt' valign=top>
  <td height=16 class=xl391 style='height:12.0pt'>".$i.".</td>
  ...
  <td colspan=2 class=xl49 style='border-right:.5pt solid black;'>SAMPAI</td>
    ...
    if ($strDataID != "")
    {
      ...
      if (is_array($arrResult) && count($arrResult) > 0)
      {
        ...
        {
          $counter++;
          if ($row['is_passed'] == 't') $row['is_passed'] = 'LULUS';
          if ($row['is_passed'] == 'f') $row['is_passed'] = 'TIDAK LULUS';

          if ($row['academic'] != "")
          {
            $arrTmp = $tblEdu->find(array("code" => $row['academic']), "name");
            if (isset($arrTmp['name'])) $row['academic'] = $arrTmp['name'];
          }
    ...
    if ($strDataID != "")
    {
      ...
      if (is_array($arrResult))
      {
        foreach ($arrResult as $row)
        {
          ...
          if ($row['reference_position'] != "") $row['reference_position'] = "( ".$row['reference_position']." )";
          $strResult .= "
 <tr height=18 style='height:13.5pt'>
