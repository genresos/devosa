<?php
  ...
  $dataPrivilege = getDataPrivileges("attendance_list.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("id"), "dataId", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);  
    $f->addInput(getWords("question"), "dataQuestion", "", array("size" => 100, "maxlength" => 127), "string", true, true, true);  
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("id"), "id", array('width' => '150'),array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("question"), "question", array('width' => '200'), array('nowrap' => '')));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    $data = array("id" => $f->getValue('dataId'),
    ...
    {
      $bolSuccess = $dataHrdQuestion->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    if ($bolSuccess) $f->setValue('dataID', $data['id']);
