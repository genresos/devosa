<?php
  ...
  if ($db->connect()) 
  {
    ...
    $arrData['dataDateFrom']      = (getPostValue('dataDateFrom') != "" && validStandardDate(getPostValue('dataDateFrom'))) ? getPostValue('dataDateFrom') : date("Y-m")."-01";
    $arrData['dataDateThru']      = (getPostValue('dataDateThru') != "" && validStandardDate(getPostValue('dataDateThru'))) ? getPostValue('dataDateThru') : date("Y-m-d");
    ...
    if ($arrData['dataBank'] != "") {
      $strKriteria .= "AND t1.bank_code = '". $arrData['dataBank']. "' ";
    }
...
//--------------------------
  ...
  {   
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("bank account"), "bank_account", array("width" => 150),  array("align" => "center"), true, true, "", "", "string", true, 25));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("bank account name"), "bank_account_name", array("width" => 150),  array("align" => "center"), true, true, "", "", "string", true, 35));
    ...
    $strSQL       = "
      SELECT t1.id, employee_id, employee_name, bank_name, division_code, t1.bank_code,
      department_code, section_code, t1.branch_code, branch_name, bank_account, bank_account_name, SUM(approved_cost) AS total_claim 
      ...
      LEFT JOIN hrd_bank AS t4 ON t1.bank_code = t4.bank_code
    ...
    $strSQL .= " GROUP BY t1.id, employee_id, employee_name, bank_account, bank_account_name, bank_name, division_code , department_code , section_code , t1.branch_code , branch_name ";
    ...
    if ($strDataClaimThru != "")
    {
      $strSQLCOUNT .= "AND total_claim <= ".$strDataClaimThru." ";
