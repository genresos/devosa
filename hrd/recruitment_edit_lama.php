<?php
  ...
  $f->addInput("MRF No.", "request_number", $arrData['request_number'], array("size" => 20), "string", true, true, true, "", "", true, array("width" => 120));
  

    //bisa ubah tanggal
    $f->addInput(getWords("request date"), "recruitment_date", $arrData['recruitment_date'], array(), "date", true, true, true);
  ...
  $f->addSelect(getWords("branch"), "branch", getDataListBranch($arrData['branch_code'], true, $emptyData), array("style" => "width:$strDefaultWidthPx"), "string", false, true, true, "", "", true, array("width" => 120));
  $f->addSelect(getWords("band / grade"), "grade_code", getDataListSalaryGrade($arrData['grade_code'], true, $emptyData), array("style" => "width:$strDefaultWidthPx"), "string", false, true, true, "", "", true, array("width" => 120));
  $f->addSelect(getWords("level"), "position", getDataListPosition($arrData['position_code'], true, $emptyData), array("style" => "width:$strDefaultWidthPx"), "string", false, true, true, "", "", true, array("width" => 120));
  $f->addSelect(getWords("status"), "employee_status", getDataListEmployeeStatus($arrData['employee_status'], true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);  
  $f->addSelect(getWords("sex"), "gender", getDataListGender($arrData['gender'], true), array("style" => "width:$strDefaultWidthPx"), "string", false, true, true);
  ...
  $f->addSelect(getWords("company"), "id_company", getDataListCompany($arrData['id_company'], $bolCompanyEmptyOption, $arrCompanyEmptyData, $strKriteria2), array("style" => "width:$strDefaultWidthPx"), "", false);
  $f->addSelect(getWords("division"), "division_code", getDataListDivision($arrData['division_code'], true), array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['division'] == ""));
  $f->addSelect(getWords("department"), "department_code", getDataListDepartment($arrData['department_code'], true), array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['department'] == ""));
  $f->addSelect(getWords("section"), "section_code", getDataListSection($arrData['section_code'], true), array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['section'] == ""));
  $f->addSelect(getWords("sub section"), "sub_section_code", getDataListSubSection($arrData['sub_section_code'], true), array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['sub_section'] == ""));


  $f->addFieldSet(getWords("required information"), 2);
  $f->addInput(getWords("number required"), "number", $arrData['number'], array("size"=> 50), "integer", true, true, true, "", "", true, array("width" => 120));
