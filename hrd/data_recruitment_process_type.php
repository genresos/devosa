<?php
  ...
  $arrPrintType = array ( // sekedar menentukan apakah perlu di print form atau gak, di datanet, khusus interview
    0 => "",
    1 => "Interview Form",
    2 => "Driver Form"
  );
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("name"), "name", "", array("size" => 50), "string", true, true, true);  
    ...
    $f->addTextArea(getWords("note"), "note", "", array("cols" => 48, "rows" => 2), "string", false, true, true);  
    ...
    $f->addButton("btnAdd", getWords("add new"), array("onClick" => "javascript:window.location.href='data_recruitment_process_type.php'"));
	$formInput = $f->render();
  }
  ...
  $myDataGrid->addColumnNumbering(new DataGrid_Column("No.", "", array('width' => 30), array('nowrap'=>'nowrap')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("step"), "step", array('width' => 70), array('nowrap' => 'nowrap')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("name"), "name", array('width' => 200), array('nowrap' => 'nowrap'), true, true, "", ""));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("form"), "letter_type", null, array('nowrap' => ''), false, false, "", "printLetter()"));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    else 
    {
	  ...
	  if ($bolSuccess = $tbl->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data))
      {
        $dataName = $data['name'];
      }
    }
