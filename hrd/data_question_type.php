<?php
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("question type"), "dataQuestionType", "", array("size" => 31, "maxlength" => 31), "string", true, true, true);  
    $f->addInput(getWords("note"), "dataNote", "", array("size" => 31, "maxlength" => 127), "string", false, true, true);   
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("question type"), "question_type", array('width' => '150'), array('nowrap' => '')));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    $data = array("question_type" => $f->getValue('dataQuestionType'),
    ...
    {
      $bolSuccess = ($dataTrainingQuestionType->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data));
    }
    if ($bolSuccess)
    {
