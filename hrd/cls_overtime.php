  ...
  */
  class clsOvertimeReport
  {
    ...
    */
	...
	$this->arrOvertimeType = array("L1" => 1.5, "L2" => 2,"L2b" => 2, "L3" => 3, "L4" => 4);
    ...
    */
    function initData($strIDEmployee)
    {
      ...
      $this->arrData[$strIDEmployee] = array(
        ...
        "total_ot_1_workday"      => 0, // total ot, L1 dalam menit
        "total_ot_2_workday"      => 0, // total ot, L2 dalam menit
        "total_ot_3_workday"      => 0, // total ot, L3 dalam menit
        "total_ot_4_workday"      => 0, // total ot, L4 dalam menit
        "total_ot_1_holiday"      => 0, // total ot, L1 dalam menit
        "total_ot_2_holiday"      => 0, // total ot, L2 dalam menit
        "total_ot_3_holiday"      => 0, // total ot, L3 dalam menit
        "total_ot_4_holiday"      => 0, // total ot, L4 dalam menit
        ...
        "ot_hk_gt3h"  => 0, // total early auto ot, dalam hari.
    ...
    */
    ...
    { 
       ...
       $arrOTGroup1 = array(); // array untuk OT yang dihitung normal
       $arrOTGroup2 = array(); // array untuk OT selebihnya
       while (dateCompare($strDateTemp, $strDateThru) <=0)
       {
          //assign OT untuk date temp
          $objTodayOT = new clsOvertimeReport($this->data, $strDateTemp, $strDateTemp, $strIDEmployee); // cls_ambil data summary ot utk 1 hari
          $objTodayOT->generateOvertimeReport();
          if (!isset($objTodayOT->arrData[$strIDEmployee])) continue;
    ...
    function generateOvertimeSalaryReport($strSalaryDate)
    {
      ...
      $strSQL  = "
          SELECT COUNT(is_overtime) AS total_ot_day, t1.id_employee, 
            SUM(l1) AS total_ot_1, SUM(l2) AS total_ot_2, SUM(l2b) AS total_ot_2b, 
            SUM(l3) AS total_ot_3, SUM(l4) AS total_ot_4,
            SUM(CASE WHEN holiday = 0 THEN l1 ELSE 0 END) AS total_ot_1_workday,
            SUM(CASE WHEN holiday = 0 THEN l2 ELSE 0 END) AS total_ot_2_workday,
            SUM(CASE WHEN holiday = 0 THEN l3 ELSE 0 END) AS total_ot_3_workday,
            SUM(CASE WHEN holiday = 0 THEN l4 ELSE 0 END) AS total_ot_4_workday,
            SUM(CASE WHEN holiday = 1 THEN l1 ELSE 0 END) AS total_ot_1_holiday,
            SUM(CASE WHEN holiday = 1 THEN l2 ELSE 0 END) AS total_ot_2_holiday,
            SUM(CASE WHEN holiday = 1 THEN l3 ELSE 0 END) AS total_ot_3_holiday,
            SUM(CASE WHEN holiday = 1 THEN l4 ELSE 0 END) AS total_ot_4_holiday,
      ...
      (
          ...
          WHERE 
          (
            ((attendance_date BETWEEN '" .$this->strStartDate."' AND '" .$this->strFinishDate."') AND toa.is_outdated = 'f')
            OR
