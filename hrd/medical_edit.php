...
$strprosenPlatform='1'; // uddin: default platform outpation untuk family
...
function getData($db, &$intRows, $strDataID = "")
{
  ...
  //uddin 20150203
  //get information outpation platform for family
  if ($strIDEmployee!=""){
    $strSQL = "select * from hrd_medical_platform_additional a , hrd_medical_type b
    where a.role_type='l_fam_limit_out'
    and a.grade_code=(select grade_code from hrd_employee where id=".$strIDEmployee.")
    and b.code='RAWATJALAN'
    and a.id_medical_type=b.id";
    $resP = $db->execute($strSQL);
    if ($rowP = $db->fetchrow($resP))
    {
      $strprosenPlatform=$rowP["amount"]/100;
    }
  }
  //echo $strprosenPlatform;
  //echo $strSQL;
  //end
  ...
  if ($bolSave && $bolError)
  {
    ...
    for ($i = 1; $i <= $intShown; $i++)
    {
      ...
      if (isset($_REQUEST['detailName'.$i])) $strName = $_REQUEST['detailName'.$i];
      else continue;
  ...
  (isset($_REQUEST['detailMedicalDate'.$i])  && validStandardDate($_REQUEST['detailMedicalDate'.$i])) ? $strMedicalDate = $_REQUEST['detailMedicalDate'.$i] : $strMedicalDate = date("Y-m-d");
  (isset($_REQUEST['detailMedicalDateThru'.$i])  && validStandardDate($_REQUEST['detailMedicalDateThru'.$i])) ? $strMedicalDateThru = $_REQUEST['detailMedicalDateThru'.$i] : $strMedicalDateThru = date("Y-m-d");
  (isset($_REQUEST['detailClaimDate'.$i]) && validStandardDate($_REQUEST['detailClaimDate'.$i])) ? $strClaimDate = $_REQUEST['detailClaimDate'.$i] : $strClaimDate = date("Y-m-d");
  ...
  $strResult .= "<tr valign=top  id=\"detailRows$i\">\n";
    ...
    $strResult .= "  <td nowrap>" .getFamilyList($db, "detailName$i",$_REQUEST['detailName'.$i],"<option value=''> </option><option value=\"". $arrData['dataEmployeeName'] ."|-1\" $strSelect>" .$arrData['dataEmployeeName']. "</option>", $strFamCriteria). "&nbsp;<font color=\"red\">*</font></td>";
    //$strResult .= "  <td  nowrap>" .getMedicalTreatmentTypeList("detailType$i", true, $strType, "","onChange=\"changeMedicalType($i);\" "). /*<input type=checkbox id=\"detailRoom$i\" name=\"detailRoom$i\" $strCheckRoom $strDisableRoom>"&nbsp;".getWords("room")."*/"&nbsp;<font color=\"red\">*</font></td >";
    //$strResult .= "  <td nowrap>" .getMedicalTreatmentCodeList($db,"detailMedicalCode$i", $strMedicalCode,"","WHERE \"type\" = '$intType' ", "style=\"width:60px\""). "&nbsp;<font color=\"red\">*</font></td>";
    $strResult .= "  <td nowrap>" .getMedicalTreatmentCodeList_v2($db,"detailMedicalCode$i", $strMedicalCode,"","", "style=\"width:150px\""). "&nbsp;<font color=\"red\">*</font></td>";
    ...
    $strResult .= "  <td nowrap><input type=text size=15 maxlength=10 name=detailMedicalDate$i id=\"detailMedicalDate$i\" value=\"" .$strMedicalDate. "\" class='date'>&nbsp;";
      ...
      else
      {
        if ($strDataID != "")
        {
          ...
          if ($rowDb['relation'] == -1) $strSelect = "selected";
          ...
          $strResult .= "<tr valign=top  id=\"detailRows$intRows\">\n";
            ...
            $strResult .= "  <td nowrap>" .getFamilyList($db, "detailName$intRows", $rowDb['name']."|".$rowDb['relation'],"<option value=''> </option><option value=\"". $arrData['dataEmployeeName']. "|-1\" $strSelect>". $arrData['dataEmployeeName']. "</option>", $strFamCriteria). "&nbsp;<font color=\"red\">*</font></td>";
            //$strResult .= "  <td nowrap>" .getMedicalTreatmentTypeList("detailType$intRows", true, $rowDb['type'], "","onChange=\"changeMedicalType($intRows);\" "). /*"<input type=checkbox id=\"detailRoom$intRows\" name=\"detailRoom$intRows\" $strCheckRoom $strDisableRoom>&nbsp;".getWords("room").*/"&nbsp;<font color=\"red\">*</font></td>";

            //$strResult .= "  <td nowrap>" .getMedicalTreatmentCodeList($db,"detailMedicalCode$intRows",$rowDb['medical_code'],"","WHERE type = '$intType' ", "style=\"width:60px\""). "&nbsp;<font color=\"red\">*</font></td>";
            $strResult .= "  <td nowrap>" .getMedicalTreatmentCodeList_v2($db,"detailMedicalCode$intRows",$rowDb['medical_code'],"","", "style=\"width:150px\""). "&nbsp;<font color=\"red\">*</font></td>";
          ...
          for ($i = $intShown; $i <= $intAdd; $i++)
          {
            ...
            if ($intRows <= $intMaxShow) {
              $strResult .= "<tr valign=top  id=\"detailRows$intRows\">\n";
                ...
                $strResult .= "  <td nowrap>" .getFamilyList($db, "detailName$intRows","","<option value=''> </option><option value=\"".$arrData['dataEmployeeName']."|-1\">".$arrData['dataEmployeeName']."</option>", $strFamCriteria  , $strDisabled."onChange=\"onCostChanged($intRows);\""). "&nbsp;<font color=\"red\">*</font></td>";
                //$strResult .= "  <td nowrap>" .getMedicalTreatmentTypeList("detailType$intRows",true,"",""," onChange=\"changeMedicalType($intRows);\" $strDisabled"). /*"<input type=checkbox name=\"detailRoom$intRows\" id=\"detailRoom$intRows\" disabled \>&nbsp;".getWords("room").*/"&nbsp;<font color=\"red\">*</font></td>";
                //$strResult .= "  <td nowrap>" .getMedicalTreatmentCodeList($db,"detailMedicalCode$intRows","","","WHERE type=0 ", "style=\"width:60px\" $strDisabled"). "&nbsp;<font color=\"red\">*</font></td>";
                $strResult .= "  <td nowrap>" .getMedicalTreatmentCodeList_v2($db,"detailMedicalCode$intRows","","","", "style=\"width:150px\" $strDisabled"). "&nbsp;<font color=\"red\">*</font></td>";
                    ...
                    if ($strDataID != "") {
                      ...
                      if ($rowM = $db->fetchrow($resM)) {
                        ...
                        while ($rowDb = $db->fetchrow($resDb)) {
                          ...
                          if ($rowDb['claim_date']!= "") $arrData['dataDate'] = $rowDb['claim_date'];
                          ...
                          $strResult .= "<tr valign=top  id=\"detailRows$intRows\">\n";
                            ...
                            function saveData($db, &$strError)
                            {
                              ...
                              while ($rowDb = $db->fetchrow($resDb)) {
                                ...
                                $arrMedOnlyEmp[$rowDb['type']][$rowDb['code']] = $rowDb['employee_only']; //uddin 20150202
                                $arrMedOnlyFam[$rowDb['type']][$rowDb['code']] = $rowDb['family_only']; //uddin 20150202
                                $arrMedPerPerson[$rowDb['type']][$rowDb['code']] = $rowDb['per_person']; //uddin 20150202
                              }
                              ...
                              for ($i = 1; $i <= $intMax; $i++)
                              {
                                if (isset($_REQUEST['detailCost'.$i]))
                                {
                                  ...
                                  if (!is_numeric($_REQUEST['detailCost'.$i]))
                                  {
                                    ...
                                    echo "tot$i:".$_REQUEST['detailCost'.$i];
                                    break;
                                  }
                              ...
                              if ($bolOK && $strIDEmployee != "")
                              {
                                ...
                                $tempyear="-";
                                $tempCost="-";
                                for ($i = 1; ($i <= $intMax and $tempyear!=""); $i++)
                                {
                                  //echo $tempCost;
                                  //$strType = (isset($_REQUEST['detailType'.$i])) ? $_REQUEST['detailType'.$i] : "-1";
                                  //$bolRoom = (isset($_REQUEST['detailRoom'.$i])) ? "t" : "f";
                                  $strTempCode = (isset($_REQUEST['detailMedicalCode'.$i])) ? $_REQUEST['detailMedicalCode'.$i] : "-1_0";
                                  $temptt=explode("_",$strTempCode);
                                  $strCode = $temptt[1];
                                  $strType = $temptt[0];
                                  $strCost = (isset($_REQUEST['detailCostApproved'.$i])) ? $_REQUEST['detailCostApproved'.$i] : "0";
                                  $strYear = (isset($_REQUEST['detailClaimDate'.$i]) and validStandardDate($_REQUEST['detailClaimDate'.$i])) ? $_REQUEST['detailClaimDate'.$i] : "";
                                  ...
                                  if ($strCost!="0"){
                                    ...
                                    // uddin 20150202: new validasi quota
                                    $arrTemp = explode("|", $_REQUEST['detailName'.$i]);
                                    $strRelation = $arrTemp[1];
                                    //uddin 20150202: cek apakah ada filter only employee
                                    if ($arrMedOnlyEmp[$strType][$strCode] == "t" and !($strRelation == "-1")){ //jika khusus employee dan relasi bukan employee maka return false
                                      $strError = "Medical type $strCode Only for Employee (not for Family)";
                                      return false;
                                    }
                                    //uddin 20150202: cek apakah ada filter only Family
                                    if ($arrMedOnlyFam[$strType][$strCode] == "t" and $strRelation == "-1"){ //jika khusus family dan relasi employee maka return false
                                      $strError = "Medical type $strCode Only for Family (not for Employee)";
                                      return false;
                                    }
                                    // end
                                    //uddin 20150202: cek apakah ada filter only Family
                                    if ($arrMedPerPerson[$strType][$strCode] == "t" and $strRelation != "-1"){ //jika khusus family dan relasi employee maka return false
                                      $strNameFamily="";
                                      if (isset($_REQUEST['detailName'.$i]))
                                      {
                                        $arrTemp = explode("|", $_REQUEST['detailName'.$i]);
                                        if (count($arrTemp) >= 2)
                                        $strNameFamily = $arrTemp[0];
                                      }
                                      $strSQLIdFamily="select id from hrd_employee_family where id_employee=".$strIDEmployee." and name='".$strNameFamily."'";
                                      $strIdFamily="";
                                      //echo $strSQLIdFamily;
                                      $resDbFam = $db->execute($strSQLIdFamily);
                                      if ($rowDb = $db->fetchrow($resDbFam)) {
                                        $strIdFamily = $rowDb['id'];
                                      }
                                      $arrQuota[$strYear] = getEmployeeMedicalQuota($db, $strIDEmployee, $strEmployeeCompany, $strYear, $strDataID, $strIdFamily); // employeeFunc.php
                                      //if ($arrQuota[$strYear][] < $strCost)
                                      // khusus untuk per person langsuk cek masing2
                                      foreach ($arrQuota AS $strYear => $arrQuotaYear)
                                      {
                                        foreach ($arrQuotaYear AS $istrType => $arrQuotaDetail)
                                        {
                                          foreach ($arrQuotaDetail AS $istrCode => $fltAmount){
                                            if ($istrCode==$strCode and $strCost > $fltAmount){
                                              $strError = getWords("medical_quota_not_enough");
                                              $strError .= getWords(". remaining quota for ". $ARRAY_MEDICAL_TREATMENT_GROUP[$strType]. " treatment for '".$strNameFamily."' in $strYear =  ". standardFormat($fltAmount, true, 0));
                                              return false;
                                            }
                                          }
                                        }
                                      ...
                                      $arrQuota[$strYear] = getEmployeeMedicalQuota($db, $strIDEmployee, $strEmployeeCompany, $strYear, $strDataID); // employeeFunc.php
                                ...
                                foreach ($arrQuota AS $strYear => $arrQuotaYear)
                                {
                                  foreach ($arrQuotaYear AS $strType => $arrQuotaDetail)
                                  {
                                    if ($strType == MEDICAL_TYPE_OUTPATIENT)
                                    {
                                      ...
                                      {
                                        $strError = getWords("medical_quota_not_enough");
                                        $strError .= getWords(". remaining quota for Outpation treatment in $strYear =  ". standardFormat($arrQuotaDetail, true, 0));
                                        return false;
                                      }
                                      continue;
                                    }
                                    ...
                                    foreach ($arrQuotaDetail AS $strCode => $fltAmount)
                                    {
                                      ...
                                      {
                                        $strError = getWords("medical_quota_not_enough");
                                        $strError .= getWords(". remaining quota for ". $ARRAY_MEDICAL_TREATMENT_GROUP[$strType]. " treatment in $strYear =  ". standardFormat($fltAmount, true, 0));
                                        return false;
                                ...
                                if ($strDataID != "") { // simpan detail claim yang dilakukan
                                  ...
                                  for ($i = 1; $i <= $intMax; $i++) {
                                    ...
                                    //(isset($_REQUEST['detailType'.$i])) ? $strType = $_REQUEST['detailType'.$i] : $strType = "0";
                                    //(isset($_REQUEST['detailMedicalCode'.$i])) ? $strMedicalCode = $_REQUEST['detailMedicalCode'.$i] : $strMedicalCode = "";
                                    $strTempCode = (isset($_REQUEST['detailMedicalCode'.$i])) ? $_REQUEST['detailMedicalCode'.$i] : "0_";
                                    $temptt=explode("_",$strTempCode);
                                    $strMedicalCode = $temptt[1];
                                    $strType = $temptt[0];

                                    if ($strType == MEDICAL_TYPE_OUTPATIENT) $idType = -1;
                                    else
                                    ...
                                    //uddin 20150203: tambah query untuk mendapatkan id family
                                    $strIdFamily="(select id from hrd_employee_family where id_employee=".$strIDEmployee." and name='".$strName."')";
                                    if ($strID == "") {
                                      if ($strName != "") { // insert new data
                                        ...
                                        $strSQL .= "medical_date, medical_date_thru, claim_date, cost, approved_cost,id_family) ";
                                        ...
                                        $strSQL .= "'$strMedicalDate', '$strMedicalDateThru', '$strClaimDate', '$strCost', '$strCostApproved',$strIdFamily) ";
                                        $resDb = $db->execute($strSQL);
                                      ...
                                      if ($strName == "") {
                                        ...
                                        $strSQL .= "cost = '$strCost', approved_cost = '$strCostApproved', id_family=$strIdFamily ";
      ...
      function getMedicalType($db)
      {
        ...
        $strResult .= "<table border=0 width=100%>\n";
          $strResult .= " <tr valign=top>\n";
            ...
            $resDb = $db->execute($strSQL);
            if ($rowDb = $db->fetchrow($resDb)) {
              if (is_numeric($rowDb['total'])) $intTotal = $rowDb['total'];
            }
            ...
            $strResult .= " <td>\n";
              ...
              while ($rowDb = $db->fetchrow($resDb)) {
                ...
                if ($i > $intBaris) {
                  ...
                  $arrType[$rowDb['type']][] = array($rowDb['code'], $rowDb['note']);
                }
              ...
              if ($db->connect()) {
                ...
                if (isset($_REQUEST['dataEmployee'])) $arrData['dataEmployee'] = $_REQUEST['dataEmployee'];
                ...
                if ($bolCanView)
                {
                  ...
                  $strDataDetail = ($bolPrint) ? getDataPrint($db,$intTotalData, $strDataID) : getData($db,$intTotalData, $strDataID);
                ...
                if ($bolPrint) {
                  ...
                  $strInputMethod = getPaymentMethodList("dataMethod",$arrData['dataMethod'],"", "style=\"width:$strDefaultWidthPx\"");
                    ...
                    if ($strDataID != "") { // ada data
                      if ($_SESSION['sessionUserRole'] == ROLE_ADMIN || $_SESSION['sessionUserRole'] == ROLE_SUPERVISOR) { // admin atau manager
                        ...
                        if ($arrData['dataStatus'] == 3) {
                          ...
                          if ($arrDataCash['dataID'] != "") $strCashStyle = "";
                          $strReadonly = "";
                        }
                      }
                    }
