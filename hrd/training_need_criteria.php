<?php
  ...
  $fFilter->addSelect(getWords("active"), "filterActive", getDataListActive(), array("onChange" => "document.formFilter.submit()"), "", false);
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("active"), "active", "", array('width' => '30', 'valign' => 'top', 'align' => 'center', 'nowrap' => ''), false, false, "","printActiveSymbol()"));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    if($bolSave)
    {
      ...
      {
        $bolSuccess = $tblHrdTrainingNeedCriteria->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
      }
    }
