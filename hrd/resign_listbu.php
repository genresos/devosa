<?
  ...
  $dataPrivilege = getDataPrivileges(basename("resign_edit.php"), $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  if (!$bolCanView) die(accessDenied($_SERVER['HTTP_REFERER']));
...
// fungsi untuk menampilkan data, tapi hanya perubahan jabatan saja
  function getData($db, $strDataDateFrom, $strDataDateThru, &$intRows, $strKriteria = "", $strOrder = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= "  <td align=right>" .standardFormat($rowDb['salary'], true). "&nbsp;</td>";$strResult .= "  <td align=right>" .standardFormat($rowDb['meal'], true). "&nbsp;</td>";
  ...
  if ($db->connect()) {
   ...
   if ($bolCanView) {
    ...
    $strInputDepartment = getDepartmentList($db,"dataDepartment",$strDataDepartment, $strEmptyOption, "$strDeptKriteria", "style=\"width:$intDefaultWidthPx\"");
    $strInputSection = getSectionList($db,"dataSection",$strDataSection, $strEmptyOption, "$strSectKriteria", "style=\"width:$intDefaultWidthPx\"");
