  ...
  */
  class clsSalaryCalculation
  {
    ...
    */
    ...
    function clsSalaryCalculation($db, $strID = "", $bolIrregular = false, $strDataDate = "", $arrKriteria = array(), $strDateFrom = "", $strDateThru = "", $bolManagerial = false)
    ...
    */
    function setSalaryDate($strSalaryDate, $strDateFrom, $strDateTo, $strTHRDateFrom, $strTHRDateTo, $strCompany, $strSalarySet, $bolHideBlank, $strNote = "")
    {
...
//        if ($rowDb['id_employee']
    ...
    function calculateBasic($strKriteria = "")
    {
      ...
      while ($row = $this->data->fetchrow($res))
      {
        ...
        if (isset ($this->arrMA[$row['allowance_code']]) && isset($this->arrDetail[$row['id_employee']]) && $row['amount'] != "")
        {
          ...
          if ($this->arrMA[$row['allowance_code']]['jams'] == 't')
            $this->arrDetail[$row['id_employee']]['base_jamsostek'] += ($row['amount'] * $fltProrate); // as base jamsostek, di fujiko jamsostek di prorate
		  ...
		  {$this->arrDetail[$row['id_employee']]['base_tax'] += $row['amount']; // as base tax
		   	if ($row['id_employee'] == 1627) echo "<br>".$row['allowance_code']." = ".$row['amount'];
		  }
        }
      ...
      while ($row = $this->data->fetchrow($res))
      {
        ...
        if (isset($this->arrDetail[$strIDEmp]))
        {
          ...
          $this->compute($strIDEmp, "seniority_allowance", $this->arrConf["seniority_allowance"], $this->arrEmployee[$strIDEmp]['work_year'], $fltProrate);
    ...
    */
    function calculateOvertime()
    {
      ...
      $objOT = new clsOvertimeReport($this->data, $this->arrData['date_from'], $this->arrData['date_thru'], "", " AND id_company = ". $this->arrData['id_company']); // cls_overtime.php
      ...
      foreach($this->arrDetail AS $strID => $arrInfo)
      {
        ...
        $this->arrDetail[$strID]['ot2b'] = $objOT->getDataAllowance($strID, "2b", $this->arrDetail[$strID]['ot_per_hour']);
       ...
       /* for ($i = 1; $i <= 4; $i++)
		...
		if ($strID == 1627) echo "<br> overtime = ".$this->arrDetail[$strID]['overtime_allowance'];}
...
//print_r ($objot);
    ...
    */
	function calculateTHR()
    {
	  ...
	  {
		unset($fltProportion);
		$join = ($this->arrEmployee[$strIDEmp]['join_date'] == "") ? "" : date_create($this->arrEmployee[$strIDEmp]['join_date']);
		if($join != ""){
		...
		if($this->arrEmployee[$strIDEmp]['join_date'] == "" || $this->arrData['date_from_thr'] == "" || $this->arrData['date_thru_thr'] == "")
		  $fltProportion = 0;
		elseif ($year_diff>=2)							
		  $fltProportion = 1;								//..jika 6 bulan lebih maka menerima 100%
		elseif ($year_diff==0 ||$year_diff==1){
			if($month_diff > 6) $fltProportion = 1;			//..jika 6 bulan lebih maka menerima 100%
			elseif($month_diff == 6){
				if ($day_diff >= 0) $fltProportion = 1;		//..jika 6 bulan lebih maka menerima 100%
			}
			elseif($month_diff==5) $fltProportion = 2/3;	//..jika 4 bulan sampai kurang dari 6 bulan, maka menerima proporsional 2/3 gaji tetap
			elseif($month_diff==4){
				if ($day_diff >= 0) $fltProportion = 2/3;	//..jika 4 bulan sampai kurang dari 6 bulan, maka menerima proporsional 2/3 gaji tetap
			}	
			elseif($month_diff==3) $fltProportion = 1/3;	//..jika 2 bulan sampai kurang dari 4 bulan, maka menerima proporsional 1/3 gaji tetap
			elseif($month_diff==2){
				if ($day_diff >= 0) $fltProportion = 1/3;	//..jika 2 bulan sampai kurang dari 4 bulan, maka menerima proporsional 1/3 gaji tetap
				else $fltProportion = 0;					//..jika di bawah 2 bulan, tidak menerima thr
			}					
		}

		if($this->irregular == 't'){$this->arrDA['thr_allowance'][$strIDEmp]["amount"] =  $fltProportion * ($this->arrDetail[$strIDEmp]['base_ot']);}
 		else if($this->irregular == 'f'){$this->arrDA['thr_allowance'][$strIDEmp]["amount"] = 0;}
...
//          if ($strJoinDate != "" && dateCompare($strJoinDate , substr($this->arrData['salary_date'],0,4)."-01-01") >= 0 && dateCompare($strJoinDate , substr($this->arrData['salary_date'],0,4)."-12-31") <= 0) 
//          {
//            $intPeriodDeduction += (intval(substr($strJoinDate, 5, 2)) - 1);
//          }
//          if ($strResignDate != "" && dateCompare($strResignDate, substr($this->arrData['salary_date'],0,4)."-01-01") >= 0 && dateCompare($strResignDate, substr($this->arrData['salary_date'],0,4)."-12-31") <= 0) 
//          {
...
//          $objTax->setDataIncludeIrregular($fltBasic, $fltBasicIrregular, $strTaxStatus, $bolNPWP, $intTaxPeriod , $this->arrDetail[$strID]['jamsostek_deduction'], $strID, TAX_METHOD,$this->arrBaseTaxPayedTaxBefore[$strID],$this->salaryCalcMonth);
			$objTax->setDataIncludeIrregular($fltBasic, $fltBasicIrregular, $strFamilyStatus, $bolNPWP , $this->arrDetail[$strID]['jamsostek_deduction'], $strID, TAX_METHOD,$this->arrBaseTaxPayedTaxBefore[$strID],$this->salaryCalcMonth, $this->salaryCalcYear, $strJoinDate, $strResignDate);
          
               
           $fltTax                                   = $objTax->getTax(true);
//           $fltIrregularTax                          = ($fltBasicIrregular == 0) ? 0 : $objTax->getTax(false);     
    	...
    	if ($intMonth > 1){
            ...
            {  	
                ...
                while ($row = $this->data->fetchrow($res))
                {
                    ...
                    $strSQL2 = "SELECT id_employee, base_tax, base_irregular_tax, tax, irregular_tax, jamsostek_deduction, base_jamsostek FROM hrd_salary_detail WHERE id_salary_master='$salaryMasterID'";
