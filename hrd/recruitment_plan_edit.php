<?
  ...
  function saveData($db, &$strError) {
    ...
    if (!is_numeric($strDataGender)) $strDataGender = 2; // bebas
    if (!is_numeric($strDataMarital)) $strDataMarital = 2; // bebas
    if (!is_numeric($strDataNumber)) $strDataNumber = 1;
  ...
  if ($db->connect()) {
    ...
    $strInputEducationLevel = getEducationList($db,"dataEducationLevel",$arrData['dataEducationLevel'], "$strEmptyOption", "", " style=\"width:$strDefaultWidthPx \" $strReadonly");

    $strInputEmployeeStatus = getEmployeeStatusList("dataEmployeeStatus",$arrData['dataEmployeeStatus'], "", " style=\"width:$strDefaultWidthPx \" $strReadonly");
    $strInputGender = getGenderList("dataGender",$arrData['dataGender'], "$strEmptyOption", " style=\"width:$strDefaultWidthPx1 \" $strReadonly");
    $strInputMarital = getMaritalList("dataMarital",$arrData['dataMarital'], "$strEmptyOption", " style=\"width:$strDefaultWidthPx1 \" $strReadonly");
