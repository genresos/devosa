<?php
  ...
  if($bolIsEmployee && !isset($_REQUEST['dataEmpId'])) header("location:candidate_edit_internal.php?dataEmpId=".$arrUserInfo['employee_id']);
  
  $isCandidate = ($_SESSION['sessionGroupRole'] == ROLE_CANDIDATE);

  $strDataID = (isset($_REQUEST['dataID'])) ? $_REQUEST['dataID'] : "";
  //CUSTOM
  $strDataEmployeeID = (isset($_REQUEST['dataEmpId'])) ? $_REQUEST['dataEmpId'] : "";
  $isNew = ($strDataID == "");
  ...
  //CUSTOM
  if($strDataID == "" && $strDataEmployeeID != "") syncArrData($arrData, $db, $strDataEmployeeID); //sinkronisasi dgn employee, 1/2 hardcode
 ...
 // }
  //else
  //{
    $f->addSelect(getWords("MRF No."), "id_recruitment_need", getDataListMRFInternal($arrData['id_recruitment_need'], true, $emptyData, false, $arrData['id_recruitment_need']), array("onchange" => "myClient.changeMRF()"), "string", false, true, true, "", "", true, array("width" => 120)); // common_data.php
  ...
  //$f->addInput(getWords("candidate name"), "candidate_name", $arrData['candidate_name'], array("size"=> 50), "string", true, true, true, "", "", true, array("width" => 120));
    ...
    if($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE)
      ...
      $strDataEmployee = $arrData['candidate_name'];
	  }
    
    $f->addInputAutoComplete(getWords("Employee ID"), "candidate_name", getDataEmployee($strDataEmployee), "style='width:250px' ". $strReadonly, "string", true);
    $f->addLabelAutoComplete("", "candidate_name", "");
    if(!$bolIsEmployee){
      $arrInfoBtn = array();
      $arrInfoBtn["onclick"] = "redirectSelf();";
      $f->addButton("getInfoBtn", getWords("get info"), $arrInfoBtn);
    }
  if ($isCandidate)
  {
  ...
  $f->addInput(getWords("email"), "email", $arrData['email'], array("size"=> 30), "string", true, true, true);
  ...
  $f->addSelect(getWords("marital status"), "marital_status", getDataListMaritalStatusCandidate($arrData['marital_status'], true, $emptyData), array(), "string", true, true, true);
  ...
  */
  ...
  $strTemplateFile = "templates/candidate_edit_internal.html";
  //candidate user
 //if ($isCandidate)
 // {
    //------------------------------------------------
...
//--------------------------------------------------------------------------------
  ...
  function getLanguageSkill($strDataID)
  {
    ...
    //$arrLanguageSkill = getDataCheckBoxCandidateLanguageSkill(null); // NOTE: common_data.php -> HARUS DI-EDIT, ganti agar menjadi <select>
	$arrLanguageSkill = getDataSelectCandidateLanguageSkill(null);
    ...
    if ($strDataID != "")
    {
      ...
      if (is_array($arrResult))
      {
		...
		$strResult .= "
              ...
              <td nowrap align=center>".generateLanguangeSkillToRadioButton($arrLanguageSkill,'writitngSkill',$counter,$row['writing_skill'])."</td>
            </tr>";
			*/
		$strResult .= "
              <td nowrap align=center>".generateLanguangeSkillToSelect($arrLanguageSkill,'listeningSkill',$counter,$row['listening_skill'])."</td>
              <td nowrap align=center>".generateLanguangeSkillToSelect($arrLanguageSkill,'readingSkill',$counter,$row['reading_skill'])."</td>
              <td nowrap align=center>".generateLanguangeSkillToSelect($arrLanguageSkill,'speakingSkill',$counter,$row['speaking_skill'])."</td>
              <td nowrap align=center>".generateLanguangeSkillToSelect($arrLanguageSkill,'writingSkill',$counter,$row['writing_skill'])."</td>
            </tr>";
	...
	// NOTE: UNTUK CANDIDATE BARU
    if ($counter == 0)
    {
      foreach($arrLanguage as $row)
      {//Untuk language dari DB
        $counter++;
		/*
        $strResult .= "
        <tr name='dataLanguageRow$counter' id='dataLanguageRow$counter'>
          ...
          <td nowrap align=center>".generateRadio("listeningSkill".$counter, $arrLanguageSkill, "", "")."</td>
          <td nowrap align=center>".generateRadio("readingSkill".$counter, $arrLanguageSkill, "", "")."</td>
          <td nowrap align=center>".generateRadio("speakingSkill".$counter, $arrLanguageSkill, "", "")."</td>
          <td nowrap align=center>".generateRadio("writingSkill".$counter, $arrLanguageSkill, "", "")."</td>
        </tr>";
		*/
		$strResult .= "
        <tr name='dataLanguageRow$counter' id='dataLanguageRow$counter'>
          ...
          <td nowrap align=center>".generateSelect("listeningSkill".$counter, $arrLanguageSkill, "", "")."</td>
          <td nowrap align=center>".generateSelect("readingSkill".$counter, $arrLanguageSkill, "", "")."</td>
          <td nowrap align=center>".generateSelect("speakingSkill".$counter, $arrLanguageSkill, "", "")."</td>
          <td nowrap align=center>".generateSelect("writingSkill".$counter, $arrLanguageSkill, "", "")."</td>
        </tr>";
          //<td align=center><a href=\"javascript:deleteLanguageSkill($counter)\" title=\"".getWords("delete")."\"><img src=\"../images/delete.gif\" border=0 alt=\"".getWords("delete")."\" /></a></td>
      }
	  $intShow = $counter;
	  $intMax = 10;
	  //Untuk language tambahan
	  for($i = $counter; $i<$intMax; $i++){
		...
		*/
		$strResult .= "
			<tr name='dataLanguageRow$counter' id='dataLanguageRow$counter' style='display:none'>
			<td nowrap>".
			generateHidden("detailLanguageDelete".$counter, '0').
			generateInput("language".$counter, "", "style='width: 100%'")."
			</td>
    ...
    $strResult .= "
		  <tr>
            <td colspan=7>&nbsp; <a href='javascript:myClient.showMoreLanguage()'>" .getWords("show more")."</a></td>
          </tr>
        </tbody>";
    /*$strResult .= "
        <tfoot>
        <tr>
          <td colspan=6><a href=\"javascript:showMoreLanguageSkill()\">".getWords("show more")."</a></td>
        </tr>
        </tfoot>";*/
    $strResult .= "
      </table>
      <input type=hidden name=hNumShowLanguage id=hNumShowLanguage value='$intShow' />
	  <input type=hidden name=hNumTotalLanguage id=hNumTotalLanguage value='$intMax' />";
    return $strResult;
  }
...
/*
	...
	*/
	...
	$arrQuestion = array();
	$db->connect();
	$db->execute("SELECT * FROM hrd_recruitment_question");
	$rowcount = $db->numrows();
	for($i = 0; $i<$rowcount; $i++){
		$currentrow = $db->fetchrow();
		$arrQuestion[$currentrow["id"]] = $currentrow["question"];
		$questionData[$currentrow["id"]] = $currentrow["question"];
	}
    ...
    for($i = 1; $i <=$rowcount ; $i++)
    {
      ...
      <tr>
        <td nowrap>".$i.".</td>
        <td>".
          $arrQuestion[$i].generateHidden("question".$i,$arrQuestion[$i])."
        </td>
  ...
  $strResult .= "
    ...
    $strResult .= "
        ...
        if (!isset($arrData[$i]['answer1'])) $arrData[$i]['answer1'] = "";
    $strResult .= "
          <td nowrap>Jika ya, sampai dengan<br>".generateTextArea("answer".$i."_1", $arrData[$i]['answer1'], "style='width: 100%'")."</td>
        </tr>";
      $i = 4;
    if (!isset($arrData[$i]['is_yes'])) $arrData[$i]['is_yes'] = '';
    $strResult .= "
    ...
    $strResult .= "
          <td nowrap>
            Jika ya, di Perusahaan <br>".generateInput("answer".$i."_1", $arrData[$i]['answer1'], "style='width: 100%'")."<br>
            Sebagai (jabatan) <br>".generateInput("answer".$i."_2", $arrData[$i]['answer2'], "style='width: 100%'")."
          </td>
        </tr>";
    $i = 5;
    if (!isset($arrData[$i]['is_yes'])) $arrData[$i]['is_yes'] = '';
    $strResult .= "
        <tr>
          ...
          <td>".nl2br($arrQuestion[$i])."</td>
          <td align=center><input type=radio name=is_yes".$i." value='t' ".getCheckedFlagString($arrData[$i]['is_yes'], true)." /></td>
          <td align=center><input type=radio name=is_yes".$i." value='f' ".getCheckedFlagString($arrData[$i]['is_yes'], true)." /></td>";
        if (!isset($arrData[$i]['answer1'])) $arrData[$i]['answer1'] = "";
    $strResult .= "
          <td nowrap>".generateTextArea("answer".$i."_1", $arrData[$i]['answer1'], "style='width: 100%'")."</td>
        </tr>";
    ...
    $strResult .= "
        <tr>
          <td nowrap>".$i.".</td>
          <td>".nl2br($arrQuestion[$i])."</td>
          <td align=center><input type=radio name=is_yes6 value='t' ".getCheckedFlagString($arrData[$i]['is_yes'], true)." /></td>
          <td align=center><input type=radio name=is_yes6 value='f' ".getCheckedFlagString($arrData[$i]['is_yes'], false)." /></td>";
        if (!isset($arrData[$i]['answer1'])) $arrData[$i]['answer1'] = "";
        if (!isset($arrData[$i]['answer2'])) $arrData[$i]['answer2'] = "";
    $strResult .= "
          <td nowrap>
            Jika ya, mohon disebutkan<br>Nama:<br>".generateInput("answer".$i."_1", $arrData[$i]['answer1'], "style='width: 100%'")."<br>
            Posisi:<br>".generateInput("answer".$i."_2", $arrData[$i]['answer2'], "style='width: 100%'")."
          </td>
        </tr>";
    $i = 7;
    if (!isset($arrData[$i]['is_yes'])) $arrData[$i]['is_yes'] = '';
    ...
    $strResult .= "
          <td nowrap>
            Jika ya, mohon disebutkan<br>
            1. ".generateInput("answer".$i."_1", $arrData[$i]['answer1'], "size=30")." Tahun:".generateInput("answer".$i."_2", $arrData[$i]['answer2'], "size=5")."<br>
            2. ".generateInput("answer".$i."_3", $arrData[$i]['answer3'], "size=30")." Tahun:".generateInput("answer".$i."_4", $arrData[$i]['answer4'], "size=5")."
          </td>
        </tr>";
    $i = 8;
    if (!isset($arrData[$i]['is_yes'])) $arrData[$i]['is_yes'] = '';
    ...
    $strResult .= "
          <td nowrap>
            Penghasilan Nett: Rp.".generateInput("answer".$i."_1", $arrData[$i]['answer1'], "size=30")."<br>
    ...
    $strResult .= "
          <td nowrap>
            Tgl. ".generateInput("answer".$i."_1", $arrData[$i]['answer1'], "size=30")."
          </td>
        </tr>";
		*/
    $strResult .= "
...
//            </tr>";
  ...
  function saveData()
  {
    ...
    if (saveDataCandidate())
    {
      ...
      $strSQL  = "UPDATE hrd_candidate SET internal = true WHERE id = '$strDataID' ";
      $resExec = $db->execute($strSQL);
  ...
  {
    global $tblCandidateLanguage;
    $intNumData = intval(getPostValue("hNumShowLanguage"));
    for($counter = 1; $counter <= $intNumData; $counter++)
    {
      $data =  array(
                  "id_candidate" => $candidateID,
  ...
  function syncArrData(&$arrData, $db, $strDataEmployeeID){
    $arrDataEmp = getEmployeeInfoByCode($db, $strDataEmployeeID);
    $arrDataCand = $arrData;
    $arrDataCand['candidate_name']            = $strDataEmployeeID;
    $arrDataCand['current_address']           = $arrDataEmp['primary_address'];
    $arrDataCand['current_address_city']      = $arrDataEmp['primary_city'];
    $arrDataCand['current_address_zip']       = $arrDataEmp['primary_zip'];
    $arrDataCand['email']                     = $arrDataEmp['email'];
    $arrDataCand['phone']                     = $arrDataEmp['primary_phone'];
    $arrDataCand['permanent_address']         = $arrDataEmp['primary_address'];
    $arrDataCand['permanent_address_city']    = $arrDataEmp['primary_city'];
    $arrDataCand['permanent_address_zip']     = $arrDataEmp['primary_zip'];
    $arrDataCand['permanent_address_phone']   = $arrDataEmp['primary_phone'];
    $arrDataCand['gender']                    = $arrDataEmp['gender'];
    $arrDataCand['height']                    = $arrDataEmp['height'];
    $arrDataCand['weight']                    = $arrDataEmp['weight'];
    $arrDataCand['religion_code']             = $arrDataEmp['religion_code'];
    $arrDataCand['nationality']               = $arrDataEmp['nationality'];
    $arrDataCand['id_card']                   = $arrDataEmp['id_card'];
    $arrDataCand['passport']                  = $arrDataEmp['passport'];
    $arrDataCand['driver_license_a']          = $arrDataEmp['driver_license_a'];
    $arrDataCand['driver_license_b']          = $arrDataEmp['driver_license_b'];
    $arrDataCand['driver_license_c']          = $arrDataEmp['driver_license_c'];
    $arrDataCand['birthplace']                = $arrDataEmp['birthplace'];
    $arrDataCand['birthdate']                 = $arrDataEmp['birthday'];
    $arrDataCand['marital_status']            = $arrDataEmp['marital_status'];
    $arrDataCand['employee_id']               = $strDataEmployeeID;
    $arrData = $arrDataCand;
  }
?>