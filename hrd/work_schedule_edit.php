<?php
  ...
  if ($db->connect()) 
  {
    ...
    if ($bolCanEdit)
    {
      ...
      $f->addSelect(getWords("division"), "dataDivision", getDataListDivision($Division, true, ""), array("style" => "width: 200px", "onChange" => "javascript:myClient.setTableName('Division', this.value)"), "string", false, true, true);  

      $f->addSelect(getWords("department"), "dataDepartment", getDataListDepartment($Department, true, ""), array("style" => "width: 200px", "onChange" => "javascript:myClient.setTableName('Department', this.value)"),  "string", false, true, true);  

      $f->addSelect(getWords("section"), "dataSection", getDataListSection($Section, true, ""), array("style" => "width: 200px", "onChange" => "javascript:myClient.setTableName('Section', this.value)"), "string", false, true, true);  
      $f->addSelect(getWords("sub section"), "dataSubSection", getDataListSubSection($SubSection, true, ""), array("style" => "width: 200px", "onChange" => "javascript:myClient.setTableName('SubSection', this.value)"), "string", false, true, true);  


      $f->addInputAutoComplete(getwords("n i k"), "dataEmployee", getDataEmployee($Employee),  array("onChange" => "javascript:myClient.setTableName('Employee', this.value)", "style" => "width:200"), "string",  false);
      $f->addLabelAutoComplete("", "dataEmployee", "");



      $f->addSelect(getWords("workday"), "dataWorkday", getDataListDayName(-1, true, -1),"", "integer", false, true, true);  
