  ...
  */
        ...
        foreach($arrTmp AS $i => $rowDb)
        {
          ...
          if ($intStatus == REQUEST_STATUS_NEW) {
            ...
            if ($intDur > INT_LIMIT_APPROVAL) {
              ...
              if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) $strClass = "class='bgBlocked'";
            }
  ...
  if ($db->connect()) {
    ...
    if(isset($_REQUEST['btnShowAlert'])) {
      ...
      if (is_numeric($status)) $strKriteria .= "AND t1.status = $status ";
      else $strKriteria .= "AND t1.status < ". REQUEST_STATUS_APPROVED. " AND t1.status <> ". REQUEST_STATUS_DENIED. " ";
    }

    if ($bolCanView) {
      if (validStandardDate($strDataDateFrom) && validStandardDate($strDataDateThru) && (isset($_REQUEST['btnShow']) || isset($_REQUEST['btnPrint']))) {
