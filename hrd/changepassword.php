<?
  ...
  $strContentFile = "../hr/templates/changepassword.html";
  ...
  //---- INISIALISASI ----------------------------------------------------
  $strWordsChangePassword = getWords("change password");
  $strWordsOldPassword = getWords("old password");
  $strWordsNewPassword = getWords("new password");
  $strWordsRetypeNewPassword = getWords("retype new password");
  ...
  function saveData($db, &$strError) {
    ...
    $strSQL  = "SELECT passwd FROM \"allUser\" WHERE id = '" .$_SESSION['sessionUserID']. "' ";
    $resDb = $db->execute($strSQL);
    if ($rowDb = $db->fetchrow($resDb)) {
      
      if (md5($strDataOld) == $rowDb['passwd']) {
        // simpan perubahan dta
        $strSQL  = "UPDATE \"allUser\" SET passwd = '" .md5($strDataNew). "' ";
