<?php
  ...
  function getData($db)
  {
    ...
    if ($db->connect() && $strDataTripTypeID != "")
    {
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("id"), "employee_id", array("rowspan" => 2,'width' => '150'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("employee name"), "employee_name", array("rowspan" => 2,'width' => '150'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("sex"), "gender", array("rowspan" => 2,'width' => '150'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("grade"), "grade_code", array("rowspan" => 2,'width' => '150'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("medical status"), "medical_quota_status", array("rowspan" => 2,'width' => '150'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("outpatient"), "", array("colspan" => 4), null, false, false, "", "", "string", true, 40));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("quota"), "quota", array("rowspan" => 2,'width' => '200'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("additional quota"), "additional_quota", array("rowspan" => 2,'width' => '150'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("claim"), "claim", array("rowspan" => 2,'width' => '150'), array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("remain"), "remain", array("rowspan" => 2,'width' => '250'), ""));

      foreach($arrMedicalType[$strMedicalType] AS $arrMedicalCode)
      {
        $myDataGrid->addColumn(new DataGrid_Column(getWords($arrTripCostType[$strCostID]['trip_cost_type_name']), "trip_cost_".$strCostID, array('width' => '75'), array('align' => 'center'), false, false));
      }

      foreach($arrMedicalType AS $strType => $arrMedicalCode)
      {
        $myDataGrid->addColumn(new DataGrid_Column(getWords($strType), "", array("colspan" => count($arrMedicalCode)), null, false, false, "", "", "string", true, 40));

        foreach($arrMedicalCode AS $strCode => $arrDetail)
        {
          $myDataGrid->addColumn(new DataGrid_Column(getWords($strCode), "amount_".$strType."_".$strCode,  array('width' => '75'), array('align' => 'center'), false, false));
      ...
      $myDataGrid->addColumn(new DataGrid_Column("", "", array("rowspan" => 2, "width" => "60"), array('align' => 'center', 'nowrap' => ''), false, false, "","printEditLink()", "", false /*show in excel*/));
      ...
      if ($dataPrivilege['check'] == 't')
        $myDataGrid->addSpecialButton("btnChecked", "btnChecked", "submit", getWords("checked"), "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");
      if ($dataPrivilege['approve'] == 't')
      {
        $myDataGrid->addSpecialButton("btnApproved", "btnApproved", "submit", getWords("approved"), "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");
        $myDataGrid->addSpecialButton("btnPaid", "btnPaid", "submit", getWords("paid"), "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");
        $myDataGrid->addSpecialButton("btnDenied", "btnDenied", "submit", getWords("denied"), "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");
      }
  ...
  function saveData() 
  {
    ...
    $arrTripCostSetting = $tblTripTypeCostSetting->findAll("id_trip_type = $strTripTypeID AND include = 't'", "id_trip_cost_type", "", null, 1, "id_trip_cost_type");
    $tblHrdTripCostPlatform = new cHrdTripCostPlatform();

    $data = array("id_trip_type" => $strTripTypeID);
    ...
    foreach($arrGrade AS $strGradeCode => $arrGradeDetail)
    {
      ...
      foreach($arrTripCostSetting AS $strCostID => $arrSettingDetail)
      {
        ...
        if(isset($arrData['detailQuota_'.$strGradeCode.'_'.$strCostID]))
        {
          ...
          $tblHrdTripCostPlatform->delete(array("id_trip_type" => $strTripTypeID, "id_trip_cost_type" => $strCostID, "grade_code" => $strGradeCode));
      ...
      if (substr($strIndex,0,15) == 'DataGrid1_chkID') 
      {
        ...
        {  
          ...
          if (isProcessable($rowDb['status'], $intStatus))
          {
            ...
            writeLog(ACTIVITY_EDIT, MODULE_PAYROLL, $rowDb['employee_name']." - ". $rowDb['created'] ." - ". $rowDb['proposal_date'], $intStatus);
          }
        }
      }
