<?php
      ...
      for ($i = 0;$i < ($intTotal);$i++) {
        if ($intCurrent == $i) { // saat ini, jdi tidak ada link
          ...
          //if ($i <= ($intStatus+1)) { // berikan link
            $strResult .= " <b><a href=\"" .$arrLinkType[$i]. "?dataID=$strDataID\">[" .($i + 1)."] ".getWords($arrMenuType[$i]). "</a></b>  |";
          //} else {
  ...
  function printEvaluationResult($db, $strDataID)
  {
    ...
    if ($rowDb = $db->fetchrow($resDb))
    {
      ...
      // cari info atasannya
      if ($rowDb['id_manager'] == "") $GLOBALS['strManager'] = "";
      else
      {
     ...
     // cari penilaian umum
      ...
      if (!is_numeric($arrW['weight1'])) $arrW['weight1'] = 0;
      if (!is_numeric($arrW['weight2'])) $arrW['weight2'] = 0;
      if (!is_numeric($arrW['weight3'])) $arrW['weight3'] = 0;
  ...
  {
    global $db;
    if (!is_numeric($fltPoint)) return "";

    if ($fltPoint < 60) return getSetting("category_e");
    else if ($fltPoint <= 70) return getSetting("category_d");
    else if ($fltPoint <= 80) return getSetting("category_c");
    else if ($fltPoint <= 90) return getSetting("category_b");
    else return getSetting("category_a");
  }
