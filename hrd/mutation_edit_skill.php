...
$strWordsStatus = getWords("status");
  ...
  if ($db->connect()) {
   ...
   //echo $strStatus;
    ...
    $strInputStatusNew = getEmployeeStatusList("dataStatusNew", $arrData['dataStatusNew'], "", " style=\"width:$strDefaultWidthPx \" $strReadonly", "<option value='99'>Resigned</option>\n");
    ...
    $strBtnPrint .= "<input type=button name=btnPrint onClick=\"window.open('mutation_edit_skill.php?btnPrint=Print&dataID=$strDataID');\" value=\"" .getWords("print")."\" $strDisabledPrint>";
  }
  

  ($bolPrint) ? $strMainTemplate = getTemplate("mutation_edit_print.html", false) : $strTemplateFile = getTemplate("mutation_edit_skill.html");
