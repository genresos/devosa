...
// getDataPrivileges ada di '../global.php'
  ...
  $f->addInput(getWords("date from"), "dataDateFrom", getInitialValue("DateFrom", date("Y-m-")."01"), array("style" => "width:$strDateWidth"), "date", true, true, true);
  $f->addInput(getWords("date thru"), "dataDateThru", getInitialValue("DateThru", date("Y-m-d")), array("style" => "width:$strDateWidth"), "date", true, true, true);
    ...
    $f->addSelect(getWords("department "), "department_code", getDataListDepartment(getInitialValue("Department", "", $strDataDepartment), true), array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['department'] == ""));
    $f->addSelect(getWords("division"), "division_code", getDataListDivision(getInitialValue("Division", "", $strDataDivision), true), array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['division'] == ""));
  ...
  $f->addSubmit("btnExcel", getWords("export excel"), array("onClick" => "document.formInput.target = ''; return validInput();"), true, true, "", "", "showReport");
 ...
 $f->setFormTarget("");
  ...
  function showReport()
  {
    ...
    */
    ...
    $arrInfo = array();
    $arrTotal = array(); // menyimpan data total
    ...
    if(($f->getValue('dataDateFrom')!="") AND ($f->getValue('dataDateThru')!=""))
       $strPeriode .= "AND (join_date::date BETWEEN '".$f->getValue('dataDateFrom')."' AND '".$f->getValue('dataDateThru')."')  ";
  ...
  function showData()
  {
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("department"), "department_name", array('width' => 150), array('nowrap' => 'nowrap'), false, false, "", "", "string", true, 12, false));//, strtoupper(getWords("rekapitulation")), true));
//    $myDataGrid->addColumn(new DataGrid_Column(getWords("division"), "division_name", array('width' => 150), array('nowrap' => 'nowrap'), false, false, "", "printEmployeeStatus()", "string", true, 16, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("position"), "position_name", array("align" => "center"), array('align' => 'center'), false, false, "", "", "string", true, 16, false));
    $myDataGrid->addColumn(new DataGrid_Column((getWords("month")), "month", array(), array("align" => "center"), false, false, "", "", "string", true, 8, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("total"), "total", array('width' => 200), array(), false, false, "", "", "string", true, 32, false));
...
//    $myDataGrid->addColumn(new DataGrid_Column(getWords("date required"), "due_date", array('width' => 90), array('align' => 'center'), false, false, "", "", "string", true, 10, false));
    ...
    if ($isExport)
    {
      ...
      if ($f->getValue("dataDateFrom") == $f->getValue("dataDateThru"))
        ...
        $myDataGrid->strTitle2 = getWords("periode")." : ".$f->getValue("dataDateFrom")." - ".$f->getValue("dataDateThru");
    }
    else
    {
      $strGridTitle = "
        <table width='100%' border=0 cellpadding=1 cellspacing=0 style='font-size: 10pt; font-weight: bold'>
          <tr>
            <td colspan=3 style='font-size: 12pt'>".strtoupper(getWords("list of employee by division and department"))."</td>
          </tr>";
      if ($f->getValue("dataDateFrom") == $f->getValue("dataDateThru"))
      {
        $strGridTitle .= "
   ...
   //  saat ini sebenarnya cumup ambil data dari hrd_employee saja, karena yang di tampilkan adalah division_code dan department_code.
    $strSQL="select  hd.department_name, hp.position_name, to_char(he.join_date, 'Month') as month, SUM(1) as total from
         ...
         where he.join_date BETWEEN '".$f->getValue('dataDateFrom')."' AND '".$f->getValue('dataDateThru')."' ";
