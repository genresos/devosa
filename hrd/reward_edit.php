<?php
  ...
  function saveData($db, $strDataID, &$strError) {
    ...
    if ($bolOK) { // input OK, tinggal disimpan
      if ($strDataID == "") {
        ...
        $strSQL .= "id_employee,reward_date, reward_code, reward_amount, note,status) ";
        $strSQL .= "VALUES(now(),'" .$_SESSION['sessionUserID']. "','" .$_SESSION['sessionUserID']. "', ";
        $strSQL .= "'$strIDEmployee','$strDataDate', '$strDataCode', ";
        $strSQL .= "'$strDataAmount','$strDataNote',0) ";
        $resExec = $db->execute($strSQL);
        $strSubject = getSubject(0,'Reward',getEmployeeIDEmail($strIDEmployee));
        ...
        $resExec = $db->execute($strSQL);
        $strSubject = getSubject(0,'Reward Updated',getEmployeeIDEmail($strIDEmployee));

        writeLog(ACTIVITY_EDIT, MODULE_PAYROLL,"$strIDEmployee - $strDataDate",0);
      }
      $strBody = "Name: ".getEmployeeNameEmail($strIDEmployee)."<br>";
      $strBody.= "Reward Type: ".$strDataCode."<br>";
      $strBody.= "Date: ".$strDataDate;
      $strBody.= "<br>Amount: ".$strDataAmount;
      $strBody =  getBody(0,'Reward',$strBody,$strmodified_byID);
  ...
  if ($db->connect()) {
    ...
    $strInputCode = getRewardTypeList($db, "dataCode", $arrData['dataCode'], $strEmptyOption,"", "style=\"width:$strDefaultWidthPx\"");
