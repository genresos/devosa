<?
  ...
  function getData($db, $strDataID, &$intRows, $strKriteria = "", $intPage = 1, $bolLimit = true, $strOrder = "") {
    ...
    $strSQL  = "SELECT t1.total_gross, t1.total_gross_irregular, t1.employee_id, t2.employee_name, t2.bank_account, t2.bank_account_name, t3.bank_name, 'IDR' as idr ";
    ...
    while($rowDb = $db->fetchrow($resDb))
    {
      if ($bolHideBlank)
      {
        if (!$bolIrregular && $rowDb['total_gross'] <= 0) continue;
        elseif($bolIrregular && $rowDb['total_gross_irregular'] <= 0) continue;
      }
      if($bolIrregular) $rowDb['total_gross'] = $rowDb['total_gross_irregular'];
   ...
   // print "<br>type data => ".$strDataType."<br>";
    ...
    if ($strDataFilterValue != "") {
      if ($strDataFilterValue == 0)
        $strKriteria .= "AND t1.total_gross >= 0 ";
