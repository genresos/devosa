<?
  ...
  function getData($db, &$intRows, $strKriteria = "", $strOrder = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strResult .= "  <td nowrap align=right><input type=hidden name=detailPrice$intRows value=\"" .(float)$rowDb['price']. "\">" .standardFormat($rowDb['price']). "&nbsp;</td>";
  ...
  function saveData($db, &$strError) {
    ...
    if (!is_numeric($strDataPrice)) $strDataPrice = 0;
