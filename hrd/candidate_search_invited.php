<?
  ...
  if (!isset($_SESSION['sessionUserID'])) {
    header("location:login.php?dataPage=candidate_search_invited.php");
    exit();
  }

  $bolCanView = getUserPermission("candidate_search_invited.php", $bolCanEdit, $bolCanDelete, $strError);
  ...
  if ($bolFull) {
    if ($bolPrint) {
      $strMainTemplate = getTemplate("candidateSearchInvitedFullPrint.html");
    } else {
      $strTemplateFile = getTemplate("candidateSearchInvitedFull.html");
    }
  } else {
    if ($bolPrint) {
      $strMainTemplate = getTemplate("candidateSearchInvitedPrint.html");
    } else {
      $strTemplateFile = getTemplate("candidate_search_invited.html");
    }
  }
  ...
  if ($db->connect()) {
    ...
    // ------ AMBIL DATA KRITERIA -------------------------
    (isset($_REQUEST['filterCandidateID'])) ? $strFilterCandidateID = trim($_REQUEST['filterCandidateID']) : $strFilterCandidateID = "";
