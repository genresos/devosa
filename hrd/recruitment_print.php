  ...
  function getData($strDataID)
  {
    ...
    if ($strDataID != "")
    {
      if ($rowDb = $tblMRF->findById($strDataID))
      {
        if ($rowDb['id_company'] != "")
        {
          $arrTmp = $tblRel->find(array("id" => $rowDb['id_company']), "company_name");
          if (isset($arrTmp['company_name'])) $rowDb['company_name'] = $arrTmp['company_name'];
        }
        if (!isset($rowDb['company_name'])) $rowDb['company_name'] = "";
        return $rowDb;
      }
    }
