   ...
   //$strSlipTopic = $arrAbsencetype[$strDataAbsencetype]['absence_type_code'];
    ...
    foreach ($myDataGrid->checkboxes as $strDataID)
    {
      ...
      //$strFormCode = $dataTrip[0]['form_code'];
       ...
       $dataAbsence1 = $tblAbsence->findAll("id = '$strDataID'", "date_from || ' until ' || date_thru AS absence_date", "", null, 1);
      ...
      $dataApproval1 = $tblAbsence->findAll("id = '$strDataID'", "created_by, created, now()::timestamp without time zone as printed_time, '".$_SESSION['sessionUserID']."' as printed_by", "", null, 1);
      ...
      //ambil data approval => ambil nama karyawan
      $dataApproval2 = $tblAbsence->findAll("id = '$strDataID'", "now()::timestamp without time zone as printed_time, '".$_SESSION['sessionUserID']."' as printed_by", "", null, 1);

      $dataApproval2 = $tblAbsence->findAll("id = '$strDataID'", "checked_by, checked_time, approved_by, approved_time", "", null, 1);
...
//print_r($dataAbsence1);
  ...
  if ($db->connect())
  {
    ...
    $f->addInput(getWords("date thru"), "dataDateThru", getInitialValue("DateFrom", $dtThru, $dtThru), array("style" => "width:$strDateWidth"), "date", false, true, true);
    ...
    $f->addSelect(getWords("deduct leave"), "dataDeductLeave", getDataListEmployeeActive($strDeductLeave, true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);

    $f->addSelect(getWords("request status"), "dataRequestStatus", getDataListRequestStatus($_getInitialValue("RequestStatus"), true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);
    $f->addLiteral("","","");
    ...
    $f->addSelect(getWords("status"), "dataEmployeeStatus", getDataListEmployeeStatus($_getInitialValue("EmployeeStatus", "", ""), true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);

    $f->addSelect(getWords("active"), "dataActive", getDataListEmployeeActive($_getInitialValue("Active"), true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);
    $f->addLiteral("","","");
