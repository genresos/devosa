<?php
 ...
 $strWordsNew      = getWords("new");
  ...
  $f->addInput(getWords("request date"). " ".getWords("from"), "date_from", date("Y-m-d"), array(), "date", false, true, true);  
  $f->addInput(getWords("date to"), "date_thru", date("Y-m-d"), array(), "date", false, true, true);  
  $f->addInput(getWords("training date"). " ".getWords("from"), "training_date_from", "", array(), "date", false, true, true);  
  $f->addInput(getWords("date to"), "training_date_thru", "", array(), "date", false, true, true);  
  $f->addLiteral("","","");

  $f->addInput(getWords("training topic"), "training_topic", "", array(), "string", false, true, true);  
  $arrType = array();
  ...
  $f->addSelect(getWords("request status"), "dataRequestStatus", getDataListRequestStatus(getInitialValue("RequestStatus"), true, $arrEmpty), array("style" => "width:$strDefaultWidthPx"), "", false);  
  $f->addLiteral("","","");
  $f->addLiteral("","","");
  ...
  {
    $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id", array('width' => 30), array('align'=>'center', 'nowrap' => 'nowrap')));
  }
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("training topic"), "topic", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("instructor"), "name_instructor", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 16));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("participants"), "participants", array("nowrap" => "nowrap"), array("nowrap" => "nowrap"), false, false, "", "", "string", true, 16));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("status"), "status",  "", array('nowrap' => ''), false, false, "","printRequestStatus()"));
    ...
    if ($bolCanEdit)
    {
      $myDataGrid->addColumn(new DataGrid_Column("", "", array('width' => 45), array('align' => 'center', 'nowrap' => 'nowrap'), false, false, "","printGlobalEditLink()", "string", false));
    }
    /*
    ...
    $myDataGrid->addColumn(new DataGrid_Column("", "", array(), array('align' => 'center', 'nowrap' => 'nowrap'), false, false, "","printShowLink()", "string", false));
  ...
  if ($f->getValue("training_date_from") != "" && $f->getValue("training_date_thru") != '') 
    $strCriteria .= " AND id IN (SELECT DISTINCT id_request FROM hrd_training_request_detailtime 
                        WHERE trainingdate BETWEEN '" .$f->getValue("training_date_from"). "' AND '" .$f->getValue("training_date_thru"). "') ";
    ...
    $strParticipants = "<table>";
      ...
      if (isset($arrParticipants[$row['id']]))
      {
        ...
        foreach ($arrParticipants[$row['id']] AS $id => $rowTmp)
        {
          ...
          if(isset($tempDict["$strTemporaryEmployeeName"])) continue;   //F_DOUBLEPARTICIPANT
          ...
          if ($intParticipant == 1)
          {
            ...
            $strParticipant1 .= "  <td nowrap align=right>" .standardFormat($rowTmp['evaluation'],true,2)."&nbsp;</td>\n";
            if ($bolExcel)
              $strEvaluation = "";
            else
              $strEvaluation = ($row['status'] == REQUEST_STATUS_APPROVED && $rowTmp['status'] != 1) ? "<a href=\"training_evaluation_edit.php?dataID=" .$rowTmp['id']."\" title=\"" .getWords('view evaluation')."\">" .$words['view']."</a>" : "";
            ...
            $strParticipant2 .= "  <td nowrap align=right>" .standardFormat($rowTmp['evaluation'],true,2)."&nbsp;</td>\n";
            if ($bolExcel)
              $strEvaluation = "";
            else
              $strEvaluation = ($row['status'] == REQUEST_STATUS_APPROVED && $rowTmp['status'] != 1) ? "<a href=\"training_evaluation_edit.php?dataID=" .$rowTmp['id']."\"  title=\"" .getWords('view evaluation')."\">" .$words['view']."</a>" : "";
...
//--------------------------------------------------------------------------------
  ...
  {
    extract($params);
    return "<a href=\"training_evaluation_edit.php?dataRequestID=" .$record['id']."\">" .getWords('evaluation'). "</a>";
  }
  ...
  function sharingLink($params)
  {
    ...
    $bolEditable = true;
     ...
     if ($bolEditable && $record['category'] != '3')
      return "<a href='training_request_edit.php?btnCreateSharing=Create&id_plan=" .$record['id']. "'>" .getWords('create'). "</a>";
    else
      return "";
  }
  ...
  function printLink($params)
  {
    ...
    $strResult = "<a href=\"javascript:openWindowDialog('templates/training_request_print.html?dataID=" .$record['id']. "');\">" .getWords("print")."</a>";
      ...
      if (substr($strIndex,0,15) == 'DataGrid1_chkID') 
      {
        ...
        {  
          ...
          if (isProcessable($rowDb['status'], $intStatus))
          {
			...
			if($intStatus == 2)	alertApproval($db, $strValue);
			
            writeLog(ACTIVITY_EDIT, MODULE_OTHER, $rowDb['id']." - ". $rowDb['created'] ." - ". $rowDb['training_type'], $intStatus);
          }
        }
      }
