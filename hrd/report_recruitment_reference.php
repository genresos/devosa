...
// getDataPrivileges ada di '../global.php'
  ...
  $f->addSelect(getWords("reference"), "reference", getDataListJobReference($arrData['reference'], true, $emptyData), array(), "string", true, true, true);
  $f->addInput(getWords("job posting date"), "data_job_date", "", array(), "date", false);
  $f->addSelect(getWords("position"), "position_code", getDataListPosition($arrData['position_code'], true, $emptyData), array(), "string", false, true, true);

  $f->addSubmit("btnPreview", getWords("show report"), array("onClick" => "document.formInput.target = '';"), true, true, "", "", "previewData");
  $f->addSubmit("btnPrint", getWords("print report"), array("onClick" => "javascript:printList()"), true, true, "", "", "previewData");
  $f->addSubmit("btnExcel", getWords("export excel"), array("onClick" => "document.formInput.target = '';"), true, true, "", "", "exportExcel");
  ...
  function getPositionDataList($default = null, $isHasEmpty = false, $emptyData = null)
  {
    ...
    if ($isHasEmpty) 
    {
      if ($emptyData == null) $arrResult[] = array("value" => "", "text" => "");
      else $arrResult[] = $emptyData;
    }
    ...
    {
      $arrResult[] = array("text" => $arrD['position'], "value" => $arrD['position']);
    }
  ...
  function getDataListJobReference($default = null, $isHasEmpty = false, $emptyData = null)
  {
    ...
    foreach ($arrData AS $arrD)
    {
      ...
      $arrResult[] = array("value" => $arrD['reference_type']."##".$arrD['reference'], "text" => $strTypeName. " - ".$arrD['reference']);
  ...
  function previewData()
  {
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("date"), "application_date", array('width' => 60), array("align" => "center"), false, false, "", "", "string", true, 32, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("candidate name"), "candidate_name", array(), array('nowrap' => 'nowrap'), false, false, "", "", "string", true, 12, false));//, strtoupper(getWords("rekapitulation")), true));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("education"), "education", array(), array('nowrap' => 'nowrap'), false, false, "", "", "string", true, 16, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("position"), "position", array(), array('nowrap' => 'nowrap'), false, false, "", "", "string", true, 16, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("invitation"), "invitation_date", array('width' => 60), array('align' => 'center'), false, false, "", "", "string", true, 10, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("work experience"), "experience", array(), array(), false, false, "", "", "string", true, 32, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("note"), "note", array(), array(), false, false, "", "", "string", true, 32, false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("job posting date"), "job_reference_date", array('width' => 60), array("align" => "center"), false, false, "", "", "string", true, 32, false));
//    $myDataGrid->addColumn(new DataGrid_Column(getWords("date required"), "due_date", array('width' => 90), array('align' => 'center'), false, false, "", "", "string", true, 10, false));
    ...
    $strPosCriteria = (($strPos = $f->getValue('position_code')) == "") ? "" : "AND UPPER(\"position\") = '" .strtoupper($strPos)."' ";    
    $strRefCriteria = "";
    ...
    $arrExp = array(); // data experience candidate
    $strSQL = "
      SELECT * FROM hrd_candidate_working_experience
      WHERE id_candidate IN
      (
        SELECT id FROM hrd_candidate 
        WHERE application_date BETWEEN '".$f->getValue('data_date_from')."' AND '".$f->getValue('data_date_thru')."'
    ...
    $arrEdu = array(); // data pendidikan terakhir
    $strSQL = "
    ...
    foreach($dataset as &$rowDb)
    {
      ...
      if (isset($arrExp[$rowDb['id']]))
      {
        ...
        {
          if ($rowDb['experience'] != "") $rowDb['experience'] .= " <br /> \n";
...
//    $myDataGrid->addColumn(new DataGrid_Column(getWords("date required"), "due_date", array('width' => 90), array('align' => 'center'), false, false, "", "", "string", true, 10, false));
    ...
    $strPosCriteria = (($strPos = $f->getValue('position_code')) == "") ? "" : "AND UPPER(\"position_code\") = '" .strtoupper($strPos)."' ";    
    $strRefCriteria = "";
    if (($strRef = $f->getValue('reference')) != "")
    {
      list($strRefType, $strRef) = explode("##", $strRef);
    ...
    if ($f->getValue("data_date_from") == $f->getValue("data_date_thru"))
      ...
      $myDataGrid->strTitle2 = getWords("period")." : ".$f->getValue("data_date_from")." - ".$f->getValue("data_date_thru");
