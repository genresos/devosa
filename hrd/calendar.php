...
//     }
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      $arrCalendar[$rowDb['tgl']][] = array("note" => $rowDb['note'],
                                            "id" => $rowDb['id'],
    ...
    for ($i = 0; $i < 7; $i++) {
      $strHasil .= "  <td nowrap align=center  class='tableHeader' width=14%>" .getNamaHariSingkat($i). "&nbsp;</td>\n";
    }
    $strHasil .= "</tr>\n";
    ...
    for($intDayCounter = 1; $intDayCounter <= $intDaysThisMonth; $intDayCounter++) 
    {
      ...
      if (isset($arrCalendar[$intDayCounter]))
      {
         if ($arrCalendar[$intDayCounter][0]['leave'] == "f") $str1stClass = "class='bgLeaveHoliday'";
         else
         {
        ...
        if (count($arrCalendar[$intDayCounter]) > 0)
        {
          ...
          foreach($arrCalendar[$intDayCounter] as $cal)
          {
            ...
            if (!$bolPrint && $_SESSION['sessionUserRole'] != ROLE_EMPLOYEE) 
              $strRemarks .= "<a class=nounderline href=\"javascript:editEvent(".$cal['id'].", ".$intDayCounter.");\">" .strtoupper($cal["note"]). "</a>";
            else
              $strRemarks .= strtoupper($cal["note"]);

            if ($cal["note"] != "" && !$bolPrint && $_SESSION['sessionUserRole'] != ROLE_EMPLOYEE) 
            {
              $strRemarks .= " &nbsp;<br>[<a class=nounderline href=\"javascript:deleteEvent(".$cal['id'].", $intDayCounter)\">" .getWords('delete'). "</a>]";
            }
...
//       }
  ...
  // $strMonth, $stryear = bulan dan tahun yang diinginkan
  // $bolPrint = apakah di print atau tidak
  function getMonthlyEmployeeShiftCalendar($db, $strMonth = "", $strYear = "", $strEmployee = "", $intDefaultHeight = 60, $bolPrint = false) {
    global $words;
    ...
    */
    ...
    $strHasil .= "<tr valign=middle height=20>\n";
    for ($i = 0; $i < 7; $i++) {
      $strHasil .= "  <td nowrap align=center  class='tableHeader' width=14%>" .getNamaHariSingkat($i + 1). "&nbsp;</td>\n";
    }
    $strHasil .= "</tr>\n";
