<?php
  ...
  ($bolPrint) ? $strMainTemplate = getTemplate("trip_payment_print.html", false) : $strTemplateFile = getTemplate("trip_payment_edit.html");
  //---- INISIALISASI ----------------------------------------------------
  $strDataDetail = "";
  $strButtons = "";
  $intTotalData = 0;
    ...
    if ($bolOK && $strIDEmployee != "") {
      ...
      if ($strDataID != "") { // simpan detail claim yang dilakukan
        ...
        for ($i = 1; $i <= $intMax; $i++) {
          ...
          if (!is_numeric($strCostOther)) $strCostOther = 0;
  ...
  if ($db->connect()) {
    ...
    $strInputAllowance = "<input type=hidden name=dataAllowance value='" .$arrData['dataAllowance']."'>" . standardFormat($arrData['dataAllowance']);
    ...
    if ($bolPrint)
    {
      ...
      $strDataAllowance = ($arrData['dataDuration'] == 0) ? 0 : standardFormat($arrData['dataAllowance'] / $arrData['dataDuration']);
    ...
    $strButtons .= "<input type=button name=btnPrint onClick=\"window.open('trip_payment_edit.php?btnPrint=Print&dataTripID=$strDataTripID');\" value=\"".getWords("print")."\" $strDisablePrint>";
  }
