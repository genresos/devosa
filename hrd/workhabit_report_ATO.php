<?php
  ...
  function getData($strKriteria){
	...
	//get all new employee	
	$strSQL = "SELECT COUNT(t1.id) AS \"jumlah\" FROM hrd_employee AS t1
	$strKriteria
	AND join_date BETWEEN '$strDateFrom' AND '$strDateThru'";
	
	$resSQL = $db->execute($strSQL);
	while($row = $db->fetchrow($resSQL)){
		$numOfEmployee = $row['jumlah'];
		$detail = getWords("new");
		$dataSet->addPoint(new Point($detail, $numOfEmployee));
	...
	$strSQL = "SELECT COUNT(t1.id) AS \"jumlah\" FROM hrd_employee AS t1
	$strKriteria
	AND resign_date BETWEEN '$strDateFrom' AND '$strDateThru'";
	
	$resSQL = $db->execute($strSQL);
	while($row = $db->fetchrow($resSQL)){
		$numOfEmployee = $row['jumlah'];
		$detail = getWords("resign");
		$dataSet->addPoint(new Point($detail, $numOfEmployee));
