    ...
    while ($rowDiv = $db->fetchrow($resDiv)) 
    {
      ...
      while ($rowDb = $db->fetchrow($resDb)) 
      {
        ...
        while ($rowTmp = $db->fetchrow($resTmp)) {
          ...
          {
            if ($intSection != 1) $strTmpData3 .= "<tr>";
...
//     $strHeader2 = " <tr class=tableHeader align=center>$strHeader2</tr>\n";
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      $intDept++;
      $intTemp++;
      $strHeader1 .= "  <td align=center title=\"" .$rowDb['position_name']."\" $strDefaultWidth>" .strtoupper($rowDb['position_code'])."&nbsp;</td>\n";
    ...
    $strResult .= "<strong>".strtoupper(getWords("employee category"))."</strong><br>";
    $strResult .= $strTableStart;
    $strResult .= $strHeader1."\n".$strData;
    $strResult .= $strTableFinish;
	...
	*/
    ...
    $strResult .= "<strong>".strtoupper(getWords("Position"))."</strong><br>";
    $strResult .= $strTableStart;
    $strResult .= $strHeader1."\n".$strData;
    $strResult .= $strTableFinish;
  ...
  if ($db->connect()) {
    ...
    $strBranchName = "";
		if ($strDataBranch != ""){
			...
			$strSQL  = "SELECT branch_name FROM hrd_branch WHERE branch_code = '$strDataBranch' ";
      $resDb = $db->execute($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
        $strBranchName = $rowDb['branch_name'];
      }
		}
			
    if (validStandardDate($strDateThru) && validStandardDate($strDateFrom))
    {
      $strKriteria .= "AND management_code IS NOT NULL AND management_code <> '' AND join_date <= '$strDateThru' AND (resign_date >= '$strDateFrom' OR resign_date IS NULL)";
