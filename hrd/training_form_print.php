<?php
  if ( !session_id() ) include_once('../global/session.php');
    ...
    foreach($_REQUEST AS $strIndex => $strValue)
    {
      if(substr($strIndex,0,5) == 'chkID')
      {
        ...
        if($rowDb = $db->fetchrow($resDb))
        {
          ...
          if ($rowDb['training_date'] != $rowDb['training_date_thru']) $strTanggalKursus .= "  Sampai ". pgDateFormat($rowDb['training_date_thru'], "d M Y");
          $strNomorKursus = "";
