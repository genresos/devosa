<?
  ...
  function getData($db, $strMonth, $strYear, $strKriteria)
  {
    ...
    $intMax = 31;
    if (in_array($strMonth, array(4,6,9,11)))
    {
      $intMax = 30;
    }
    else if ($strMonth == 2)
  ...
  $strTableEmployeeID    = getwords('employee id');
    $strResult = "
      <table cellspacing=0 class='gridTable'>
        <tr class='tableHeader'>
          <td class='tableHeader' rowspan=2>".$strTableNo."</td>
          <td class='tableHeader' rowspan=2 nowrap>$strTableEmployeeID</td>
          <td class='tableHeader'  width=250 colspan=2 rowspan=2 nowrap>$strTableName</td>
          <td class='tableHeader' colspan=$intMax align=center>$strTableDate</td>
        </tr>
        <tr class='tableHeader'>
          $strResultT
        </tr>
    ...
    $arrShift = array(); // data shift karyawan
    $arrTotal = array(); // menampung data total
    $strSQL = "
...
foreach ($arrNoList AS $rowDb) //untuk karyawan yang tidak mempunyai division code
      {
        ...
        $strResult .= "
          <tr>
            ...
            <td nowrap width=50 style='border-left:none'><input type='button' name='genShift$intRow' value='roster'  onClick='roast($intRow)'></td>
        ";
        for($tgl = 1; $tgl <= $intMax; $tgl++)
        {
        ...
        $strRoster  = "
          <table cellspacing=0 class=plain>
            <tr>
              ...
              <td>
                <input type=\"button\" name=\"apply$intRow\" onClick=\"apply($intRow)\" value=\"".getWords("apply")."\">&nbsp;
                <input type=\"button\" name=\"clear$intRow\" onClick=\"clearShift($intRow)\" value=\"".getWords("clear")."\">&nbsp;
              </td>
            </tr>
            <tr>
              <td>Range</td>
              <td>:</td>
              <td>".getComboFromArray($arrDay, "dataStart$intRow", 0, "", "", false)
                   ."&nbsp;&nbsp;&nbsp;-&nbsp;&nbsp;&nbsp;&nbsp;"
                   .getComboFromArray($arrDay, "dataEnd$intRow", $intMax-1, "", "", false)."</td>
              <td><input type=\"button\" name=\"applyAll$intRow\" onClick=\"applyAll($intRow)\" value=\"".getWords("apply to all")."\"></td>
            </tr>
          </table>
        ";
    ...
    foreach($arrDivList AS $strDivCode => $arrTmpDiv) // per division
    {
      $strResult .= "
        <tr>
          <td colspan='$intColumn' nowrap class='bgNewRevised'>
          <strong>$strDivCode - " .$arrTmpDiv['name']."</strong>&nbsp;
          </td>
        </tr>
      ";
      // tampilkan data employee di divisi tersebut
      $arrDivEmpl = (isset($arrTmpDiv['employee'])) ? $arrTmpDiv['employee'] : array();
      ...
      if (isset($arrDeptList[$strDivCode]))
      {
        ...
        {            
          ...
          $arrDeptEmpl = (isset($arrTmpDept['employee'])) ? $arrTmpDept['employee'] : array();
          ...
          if (isset($arrSectList[$strDivCode][$strDeptCode]))
          {
            ...
            //if ($strDeptCode == 'MRL') print_r($arrTmpSect);
              ...
              $arrSectEmpl = (isset($arrTmpSect['employee'])) ? $arrTmpSect['employee'] : array();
    ...
    // footer
    global $strDataEmployee, $strDataDivision, $strDataDepartment, $strDataSection, $strDataSubSection, $strDataBranch;
    $strResult .= "
        <tr>
           <td colspan=4>&nbsp;</td>
           <td colspan = $intMax nowrap>
            ...
            <input type=hidden name='dataBranch'     id='dataBranch'     value='$strDataBranch'>
  ...
  function saveData($db, &$strError, $strKriteria)
  {
    ...
    $arrShiftList = array(); // untuk menyimpan info tentang shift
    if ($intTotalData > 0 )
    {
  ...
  if ($db->connect()) {
    ...
    $strInputActive = getEmployeeActiveList("dataActive", $strDataActive, $strEmptyOption," style=\"width:$intDefaultWidthPx\" ");
  ...
  if (isset($_REQUEST['btnPrint']))
  {
     ...
     $resDb = $db->execute($strSQL);
     while ($rowDb = $db->fetchrow($resDb))
        $arrShiftType[] = array("code" => $rowDb['code'], "name" => $rowDb['note']);
