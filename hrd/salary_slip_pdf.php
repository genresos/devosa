<?php
  ...
  if ($db->connect())
  {
    ...
    if (!is_numeric($fltHourPerMonth)) $fltHourPerMonth = "173"; // default
    ...
    $arrIncome = array(
      ...
      "transport" => array("name" => "Transportasi", "code" => "transport"),
      "housing" => array("name" => "Perumahan", "code" => "housing"),
      "conjuncture" => array("name" => "Konjungtur", "code" => "conjuncture"),
      "leave" => array("name" => "Cuti", "code" => "leave"),
      "meal" => array("name" => "Uang Makan", "code" => "lunch"),
    );

    $fltZakatPercent = (isset($arrSetting["zakat_deduc"])) ? $arrSetting["zakat_deduc"] : "0";
    if (!is_numeric($fltZakatPercent)) $fltZakatPercent = 0;

    $strBasicSalaryName = (isset($arrSetting["basicsalary_name"])) ? $arrSetting["basicsalary_name"] : "Gaji";

    if (isset($arrSetting["transport_name"])) $arrIncome["transport"]['name'] = $arrSetting["transport_name"];
    if (isset($arrSetting["housing_name"])) $arrIncome["housing"]['name'] = $arrSetting["housing_name"];
    if (isset($arrSetting["conjuncture_name"])) $arrIncome["conjuncture"]['name'] = $arrSetting["conjuncture_name"];
    if (isset($arrSetting["leave_name"])) $arrIncome["leave"]['name'] = $arrSetting["leave_name"];
    if (isset($arrSetting["meal_name"])) $arrIncome["meal"]['name'] = $arrSetting["meal_name"];
    ...
    foreach ($_REQUEST AS $kode => $value)
    {
      if (substr($kode,0,5) == 'chkID')
      {
        ...
        if ($rowDb = $db->fetchrow($resDb))
        {
          ...
          //--- CARI INCOME DULU
          // cari basic
          $strDetailIncome = array();
          $strDetailIncome[] = array("field" => $strBasicSalaryName, "value" => standardFormat($rowDb['basic_salary'] - $rowDb['attendanceDeduction'],true,0));
          ...
          foreach ($arrIncome AS $index => $arrInfo)
          {
            ...
            $strDetailIncome[] = array("field" => $strName, "value" => standardFormat($rowDb[$strField],true,0));
            $fltTotalIncome += $rowDb[$strField];
          }

          $fltOT = $rowDb['ot1'] + $rowDb['ot2'] + $rowDb['ot3'] + $rowDb['ot4'];
          $strDetailIncome[] = array("field" => $strOTName, "value" => standardFormat($fltOT,true,0));
          ...
          foreach ($arrAllowType AS $strCode => $strName)
          {
            ...
            $strDetailIncome[] = array("field" => $strName, "value" => standardFormat($fltAmount,true,0));
            $fltTotalIncome += $fltAmount;
          }

          // -- CARI POTONGAN
          $strDetailDeduction = array();
          $strDetailDeduction[] = array("field" => $strJamsostekName, "value" => standardFormat($rowDb['jamsostekDeduction'],true,0));
          $fltTotalDeduction += $rowDb['jamsostekDeduction'];
          if ($strLoanShow)
          {
            $strDetailDeduction[] = array("field" => $strLoanName, "value" => standardFormat($rowDb['loan'],true,0));
          }
          ...
          foreach ($arrDeducType AS $strCode => $strName) {
            ...
            $strDetailDeduction[] = array("field" => $strName, "value" => standardFormat($fltAmount,true,0));
