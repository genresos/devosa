  ...
  */
  class clsSalaryCalculation
  {
    ...
    var $strKriteria; // detail data pinjaman, karena harus diupdate juga infonya
		var $approvedOvertimeId; //overtime id yang di approved
    ...
    */
    ...
    function clsSalaryCalculation($db, $strID = "", $bolIrregular = false, $strDataDate = "", $arrKriteria = array(), $strDateFrom = "", $strDateThru = "", $strDateFromSalary = "", $strDateThruSalary = "", $bolManagerial = false, $strApprovedOvertimeId = null)

    {
      ...
      $this->arrDA        = array();
      $this->arrDD        = array();
      $this->approvedOvertimeId = $strApprovedOvertimeId;
			$this->arrayApprovedOvertime = null;
			if (!empty($this->approvedOvertimeId)){
				$this->createArrayApprovedOvertimeByEmployee();
			}
      $this->strKriteria  = "";
      // ambil data setting
    ...
    */
    function initEmployee($strKriteria = "")
    {
      ...
      $strSQL = "
        SELECT te.id, te.employee_id, te.employee_name, te.id_company,  te.management_code, te.division_code, te.zakat, te.get_jamsostek, te.get_bpjs, te.id_cost_center,
      ...
      while ($row = $this->data->fetchrow($res))
      {
        if ($row['id'] != "")
        {
          ...
          $row['grouper'] = $row['id_cost_center'];
    ...
    function initSalaryMaster($strDataDate, $strDateFrom = "", $strDateThru = "", $strDateFromSalary = "", $strDateThruSalary = "")
    {
      ...
      if ($this->strDataID != "")
      {
        ...
        if ($row = $this->data->fetchrow($res))
        {
          ...
          $this->arrData['is_managerial'] = $row['is_managerial'];
        ...
        if (strtotime($strJoinDate) > strtotime($intStart))
        {
          ...
          else
          {
            if ($this->arrEmployee[$strID]['is_all_day']=='t')
            {
              ...
              $intResult = $objDt->getTotalWorkDayNoHoliday($this->data,$strJoinDate, $intFinish);
            }
            elseif ($this->arrEmployee[$strID]['is_sat_in']=='t') {
          ...
          if ($strResignDate != "")
          {
            if(strtotime($strResignDate) < strtotime($intFinish))
            {
              ...
              else
              {
                if ($this->arrEmployee[$strID]['is_all_day']=='t')
                {
                  $fltprorate = strtotime($strResignDate) - strtotime($intStart);
                  $fltprorate = floor($fltprorate/(60*60*24))+1;
                  $intResult = $fltprorate;
                }
                elseif ($this->arrEmployee[$strID]['is_sat_in']=='t') {
...
//      die($strSQL);
	...
	function calculateAbsenceDeduction()
    {
        ...
        while ($row = $this->data->fetchrow($res))
        {
              ...
              && $this->arrEmployee[$row['id']]['position_group'] >= 2)
          {
            ...
            $intDeductionBase += $this->arrDA['Position_Allowance'][$row['id']]['amount'];
            $intDeductionBase += $this->arrDA['Prestasi_Allowance'][$row['id']]['amount'];
            $intDeductionBase += $this->arrDA['Excess_Hours'][$row['id']]['amount'];
          }
    ...
    */
	function calculateLateDeduction()
    {
        ...
        while ($row2 = $this->data->fetchrow($res2))
        {
            ...
            {
              $this->arrDetail[$row2['id']]['late_deduction'] = $row2['total_late_frequency'] / $intNumberOfDays * 0.125 * $this->arrDA['Position_Allowance'][$row2['id']]['amount'] * $fltInverseProrate;
            }
            else
            {
    ...
    */
    function calculateJamsostek()
    {
      ...
      foreach($this->arrDetail AS $strID => $arrInfo)
      {
        ...
        $basePension = $this->arrDetail[$strID]['base_jamsostek'];
        $baseBPJS = $this->arrDetail[$strID]['base_jamsostek'];
      ...
      if ($this->strDataID == "") // insert new
      {
        ...
        $strSQL = "
          INSERT INTO hrd_salary_master (
            ...
            created, created_by, modified, modified_by, is_managerial
          )
          VALUES (
            ...
            now(), '$strUserID', now(), '$strUserID', $strBoolean
          );
        ";
      }
      else
    ...
    */
    ...
    /* createArrayApprovedOvertimeByEmployee */
		function createArrayApprovedOvertimeByEmployee(){
			global $db;
			if (!empty($this->approvedOvertimeId) && $db->connect()){
				$strSQL = "SELECT * FROM hrd_salary_detail WHERE id_salary_master='".$this->approvedOvertimeId."'";
				$resDb = $db->execute($strSQL);
      	while ($rowDb = $db->fetchrow($resDb)){
      		$this->arrayApprovedOvertime[$rowDb['id_employee']] = $rowDb;
      	}
			}
		}
		/* End createArrayApprovedOvertimeByEmployee */
		/* getApprovedOvertimeData : fungsi untuk mengambil nilai overtime yang sudah di approved */
		function getApprovedOvertimeData(){
			if (!empty($this->approvedOvertimeId) && !empty($this->arrayApprovedOvertime)){
				foreach($this->arrDetail AS $strID => $arrInfo){
					$this->arrDetail[$strID]['ot1_min'] = isset($this->arrayApprovedOvertime[$strID]['ot1_min']) ? $this->arrayApprovedOvertime[$strID]['ot1_min'] : 0;
					$this->arrDetail[$strID]['ot2_min'] = isset($this->arrayApprovedOvertime[$strID]['ot2_min']) ? $this->arrayApprovedOvertime[$strID]['ot2_min'] : 0;
					$this->arrDetail[$strID]['ot3_min'] = isset($this->arrayApprovedOvertime[$strID]['ot3_min']) ? $this->arrayApprovedOvertime[$strID]['ot3_min'] : 0;
					$this->arrDetail[$strID]['ot4_min'] = isset($this->arrayApprovedOvertime[$strID]['ot4_min']) ? $this->arrayApprovedOvertime[$strID]['ot4_min'] : 0;
					$this->arrDetail[$strID]['total_ot_min'] = isset($this->arrayApprovedOvertime[$strID]['total_ot_min']) ? $this->arrayApprovedOvertime[$strID]['total_ot_min'] : 0;
					$this->arrDetail[$strID]['otx_min'] = isset($this->arrayApprovedOvertime[$strID]['otx_min']) ? $this->arrayApprovedOvertime[$strID]['otx_min'] : 0;
					$this->arrDetail[$strID]['ot_day'] = isset($this->arrayApprovedOvertime[$strID]['ot_day']) ? $this->arrayApprovedOvertime[$strID]['ot_day'] : 0;
					$this->arrDetail[$strID]['ot_per_hour'] = isset($this->arrayApprovedOvertime[$strID]['ot_per_hour']) ? $this->arrayApprovedOvertime[$strID]['ot_per_hour'] : 0;
					$this->arrDetail[$strID]['ot1'] = isset($this->arrayApprovedOvertime[$strID]['ot1']) ? $this->arrayApprovedOvertime[$strID]['ot1'] : 0;
					$this->arrDetail[$strID]['ot2'] = isset($this->arrayApprovedOvertime[$strID]['ot2']) ? $this->arrayApprovedOvertime[$strID]['ot2'] : 0;
					$this->arrDetail[$strID]['ot3'] = isset($this->arrayApprovedOvertime[$strID]['ot3']) ? $this->arrayApprovedOvertime[$strID]['ot3'] : 0;
					$this->arrDetail[$strID]['ot4'] = isset($this->arrayApprovedOvertime[$strID]['ot4']) ? $this->arrayApprovedOvertime[$strID]['ot4'] : 0;
					$this->arrDetail[$strID]['overtime_allowance'] = isset($this->arrayApprovedOvertime[$strID]['overtime_allowance']) ? $this->arrayApprovedOvertime[$strID]['overtime_allowance'] : 0;
					if ((isset($this->arrConf['overtime_allowance_tax'])) && $this->arrConf['overtime_allowance_tax'] == 't')
						$this->arrDetail[$strID]['base_tax']       += $this->arrDetail[$strID]['overtime_allowance'];
					if ((isset($this->arrConf['overtime_allowance_jams'])) && $this->arrConf['overtime_allowance_jams'] == 't')
						$this->arrDetail[$strID]['base_jamsostek'] += $this->arrDetail[$strID]['overtime_allowance'];
