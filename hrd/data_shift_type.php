<?php
  ...
  function getDataRows($rowDb, $intRows) {
    ...
    $strResult .= "  <td nowrap align=right><input type=hidden name=detailStart$intRows value=\"" .$rowDb['start_time']. "\" disabled>" .$rowDb['start_time']. "&nbsp;</td>";
    $strResult .= "  <td nowrap align=right><input type=hidden name=detailFinish$intRows value=\"" .$rowDb['finish_time']. "\" disabled>" .$rowDb['finish_time']. "&nbsp;</td>";
  ...
  function getDataRows2($rowDb, $intRows) {
    ...
    $strResult .= "  <td nowrap><input type=checkbox name='chkID2$intRows' value=\"" .$rowDb['id']. "\">\n";
    $strResult .= "  <input type=hidden name=detailDays$intRows value=\"" .count($arrDays) . "\" disabled>";
    $strResult .= "  <td nowrap><input type=hidden name=detailPattern$intRows value=\"" .$rowDb['pattern']. "\" disabled>" .$rowDb['pattern']. "&nbsp;</td>";
  ...
  function saveData($db, &$strError) {
    ...
    */
    ...
    if ($strDataID == "") {
      ...
      $strSQL .= "\"code\",\"start_time\", finish_time, shift_off, is_shift, \"break_duration\", note, shift_allowance, additional_ot) ";
      $strSQL .= "VALUES(now(),'" .$_SESSION['sessionUserID']. "','" .$_SESSION['sessionUserID']. "', ";
      $strSQL .= "'$strDataCode', '$strDataStart','$strDataFinish', '$strDataShiftOff', '$strDataIsShift', '$strDataBreak', '$strDataNote', $strDataShiftAllowance, $strDataAddOT); ";
  ...
  foreach ($arrPositionGroup AS $strCode)
  {
     $strInputAllowanceGroup .= "
               <tr>
                  ...
                  <td>:</td>
                  <td><input name=\"dataAllowance".strtoupper($strCode)."\" type=\"text\" id=\"dataAllowance".strtoupper($strCode)."\" size=\"20\" maxlength=\"10\" value=\"0\" ></td>
                </tr>";
