<?php
  ...
  if ($db->connect()) 
  {
    ...
    $strReadonly = (scopeCBDataEntry($arrData['dataEmployeeID'], $_SESSION['sessionUserRole'], $arrUserInfo)) ? "readonly" : "";
    ...
    $fFilter->addInputAutoComplete(getWords("employee ID"), "dataEmployeeID", getDataEmployee($arrData['dataEmployeeID']), "style=width:$strDefaultWidthPx ".$strReadonly, "string", false);
    $fFilter->addLabelAutoComplete("", "dataEmployeeID", "");

    $fFilter->addInput(getWords("minimum cost filter"), "dataClaimFrom", $arrData['dataClaimFrom'], "", "numeric", false, true, true);
    $fFilter->addInput(getWords("maximum cost filter"), "dataClaimThru", $arrData['dataClaimThru'], "", "numeric", false, true, true);
    ...
    $fFilter->addSelect(getWords("status"), "dataEmployeeStatus", getDataListEmployeeStatus($arrData['dataStatus'], true, array("value" => "", "text" => "", "selected" => true)), array("style" => "width:$strDefaultWidthPx"), "", false);  
    ...
    $fFilter->addSelect(getWords("sub section"), "dataSubsection", getDataListSubSection($arrData['dataSubsection'], true), array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['sub_section'] == ""));
...
if (!isset($_REQUEST['btnExportXLS']))
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("employee id"), "employee_id", array('width' => 100), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 15));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("bank account"), "bank2_account", array("width" => 150),  array("align" => "center"), true, true, "", "", "string", true, 25));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("bank"), "bank_name", array("width" => 150),  array("align" => "center"), true, true, "", "", "string", true, 25));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("bank account name"), "bank2_account_name", array("width" => 150),  array("align" => "center"), true, true, "", "", "string", true, 35));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("total claim"), "total_claim", array("width" => 150),  array("align" => "right"), true, true, "", "formatNumber()", "numeric", true, 15));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("branch"), "branch_name", array("width" => 100),  array("nowrap" => "nowrap"), true, true, "", "", "string", true, 10));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("division code"), "division_code", array("width" => 100),  array("nowrap" => "nowrap"), true, true, "", "", "string", false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("department code"), "department_code", array("width" => 150),  array("nowrap" => "nowrap"), true, true, "", "", "string", false));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("section code"), "section_code", array("width" => 100),  array("nowrap" => "nowrap"), true, true, "", "", "string", false));
    ...
    //format tanggal
    if (!validStandardDate($strDataDateFrom)) {
      $strDataDateFrom = date("Y-m-d");
    }
    if (!validStandardDate($strDataDateThru)) {
      $strDataDateThru = date("Y-m-d");
    }


    $strSQL       = "
      SELECT t1.id, employee_id, employee_name, bank_name, division_code, t3.status,grade_code,position_code,active,bank2_code,id_company,sub_section_code, 
      department_code, section_code, t1.branch_code, branch_name, bank2_account, bank2_account_name, SUM(approved_cost)::TEXT AS total_claim 
    ...
    $strSQL .= " GROUP BY t1.id, employee_id,t3.status, employee_name, bank2_account, bank2_account_name, bank_name, division_code , department_code , section_code , t1.branch_code , branch_name ";
