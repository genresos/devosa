  ...
  function getData($db, &$intRows, $strDataID = "")
  {
    ...
    if ($bolSave && $bolError)
    {
       ...
       {  
          ...
          (isset($_REQUEST['detailMedicalDate'.$i])) ? $strMedicalDate = $_REQUEST['detailMedicalDate'.$i] : $strMedicalDate = date("Y-m-d");
          (isset($_REQUEST['detailMedicalDateThru'.$i])) ? $strMedicalDateThru = $_REQUEST['detailMedicalDateThru'.$i] : $strMedicalDateThru = date("Y-m-d");
          (isset($_REQUEST['detailClaimDate'.$i])) ? $strClaimDate = $_REQUEST['detailClaimDate'.$i] : $strClaimDate = date("Y-m-d");
    ...
    for ($i = $intShown; $i <= $intAdd; $i++)
    {
      ...
      $strResult .= "  <td nowrap>" .getFamilyList($db, "detailName$intRows","","<option value=''> </option><option value=\"".$arrData['dataEmployeeName']."|-1\">".$arrData['dataEmployeeName']."</option>", $strFamCriteria  , $strDisabled). "&nbsp;<font color=\"red\">*</font></td>";
    ...
    if ($bolOK && $strIDEmployee != "")
    {
         ...
         foreach ($arrQuotaYear AS $strType => $arrQuotaDetail)
         {
           if ($strType == MEDICAL_TYPE_OUTPATIENT)
           {
              if (isset($arrTotal[$strYear][$strType]) && $arrTotal[$strYear][$strType] > $arrQuotaDetail)
              {
                ...
                $strError .= getWords(". remaining quota for ". $ARRAY_MEDICAL_TREATMENT_GROUP[$strType]. " treatment in $strYear =  ". standardFormat($arrQuotaDetail, true, 0));
                return false;
              }
              continue;
           }
      ...
      if ($strDataID != "") { // simpan detail claim yang dilakukan
        ...
        for ($i = 1; $i <= $intMax; $i++) {
          ...
          if ($strID == "") {
            if ($strName != "" AND $strDE != 1) { // insert new data
