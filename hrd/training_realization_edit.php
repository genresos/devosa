<?php
  ...
  $dataPrivilege = getDataPrivileges("training_realization_list.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove);
...
//---- INISIALISASI ----------------------------------------------------
  ...
  function getCurrencyList($strName, $strDefault = "IDR")
  {
    ...
    if (!$bolOK && $strDefault != "") $strResult .= "<option value='$strDefault' selected>$strDefault</option>";
    $strResult .= "</select>";
    return $strResult;
  }
  ...
  function getTrainingExpense($db, $strDataRequestID = "", $strDataEmployeeID = "")
  {
    ...
    $strResult .= "  <td colspan=4>&nbsp;<a href=\"javascript:showMoreInput();\">" .getWords('more'). "</a></td></tr>\n";
    $strResult .= "  </table>\n";
  ...
  function getTimeDetail($db, $strDataRequestID = "", $strDataEmployeeID = "", $arrData = array(), $strInputDuration = "")
  {
    global $words;
    global $_REQUEST;
    ...
    if ($bolOK)
    {
      ...
      if ($strDataRequestID != "")
      {
        ...
        for ($i = 1; $i <= $intTotal; $i++)
        {
          ...
          {
            $fltAmount = (isset($_REQUEST['detailAmount'.$i])) ? str_replace(",", "", $_REQUEST['detailAmount'.$i]) : 0;
  ...
  if ($db->connect()) 
  {
    ...
    $arrTmp = array("public", "inhouse", "internal", "sharing session");
    ...
    if (!$bolCanEdit) $strDisabledSave = "disabled ";
    else if ($objUP->isUserEmployee())
    {
      if ($arrData['dataStatus'] >= REQUEST_STATUS_APPROVED) $strDisabledSave = "disabled";
    }
