...
//    echo $strCriteria;
    ...
    $myDataGrid->addColumn(new DataGrid_Column("ID", "employee_id", array('width' => 70), array("nowrap" => "nowrap"), true, true, "", "", "string", true, 12, false));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("due date"), "dueDate", array("width" => 70),  array("nowrap" => "nowrap"), true, true, "", "formatDate()", "string", true, 12));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("jamsostek no."), "jamsostek_no", array("width" => 80), null, true, true, "", "", "string", true, 15));
		$myDataGrid->addColumn(new DataGrid_Column(getWords("allowance list"), "allowance_list", array("width" => 80), null, true, true, "", "", "string", true, 15));
    if ($isExport)
    {
    ...
    foreach($dataset as $rowDb)
    {
      ...
      $arrayAllowance = getFKRSalaryList($db, $rowDb['id']);
      $rowDb['allowance_list'] = createAllowanceTable($arrayAllowance, $isExport);
      if ($bolUpdateOnly)
      {
  ...
  function getEmployeeUpdated($strCriteria)
  {
    ...
    return $tblEmployee->findAll("flag <> 0 AND (\"link_id\" is not NULL) ".$strCriteria, 
                                  "*, (EXTRACT(YEAR FROM AGE(birthday))) AS umur",
                                  null, null, null,
                                  "link_id"
                                 );
  ...
  function createAllowanceTable($arrayAllowance = null, $isExport = false){
  	$allowanceTable = '';
  	if (!empty($arrayAllowance)){
  		if ($isExport){
  			$totalAllowance = 0;
  			for ($i = 0;$i < count($arrayAllowance);$i++){
  				$totalAllowance = $totalAllowance + $arrayAllowance[$i]['amount'];
  				if ($allowanceTable == ''){
  					$allowanceTable = $arrayAllowance[$i]['name'].' => '.number_format($arrayAllowance[$i]['amount'],0,',','.');
  				}else{
  					$allowanceTable .= ', '.$arrayAllowance[$i]['name'].' => '.number_format($arrayAllowance[$i]['amount'],0,',','.');
  				}
  			}
  			$allowanceTable .= ', Total => '.number_format($totalAllowance,0,',','.');
  		}else{
	  		$allowanceTable = '<table>';
	  		$totalAllowance = 0;
	  		for ($i = 0;$i < count($arrayAllowance);$i++){
	  			$totalAllowance = $totalAllowance + $arrayAllowance[$i]['amount'];
	  			$allowanceTable .= '<tr><td>'.$arrayAllowance[$i]['name'].'</td><td align="right">'.number_format($arrayAllowance[$i]['amount'],0,',','.').'</td></tr>';
	  		}
	  		$allowanceTable .= '<tr><td><b>Total</b></td><td align="right"><b>'.number_format($totalAllowance,0,',','.').'</b></td></tr>';
	  		$allowanceTable .= '</table>';
	  	}
  	}
