<?
  ...
  $dataPrivilege = getDataPrivileges("absence_edit.php", $bolCanView, $bolCanEdit,  $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  $bolCanView = ($bolCanApprove || $bolCanCheck || $bolCanAcknowledge);
  if (!$bolCanView) die(getWords('action denied'));
  ...
  function saveData($db) {
    ...
    writeLog(ACTIVITY_EDIT, MODULE_EMPLOYEE, $strDataEmployeeName ." - ". $strDataDateFrom ." - ". $strDataAbsenceTypeCode." - ". $strDataNote, $intStatus);
  ...
  if ($db->connect()) {
    ...
    if ($bolCanView) {
      if (isset($_REQUEST['btnSave'])) saveData($db);
