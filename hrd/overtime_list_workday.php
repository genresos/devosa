<?
  ...
  function getData($db, $strDataDateFrom, $strDataDateThru, &$intRows, $strKriteria = "",$strOrder = "")
  {
    ...
    $strSQL  = "
    SELECT t1.*,
    t2.employee_id,  t2.employee_name, t2.division_code, t2.department_code, t2.section_code, t2.sub_section_code,
    ...
    AND holiday_ot = FALSE $strKriteria ORDER BY t1.overtime_date DESC, t2.division_code, t2.employee_name";
    ...
    while ($rowDb = $db->fetchrow($resDb))
    {
      ...
      $strResult .= " <td>".getUserName($db, $rowDb['approved2_by'])."</td>";
      $strResult .= " <td>".substr($rowDb['approved2_time'],0,16)."</td>";
    ...
    foreach ($_REQUEST as $strIndex => $strValue)
    {
      ...
      if (substr($strIndex,0,5) == 'chkID')
      {
        ...
        if ($rowDb = $db->fetchrow($resDb))
        {
          ...
          $strBody.= "Overtime Date: ".$rowDb['overtime_date'];
          ...
          if (isProcessable($rowDb['status'], $intStatus))
          {
            ...
            writeLog(ACTIVITY_EDIT, MODULE_EMPLOYEE, $rowDb['employee_name']." - ". $rowDb['overtime_date'], $intStatus);
          }
        }
      }
      $resExec = $db->execute($strSQL);
...
//    //$bolCanApprove2 = isEligibleApprove2($db);
    ...
    $strInputDateFrom = "<input type=text name=dataDateFrom id=dataDateFrom size=15 maxlength=10 value=".($strDateFrom = getInitialValue("DateFrom", date("Y-m-")."01")).">";
