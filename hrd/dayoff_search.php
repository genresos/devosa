<?php
    ...
    while ($rowDb = $db->fetchrow($resTmp))
    {
      ...
      if (!$bolPrint)
      {
        if ($rowDb['status'] != 0)
        {
          ...
          $strShowInfo .= "  <td nowrap align=center><a href=\"javascript:openWindowById('detailRecord$intRows')\" title=\"" .getWords("show record info")."\">" .getWords("show")."$strDiv</a></td>\n";
        }
        else
        {
      ...
      if ($bolIsEmployee && $strUserIDEmp == $rowDb['id_employee']) $strChkDis = " disabled ";
      $strResult .= "
          <tr class=\"" .getApprovalClassStyle($rowDb['status']). "\">
            ...
            <td align=center>&nbsp;<a href=overtime_application_edit.php?btnPrint=Print&dataID=" .$rowDb['id_application']. " target='_blank'>" .getWords('print')."</a></td>
            <td align=center>&nbsp;<a href=overtime_application_edit.php?dataID=" .$rowDb['id_application']. ">" .getWords('edit'). "</a></td>
  ...
  {
    $strResult = "";
    if ($intStatus == REQUEST_STATUS_NEW) $strResult = "bgNewData";
    else if ($intStatus == REQUEST_STATUS_VERIFIED) $strResult = "bgVerifiedData";
    else if ($intStatus == REQUEST_STATUS_CHECKED) $strResult = "bgCheckedData";
  ...
  function deleteData($db)
  {
    ...
    $arrProcess = array(); // untuk menampung id yang sudah diproses
    ...
    foreach ($_REQUEST as $strIndex => $strValue) {
      if (substr($strIndex,0,5) == 'chkID') 
      {
        ...
        if (isset($arrProcess[$strValue])) continue;
        else $arrProcess[$strValue] = $strValue;
  ...
  function approvedData($db)
  {
    ...
    $arrProcess = array(); // untuk menyimpan id yang sudah diproses
    
    if ($objUP->isUserEmployee() || !$bolCanApprove) return false;
    ...
    foreach ($_REQUEST as $strIndex => $strValue)
    {
      if (substr($strIndex,0,5) == 'chkID')
      {
        ...
        if (isset($arrProcess[$id])) continue;
        else $arrProcess[$id] = $id;
  ...
  function changeStatusData($db,$intStatus)
  {
    ...
    $strUpdate = "";
    if ($intStatus == REQUEST_STATUS_VERIFIED) $strUpdate = "acknowledge_time = now(), acknowledge_by = '$strmodified_by', ";
    else if ($intStatus == REQUEST_STATUS_CHECKED) $strUpdate = "checked_time = now(), checked_by = '$strmodified_by', ";
    else if ($intStatus == REQUEST_STATUS_DENIED) $strUpdate = "denied_time = now(), denied_by = '$strmodified_by', ";
    $i = 0;
    $bolOK = true;
    $db->execute("begin");
    foreach ($_REQUEST as $strIndex => $strValue) 
  ...
  if ($db->connect()) 
  {
    ...
    {
      if ($bolCanApprove) approvedData($db);
    }
    ...
    if (isset($_SESSION['sessionDataDateFrom']))  $strDataDateFrom = $_SESSION['sessionDataDateFrom'];
    if (isset($_SESSION['sessionDataDateThru']))  $strDataDateThru = $_SESSION['sessionDataDateThru'];
