<?php
  ...
  $dataPrivilege = getDataPrivileges("training_request_list.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove);
  if (!$bolCanView) die(getWords('view denied'));

  $bolPrint = (isset($_REQUEST['btnPrint']));

  ($bolPrint) ? $strMainTemplate = getTemplate("training_evaluation_employee_print.html") : $strTemplateFile = getTemplate("training_evaluation_employee.html");
  ...
  function getData($db, $strRequestID, $strEmployeeID, $strTrainerID = "", $bolReadonly = true) 
  {
    ...
    if ($row = $db->fetchrow($res))
    {
      ...
      if ($row['training_date'] != $row['training_date_thru'])
        $arrData['dataDate'] .= " ".getWords("until")." ".$objDt->getDateFormat($row['training_date_thru'], "d M Y");
    }
    ...
    if (!isset($_REQUEST['btnPrint']))
    {
      ...
      $arrData['dataEmployee'] .= "<input type='submit' name='btnGetInfo' id='btnGetInfo' value=\"" .getWords("show evaluation"). "\"> "; // untuk refresh data setelah memilih karyawan
   } else {
     $strSQL = "
        SELECT t1.*, t2.employee_name, t2.employee_id 
        FROM (
      ...
      */  if (!isset($_REQUEST['btnPrint'])) 
      {
        ...
        $arrData['dataDetail'] .= "
          <tr>
            <td colspan='$intCols' style='font-weight:bold;color:white;background-color:darkgray'>" .strtoupper(getWords("feedback")). "&nbsp;</td>
          </tr>
          <tr>
            <td nowrap colspan=2>
