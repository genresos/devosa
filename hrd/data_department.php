<?php
  ...
  $strWordsLISTOFDIVISIONDEPARTMENTANDSECTION = getWords ("list of division, department and section");
...
//    $strSQL .= "WHERE management_code LIKE '".getCompanyCode()."%' $strKriteria ORDER BY $strOrder management_code ";
    ...
    while ($rowMan = $db->fetchrow($resMan)) {
     $intRows++;
      ...
      $strResult .= "  <td nowrap align=center><a href='javascript:addDataDivision($intRows)'><img src=\"../images/add.png\" border=0 alt='".getWords("add")."' title='" .$words['add department']. "' /></a>&nbsp;</td>\n";
     ...
     //$bolSatHol = ($rowDiv['saturday_holiday'] == 't') ? "&radic;" : "";
      ...
      while ($rowDiv = $db->fetchrow($resDiv)) {
       ...
       $intDiv++;
        ...
        $strResult .= "  <td nowrap align=center><a href='javascript:addData($intRows)'><img src=\"../images/add.png\" border=0 alt='".getWords("add")."' title='" .$words['add department']. "' /></a>&nbsp;</td>\n";
        ...
        while ($rowDb = $db->fetchrow($resDb)) {
          ...
          $strResult .= "  <td nowrap align=center><a href='javascript:addDataSection($intRows)'><img src=\"../images/add.png\" border=0 alt='".getWords("add")."' title='" .$words['add section']. "' /></a>&nbsp;</td>\n";

          $intSection = 0;
          // cari data section
          $strSQL  = "SELECT * FROM hrd_section WHERE department_code = '" .$rowDb['department_code']. "' AND division_code = '" .$rowDb['division_code']. "' AND management_code = '" .$rowMan['management_code']. "' ORDER BY section_code ";
          ...
          while ($rowSec = $db->fetchrow($resSec)) {
            ...
            $strResult .= "  <td nowrap align=center><a href='javascript:addDataSubSection($intRows)'><img src=\"../images/add.png\" border=0 alt='".getWords("add")."' title='" .$words['add subsection']. "' /></a>&nbsp;</td>\n";

            //cari dta subsection
            $intSubSection = 0;
            $strSQL  = "SELECT * FROM hrd_sub_section WHERE department_code = '" .$rowSec['department_code']. "' AND division_code = '" .$rowSec['division_code']. "' AND management_code = '" .$rowMan['management_code']. "' ";
 ...
 $strButtonListing = generateRoleButtons($bolCanEdit, $bolCanDelete, false, false,false );
