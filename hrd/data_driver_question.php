<?php
  ...
  $dataPrivilege = getDataPrivileges(basename("data_reference_type.php"), $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  ...
  $strWordsReferenceQuestion = getWords("input driver question");
  ...
  $isNew = ($strDataID == "");
  $tbl = new cModel("hrd_recruitment_driver_question", "driver question");

  $formInput = "";
  if ($bolCanEdit)
  {
    $f = new clsForm("formInput", 1, "100%", "");
    $f->caption = strtoupper(vsprintf(getWords("input data %s"), getWords("Driver question")));
    $f->addHidden("dataID", $strDataID);
    $f->addInput(getWords("question"), "question", "", array("size" => 50), "string", true, true, true);
    $f->addInput(getWords("note"), "note", "", array("size" => 50), "string", false, true, true);
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("question"), "question", array(), array('nowrap' => 'nowrap'), true, true, "", ""));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("note"), "note", array(), array('nowrap' => 'nowrap'), true, true, "", ""));
...
//--------------------------------------------------------------------------------
  ...
  function saveData()
  {
   ...
   if ($isNew) 
    ...
    {
      $bolSuccess = $tbl->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    //---------- END Jika Data Baru---------------------------------------------//
