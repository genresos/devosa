<?php
  ...
  function getData($db) {
   ...
   // echo $strSQL;
    ...
    if ($db->connect())
    {
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("id"), "id", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("training date"), "training_date", "", array('nowrap' => '')));
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("expected result"), "result", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("institution"), "name_vendor", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("trainer"), "name_instructor", "", array('nowrap' => '')));

      $myDataGrid->addColumn(new DataGrid_Column(getWords("participant"), "participant", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("total"), "total_participant", array('nowrap' => ''),  array("align" => "right"), false, true, "", "", "numeric", true, 15));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("total cost"), "total_cost", array('nowrap' => ''),  array("align" => "right"), false, true, "", "formatNumber()", "numeric", true, 15));
	  $myDataGrid->addColumn(new DataGrid_Column(getWords("duration") ." ". getWords("days"), "total_duration", array('nowrap' => ''),  array("align" => "right"), false, true, "", "", "numeric", true, 15));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("duration") ." ". getWords("minutes"), "total_hour", array('nowrap' => ''),  array("align" => "right"), false, true, "", "", "numeric", true, 15));
      ...
      foreach($dataset AS $strKey => $arrDetail)
      {
        ...
        if (isset($arrParticipant[$arrDetail['id']]))
        {
          ...
          {
            if ($strParticipant != "") $strParticipant .= "<br>";
  ...
  if ($db->connect()) 
  {
    ...
    $f->addInput(getWords("date from"), "dataDateFrom", getInitialValue("DateFrom", date("Y-m-")."01"), array("style" => "width:$strDateWidth"), "date", false, true, true);
    ...
    $f->addSelect(getWords("training type"), "dataTrainingType", getDataListTrainingType("", true,$arrEmpty), array("style" => "width:$strDefaultWidthPx"), "", false);
    $f->addSelect(getWords("institution"), "dataInstitution", getDataListTrainingVendor("", true, $arrEmpty), array("style" => "width:$strDefaultWidthPx"), "", false);
    $f->addLiteral("","","");
