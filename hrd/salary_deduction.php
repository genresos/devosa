<?php
  ...
  $strSpan2 = 11; // colspan untk bagian paging
  $arrColumns = array();
  $strDeductionList = "";
  ...
  function getData($db, $intRows, $strKriteria = "", $intPage = 1, $bolLimit = true, $strOrder = "")
  {
    ...
    $intRowsLimit = $intTotalData;
    if (!is_numeric($intRowsLimit)) $intRowsLimit = 500;
    ...
    $strSQL .= "t1.employee_status, t1.position_code, t1.grade_code, t1.employee_id_2, ";
    $strSQL .= "t1.division_code, t1.functional_code, t1.branch_penugasan_code, t1.branch_cost_center_code ";
    $strSQL .= "FROM hrd_employee AS t1 ";
    $strSQL .= "LEFT JOIN hrd_position AS t6 ON t1.position_code = t6.position_code ";
    $strSQL .= "WHERE t1.id_company = $strIDCompany AND position_group >= 2 $strKriteria ORDER BY $strOrder employee_name ";
//    die($strSQL);
    if ($bolLimit) {
...
*/
      $strEmpStatus = ($rowDb['employee_status'] == "")? "" : getWords($ARRAY_EMPLOYEE_STATUS[$rowDb['employee_status']]);
      ...
      foreach($arrColumns AS $strKode=>$strAmount) {
        ...
        $strResult .= "<td align='center'><input type=\"text\" name=\"dataDeduction$i"."_$intRows\" size=$intTextWidth maxlength=20 value=\"$fltAmount\" class=numeric $strDisable></td>\n";
      }
      ...
      if ($strDataID != "") {
        ...
        for ($j = 1; $j<=$intColumn;$j++)
        {
          ...
          if ($strKode != "") {
            ...
            if (!is_numeric($fltDeduction)) $fltDeduction = 0;
...
/*
  ...
  function getDeductionData($db)
  {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      $i++;
      $strDataColumn .= "<td align=center ><input type=hidden name=dataDeductionType$i value=\"" .$rowDb['code']."\"> ";
      $strDataColumn .= ($rowDb['name'])."<br />\n";
      $strDataColumn .= "<input $strDisable type=\"submit\" name=\"btnDefault".$rowDb['code']."\" value=\"Get Default\"  style=\"width:75px;font-size:7pt\" onclick=\"return getDefault('" .$rowDb['code']."','" .$rowDb['name']."');\" </td>\n";
    ...
    // handle jika kosong
    if ($i == 0) {
      $strDataColumn .= "<td align=center nowrap>&nbsp;</td>\n";
      $strSpan1++;
      $strSpan2++;
    }
  ...
  function importData($db)
  {
    ...
    if (is_uploaded_file($_FILES["fileData"]['tmp_name']) && $strDeductionType != "") {
      ...
      $arrEmp = array(); // menampung daftar karyawan yagn sudah ada
      ...
      $arrEmpUnknown = array(); // mencatat daftar employee ID yang gak teradftar
      $arrForm = array(); // mencata daftar form yang sudah ada, indexnya adalah form number

      // UPDATE DULU SISA CUTI YG ADA
      //$intRows = 15;
      for ($i=1;$i <= $intRows; $i++) {
        // tampung di variabel, biar pendek codingnya :D
        $arrData = (isset($data->sheets[0]['cells'][$i])) ? $data->sheets[0]['cells'][$i] : array();
  ...
  if ($db->connect()) {
    ...
    if (isset($_REQUEST['dataIDSalarySet'])) $strIDSalarySet = $_REQUEST['dataIDSalarySet'];
    ...
    if (isset($_REQUEST['dataPosition'])) $strDataPosition = $_REQUEST['dataPosition'];
    if (isset($_REQUEST['dataGrade'])) $strDataGrade = $_REQUEST['dataGrade'];
    if (isset($_REQUEST['dataFamilyStatus'])) $strDataFamilyStatus = $_REQUEST['dataFamilyStatus'];
    ...
    if (isset($_REQUEST['dataStatus'])) $strDataStatus = $_REQUEST['dataStatus'];
    if (isset($_REQUEST['dataActive'])) $strDataActive = $_REQUEST['dataActive'];
    if (isset($_REQUEST['dataCostCenter'])) $strDataCostCenter = $_REQUEST['dataCostCenter'];
   ...
   $strKriteria .= $strKriteriaCompany;
    ...
    $arrBasicSalarySet = $tblBasicSalarySet->findAll($strKriteriaCompany, "id, start_date, note, id_company, id_salary_set_source", "", null, 1, "id");
    ...
    $strInputStartDate = getComboFromArray($arrSetSource, "dataIDSalarySet", $strIDSalarySet,"style=\"width:$strDefaultWidthPx\"");

    $tblEmployeeDeduction = new cHrdEmployeeDeduction;
    if(isset($strIDSalarySetSource) &&  $strIDSalarySetSource != "" && $tblEmployeeDeduction->findCount("id_salary_set = $strIDSalarySetSource") > 0)
    {
      $arrEmployeeDeduction  = $tblEmployeeDeduction->findAll("id_salary_set = $strIDSalarySetSource", "*", "", null, 1, "id");
    ...
    $strInputSection = getSectionList($db,"dataSection",$strDataSection, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputSubSection = getSubSectionList($db,"dataSubSection",$strDataSubSection, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputCostCenter = getCostCenterList($db,"dataCostCenter",$strDataCostCenter, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputDepartment = getDepartmentList($db,"dataDepartment",$strDataDepartment, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputBranch = getBranchList($db,"dataBranch",$strDataBranch, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" ");
    $strInputDivision = getDivisionList($db,"dataDivision",$strDataDivision, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputActive = getEmployeeActiveList("dataActive", $strDataActive, $strEmptyOption, $strDisable);
      //handle user company-access-right
    $strInputCompany = getCompanyList($db, "dataCompany",$strDataCompany, $strEmptyOption2, $strKriteria2, "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputPosition = getPositionList($db,"dataPosition",$strDataPosition, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputGrade = getSalaryGradeList($db,"dataGrade", $strDataGrade, $strEmptyOption, "", "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputEmployeeStatus = getEmployeeStatusList("dataStatus", $strDataStatus, $strEmptyOption, "style=\"width:$strDefaultWidthPx\" $strDisable");
    $strInputFamilyStatus = getFamilyStatusList($db, "dataFamilyStatus", $strDataFamilyStatus, $strEmptyOption , "", "style=\"width:$strDefaultWidthPx\" $strDisable");
