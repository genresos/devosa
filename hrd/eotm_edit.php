<?php
  ...
  // ------------------------------------------------------------------------------------------------------------------------------
    if ($bolCanEdit)
    {
      ...
      $f->addSelect(getWords("release month"), "dataMonth", getDataListMonth($arrData['dataMonth']), "style='width:250px'", "", true);
      $f->addSelect(getWords("release year"), "dataYear", getDataListYear($arrData['dataYear']), "style='width:250px'", "", true);
      $f->addSelect(getWords("company"), "dataCompany", getDataListCompany($arrData['dataCompany'], $bolCompanyEmptyOption, $arrCompanyEmptyData, $strKriteria2), array("style" => "width:$strDefaultWidthPx"), "", false);

      $f->addInputAutoComplete(getWords("employee ID"), "dataEmployee", getDataEmployee($arrData['dataEmployee']), "style='width:250px' ", "string", true);
