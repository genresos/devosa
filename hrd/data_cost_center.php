<?php
  ...
  if ($db->connect())
  {
    ...
    if ($bolCanEdit)
    {
      ...
      $f->addTextArea(getWords("cost center code"), "dataCode", "", array("cols" => 97, "rows" => 2), "string", true, true, true);
      $f->addSelect(getWords("company"), "dataCompany", getDataListCompany(-1, true,""));
      $f->addTextArea(getWords("cost center name"), "dataName", "", array("cols" => 97, "rows" => 2), "string", true, true, true);
...
//      $f->addSelect(getWords("member :: company"), "dataMemberCompany", getDataListCompany(-1, true,""), array("multiple" => "multiple"));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("code"), "code", null, array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("company"), "dataid", null, array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("cost center name"), "name", null, array('nowrap' => '')));
    ...
    if ($bolCanEdit){
      ...
      // $myDataGrid->addColumn(new DataGrid_Column("", "", array('width' => '60'), array('align' => 'center', 'nowrap' => ''), false, false, "","printPermissionLink()", "", false /*show in excel*/));
    }
    if ($bolCanDelete)
...
//print_r($_REQUEST);die();
    ...
    $data = array("name" => $f->getValue('dataName'),
    ...
    {
      $bolSuccess = $dataHrdCostCenter->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
      $intDataID = $f->getValue('dataID');
    }
    if ($bolSuccess) $f->setValue('dataID', $intDataID);
