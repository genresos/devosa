<?php
  ...
  $dataPrivilege = getDataPrivileges("salary_report_overtime.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove);
...
//  //this save button will hide after save <toggle>
  ...
  if ($showReport)
  {
    ...
    else
    {
      ...
      $myDataGrid->addColumn(new DataGrid_Column("Total OT Day", "total_ot_day", array('width'=> 120), array('align' => 'center'), false, false, "","", "string", true, 12));
      ...
      $myDataGrid->addColumn(new DataGrid_Column("Overtime Allowance", "overtime_allowance", array('width'=> 120), array('align' => 'center'), false, false, "","formatNumber()", "numeric", true, 12));
  ...
  function getOvertimeReport($db, $intYear, $intMonth, $strKriteria = "")
  {
    ...
    while ($row = $db->fetchrow($res))
    {
        ...
        $strSQL2 = "SELECT t0.*, \"join_date\", \"resign_date\", t1.\"employee_name\", t2.\"position_name\", t3.functional_name, t1.employee_status
                    ...
                    WHERE \"id_salary_master\" ='$salaryMasterID' $strKriteria AND total_ot_min > 0 ORDER BY t1.\"employee_name\"";
