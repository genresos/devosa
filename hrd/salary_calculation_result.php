<?php
  ...
  function saveData()
  {
    ...
    if (count($myDataGrid->checkboxes)){
    	$arraySelectedEmployee = array();
			foreach ($myDataGrid->checkboxes as $strValue){
				$arraySelectedEmployee[] = $strValue;
			}
  ...
  function getSlip()
  {
   ...
   // die();
    ...
    foreach ($myDataGrid->checkboxes as $strValue)
    {
      ...
      if ($objSalary->arrEmployee[$strIDEmployee]['is_gross_up'] == 't') {
        $GLOBALS['strIncome'] .= wrapRowBorderLeftRight("Tax Allowance", "Rp", standardFormat($objSalary->getEmployeeSalaryDetail($strIDEmployee, "tax")), true /*isNumeric*/); //form_function.php
      ...
      if(!$bolIrregular)
      {
        ...
        $strSQL  = "SELECT t1.*, t2.id AS id_type, t3.resign_date FROM hrd_loan as t1
                    ...
                    AND (payment_thru + interval '1 months') > '". $objSalary->arrData['date_thru_salary']."'
                      AND t1.id_employee = '$strIDEmployee'";
        $resDb = $db->execute($strSQL);
        ...
        while ($rowDb = $db->fetchrow($resDb))
        {
          ...
          if ($rowDb['resign_date'] != "" || $rowDb['resign_date'] != NULL)
          {
            if($rowDb['resign_date'] >= $objSalary->arrData['date_from_salary'] && $rowDb['resign_date'] <= $objSalary->arrData['date_thru_salary'])
            {
              ...
              $intMultiplier = ($intPaymentThruYear - $intResignDateYear) * 12 + $intPaymentThruMonth - $intResignDateMonth;
              if ($intPaymentThruYear === $intResignDateYear && $intPaymentThruMonth === $intResignDateMonth) {
                $intMultiplier += 1;
              }
      ...
      $GLOBALS['strDeduction'] .= wrapRowBorderLeftRight("Tax", "Rp", standardFormat($objSalary->getEmployeeSalaryDetail($strIDEmployee, "tax") + $objSalary->getEmployeeSalaryDetail($strIDEmployee, "irregular_tax")), true /*isNumeric*/); //form_function.php
...
<table>
  ...
  function setStatus()
  {
    ...
    if (!empty($doRelease)){
    	header("location: salary_calculation_result.php?dataID=".$strDataID);
  ...
  function getDataGrid($db, $strCriteria, $bolLimit = true, $isFullView = false, $isExport = false)
  {
    ...
    $myDataGrid->pageSortBy = "t2.join_date";
...
*/
    ...
    // $myDataGrid->addColumn(new DataGrid_Column(getWords("cost center"), "grouper", array("rowspan" => 2, "width" => 30, "style" => "display:none"), array("style" => "display:none"), true, true, "", "", "string", true, 8, false));
    ...
    if (!$bolIrregular)
    {
      ...
      if ($objSalary->arrData['is_managerial'] == 't'){
        $totalAllowanceNonBenefitNonTax = $totalAllowanceNonBenefitNonTax + count($newArrayAllowance['managerial_allowance']);
      }
      ...
      if ($objSalary->arrData['is_managerial'] == 't'){
        for($i = 0;$i < count($newArrayAllowance['managerial_allowance']);$i++){
          $allowanceData = $newArrayAllowance['managerial_allowance'][$i];
          $myDataGrid->addColumn(new DataGrid_Column(getWords($allowanceData['name']), "alw_".$allowanceData['allowance_code'], array("rowspan" => 1, "width" => 270),  array("align" => "right"), false, true, "", "formatNumeric()", "numeric", true, 15, true, $allowanceData['allowance_code']));
        }
      }
      ...
      if ($objSalary->arrData['is_managerial'] == 't'){
        $totalDeduction = $totalDeduction + count($newArrayAllowance['managerial_deduction']);
      }
      $myDataGrid->addSpannedColumn(getWords("deduction"), $totalDeduction + 1);
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("tax"), "tax", array("width" => 270),  array("nowrap" => "nowrap", "align" => "right"), false, true, "", "formatNumeric()", "numeric", true, 15, true, "tax"));
      if ($objSalary->arrData['is_managerial'] == 't'){
        for($i = 0;$i < count($newArrayDeduction['managerial_deduction']);$i++){
          $deductionData = $newArrayDeduction['managerial_deduction'][$i];
          $myDataGrid->addColumn(new DataGrid_Column(getWords($deductionData['name']), "ded_".$deductionData['deduction_code'], array("rowspan" => 1, "width" => 270),  array("align" => "right"), false, true, "", "formatNumeric()", "numeric", true, 15, true, "ded_".$deductionData['deduction_code']));
        }
      }
      ...
      for($i = 0;$i < count($newArrayAllowance['benefit_non_tax']);$i++){
        ...
        $deductionNameView = implode(' ',$deductionName);
    ...
    {
      $myDataGrid->addColumn(new DataGrid_Column(getWords("no rekening"), "bank_account", array("rowspan" => 2, "width" => 270),  array("align" => "right"), false, true, "", "", "string", true, 6, false, ""));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("bank account name"), "bank_account_name", array("rowspan" => 2),  array("nowrap" => "nowrap"), true, true, "", "", "string", true, 35, false));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("bank"), "bank_name", array("rowspan" => 2, "width" => 270),  array("align" => "right"), false, true, "", "", "string", true, 6, false, ""));
      ...
      $myDataGrid->addColumn(new DataGrid_Column(getWords("transfer status"), "transfer_status_name", array("rowspan" => 2, "width" => 270),  array("align" => "right"), false, true, "", "", "string", true, 6, false, "transfer_status"));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("release note"), "release_note", array("rowspan" => 2, "width" => 270),  array("align" => "right"), false, true, "", "", "string", true, 6, false, "release_note"));
    }
    if ($isExport)
    {
...
//    $strOrderBy = "employee_name";
    ...
    $strSQL = "
      ...
      LEFT JOIN (
        SELECT id, employee_name, join_date, id_company, gender, salary_currency, branch_cost_center_code, branch_code,
        division_code, department_code, section_code, sub_section_code, employee_id
        FROM hrd_employee AS t2 WHERE 1=1 $strCriteria
    ...
    //get Data and set to Datagrid's DataSource by set the data binding (bind method)
    $strSQL = "
      SELECT t1.*, t2.employee_name, t2.bank_account_name, t2.join_date, t2.resign_date,  t2.gender, t2.salary_currency, t2.active,
      t3.position_group, t4.functional_name, bank_account, branch_penugasan_code, branch_bpjs_ks_code, 
      t5.transfer_code, t2.employee_id_2, t2.bank_code, branch_cost_center_code, t2.due_date,
      ...
      LEFT JOIN (
        SELECT id, employee_name, join_date, resign_date,  id_company, gender, salary_currency, active, functional_code,
        bank_account, bank_account_name, branch_code, branch_cost_center_code, branch_penugasan_code, branch_bpjs_ks_code, employee_id_2,
        bank_code, due_date, division_code, department_code, section_code, sub_section_code, employee_id
        FROM hrd_employee AS t2 WHERE 1=1 $strCriteria
      ) AS t2 ON t1.id_employee = t2.id
      LEFT JOIN (
      ...
      for($i = 0;$i < count($newArrayAllowance['other_allowance']);$i++){
        ...
        $rowDb['alw_'.$strCode] = $objSalary->arrDA[$strCode][$rowDb['id_employee']]['amount'];
			...
			for($i = 0;$i < count($newArrayAllowance['benefit_tax']);$i++){
				...
				$strCode = $allowanceData['allowance_code'];
				if (isset($objSalary->arrDA[$strCode][$rowDb['id_employee']]))
          $rowDb[$strCode] = $objSalary->arrDA[$strCode][$rowDb['id_employee']]['amount'];
			}
      ...
      foreach ($arrLoanType AS $strCode => $strName)
      {
        ...
        for($i = 0;$i < count($ARRAY_LOAN_CATEGORY);$i++){
        	...
        	if (isset($rowDb[$arrayLoanCategoryCode[$i]])){
        		...
        		$rowDb[$arrayLoanCategoryCode[$i]] = $loanAmount;
        	}
        }  
      }
  ...
  function dialogBoxReleased(){
  	...
  	$dialogBox .= '</tr>';
  	$dialogBox .= '<tr>';
  	$dialogBox .= '<td width="120" valign="top">Released Number</td><td><input type="text" id="release_number" name="release_number" class="numeric" style="width: 70px;"></td>';
  	$dialogBox .= '</tr>';
  	$dialogBox .= '<tr>';
  ...
  if ($db->connect()) {
    ...
    if ($objSalary->strDataID != "")
    {
      ...
      $strSalaryPeriode = date('M Y', strtotime($strDateThru));
...
//print_r($objSalary->arrData);die();
    ...
    $strCalculationMenu = "<b><a href='salary_calculation.php'>" .getWords("salary calculation list")."</a></b> | <b><a href='salary_calculation_managerial.php'>" .getWords("salary calculation list managerial")."</a></b>";//getCalculationMenu($strDataID, 5, $intStatus);
    ...
    (isset($_POST['dataFilterValue'])) ? $strDataFilterValue = $_POST['dataFilterValue'] : $strDataFilterValue = 2;
    ...
    if ($strDataSalaryCurrency != "") {
      $strKriteria .= "AND t2.salary_currency = '$strDataSalaryCurrency' ";
    }
    ...
    if ($strDataTransferStatus != "" && $strDataTransferStatus != "-1") {
      $strKriteriaTransfer .= "AND transfer_code = ".substr($strDataTransferStatus, -1)."::TEXT ";
    }
    ...
    if ($strDataFilterValue != "") {
      if ($strDataFilterValue == 0)
        $strKriteriaTransfer .= "AND total_gross >= 0 ";
    ...
    $strSQL  = "SELECT id, code, remark FROM hrd_salary_transfer_type ORDER BY code";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb)) {
        $strCode = $rowDb['code'];
    ...
    $strInputFilterValue = getComboFromArray($arrSalaryFilterValue, "dataFilterValue", $strDataFilterValue, false, " style=\"width:$strDefaultWidthPx\"");
		$strInputExcelExportType = "<input type=checkbox name='dataNoGroup' value=\"1\" checked>";
