<?php
    ...
    {    
        ...
        if (($strIDEmployee!=0) OR ($strIDEmployee!="")) 
         {   
                ...
                $arrDataAttendance[]=array('finger_id'=>$strNIK,'id_employee'=>$strIDEmployee,'date'=>$strDateConvert,'in'=>$strIN,'out'=>$strOUT); // format baru
  ...
  function importData($db, &$intTotal)
  {
    ...
    $dataCompany = $tblCompany->find("id = $strDataCompany", "attendance_file_path, attendance_file_type", "");
    if (is_uploaded_file($_FILES['fileAttendance']['tmp_name']))
    {
      $strFileName = $_FILES['fileAttendance']['tmp_name'];
      //die ("D");
      //print_r($strFileName);
      $strResult = processAttendance($db, $strFileName, $strDateFrom, $strDateThru, $strDataCompany, false);
      syncOvertimeApplication($db, $strDateFrom, $strDateThru, "");
    }
    else //if(is_dir($strFilePath = $dataCompany['attendance_file_path']))
    {
      $strFilePath = $dataCompany['attendance_file_path'];
      $strFileType = $dataCompany['attendance_file_type'];
      $strResult = processAttendanceAutomatic($db, $strFilePath, $strFileType, $strDateFrom, $strDateThru, $strDataCompany, false);
      syncOvertimeApplication($db, $strDateFrom, $strDateThru, "");
    }
    return $strResult;
  }
