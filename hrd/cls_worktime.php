  ...
  class clsWorkTime
  {
    ...
    */
    ...
    /*
      ...
      output : jumlah total hari kerja diantara tanggal awal dan akhir itu
    */
    function getTotalWorkDaySaturdayIn($db, $strFrom, $strThru)
    {
      global $arrWorkDay; // untuk menampung total workday, datefrom-datethru, agar menghemat pencarian, jika sudah pernah ada
	  $this->data = $db;
      if (isset($arrWorkDay["$strFrom.$strThru"])) return $arrWorkDay["$strFrom.$strThru"]; // langsung dibalikin
    ...
    /*
      ...
      output : jumlah total hari kerja diantara tanggal awal dan akhir itu
    */
    function getTotalWorkDayNoHoliday($db, $strFrom, $strThru)
    {
      global $arrWorkDay; // untuk menampung total workday, datefrom-datethru, agar menghemat pencarian, jika sudah pernah ada
	  $this->data = $db;
      if (isset($arrWorkDay["$strFrom.$strThru"])) return $arrWorkDay["$strFrom.$strThru"]; // langsung dibalikin
    ...
    function isHoliday($strDate, $strID = "")
    {
      ...
      if ($this->objDate->validDate($strDate))
      {
        ...
        if ($strID != "")
        {
          ...
          {
            if ($objS->checkEmployeeShiftOff($strID, $strDate, $strDate)) $bolResult = true;
            $bolFound = true;
          }
          unset($objS);
        }
    ...
    */
    function isPublicHoliday($strDate)
    {
      ...
      if (!$this->objDate->validDate($strDate)) return false;
