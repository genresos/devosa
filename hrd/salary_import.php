<?php
  ...
  function importProcess($db){
    ...
    if (($salaryTypeAllowance!="")AND ($salaryTypeDeduction!="")){
        $strMessage = "<h3>Please Select Only One Salary Type(Allowance or Deduction)</h3>";
    }
    else{
        ...
        for ($i=2; $i<=$baris; $i++)
        {
         ...
         $data2= $data->val($i,3); // Amount
         $arrData[]=array("nik"=>$data1,"amount"=>$data2);
        }

        if ($getPost['salaryTypeAllowance']!=""){
            //Simpan data allowance
            $strMessage=saveDataSalaryAllowance($arrData,$salaryTypeAllowance,"hrd_employee_allowance","allowance_code",$salarySet);
        }
        elseif ($getPost['salaryTypeDeduction']!=""){
            //Simpan data deduction
            $strMessage=saveDataSalaryAllowance($arrData,$salaryTypeDeduction,"hrd_employee_deduction","deduction_code",$salarySet);
        }
    }
  ...
  function getListSalarySet ($strParameter){
    ...
    if ($db->connect())
    {
      $strSQL = "SELECT t1.id,t1.id_company, t1.start_date ,t2.company_name FROM hrd_basic_salary_set AS t1
                 LEFT JOIN hrd_company AS t2 ON t1.id_company=t2.id ORDER BY t1.start_date DESC";
      $resDb = $db->execute($strSQL);
      $str.="<select name='".$strParameter."'>";
      while ($r = $db->fetchrow($resDb)){
