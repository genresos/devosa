<?php
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("training category"), "dataCategory",   "", array("size" => 49), "string",  true, true, true);  
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("category"), "training_category", array('width' => '250'),array('nowrap' => '')));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    $data = array("training_category" => $f->getValue('dataCategory'),
    ...
    {
      $bolSuccess = $dataHrdTrainingCategory->update(/*pk*/"id='".$f->getValue('dataIDCategory')."'", /*data to update*/ $data);
    }
    if ($bolSuccess)
    {
