...
echo $_SESSION['sessionDefaultModuleID']; */
  ...
  function getDataContract($db) {
    global $words;
    global $_SESSION;
    global $strDataDivision, $strDataBranch,$strDataEmployee;
    global $strDataDepartment,$strDataSection,$strDataSubSection;
    ...
    if ($strDataEmployee != "")   $strSQL .= "AND employee_id = '$strDataEmployee' ";
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      if (isset($arrMutation[$rowDb['id']])) {
        ...
        $strLinkPar .= "&dataEmployee=".getNIK($db, $rowDb['employee_id']);
        $strConfirm = "";
      }
      ...
      if (isset($arrMutationResign[$rowDb['id']])) {
        ...
        $strLinkPar2 .= "&dataEmployee=".getNIK($db, $rowDb['employee_id']);
        $strConfirm2 = "";
      }
...
if ($strDataBranch != "")   $strSQL .= "AND branch_code = '$strDataBranch' ";
  ...
  function getDataBirthday($db) {
    ...
    $strAkhir = date("m-d",$tsAkhir);
		$intDateFrom = mktime(0,0,0, date('m'), date('d') - 3, date('Y'));
		$intDateThru = mktime(0,0,0, date('m'), date('d') + 3, date('Y'));
    $dateToday = date('d M');

    // kriteria contract yang ditampilkan adalah, yang duedatenya sudah lewat 1 minggu, dan hampmir habis 1 bulan lagi
    $strSQL  = "select a.birthday, a.employee_name,  a.employee_ID, gender, to_char(birthday,'DD Mon') as sekarang
		from hrd_employee AS a where a.active = 1 AND to_char(current_date,'Mon') = to_char(birthday,'Mon') ";// extract(month from birthday BETWEEN '$dateFrom' AND '$dateThru'
    /*
    ...
    */
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strClass = ($rowDb['sekarang'] == $dateToday) ? "class = 'bgLate'" : "";
			$explodeBirthday = explode('-',$rowDb['birthday']);
			$intBirthday = mktime(0,0,0,$explodeBirthday[1], $explodeBirthday[2], date('Y'));
			if ($intBirthday >= $intDateFrom && $intBirthday <= $intDateThru){
				$intRows++;
	      $strResult .= "<tr valign=top $strClass>\n";
	      /*
 ...
 $strSQL = "select c.employee_name AS Nama, a.date_from AS Tgl_Mulai, a.date_thru AS Tgl_Selesai,
                ...
                where a.status in(2,3)
                and a.id_employee = c.id
      and current_date between a.date_from and a.date_thru $strKriteria
                order by description";
...
function getDataWarning($db)
    ...
    $strSQL = "SELECT t1.warning_date, t1.due_date, t2.code, t3.employee_name, t3.employee_id, t3.employee_id_2
               ...
               AND t1.status = 6 $strKriteria ORDER BY t1.due_date DESC";
...
t0.id IN (SELECT id_mutation from hrd_employee_mutation_resign) $strKriteria";
    ...
    if ($bolCanAcknowledge)
    {
      ...
      //Alert Absence Partial
      $dataPrivilege = getDataPrivileges("absence_partial_list.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
      if ($bolCanCheck)
      {
          $strLink = "javascript:goAlert('absence_partial_list.php',".REQUEST_STATUS_NEW.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence_partial as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "WHERE status = " .REQUEST_STATUS_NEW." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Amnesty Late - need to be checked: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanApprove)
      {
          $strLink = "javascript:goAlert('absence_partial_list.php',".REQUEST_STATUS_CHECKED.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence_partial as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "WHERE status = " .REQUEST_STATUS_CHECKED." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Amnesty Late - need to be approved: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanApprove2)
      {
          $strLink = "javascript:goAlert('absence_partial_list.php',".REQUEST_STATUS_APPROVED.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence_partial as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "WHERE status = " .REQUEST_STATUS_APPROVED." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          ...
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Amnesty Late - need to be approve 2'd: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanAcknowledge)
      {
          $strLink = "javascript:goAlert('absence_partial_list.php',".REQUEST_STATUS_APPROVED_2.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence_partial as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "WHERE status = " .REQUEST_STATUS_APPROVED_2." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Amnesty Late - need to be acknowledged: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
    ...
    /*Managerial Absence*/
      $dataPrivilege = getDataPrivileges("absence_approval_managerial.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
      if ($bolCanCheck)
      {
          $strLink = "javascript:goAlert('absence_approval_managerial.php',".REQUEST_STATUS_NEW.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "LEFT JOIN hrd_position AS t2 ON t1.position_code = t2.position_code ";
          $strSQL  .= "WHERE approver_id='".$arrUserInfo['employee_id']."' AND approver_id IS NOT NULL AND status = " .REQUEST_STATUS_NEW." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Absence List Managerial - need to be checked: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanApprove)
      {
          $strLink = "javascript:goAlert('absence_approval_managerial.php',".REQUEST_STATUS_CHECKED.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "LEFT JOIN hrd_position AS t2 ON t1.position_code = t2.position_code ";
          $strSQL  .= "WHERE approver_id='".$arrUserInfo['employee_id']."' AND approver_id IS NOT NULL AND status = " .REQUEST_STATUS_CHECKED." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Absence List Managerial - need to be approved: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanApprove2)
      {
          $strLink = "javascript:goAlert('absence_approval_managerial.php',".REQUEST_STATUS_APPROVED.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "LEFT JOIN hrd_position AS t2 ON t1.position_code = t2.position_code ";
          $strSQL  .= "WHERE approver_id='".$arrUserInfo['employee_id']."' AND approver_id IS NOT NULL AND status = " .REQUEST_STATUS_APPROVED." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Absence List Managerial - need to be approve 2'd: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanAcknowledge)
      {
          $strLink = "javascript:goAlert('absence_approval_managerial.php',".REQUEST_STATUS_APPROVED_2.")";
          $strSQL  = "SELECT COUNT(t0.id) AS total FROM hrd_absence as t0 ";
          $strSQL  .= "LEFT JOIN hrd_employee as t1 ON t0.id_employee = t1.id ";
          $strSQL  .= "LEFT JOIN hrd_position AS t2 ON t1.position_code = t2.position_code ";
          $strSQL  .= "WHERE approver_id='".$arrUserInfo['employee_id']."' AND approver_id IS NOT NULL AND status = " .REQUEST_STATUS_APPROVED_2." ".$strKriteriaCompany." ". $strKriteria;
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;Absence List Managerial - need to be acknowledged: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
    ...
    //End of Alert Recruitment
      ...
      //Alert FKR
      $dataPrivilege = getDataPrivileges("fkr_list.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);

      if ($bolCanCheck)
      {
          $strLink = "javascript:goAlert('fkr_list.php',".REQUEST_STATUS_NEW.")";
          $strSQL  = "SELECT COUNT(id) AS total FROM hrd_fkr WHERE status = " .REQUEST_STATUS_NEW. " ";
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;FKR List - need to be checked: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanApprove)
      {
          $strLink = "javascript:goAlert('fkr_list.php',".REQUEST_STATUS_CHECKED.")";
          $strSQL  = "SELECT COUNT(id) AS total FROM hrd_fkr WHERE status = " .REQUEST_STATUS_CHECKED. " ";
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;FKR List - need to be approved: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanApprove2)
      {
          $strLink = "javascript:goAlert('fkr_list.php',".REQUEST_STATUS_APPROVED.")";
          $strSQL  = "SELECT COUNT(id) AS total FROM hrd_fkr WHERE status = " .REQUEST_STATUS_APPROVED. " ";
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;FKR List - need to be approve 2'd: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
      ...
      if ($bolCanAcknowledge)
      {
          $strLink = "javascript:goAlert('fkr_list.php',".REQUEST_STATUS_APPROVED_2.")";
          $strSQL  = "SELECT COUNT(id) AS total FROM hrd_fkr WHERE status = " .REQUEST_STATUS_APPROVED_2. " ";
          $resDb = $db->execute($strSQL);
          if ($rowDb = $db->fetchrow($resDb)) {
              if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
                  $strResult .= " <tr valign=top class=$strClass>\n";
                  $strResult .= "  <td align=left nowrap>&nbsp;FKR List - need to be acknowledged: </td>\n";
                  $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
                  $strResult .= " </tr>\n";
              }
          }
    ...
    # Alert Warning
    $dataPrivilege = getDataPrivileges("warning_list.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);

    if ($bolCanCheck)
    {
      $strLink = "javascript:goAlert('warning_list.php',".REQUEST_STATUS_NEW.")";
      $strSQL  = "SELECT COUNT(t1.id) AS total FROM hrd_employee_warning AS t1 LEFT JOIN hrd_employee AS t2 ON t1.id_employee = t2.id WHERE t1.status = " .REQUEST_STATUS_NEW. " ";
      $resDb = $db->execute($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
        if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
          $strResult .= " <tr valign=top class=$strClass>\n";
          $strResult .= "  <td align=left nowrap>&nbsp;Warning List - need to be checked: </td>\n";
          $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
          $strResult .= " </tr>\n";
        }
      }
    ...
    if ($bolCanApprove)
    {
      $strLink = "javascript:goAlert('warning_list.php',".REQUEST_STATUS_CHECKED.")";
      $strSQL  = "SELECT COUNT(t1.id) AS total FROM hrd_employee_warning AS t1 LEFT JOIN hrd_employee AS t2 ON t1.id_employee = t2.id WHERE t1.status = " .REQUEST_STATUS_CHECKED. " ";
      $resDb = $db->execute($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
        if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
          $strResult .= " <tr valign=top class=$strClass>\n";
          $strResult .= "  <td align=left nowrap>&nbsp;Warning List - need to be approved: </td>\n";
          $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
          $strResult .= " </tr>\n";
        }
      }
    ...
    if ($bolCanApprove2)
    {
      $strLink = "javascript:goAlert('warning_list.php',".REQUEST_STATUS_APPROVED.")";
      $strSQL  = "SELECT COUNT(t1.id) AS total FROM hrd_employee_warning AS t1 LEFT JOIN hrd_employee AS t2 ON t1.id_employee = t2.id WHERE t1.status = " .REQUEST_STATUS_APPROVED. " ";
      $resDb = $db->execute($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
        if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
          $strResult .= " <tr valign=top class=$strClass>\n";
          $strResult .= "  <td align=left nowrap>&nbsp;Warning List - need to be approve 2'd: </td>\n";
          $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
          $strResult .= " </tr>\n";
        }
      }
    ...
    if ($bolCanAcknowledge)
    {
      $strLink = "javascript:goAlert('warning_list.php',".REQUEST_STATUS_APPROVED_2.")";
      $strSQL  = "SELECT COUNT(id) AS total FROM hrd_employee_warning WHERE status = " .REQUEST_STATUS_APPROVED_2. " ";
      $resDb = $db->execute($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
        if ($rowDb['total'] != "" && $rowDb['total'] > 0) {
          $strResult .= " <tr valign=top class=$strClass>\n";
          $strResult .= "  <td align=left nowrap>&nbsp;Warning List - need to be acknowledged: </td>\n";
          $strResult .= "  <td align=right nowrap><strong><a href=\"$strLink\"><a href=\"$strLink\">" .$rowDb['total']. "&nbsp; data</a></a></strong></td>\n";
          $strResult .= " </tr>\n";
        }
      }
...
*/
  ...
  if ($db->connect()) {
    ...
    //comment by adnan , add
        ...
        if ($_SESSION['sessionUserRole'] != ROLE_EMPLOYEE && $_SESSION['sessionUserRole'] != ROLE_CANDIDATE)
          $strDetailContract = getDataContract($db);
        else if($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) $strDetailContract = getDataContract($db);
        else $strDetailContract = "";
        //$strDetailContract = getDataContract($db);
        $strDetailDocuments = "";
        if ($_SESSION['sessionUserRole'] != ROLE_EMPLOYEE && $_SESSION['sessionUserRole'] != ROLE_CANDIDATE)
        	$strDetailDocuments = getDataDocuments($db);
        else if($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) $strDetailDocuments = getDataDocuments($db);
        getEmployeeInfo($db);
