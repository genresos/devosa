<?php
  ...
  if ($db->connect()) 
  {
    ...
    $f->addSelect(getWords("join month"), "dataJoinMonth", getDataListMonth(intval(date('m')), true), array("style" => "width:$strDefaultWidthPx"), "", false);  
    $f->addLiteral("", "", "");
  ...
  // GENERATE CRITERIA
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getwords("n i k"), "employee_id", array('width' => '75'), array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("employee name"), "employee_name", array('width' => '150'), array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("division"), "division_code", array(), array(), true, true, "", "getDivisionName()", "", true));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("department"), "department_code", array(), array(), true, true, "", "getDepartmentName()", "", true));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("section"), "section_code", array(), array(), true, true, "", "getSectionName()", "", true));
    //untuk di html, pd html: header di-hide, content di-show
    $myDataGrid->addColumn(new DataGrid_Column(getWords("cut off counter"), "cut_off_counter", array('width' => '75','style' => 'display:none'), array('align' => 'center'), false, false, "", ($bolCanEdit) ? "printCounter()" : "", "", false));
    //untuk di excel, pd html: header di-show, content di hide
    $myDataGrid->addColumn(new DataGrid_Column(getWords("cut off counter"), "cut_off_counter", array('width' => '75'), array('align' => 'center','style' => 'display:none'), true, false, "", "", "", true));
    //untuk di html, pd html: header di-hide, content di-show
    $myDataGrid->addColumn(new DataGrid_Column(getWords("cut off date"), "cut_off_date", array('style' => 'display:none'), array('align' => 'center', 'nowrap' => ''), false, false, "", ($bolCanEdit) ? "printDate()" : "", "", false));
    //untuk di excel, pd html: header di-show, content di hide
    $myDataGrid->addColumn(new DataGrid_Column(getWords("cut off date"), "cut_off_date", array(), array('align' => 'center','style' => 'display:none'), true, false, "", "", "", true));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("join date"), "join_date", array('width' => '75'), array('nowrap' => '')));
  ...
  function printDate($params)
  {
    ...
    return  generateInput("detailCutOffDate_".$record['id_employee'], $value). " " .generateButton("btnCutOffDate_".$record['id_employee'], "..");
  }
  ...
  function saveData() 
  {
    ...
    foreach($arrEmployee AS $strIDEmployee => $arrDetailEmployee)
    {
      if(isset($arrData['detailCutOffCounter_'.$strIDEmployee]))
      {
        ...
        $tblHrdLeaveAllowanceBase->delete(array("id_employee" => $strIDEmployee));
