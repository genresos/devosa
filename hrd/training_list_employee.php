<?php
  ...
  function getDataLama($db, &$intRows, $strFilterYear, $strKriteria = "", $intPage = 1, $bolLimit = true, $strOrder = "") {
    ...
    if (!is_numeric($strFilterYear)) $strFilterYear = date("Y");
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      if ($rowDb['date_thru'] != "") $strTgl .= " -- <br>".pgDateFormat($rowDb['date_thru'], "d-M-y");
      ...
      if (!$bolPrint) {
        $strResult .= "  <td nowrap align=center><a href='training_evaluation_edit.php?dataID=" .$rowDb['id']. "'>" .getWords('evaluation'). "</a>&nbsp;</td>";
      }
...
function getData($db, &$intRows, $strFilterYear, $strKriteria = "", $intPage = 1, $bolLimit = true, $strOrder = "") {
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      if ($rowDb['training_date_thru'] != "") $strTgl .= " -- ".pgDateFormat($rowDb['training_date_thru'], "d-M-y");
  ...
  if ($db->connect()) {
    ...
    if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE )
      if ($arrUserInfo['isDeptHead']) $strFilterDepartment = $arrUserInfo['department_code'];
      else if ($arrUserInfo['isGroupHead'])  $strFilterSection = $arrUserInfo['section_code'];
      else $strFilterEmployeeID = $arrUserInfo['employee_id'];
    ...
    */
    ...
    $strInputFilterDepartment = getDepartmentList($db,"filterDepartment",$strFilterDepartment, $strEmptyOption,""," style=\"width:$intDefaultWidthPx\" $strDisabled");
    $strInputFilterSection = getSectionList($db,"filterSection",$strFilterSection, $strEmptyOption,""," style=\"width:$intDefaultWidthPx\" $strDisabled");
