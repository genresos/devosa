<?php
  ...
  */
  ...
  class clsAttendanceClass
  {
    ...
    var $arrAttendanceChanges;  // daftar kehadiran
    ...
    function resetAttendance()
    {
      ...
      $this->arrAttendanceChanges  = array();
    ...
    function saveAttendance($strDataSource = "")
    {
      $strSQL = "";
      foreach($this->arrAttendanceChanges as $tmpAttDate => $tmpArrEmpAtt)
      {
        foreach($tmpArrEmpAtt as $tmpIDEmp => $tmpArrAtt)
        {
          if (isset($tmpArrAtt['id']) && $tmpArrAtt['id'] != "")
          {
            //Update
            $strSQL  .= "UPDATE hrd_attendance SET ";
            foreach($tmpArrAtt as $tmpField => $tmpVal)
              $strSQL .= "$tmpField = $tmpVal, ";
            $strSQL .= "created = now() WHERE id = ".$tmpArrAtt['id']."; ";
          }
          else
          {
            unset($tmpArrAtt['id']);
            ...
            foreach($tmpArrAtt as $tmpField => $tmpVal)
            {
              $strSQLFields .= "$tmpField, "; 
              $strSQLVals   .= "$tmpVal, "; 
            }
            $strSQLFields   .= "created)";
            $strSQLVals   .= "now())";

            
            $strSQL .= "INSERT INTO hrd_attendance $strSQLFields VALUES $strSQLVals; ";
          }
        }
      }
    ...
    function getEmployeeInfo()
    {
      ...
      $strSQL = "
        ...
        WHERE 1=1 
        AND ((join_date, resign_date) OVERLAPS (DATE '".$this->strDateFrom ."', DATE '".$this->strDateThru."') 
          OR (join_date <= DATE '".$this->strDateFrom ."' AND resign_date is null))
        ".$strTempKriteria." ";
    ...
    function getAttendanceData()
    {
      $strSQL  = "
        SELECT  
        t0.id, attendance_date, normal_start, normal_finish, attendance_start, attendance_finish, overtime_start, overtime_finish, 
        overtime_start_early, overtime_finish_early, overtime_start_auto, overtime_finish_auto, 
        overtime_start_early_auto, overtime_finish_early_auto, 
        not_late, t0.note, data_source, is_absence, holiday, 
        code_shift_type, late_duration, early_duration, 
        overtime, overtime_calculated, is_overtime, l1, l2, l3, l4, id_employee 
        FROM hrd_attendance AS t0 
        LEFT JOIN hrd_employee AS t1 ON t0.id_employee = t1.id 
        WHERE (attendance_date = '" .$this->strDateFrom."' 
        OR attendance_date BETWEEN '" .$this->strDateFrom."' AND '" .$this->strDateThru."') 
        AND (attendance_date <= resign_date 
        OR (active = 1)) ";
  ...
  class clsAttendanceInfo
  {
	...
	//adnan cek false biar sabtu minggu jamnya ke hitung
           if ($arrTemp['shift_off'] == "t" || isHoliday($strTmpAttendanceDate, false /*if false means only checks public holiday*/))
           {
              $this->strNormalStart  = "";
              $this->strNormalFinish = "";
    ...
    function getEmployeeOvertimeApplication(&$objAttendanceClass)
    {
      ...
      // fungsi untuk membuat data attendance dari objAttendance yang sedang diproses
    function createCurrentAttendance(&$objAttendanceClass, $strDataSource = "")
    {
      ...
      //echo "1".$strAttendanceStart."|".$strAttendanceFinish."<br>";;

      if (!isset($objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee]))
      {
        $objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee] =  (isset($objAttendanceClass->arrAttendance[$this->strAttendanceDate][$this->strIDEmployee])) ? $objAttendanceClass->arrAttendance[$this->strAttendanceDate][$this->strIDEmployee] : array();
        $instantObj =&  $objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee];
        ...
        $strOvertimeStart     = ($this->strOvertimeStart == "") ? "NULL" : "'" .$this->strOvertimeStart."'";
        $strOvertimeFinish    = ($this->strOvertimeFinish == "") ? "NULL" : "'" .$this->strOvertimeFinish."'";
        $strOvertimeStartEarly     = ($this->strOvertimeStartEarly == "") ? "NULL" : "'" .$this->strOvertimeStartEarly."'";
        $strOvertimeFinishEarly    = ($this->strOvertimeFinishEarly == "") ? "NULL" : "'" .$this->strOvertimeFinishEarly."'";
        $strOvertimeStartAuto     = ($this->strOvertimeStartAuto == "") ? "NULL" : "'" .$this->strOvertimeStartAuto."'";
        $strOvertimeFinishAuto    = ($this->strOvertimeFinishAuto == "") ? "NULL" : "'" .$this->strOvertimeFinishAuto."'";
        $strOvertimeStartEarlyAuto     = ($this->strOvertimeStartEarlyAuto == "") ? "NULL" : "'" .$this->strOvertimeStartEarlyAuto."'";
        $strOvertimeFinishEarlyAuto    = ($this->strOvertimeFinishEarlyAuto == "") ? "NULL" : "'" .$this->strOvertimeFinishEarlyAuto."'";
        $strNote              = ($this->strNote == "") ? "''" : "'" .$this->strNote."'";
        $strDataSource        = ($this->strDataSource == "") ? "''" : "'" .$this->strDataSource."'";
        ...
        $strShiftCode         = ($this->strShiftCode == "") ? "''" : "'" .$this->strShiftCode."'";
        $strIsOvertime         = ($this->strIsOvertime == 't') ? "'t'" : "'f'";
        $strIsAbsence         = ($this->bolAbsence) ? "'t'" : "'f'";
        $bolYesterday         = ($this->bolYesterday);
        $strStatus    = 0; // sementara langsung OK
        //echo "2".$strAttendanceStart."|".$strAttendanceFinish."<br>";

        $instantObj['attendance_date'] = "'".$this->strAttendanceDate."'";
        $instantObj['id_employee'] = "'".$this->strIDEmployee."'";
        $instantObj['normal_start'] = $strNormalStart;
        $instantObj['normal_finish'] = $strNormalFinish;
        $instantObj['attendance_start'] = $strAttendanceStart;
        $instantObj['attendance_finish'] = $strAttendanceFinish;
        $instantObj['overtime_start'] = $strOvertimeStart;
        $instantObj['overtime_finish'] = $strOvertimeFinish;
        $instantObj['overtime_start_early'] = $strOvertimeStartEarly;
        $instantObj['overtime_finish_early'] = $strOvertimeFinishEarly;
        $instantObj['overtime_start_auto'] = $strOvertimeStartAuto;
        $instantObj['overtime_finish_auto'] = $strOvertimeFinishAuto;
        $instantObj['overtime_start_early_auto'] = $strOvertimeStartEarlyAuto;
        $instantObj['overtime_finish_early_auto'] = $strOvertimeFinishEarlyAuto;
        $instantObj['not_late'] = $strNotLate;
        $instantObj['note'] = $strNote;
        $instantObj['total_duration'] = $this->intTotalDuration;
        $instantObj['late_duration'] = $this->intLate;
        $instantObj['early_duration'] = $this->intEarly;
        $instantObj['l1'] = $this->fltOT1;
        $instantObj['l2'] = $this->fltOT2;
        $instantObj['l3'] = $this->fltOT3;
        $instantObj['l4'] = $this->fltOT4;
        $instantObj['overtime'] = $this->fltTotalOT;
        $instantObj['shift_type'] = $strShiftType;
        $instantObj['code_shift_type'] = $strShiftCode;
        $instantObj['status'] = $strStatus;
        $instantObj['is_absence'] = $strIsAbsence;
        $instantObj['is_overtime'] = $strIsOvertime;
        $instantObj['holiday'] = $strHoliday;
        $instantObj['data_source'] = $strDataSource;
        //echo "3".$objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee]['attendance_start']."|".$objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee]['attendance_finish']."<br>";

      }
      else 
      {
        $instantObj =&  $objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee];
        $instantObj['attendance_start'] = ($instantObj['attendance_start'] != "NULL") ? $instantObj['attendance_start'] : $strAttendanceStart ;
        $instantObj['attendance_finish'] = ($instantObj['attendance_finish'] != "NULL") ? $instantObj['attendance_finish'] : $strAttendanceFinish;
        //echo "4".$objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee]['attendance_start']."|".$objAttendanceClass->arrAttendanceChanges[$this->strAttendanceDate][$this->strIDEmployee]['attendance_finish']."<br>";
      }
    }
    ...
    function calculateOvertime()
    {
      ...
      {
        // reset dulu
        $this->fltOT1 = $this->fltOT2 = $this->fltOT3 = $this->fltOT4 = $fltTotalOT = $fltOTCalculated = 0;
        // hitung
