<?php
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("position"), "position_code", array("rowspan" => 2, "width" => 100), array(), false, false, "", "", "string", true, 12));
  $myDataGrid->addColumn(new DataGrid_Column(getwords("n i k"), "employee_id", array("rowspan" => 2, "width" => 100), array(), false, false, "", "", "string", true, 12));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("status"), "employee_status", array("rowspan" => 2, "width" => 50), array(), false, false, "", "printEmployeeStatus1()", "string", true, 6));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("organization structure"), "", array("colspan" => 4), array('nowrap' => 'nowrap'), true, false, "", "", "string", true));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("unit"), "section_name", array("width" => 100), array('nowrap' => 'nowrap'), true, false, "", "", "string", true, 12));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("note"), "note", array("rowspan" => 2), array(), true, true, "", "", "string", true, 12));
  ...
  if ($f->getValue('status') != '') $arrCriteria[] = "status =".intval($f->getValue('status'));
...
*/
  ...
  function generateEmployeeFromFKR($db)
  {
    ...
    if ($row = $db->fetchrow($res))
    {
      ...
      if ($row['id_employee'] != "") return false; //anggap sudah ada, tidak perlu dibuat lagi
    }
    if ($strCandidate == "") return false; // kandidat tidak ada, abaikan
    $arrCandidate = array();
    $strSQL = "
  ...
  function printEmployeeLink($params)
  {
    ...
    if ($record['status'] != REQUEST_STATUS_APPROVED && $record['status'] != getWords("approved")) return "";
  ...
  {
    extract($params);
    return "<a href=\"javascript:openWindowDialog('fkr_print.php?dataID=" .$record['id']. "')\">" .getWords('print'). "</a>";
  }
