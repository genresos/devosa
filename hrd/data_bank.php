<?php
  ...
  if ($bolCanEdit)
  {
    ...
    if($isNew){
    $f->addInput(getWords("bank code"), "dataCode", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);  
    }
    else{
    	$f->addInput(getWords("bank code"), "dataCode", "", array("size" => 30, "maxlength" => 31, "readOnly" => true), "string", true, true, true);  
    }
    $f->addInput(getWords("bank name"), "dataName", "", array("size" => 100, "maxlength" => 127), "string", true, true, true);  
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("bank code"), "bank_code", array('width' => '150'),array('nowrap' => '')));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    $data = array("bank_code" => $f->getValue('dataCode'),
    ...
    {
      $bolSuccess = $dataHrdBank->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    if ($bolSuccess)
    {
      if ($isNew)
