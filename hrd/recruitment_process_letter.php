<?
  ...
  function saveData($db, &$strDataID, &$strError) {
    ...
    if ($bolOK) { // input OK, tinggal disimpan
      if ($strDataDetailID === "") $strDataDetailID = "null";
      // simpan data
...
<?
  ...
  if ($db->connect()) {
    ...
    // beri default dulu
    if ($strDataDetailID === "") $arrData['dataCode'] = "ST";
    else $arrData['dataCode'] = "PPP";
    ...
    if ($strDataID == "") { // ambil dari template
      ...
      if ($strDataDetailID != "") $strDataContentFile = getTemplate("letter1.html", false);
      ...
      //if ($arrData['dataDetailID'] == "") {
        ...
        if ($rowDb = $db->fetchrow($resDb)) {
          ...
          if ($rowDb['city'] != "") $strDataAddress .= "<br>".$rowDb['city'];
          if ($rowDb['zip'] != "") $strDataAddress .= " ".$rowDb['zip'];
    ...
    $strButtons .= "<input type=button name='btnPrint' id='btnPrint' value=\"" .getWords("print")."\" onClick=\"printPage();\">&nbsp;";
    $strButtons .= "<input type=reset name='btnReset' value=\"" .getWords('reset')."\">&nbsp";
    $strButtons .= "<input type=button name='btnClose' id='btnClose' value=\"" .getWords("close")."\" onClick=\"closeWindow();\">&nbsp;";
