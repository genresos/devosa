<?
  ...
  function saveData($db, $strDataID, &$strError) {
    ...
    for ($i = 1; $i <= $intMax; $i++) {
      ...
      if (!is_numeric($strPoint)) $strPoint = 0;
  ...
  if ($db->connect()) {
    ...
    if ($strDataID == "") {
      ...
      if ($bolCanView) {
        ...
        $strDataDetail = ($bolPrint) ? getDataPrint($db,$intTotalData, $strKriteria,"", $bolReadonly) : getData($db,$intTotalData, $strKriteria,"", $bolReadonly);

        // tampilkan keterangan tentang status hasil, untuk pritn aja
        if ($bolPrint)
        {

          if ($strTotal > 90) $strNoteKriteria = $strCategoryA;
          else if ($strTotal > 80) $strNoteKriteria = $strCategoryB;
          else if ($strTotal > 70) $strNoteKriteria = $strCategoryC;
          else if ($strTotal > 59) $strNoteKriteria = $strCategoryD;
          else $strNoteKriteria = $strCategoryE;
          $strTotal = standardFormat($strTotal);
        }
        // tambah button untuk print
        $strButtons .= "&nbsp;<input type=button name='btnPrint' value=\"" .getWords("print")."\" onClick=\"window.open('evaluation_competency.php?btnPrint=Print&dataID=$strDataID')\"> ";  
      } else {
