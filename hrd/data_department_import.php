  ...
  function importData($db, &$intTotal) {
    ...
    if (is_uploaded_file($HTTP_POST_FILES["dataFile"]['tmp_name'])) {
      ...
      for ($i=3;$i <= $intRows; $i++) {
          ...
          // handle satu persatu
        (isset($data->sheets[0]['cells'][$i][1])) ? $strDivCode = trim($data->sheets[0]['cells'][$i][1]) : $strDivCode = "";
        (isset($data->sheets[0]['cells'][$i][2])) ? $strDivName = trim($data->sheets[0]['cells'][$i][2]) : $strDivName = "";
        (isset($data->sheets[0]['cells'][$i][3])) ? $strDeptCode = trim($data->sheets[0]['cells'][$i][3]) : $strDeptCode = "";
        (isset($data->sheets[0]['cells'][$i][4])) ? $strDeptName = trim($data->sheets[0]['cells'][$i][4]) : $strDeptName = "";
        (isset($data->sheets[0]['cells'][$i][5])) ? $strSectCode = trim($data->sheets[0]['cells'][$i][5]) : $strSectCode = "";
        (isset($data->sheets[0]['cells'][$i][6])) ? $strSectName = trim($data->sheets[0]['cells'][$i][6]) : $strSectName = "";
        (isset($data->sheets[0]['cells'][$i][7])) ? $strSubCode = trim($data->sheets[0]['cells'][$i][7]) : $strSubCode = "";
        (isset($data->sheets[0]['cells'][$i][8])) ? $strSubName = trim($data->sheets[0]['cells'][$i][8]) : $strSubName = "";

        if ($strDivCode != "") $strDivision = $strDivCode;
        if ($strDeptCode != "") $strDepartment = $strDeptCode;
        if ($strSectCode != "") $strSection = $strSectCode;
        if ($strSubCode != "") $strSubSection = $strSubCode;
