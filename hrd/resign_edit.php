<?php
  ...
  $strWordsDueDate             = getWords("end of contract date");
  $strWordsDepartment             = getWords("department");
  $strWordsDivision             = getWords("division");
  $strWordsCompany             = getWords("company");
  $strWordsFunctional            = getWords("functional position");
  $strWordsBranchPenugasan            = getWords("branch office");
  ...
  $strWordsECStatus               = getWords("ec status");
  if ($strDataID == "") {
  $arrData                = array(
    "dataDate"            => $strNow,
    "dataEmployee"        => "",
...
//die($strSQL);
      if ($rowDb = $db->fetchrow($resDb)) {
        ...
        $arrData['dataJoinDate']       = $rowDb['join_date'];
        $arrData['dataDueDate']       = $rowDb['join_date'];
  ...
  function saveData($db, &$strError) {
	...
	(isset($_REQUEST['dataECStatus'])) ? $strDataECStatus = $_REQUEST['dataECStatus'] : $strDataECStatus = 0;
    ...
    if(!is_numeric($strBasicSalary)) $strBasicSalary = 0;
    ...
    if ($bolOK) { // input OK, tinggal disimpan
      if ($strDataID == "") {
        ...
        $strSQL .= "id_employee,proposal_date, note, status) ";
        ...
        $strSQL .= "'$strIDEmployee','$strDataDate', '$strDataNote', 0)";
        $resExec = $db->execute($strSQL);

        $strSubject = getSubject(0,'Severance Proposal',$strIDEmployee);
        $strBody.= "Name: ".getEmployeeNameEmail($strIDEmployee)."<br>";
        $strBody.= "Proposal Date: ".$strDataDate."<br>";
        ($strDataNote != "") ? $strDataNote = $strDataNote : $strDataNote = "-";
        $strBody.= "Resign Reason: ".$strDataNote;
        ...
        $strSQL .= "\"loan\", other_loan, other_obligation, obligation_note, ec_status) ";
        ...
        $strSQL .= "'$strLoan', '$strOtherLoan', '$strObligation', '$strNoteObligation', $strDataECStatus)";
        $resExec = $db->execute($strSQL);

      } else {

        // cek status, jika sudah approved, gak boleh diedit lagi
        $strSQL = "SELECT status, id_mutation FROM hrd_employee_mutation_resign as t2 LEFT JOIN hrd_employee_mutation as t1 ON t1.id = t2.id_mutation WHERE t2.id = '$strDataID' ";
        $resDb = $db->execute($strSQL);
        if ($rowDb = $db->fetchrow($resDb)) {
          $strDataIDMutation = $rowDb['id_mutation'];
          if ($rowDb['status'] >= REQUEST_STATUS_APPROVED_2) {
        ...
        //hapus dulu semua data
        $strSQL  = "DELETE FROM hrd_employee_mutation_resign WHERE id = '$strDataID'; ";
        $resDb = $db->execute($strSQL);
          // simpan data salary
        $strSQL  = "INSERT INTO hrd_employee_mutation_resign (created, modified_by, created_by, ";
        $strSQL .= "id, id_mutation, resign_date, working_days, ";
        $strSQL .= "\"attendance\", leave_remain, \"leave_remain1\", ";
        $strSQL .= "basic_salary, \"salary\", \"meal\", \"conjuncture\", ";
        $strSQL .= "leave_allowance1, leave_allowance2,";
        $strSQL .= "\"pesangon\", other_right, right_note, ";
        $strSQL .= "\"loan\", other_loan, other_obligation, obligation_note, ec_status) ";
        $strSQL .= "VALUES(now(), '$strmodified_byID', '$strmodified_byID', '$strDataID', '$strDataIDMutation', ";
        $strSQL .= "'$strResignDate', '$strWorkingDays', '$strAttendance', ";
        $strSQL .= "'$strLeaveRemain1', '$strLeaveRemain2', '$strBasicSalary', ";
        $strSQL .= "'$strSalary', '$strMeal', '$strConjuncture', '$strLeave1', ";
        $strSQL .= "'$strLeave2', '$strPesangon', '$strOtherRight', '$strNoteRight', ";
        ...
        $strSQL .= "SET modified = '".date('Y-m-d H:i')."', modified_by = '" .$_SESSION['sessionUserID']. "', ";
        $strSQL .= "id_employee = '$strIDEmployee', proposal_date = '$strDataDate', ";
        $strSQL .= "note = '$strDataNote' WHERE id = '$strDataIDMutation' ";
        $resExec = $db->execute($strSQL);

        $strSubject = getSubject(0,'Severance Proposal Updated',$strIDEmployee);
        $strBody.= "Name: ".getEmployeeNameEmail($strIDEmployee)."<br>";
        $strBody.= "Proposal Date: ".$strDataDate."<br>";
        ($strDataNote != "") ? $strDataNote = $strDataNote : $strDataNote = "-";
        $strBody.= "Resign Reason: ".$strDataNote;
  ...
  function getInfoEmployee($db, $bolStart = true) {
    ...
    if ($strID != "") {
      ...
      if (!is_numeric($intSalaryDay)) $intSalaryDay = 25; //default
      ...
      $strSQL  = "SELECT emp.id, join_date, employee_status, due_date, position_code, functional_code, department_code, branch_penugasan_code, ";
      $strSQL .= "division_code, comp.company_code, ";
      ...
      $strSQL .= "FROM hrd_employee emp LEFT JOIN hrd_company comp ON ";
      $strSQL .= "emp.id_company = comp.id WHERE employee_id = '$strID' ";
      $resDb = $db->execute($strSQL);
      if ($rowDb  =  $db->fetchrow($resDb)) {
        $arrData['dataJoinDate'] = $rowDb['join_date'];
        $arrData['dataDueDate'] = $rowDb['due_date'];
        $arrData['dataEmployeeStatus'] = $rowDb['employee_status'];
        $arrData['dataCompany'] = $rowDb['company_code'];
        ...
        if ($rowDb['lama'] < 3) $arrData['dataPesangon'] = 0;
        else if ($rowDb['lama'] < 6) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 2);
        else if ($rowDb['lama'] < 9) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 3);
		  	else if ($rowDb['lama'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 4);
		  	else if ($rowDb['lama'] < 15) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 5);
		  	else if ($rowDb['lama'] < 18) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 6);
		  	else if ($rowDb['lama'] < 21) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 7);
		  	else if ($rowDb['lama'] < 24) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 8);
		  	// else if ($rowDb['lama'] < 18) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 6);
        else $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 10);
			}elseif ($_REQUEST['dataType']==2){
				if ($rowDb['lama'] < 1 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = $arrData['dataBasicSalary']*1;
          else if ($rowDb['lama'] < 2 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 2);
          else if ($rowDb['lama'] < 3 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 3);
		  		else if ($rowDb['lama'] < 4 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 4);
				  else if ($rowDb['lama'] < 5 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 5);
				  else if ($rowDb['lama'] < 6 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 6);
				  else if ($rowDb['lama'] < 7 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 7);
				  else if ($rowDb['lama'] < 8 && $rowDb['lamabulan'] < 12) $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 8);
				  else $arrData['dataPesangon'] = ($arrData['dataBasicSalary'] * 9);
			}
			// cari data pinjaman
      $arrData['dataLoan'] = 0;
      $strSQL  = "SELECT *, EXTRACT(month FROM AGE(payment_from)) AS paid  ";
      $strSQL .= "FROM hrd_loan WHERE status >=".REQUEST_STATUS_APPROVED;
      ...
      while ($rowTmp = $db->fetchrow($resTmp)) {
       ...
       if (!is_numeric($fltMeal)) $fltMeal = 0;
       ...
       if (!is_numeric($fltConjuncture)) $fltConjuncture = 0;
  ...
  function getTypeName($varname, $default=""){
	$arrType=array(1=>"Resign", 2=>"Fired");
	$strResult ="";
  ...
  if ($db->connect()) {
    ...
    (isset($_REQUEST['dataEmployee'])) ? $strDataEmployee = trim($_REQUEST['dataEmployee']) : $strDataEmployee = trim($arrData['dataEmployee']);
    $strInputDate = "<input type=text size=15 maxlength=10 name=dataDate id=dataDate value=\"" .$arrData['dataDate']. "\" $strReadonly>";
    $strInputEmployee = "<input type=text name=dataEmployee id=dataEmployee size=15 maxlength=30 value=\"" .$strDataEmployee. "\" style=\"width:$strDefaultWidthPx\" $strReadonly>";
	$strInputType = getTypeName("dataType", $arrData['dataType']);
    $strInputResignDate = "<input type=text size=15 maxlength=10 name=dataResignDate id=dataResignDate value=\"" .$arrData['dataResignDate']. "\" $strReadonly>";
    ...
    $strInputFunctional  = $arrData['dataFunctional'];
    $strInputFunctional .= "<input type=hidden name=dataFunctional value=\"" .$arrData['dataFunctional']."\">";
    $strInputDueDate  = pgDateFormat($arrData['dataDueDate'], "d M Y")."&nbsp;";
    $strInputDueDate .= "<input type=hidden name=dataDueDate value=\"" .$arrData['dataDueDate']."\">";
    $strInputBranchPenugasan  = $arrData['dataBranchPenugasan'];
    $strInputBranchPenugasan .= "<input type=hidden name=dataBranchPenugasan value=\"" .$arrData['dataBranchPenugasan']."\">";    
    $strInputDepartment  = $arrData['dataDepartment'];
    $strInputDepartment .= "<input type=hidden name=dataDepartment value=\"" .$arrData['dataDepartment']."\">";
		$strInputDivision  = $arrData['dataDivision'];
    $strInputDivision .= "<input type=hidden name=dataDivision value=\"" .$arrData['dataDivision']."\">";
    $strInputCompany  = $arrData['dataCompany'];
    $strInputCompany .= "<input type=hidden name=dataCompany value=\"" .$arrData['dataCompany']."\">";
    ...
    $strInputOtherRight = "<input type=text name=dataOtherRight id=dataOtherRight size=15 maxlength=20 value=\"" .$arrData['dataOtherRight']. "\" style=\"width:$strDefaultWidthPx\" $strReadonly class='numeric' $strAction>";
    $strInputPesangon = "<input type=text name=dataPesangon id=dataPesangon size=15 maxlength=20 value=\"" .$arrData['dataPesangon']. "\" style=\"width:$strDefaultWidthPx\" $strReadonly class='numeric' $strAction>";
    $strInputRightNote = "<input type=text name=dataRightNote id=dataRightNote size=30 maxlength=100 value=\"" .$arrData['dataRightNote']. "\" style=\"width:$strDefaultWidthPx\" $strReadonly>";

    $strAction = "onChange=\"getTotalObligation()\"";

    $strInputLoan = "<input type=text name=dataLoan id=dataLoan size=15 maxlength=20 value=\"" .$arrData['dataLoan']. "\" style=\"width:$strDefaultWidthPx\" $strReadonly class='numeric' $strAction>";
    $strInputOtherLoan = "<input type=text name=dataOtherLoan id=dataOtherLoan size=15 maxlength=20 value=\"" .$arrData['dataOtherLoan']. "\" style=\"width:$strDefaultWidthPx\" $strReadonly class='numeric' $strAction>";
    $strInputOtherObligation = "<input type=text name=dataOtherObligation id=dataOtherObligation size=15 maxlength=20 value=\"" .$arrData['dataOtherObligation']. "\" style=\"width:$strDefaultWidthPx\" $strReadonly class='numeric' $strAction>";
    $strInputObligationNote = "<input type=text name=dataObligationNote id=dataObligationNote size=30 maxlength=100 value=\"" .$arrData['dataObligationNote']. "\" style=\"width:$strDefaultWidthPx\" $strReadonly>";
    ...
    $strInputECStatus = "<select name=dataECStatus>";
    $strInputECStatus .= "<option value=1 ";
    if($arrData['dataECStatus'] == 1) $strInputECStatus .= "selected";
    $strInputECStatus .=">Close</option>";
    $strInputECStatus .= "<option value=0 ";
    if($arrData['dataECStatus'] == 0) $strInputECStatus .= "selected";
    $strInputECStatus .=">Open</option>";
    $strInputECStatus .= "</select>";
    // hitung total hak
    ...
    $strBtnPrint .= "<input type=button name=btnPrint onClick=\"window.open('resign_edit.php?btnPrint=Print&dataID=$strDataID');\" value=\"" .getWords("print")."\" $strDisabledPrint>";

  }

  ($bolPrint) ? $strMainTemplate = getTemplate("resign_edit_print.html", false) : $strTemplateFile = getTemplate("resign_edit.html");
