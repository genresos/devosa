<?php
  ...
  $strWordsSelectApprovedOvertime = getWords("select approved overtime");
  ...
  function saveData($db, &$strError)
  {
    ...
    if (!validStandardDate($strDataTHRDateFrom) && $strDataTHRDateFrom != "") {
      ...
      return 0;
    } else     if (!validStandardDate($strDataOvertimeDateFrom)) {
...
/*
    ...
    */
    ...
    $objSalary = new clsSalaryCalculation($db, "", $bolIrregular, $strDataDate, array("id_company" => $strDataCompany),  $strDataDateFrom, $strDataDateThru, $strDataSalaryDateFrom, $strDataSalaryDateThru, true);
    ...
    setEmployeeNoAccounttoHold($db, $intID, $arrAlpha);
  ...
  function getDataGrid($db, $strCriteria, $bolLimit = true, $isFullView = false)
  {
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("salary date"), "salary_date", array("width" => 70),  array("nowrap" => "nowrap"), true, true, "", "formatDate()", "string", true, 12));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("overtime period from"), "date_from_overtime", array("width" => 70),  array("nowrap" => "nowrap"), true, true, "", "formatDate()", "string", true, 12));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("overtime period thru"), "date_thru_overtime", array("width" => 70), array("nowrap" => "nowrap"), true, true, "", "formatDate()", "string", true, 12));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("created"), "created", array("width" => 50),  array( "nowrap" => "nowrap"), true, true, "", "formatDate()", "string", true, 12));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("approved"), "approved_time", array("width" => 50),  array( "nowrap" => "nowrap"), true, true, "", "formatDate()", "string", true, 12));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("status"), "status", array('width' => '60'), "", true, true, "","printRequestStatus()"));
    ...
    $strCriteria.= "AND is_managerial is true AND is_overtime_only is false";
    $myDataGrid->totalData = $tblSalary->findCount($strCriteria);
    $dataset = $tblSalary->findAll($strCriteria,
                             "id, id_company, salary_date, date_from, date_thru, date_from_thr, date_thru_thr, date_from_salary, date_thru_salary, date_from_overtime, date_thru_overtime, irregular, hide_blank, note, created :: date, approved_time :: date, approved_by,status" ,
                             $strOrderBy,
                             $strPageLimit,
                             $intPageNumber);
  ...
  if ($db->connect())
  {
    ...
    $strOvertimeDateFrom = $arrDatePrevMonth[0]."-".$arrDatePrevMonth[1]."-"."01";
    $strOvertimeDateThru = $arrDatePrevMonth[0]."-".$arrDatePrevMonth[1]."-".lastday($arrDt[1], $arrDt[0]);
...
//    $strInputOvertimeDateFrom     = "<input type=text name=dataOvertimeDateFrom id=dataOvertimeDateFrom size=15 maxlength=10 value=\"$strDefaultFrom\">";
//    $strInputOvertimeDateThru     = "<input type=text name=dataOvertimeDateThru id=dataOvertimeDateThru size=15 maxlength=10 value=\"$strDefaultThru\">";
//    $strInputSalaryDateFrom     = "<input type=text name=dataSalaryDateFrom id=dataSalaryDateFrom size=15 maxlength=10 value=\"$strDefaultFrom\">";
//    $strInputSalaryDateThru     = "<input type=text name=dataSalaryDateThru id=dataSalaryDateThru size=15 maxlength=10 value=\"$strDefaultThru\">";
		/* Create approved overtime selection*/
    ...
    $strSQL .= "sm.id_company = comp.id WHERE is_overtime_only is true AND status >= ".REQUEST_STATUS_APPROVED_2;
    $resDb = $db->execute($strSQL);
    $arrApprovedOvertime = array();
    $arrApprovedOvertime[0] = "";
    while ($rowDb = $db->fetchrow($resDb)) {
    	$arrApprovedOvertime[$rowDb['id']] = $rowDb['company_name'].' : '.$rowDb['date_from_overtime'].' - '.$rowDb['date_thru_overtime'];
    }
    $strSelectApprovedOvertime = getComboFromArray($arrApprovedOvertime, "dataApprovedOvertime", "", "", "style=\"width:350px;\"");
    /* End create approved overtime */
