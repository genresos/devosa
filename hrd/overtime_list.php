<?php
  ...
  $dataPrivilege = getDataPrivileges("overtime_edit.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  ...
  function getData($db, $strDataDateFrom, $strDataDateThru, &$intRows, $strKriteria = "", $strOrder = "")
  {
    ...
    $intRows = 0;
    $strResult = "";
    $arrStatusDenied = array(2,8); // daftar status yang sifatnya ditolak :D
    $arrStatusEmplEditable = array_merge($arrStatusDenied, array(0,5,7)); // daftar status yang sifatnya bisa diedit oleh employee
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      if ($rowDb['status'] == 0) {
        if (isOutDated($rowDb['overtime_date'])) {
          ...
          $strHiddenNote .= "<input type=hidden name=detailDate$intRows  disabled value=\"" .pgDateFormat($rowDb['application_date'], "d M Y")."\">";
          if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) $strBgClass = "bgBlocked";
        }
      }
      ...
      $strResult .= "  <td $strBgClass1>" .pgDateFormat($rowDb['overtime_date'], "d-M-y"). "&nbsp;$strHiddenNote</td>\n";
...
*/
          ...
          if ($rowDb = $db->fetchrow($resDb)) {
            ...
            if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) { // atasan
              ...
              if ($rowDb['status'] < 3) {
                ...
                if ($arrUserInfo['isDeptHead']) $intNextStatus = 5;
                else $intNextStatus = 4;
  ...
  if ($db->connect()) {
    ...
    if(isset($_REQUEST['btnShowAlert'])) { // request dari alert
      ...
      else $strKriteria .= "AND status IN (0,1,3,7,8,10) ";
    }
    ...
    $strInputEmployeeStatus = getEmployeeStatusList("dataEmployeeStatus",$strDataEmployeeStatus,  $strEmptyOption2, "style=\"width:$intDefaultWidthPx\" $strDisabled");
	$strInputActive = getEmployeeActiveList("dataActive", $strDataActive, $strEmptyOption2, "style=\"width:$intDefaultWidthPx\" $strDisabled");
