<?
    ...
    while ($rowDb = $db->fetchrow($resDb)) {
      ...
      */
      ...
      while ($rowTmp = $db->fetchrow($resTmp)) {
        if ($strCandidateList != "") $strCandidateList .= "<BR>\n";
      ...
      $strCreateRequest = ($rowDb['status'] == REQUEST_STATUS_DENIED) ? "&nbsp;" : "<button type=button nama=btnCreate$intRows onClick=\"location.href = 'recruitment_edit.php?btnCreate=Create&idPlan=" .$rowDb['id']."'\">" .getWords("create request")."</button>";
  ...
  if ($db->connect()) {
    ...
    if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) $strDataDepartment = $arrUserInfo['department_code'];
    ...
    $strFilterDepartment = getDepartmentList($db,"dataDepartment",$strDataDepartment, $strEmptyOption,"$strDeptKriteria", "style=\"width:$intDefaultWidthPx\" ");
