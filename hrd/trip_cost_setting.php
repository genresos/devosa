...
//--- DAFTAR FUNSI------------------------------------------------------
  ...
  function getData($db, &$intRows)
  {
    ...
    $strTmp = "<input type=hidden name='dataStatus$intRows' value=\"Male (spouse work here)\">";
    $strResult .= "  <td align=center class=tableHeader nowrap>$strTmp&nbsp;Male<br>(spouse work here)</td>\n";
    ...
    $strTmp = "<input type=hidden name='dataStatus$intRows' value=\"Female (spouse work here)\">";
    $strResult .= "  <td align=center class=tableHeader nowrap>$strTmp&nbsp;Female<br>(spouse work here)</td>\n";
    ...
    // cari daftar family status
    $strSQL  = "SELECT family_status_code FROM hrd_family_status ORDER BY oid";
    ...
    foreach($arrFamilyStatus as $strFamilyStatusCode)
    {
      ...
      $strTmpMale0 = "
        <input type=text size=10 maxlength=20 name=detailAmount_".$intRows." value=\"$fltAmountMale0\" class=numeric>
      ...
      $strTmpMale1 = "
        <input type=text size=10 maxlength=20 name=detailAmount_".$intRows." value=\"$fltAmountMale1\" class=numeric>
        <input type=hidden name='detailGender_".$intRows."' value='1'> 
        <input type=hidden name='detailInspouse_".$intRows."' value='t'>
        <input type=hidden name='detailFamilyStatus_".$intRows."' value='$strFamilyStatusCode'>
        <input type=hidden name='chkID_".$intRows++."' value=$intIDMale1>";
      $strTmpFemale0 = "
        <input type=text size=10 maxlength=20 name=detailAmount_".$intRows." value=\"$fltAmountFemale0\" class=numeric>
        <input type=hidden name='detailGender_".$intRows."' value='0'> 
        <input type=hidden name='detailInspouse_".$intRows."' value='f'>
        <input type=hidden name='detailFamilyStatus_".$intRows."' value='$strFamilyStatusCode'>
        <input type=hidden name='chkID_".$intRows++."' value=$intIDFemale0>";
      $strTmpFemale1 = "
        <input type=text size=10 maxlength=20 name=detailAmount_".$intRows." value=\"$fltAmountFemale1\" class=numeric>
        <input type=hidden name='detailGender_".$intRows."' value='0'> 
        <input type=hidden name='detailInspouse_".$intRows."' value='t'>
        <input type=hidden name='detailFamilyStatus_".$intRows."' value='$strFamilyStatusCode'>
  ...
  function getDataSecondary($db, &$intRows)
  {
    ...
    foreach ($arrMedicalType as $strType => $arrMedicalCode)
    {
      if(count($arrMedicalCode) > 1) 
        ...
        $strSpan = "rowspan=2";
      $strHeader1 .= "<td nowrap align=\"center\" $strSpan>".GetWords($ARRAY_MEDICAL_TREATMENT_GROUP[$strType])."&nbsp;</td>";

      foreach ($arrMedicalCode as $strID => $strCode)
      {
        if (count($arrMedicalCode) > 1)
          $strHeader2 .= "<td nowrap align=\"center\">".GetWords($strCode)."&nbsp;</td>";
    ...
    { $i++;
      ...
      { 
        foreach ($arrMedicalCode as $strID => $strCode)
        {
          ...
          $strTmp = "
          <input type=text size=10 maxlength=20 name=detailAmountSecondary_$intRows value=\"$fltAmount\" class=numeric>
