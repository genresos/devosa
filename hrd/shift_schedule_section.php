<?php
  ...
  $dataPrivilege = getDataPrivileges("shift_schedule.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
  ...
  function getData($db, $strMonth, $strYear, $strKriteria)
  {
    ...
    $strSQL = "
      ...
      FROM \"hrd_shift_schedule_section\" WHERE EXTRACT(year from \"shift_date\") = '$strYear'
    ...
    $arrShiftType = array(); // data shift type
    ...
    while ($rowDb = $db->fetchrow($resDb))
    {
        $strResult .= "
          <tr>
            <td>$intRow<input type=hidden name='dataSection$intRow' id='dataSection$intRow' value='" .$rowDb['section_code']."'></td>
    ...
    // footer
    global $strDataDivision, $strDataDepartment, $strDataGroup;
    $intTemp = $intMax-1;
    $strResult .= "
        <tr>
          <td colspan=4>&nbsp;</td>
          <td colspan = $intMax nowrap>
            ...
            <input type=hidden name='dataGroup' id='dataGroup' value='$strDataGroup'>
  ...
  function saveData($db, &$strError)
  {
    ...
    for ($intX = 1; $intX <= $intTotalData; $intX++)
    {
      ...
      if ($strGroupId != "")
      {
        ...
        $strSQL = "
          DELETE FROM \"hrd_shift_schedule_section\"
        ...
        for ($intTgl = 1; $intTgl <= $intTotalDays; $intTgl++)
        {
          ...
          if ($strCode != "" && isset($arrShiftList[$strCode]))
          {
            ...
            $strSQL  = "INSERT INTO \"hrd_shift_schedule_section\" (modified_by, created_by, ";
            ...
            if ($strSQL != "") $resExec = $db->execute($strSQL);
  ...
  if ($db->connect()) {
    ...
    else //jika shift groupnya berdasarkan sectionnya
    {
      ...
      $strInputGroup = getSectionList($db,"dataGroup",$strDataGroup,$strEmptyOption,"","style=\"width:$intDefaultWidthPx\"");
    }
    ...
    if ($strDataDepartment != "") {
        $strKriteria .= "AND t1.division_code = '$strDataDepartment' ";
    }
    if ($strDataDivision != "") {
