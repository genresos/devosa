<?php
  ...
  $dataPrivilege = getDataPrivileges("training_request_edit.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove, $bolCanCheck, $bolCanAcknowledge, $bolCanApprove2);
...
//---- INISIALISASI ----------------------------------------------------
  ...
  function getData($db)
  {
	...
	if (isset($_REQUEST['fileID']) && !isset($_REQUEST['btnSave'])) {
      if ($bolCanEdit) deleteFile($db,$_REQUEST['fileID']);
      else {
  ...
  function getTrainingParticipant($db, $strDataID = "")
  {
    ...
    if($bolIsEmployee) $intMaxShow = 1;
    $intAdd = 50; // maksimum tambahan
    $intRows = 0;
    $intShown = 0;
    $strResult = "";
    $strButtons = "";
    $arrStatus = array("ok", "cancel");
    ...
    foreach($arrParticipant AS $id => $rowDb)
    {
      ...
      $strResult .= "  <td align=center title='cost " .standardFormat($rowDb['cost'])."'>g<input type=checkbox id='chkID$intRows' name='chkID$intRows' $strAction></td>\n";
    ...
    if(!$bolIsEmployee){
      ...
      $strResult .= "  <td colspan=10>&nbsp;<a href=\"javascript:showMoreInput();\">" .getWords('more'). "</a></td></tr>\n";
    }
  ...
  function getTrainingTrainer($db, $strDataID = "")
  {
    ...
    foreach($arrParticipant AS $id => $rowDb)
    {
      ...
      $strResult .= "  <td align=center title='cost " .standardFormat($rowDb['cost'])."'><input type=checkbox id='chkTrainerID$intRows' name='chkTrainerID$intRows' $strAction></td>\n";
      $strResult .= "</tr>\n";
    ...
    $strResult .= " <tr valign=top><td>&nbsp;</td>\n";
    $strResult .= "  <td colspan=3>&nbsp;<a href=\"javascript:showMoreTrainerInput();\">" .getWords('more'). "</a></td></tr>\n";
    $strResult .= "  </table>\n";
  ...
  function getTimeDetail($db, $strDataID = "", $arrData, $strInputDuration)
  {
    ...
    foreach($arrtime AS $id => $rowDb)
    {
      ...
      $strResult .= "<td><input type=text size=10 maxlength=10 name=dataDateTraining$intRows id=dataDateTraining$intRows value=\"" .substr($rowDb['trainingdate'],0,10). "\" class='date-empty'> ";
      $strResult .= " <input name=\"btnDate$intRows\" type=button id=\"btnDate$intRows\" value='..'></td>";
      $strResult .= "  <td align=center><input type=text size=5 name=dataStart$intRows id=dataStart$intRows value=\"" .substr($rowDb['timestart'],0,5). "\" onBlur=\"getDuration('$intRows')\" class='input_mask mask_shorttime'></td>";
      $strResult .= "  <td align=center><input type=text size=5 name=dataFinish$intRows id=dataFinish$intRows value=\"" .substr($rowDb['timefinish'],0,5). "\" onBlur=\"getDuration('$intRows')\" class='input_mask mask_shorttime'></td>";
    ...
    // footer
    $strResult .= "  <tr ><td colspan=6 title='$strInfo'>&nbsp;<a href=\"javascript:showMoreInputTime();\">" .getWords('more'). "</a></td></tr>\n";
    $strResult .= "  </table>\n";
...
// print_r($_REQUEST);
   ...
   if ($strIDEmployee == "") {
    ...
    if ($bolOK)
    {
      ...
      $arrEmployee = array(); // nampung ID dari employee peserta
      for ($i = 1; $i <= $intTotal; $i++) 
      {
        ...
        if ($strEmpID != "")
        {
          ...
          if ($bolOK) 
          {
            $arrTmp = isLeaveExists($db, $strID, $strDataTrainingDate, $strDataTrainingDateThru, $strDataID); // activity.php
            if ($arrTmp['status']) { // ada data conflic
              ...
              if ($arrTmp['from'] != $arrTmp['thru']) $strError .= " - " .pgDateFormat($arrTmp['thru'], "d-M-y");
              $strError .= "</a>";
              break;
            }
          }
          ...
          if ($bolOK) 
          {
            $arrTmp = isTrainingExists($db, $strID, $strDataTrainingDate, $strDataTrainingDateThru, $strDataID); // activity.php
            if ($arrTmp['status']) 
            { // ada data conflic
              $bolOK = false;
      ...
      if ($strDataID != "")
      {
        ...
        for ($i = 1; $i <= $intTotal; $i++)
        {
          if (isset($_REQUEST['detailEmployeeID'.$i]))
          {
            ...
            $fltCostOther = (isset($_REQUEST['detailAmountOther'.$i])) ? str_replace(",", "", $_REQUEST['detailAmountOther'.$i]) : 0;
        ...
        */
        ...
        $resExec = $db->execute($strSQL);
        if ($strDataCategory == 0 || $strDataCategory == 1) $intTotalTrainer = 0;
        else $intTotalTrainer = (isset($_REQUEST['numTrainerShow'])) ? $_REQUEST['numTrainerShow'] : 0;
  ...
  if ($db->connect()) 
  {
   ...
   die();
    ...
    if ($bolCanView)
    {
      ...
      if ($strDataID == "")
      {
        ...
        if (!is_numeric($strInputLastNumber)) $strInputLastNumber = 0;
        else $strInputLastNumber = (int)$strInputLastNumber;
        $strInputLastNumber++;
    ...
    $strInputInstitution =  getDataTrainingInstitution($db,"dataInstitutionID",$arrData['dataInstitutionID'], "text", "", " style=\"width:$strDefaultWidthPx \" disabled");
    ...
    $strInputTrainer = getDataTrainingInstructor($db,"dataInstructorID",$arrData['dataInstructorID'], "text", "", " style=\"width:$strDefaultWidthPx \" disabled");
    ...
    $strInputDivision = getDataDivision($db,"dataDivision",$arrData['dataDivision'], "text", "", " style=\"width:$strDefaultWidthPx \" disabled");
    ...
    $strInputDepartment = getDataDepartment($db,"dataDepartment",$arrData['dataDepartment'], "text", "", " style=\"width:$strDefaultWidthPx \" disabled");
    ...
    $strInputSection = getDataSection($db,"dataSection",$arrData['dataSection'], "text", "", " style=\"width:$strDefaultWidthPx \" disabled");
	...
	$strInputSubSection = getDataSubSection($db,"dataSubSection",$arrData['dataSubSection'], "text", "", " style=\"width:$strDefaultWidthPx \" disabled");
	...
	$strInputTopic = getTrainingTopicList($db,"dataPlanID",$arrData['dataPlan'],  "text", "WHERE status = 2", " style=\"width:$strDefaultWidthPx \" $strReadonly");
   ...
   $strInputSection = "<input type=text id='dataSection'  style=\"width:$strDefaultWidthPx \" disabled>";
    ...
    $arrTmp = array("Public", "External", "Internal", "Sharing Session");
    $strInputCategory = getComboFromArray($arrTmp, "dataCategory", $arrData['dataCategory'], "", " style=\"width:$strDefaultWidthPx \" ");
    $arrTmp = array("Domestic", "Foreign");
    $strInputPlace = getComboFromArray($arrTmp, "dataPlace", $arrData['dataPlace'], "", " style=\"width:$strDefaultWidthPx \" ");
    ...
    $arrTmp = array("Employee", "Company", "Free");
    $strInputPaidBy = getComboFromArray($arrTmp, "dataPaidBy", $arrData['dataPaidBy'], "", " style=\"width:$strDefaultWidthPx \" ");
