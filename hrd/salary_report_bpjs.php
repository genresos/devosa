<?php
  ...
  $dataPrivilege = getDataPrivileges("salary_report_bpjs.php", $bolCanView, $bolCanEdit, $bolCanDelete, $bolCanApprove);
...
//  //this save button will hide after save <toggle>
  ...
  if ($showReport)
  {
    ...
    else
    {
      ...
      $myDataGrid->setCaption("Report BPJS - $intYear - $intMonth");
  ...
  function getOvertimeReport($db, $intYear, $intMonth, $strKriteria = "")
  {
    ...
    while ($row = $db->fetchrow($res))
    {
        ...
        $strSQL2 = "SELECT t0.*, \"join_date\", \"resign_date\", t1.\"employee_name\", t2.\"position_name\", t3.functional_name, t1.employee_status, t1.get_bpjs, t2.is_bpjs_ks_base_umk, t4.minimum_salary, t2.bpjs_ks_base, branch_bpjs_ks_code
                    FROM \"hrd_salary_detail\" AS t0
                        ...
                        LEFT JOIN \"hrd_branch\" AS t4 ON t4.\"branch_code\" = t1.\"branch_bpjs_ks_code\"
                    WHERE t1.get_bpjs = 1 AND (t0.bpjs_allowance > 0 OR t0.bpjs_deduction > 0) AND \"id_salary_master\" ='$salaryMasterID' $strKriteria ORDER BY t1.\"employee_name\"";
        ...
        while ($row2 = $db->fetchrow($res2))
        {
            ...
            $arrResult[$row2['id_employee']]['branch_bpjs_ks_code'] = $row2['branch_bpjs_ks_code'];
            ...
            if($row2['get_bpjs']==1)
            {
              if ($row2['is_bpjs_ks_base_umk']=='t') {
                $arrResult[$row2['id_employee']]['bpjs_base'] = $row2['minimum_salary'];
              }
              else {
                $arrResult[$row2['id_employee']]['bpjs_base'] = $row2['bpjs_ks_base'];
              }
