...
class cDataGrid2 extends cDataGrid
  ...
  function getData($db)
  {
    ...
    if ($db->connect())
    {
      ...
      $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID1", "id_plan", array('rowspan' => 2, 'width' => 30), array('align'=>'center', 'nowrap' => '')));
      $myDataGrid->addColumnNumbering(new DataGrid_Column(getWords("no."), "", array('width'=>'30'), array('nowrap'=>'')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("topic"), "topic", "", array('nowrap' => '')));
	  
      $myDataGrid->addColumn(new DataGrid_Column(getWords("creator"), "creator", "", array('nowrap' => '')));
	   $myDataGrid->addColumn(new DataGrid_Column(getWords("div."), "division_code", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("dept."), "department_code", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("section"), "section_code", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("sub sect."), "sub_section_code", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("type"), "training_type",  "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("purpose"), "purpose",  "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("duration"), "duration", "", array('nowrap' => '')));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("expected date"), "expected_date",  "", array('nowrap' => ''), true, true, "", "formatDate()", "string", true, 12, false));
      $myDataGrid->addColumn(new DataGrid_Column(getWords("institution"), "name_vendor",  "", array('nowrap' => '')));
	  $myDataGrid->addColumn(new DataGrid_Column(getWords("cost"), "cost","", array("nowrap" => "nowrap", "align" => "right"), false, true, "", "formatNumeric(ignoreZero=false,decimal=2)", "numeric", true, 15, true,"cost"));
      ...
      $myDataGrid->addColumn(new DataGrid_Column("", "", array("width" => "60"), array('align' => 'center', 'nowrap' => ''), false, false, "","printCreatedLink()", "", false /*show in excel*/));
      ...
      generateRoleButtons($dataPrivilege['edit'], $dataPrivilege['delete'], "", $dataPrivilege['approve'], "", true, $myDataGrid);
  ...
  function printCreatedLink($params)
  {
	...
	if($record['status']==2){
    return "
      <a href=\"training_request_edit.php?btnCreate=Create&id_plan=".$record['id']."\">" .getWords('create request'). "</a>";
	  }else{
...
echo "G";
  ...
  if ($db->connect()) 
  {
    ...
    $f->caption = strtoupper($strWordsFILTERDATA);
	 $f->addSelect(getWords("Year"), "dataYear", getDataYear(), array("style" => "width:$strDateWidth"), "date", false, true, true);
