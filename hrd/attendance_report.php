...
function getData(
    ...
    if (true)//!empty($chkEmp) || !empty($chkDept) || !empty($chkSect)
    {
        ...
        while ($rowDb = $db->fetchrow($resDb)) {
            ...
            $arrResult[$intRows]['employee_id_2'] = $rowDb['employee_id_2'];
...
function showHeader()
{
    ...
    $strResult = "";
    if ($bolPrint) {
        $strResult .= "<table align='left' class='gridTable'>";
        $strResult .= "  <tr><td><h2>Legend</h2></td></tr> ";
        foreach ($arrAbsType AS $kode => $nama) {
            $strResult .= "  <tr><td nowrap >&nbsp;<font size=0.2 pt>" . strtoupper(
                    $kode
                ) . "</font></td><td nowrap>&nbsp;<font size=0.2 pt>" . strtoupper($nama) . "</font></td></tr>\n";
        }
        $strResult .= "  <tr><td>&nbsp;</td></tr> ";
        $strResult .= "</table>";
    }
    // bikin header table
    $strResult .= "<table cellspacing=0 cellpadding=0 border=1 class='gridTable'>\n";
    // bikin header table
    ...
    //$strResult .= "  <td nowrap rowspan=2>&nbsp;" . strtoupper(getWords("no")) . "</td>\n";
    // $strResult .= "  <td nowrap rowspan=2>&nbsp;" .strtoupper(getWords("id"))."</td>\n";
    $strResult .= "  <td nowrap rowspan=2>&nbsp;" . strtoupper(getwords("n i k")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2>&nbsp;" . strtoupper(getwords("n i k corporate")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2>&nbsp;" . strtoupper(getWords("employee name")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2 $strDefaultWidth>&nbsp;" . strtoupper(getWords("attendance")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2 $strDefaultWidth>&nbsp;" . strtoupper(getWords("late")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2 $strDefaultWidth>&nbsp;" . strtoupper(getWords("late (min)")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2 $strDefaultWidth>&nbsp;" . strtoupper(getWords("early")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2 $strDefaultWidth>&nbsp;" . strtoupper(getWords("early (min)")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2 $strDefaultWidth>&nbsp;" . strtoupper(getWords("holiday")) . "</td>\n";
    $strResult .= "  <td nowrap rowspan=2 $strDefaultWidth>&nbsp;" . strtoupper(
            getWords("shift allowance")
        ) . "</td>\n";
    $strResult .= "  <td nowrap colspan=" . ($intNumAbsType + 1) . ">&nbsp;" . strtoupper(
            getWords("absence")
        ) . "</td>\n";
    ...
    $strResult .= " <tr align=center class=tableHeader>\n";
    foreach ($arrAbsType AS $kode => $nama) {
        $strResult .= "  <td nowrap title=\"$nama\" $strDefaultWidth>&nbsp;" . strtoupper($kode) . "</td>\n";
    }
...
function showRows($strNo, $rowData, $strClass = "")
{
    ...
    $strSQL = "SELECT COUNT(*) AS total from hrd_attendance AS t1
                ...
                WHERE attendance_date BETWEEN '" . $_SESSION['sessionFilterDateFrom'] . "' AND '" . $_SESSION['sessionFilterDateThru'] . "'";
    ...
    // $strResult .= "  <td nowrap >&nbsp;" .$rowData['id_employee']."</td>\n";
    $strResult .= "  <td nowrap >" . $rowData['employee_id'] . "</td>\n";
    $strResult .= "  <td nowrap >" . $rowData['employee_id_2'] . "</td>\n";
    $strResult .= "  <td nowrap >" . $rowData['employee_name'] . "</td>\n";
    $strResult .= "  <td nowrap align=center>" . $rowData['attendance'] . "</td>\n";
    $strResult .= "  <td nowrap align=center>" . $rowData['late'] . "</td>\n";
    $strResult .= "  <td nowrap align=center>" . minuteToTime($rowData['totalLate']) . "</td>\n";
    $strResult .= "  <td nowrap align=center>" . $rowData['early'] . "</td>\n";
    $strResult .= "  <td nowrap align=center>" . minuteToTime($rowData['totalEarly']) . "</td>\n";
    $strResult .= "  <td nowrap align=center>" . $rowData['holiday'] . "</td>\n";
    $strResult .= "  <td nowrap align=center>" . $total['total'] . "</td>\n";
    // hitung absen
    foreach ($arrAbsType AS $kode => $nama) {
        $strResult .= "  <td nowrap align=center>" . $rowData['absence_' . $kode] . "</td>\n";
    }
    //$strResult .= "  <td nowrap align=center>&nbsp;" .$rowData['absenceleave']."</td>\n";
    //$strResult .= "  <td nowrap align=center>&nbsp;" .$rowData['special']."</td>\n";
    $strResult .= "  <td nowrap align=center>" . $rowData['absence'] . "</td>\n";
    // hitung cuti
    /*foreach ($ARRAY_LEAVE_TYPE AS $kode => $nama) {
...
function showDataDepartment($db, $arrData)
{
    ...
    foreach ($arrMan AS $strManCode => $strManName) {
        ...
        if ($bolShowTotal && ($bolShowEmp || $bolShowTotalSect || $bolShowTotalDept)) {
        ...
        $arrTotalMan['employee_name'] = "<strong>" . strtoupper(
                getWords("total") . " " . $strManCode
            ) . "</strong>";
        ...
        foreach ($arrTmpDiv AS $strDivCode => $strDivName) {
            if ($bolShowTotal && ($bolShowEmp || $bolShowTotalSect || $bolShowTotalDept)) {
                $strResult .= " <tr valign=top>\n";
                $strResult .= "  <td nowrap colspan=$intColspan>&nbsp;<strong>[$strDivCode] $strDivName</strong></td>\n";
                $strResult .= " </tr>\n";
            }
...
if ($db->connect()) {
    ...
    $chkEmp = (isset($_REQUEST['chkEmp'])) ? "checked" : "1";
    ...
    $strInputDateFrom = "<input type=text name=dataDateFrom id=dataDateFrom size=15 maxlength=10 value=\"$strDataDateFrom\" onChange=\"document.formData.dataDateForm.value=this.value\">";
    $strInputDateThru = "<input type=text name=dataDateThru id=dataDateThru size=15 maxlength=10 value=\"$strDataDateThru\" onChange=\"document.formData.dataDateThru.value=this.value\">";
    ...
    $strInputEmployee = "<input type=text name=dataEmployee id=dataEmployee size=15 maxlength=30 value=\"$strDataEmployee\" $strEmpReadonly>";
    ...
    $strChkEmployee = "<input type=\"checkbox\" name=\"chkEmp\" $chkEmp>";
