...
$strDetailFooter = "<tr class='tableHeader'><td>&nbsp;</td><td colspan><input name='chkAll' type='checkbox' id='chkAll' value='checkbox' onClick='checkAll();'>
		<td><input name='btnDelete' type='button' id='btnDelete' value='" . getWords("delete") . "' onClick='DeleteData();'></td>
		<td colspan=2 nowrap align='left'>&nbsp;</td><td colspan=2><input name='btnDefaultEarly' type='button' id='btnDefaultEarly' value='" . getWords(
        "use default"
    ) . "' onClick='checkDefaultEarly();'></td><td colspan=2 nowrap align='left'>&nbsp;</td><td colspan=2><input name='btnDefaultAfternoon' type='button' id='btnDefaultAfternoon' value='" . getWords(
        "use default"
    ) . "' onClick='checkDefaultAfternoon();'></td><td colspan=9>&nbsp;</td> </tr>";
//----------------------------------------------------------------------
//--- DAFTAR FUNSI------------------------------------------------------
...
function getData($db, &$arrData, $strDataID = "")
{
    ...
    if ($strDataID != "") {
        ...
        $strSQL .= "WHERE t1.id_application = '$strDataID' $strKriteria ";
        ...
        while ($rowDb = $db->fetchrow($resDb)) {
            if ($isEmployee) {
                if ($rowDb['id_employee'] == $arrUserInfo["id_employee"]) {
    ...
    while ($i < $maxOTMember) {
        ...
        $arrData['dataDetail'] .= "  <td><input type=text name='detailStartPlan$i' id='detailStartPlan$i'  size=5 maxlength=5 onChange='checkAttendance($i)' readonly $strDis></td>";
        $arrData['dataDetail'] .= "  <td><input type=text name='detailFinishPlan$i' id='detailFinishPlan$i' size=5 maxlength=5 onChange='checkAttendance($i)' readonly $strDis></td>";
...
function saveData($db, &$strDataID, &$strError)
{
    ...
    for ($i = 1; $i <= $intTotal; $i++) {
        ...
        if ($strIDEmployee != "") {
            ...
            $mailNote = !empty($strPurpose) ? $strPurpose : $strDataNote;
            $strBody = "";
            $strBody .= "Name: " . getEmployeeNameEmail($strIDEmployee) . "<br>";
            $strBody .= "Date: " . $strDataDate . "<br>";
            $strBody .= "Note: " . $mailNote;
            $strStartEarlyPlanEmail = ($strStartEarlyPlan == null) ? " " : " ";
            $strFinishEarlyPlanEmail = ($strFinishEarlyPlan == null) ? " " : " ";
            $strBody .= "<br>Afternoon Early: " . $strStartEarlyPlanEmail . " - " . $strFinishEarlyPlanEmail;
            $strBody .= "<br>Afternoon Plan: " . $strStartPlan . " - " . $strFinishPlan;
            $strBody .= "<br>Total Time: " . $arrOT['total'] . " hour(s)";
            $strBody = getBody(0, 'Overtime', $strBody, $_SESSION['sessionUserID']);
            $strSubject = getSubject(0, 'Overtime', getEmployeeIDEmail($strIDEmployee));
...
function searchEmployee($db, $strCategory, $strKey)
{
    ...
    if ($_SESSION['sessionUserRole'] == ROLE_EMPLOYEE) {
        $strSQL .= "AND employee_id='" . $arrUserInfo['employee_id'] . "'";
    } else if ($_SESSION['sessionUserRole'] == ROLE_SUPERVISOR) {
        if (!empty($strDataDivision) && $strCategory != "division") {
            $strSQL .= "AND division_code='" . $strDataDivision . "'";
        }
        if (!empty($strDataDepartment) && $strCategory != "department") {
            $strSQL .= "AND department_code='" . $strDataDepartment . "'";
        }
        if (!empty($strDataSection) && $strCategory != "section") {
            $strSQL .= "AND section_code='" . $strDataSection . "'";
        }
        if (!empty($strDataSubSection) && $strCategory != "sub_section") {
            $strSQL .= "AND sub_section_code='" . $strDataSubSection . "'";
        }
    } else if ($_SESSION['sessionUserRole'] == ROLE_BRANCH_ADMIN) {
        $strSQL .= "AND branch_code='" . $arrUserInfo['branch_code'] . "'";
    } else if ($_SESSION['sessionUserRole'] == ROLE_DIVISION_BRANCH_ADMIN || $_SESSION['sessionUserRole'] == ROLE_SUPERVISOR_BRANCH) {
        if (!empty($strDataDivision) && $strCategory != "division") {
            $strSQL .= "AND division_code='" . $strDataDivision . "'";
        }
        if (!empty($strDataDepartment) && $strCategory != "department") {
...
function getDataBreak($db, $tipe, $strDataID)
{
    ...
    if ($i == 0 && getSetting("ot_auto_break") == 't') {
        ...
        $strResult .= "<tr valign=top id='detailData$tipe" . "_$i'>\n";
        $strResult .= "  <td><input type=text name=dataBreak$tipe" . "_$i size=12 maxlength=10 value=$strStartBreak></td>\n";
        $strResult .= "  <td><input type=text name=dataDuration$tipe" . "_$i size=12 maxlength=10 value=$strDurationBreak></td>\n";
        $strResult .= "  <td><input type=text name=dataBreakFinish$tipe" . "_$i size=12 maxlength=10 value=$strDefaultBreak></td>\n";
        $strResult .= "  <td><input type=\"text\" name=\"dataNote$tipe" . "_$i\" size=\"50\" maxlength=\"50\" value='Default Break Time'></td>\n";
        $strResult .= "</tr>\n";
        $i++;
    }
    if ($i == 0) {
    ...
    while ($i <= $intMaxDetail) {
        ...
        //$strResult .= "  <td>&nbsp;</td>\n";
        $strResult .= "  <td><input type=text name=dataBreakFinish$tipe" . "_$i size=12 maxlength=10 value=$strDefaultBreak $strDis></td>\n";
...
if ($db->connect()) {
    ...
    $strInputNote = "<textarea name='dataNote' required='required' id='dataNote' rows=3 style=width:$intDefaultWidthPx>" . $arrData['dataNote'] . "</textarea>";
    ...
    $strInputEmployee = "<input type=text name=dataEmployee id=dataEmployee size=12 maxlength=30  style=width:$intDefaultWidthPx >";
    // tambahkan tombol add  pada bagian employee
    $strInputEmployee .= " <input type=button id=btnAdd name=btnAdd value='" . getWords(
            "add to list"
    ...
    $strInputDate = "<input type=text name=dataDate id=dataDate size=12 maxlength=10 value=\"" . $arrData['dataDate'] . "\" >";
