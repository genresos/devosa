<?
 ...
 //---- INISIALISASI ----------------------------------------------------
  ...
  function getData($db) 
  {
    ...
    if ($strDataID != "") 
    {
      ...
      if ($rowDb = $db->fetchrow($resDb)) {
        ...
        $arrData['dataDate'] = $rowDb['advertisement_date'];
  ...
  function getPositionAdv($db, $strDataID = "") 
  {
    ...
    $strResult .= "  <td nowrap>&nbsp;" ."MRF"."</td>\n";
    $strResult .= "  <td nowrap>&nbsp;" .getWords('position')."</td>\n";
    ...
    foreach($arrDetail AS $id => $rowDb)
    {
      ...
      $strResult .= "  <td>" .getComboFromArray($arrRecruitmentID, "detailRecruitment$intRows", $rowDb['id_recruitment_need'], $strEmptyOption, " $strAction ")."</td>";
      $strResult .= "  <td><input type=text size=30 maxlength=100 name=detailPosition$intRows id=detailPosition$intRows value=\"" .$rowDb['position_name']. "\" ></td>";
      $strResult .= "  <td><input type=text size=20 maxlength=100 name=detailNote$intRows id=detailNote$intRows value=\"" .$rowDb['note']. "\"></td>";
    ...
    for ($i = 1; $i <= $intAdd; $i++) 
    {
      ...
      $strAction = "onChange = \"onRecruitmentChanged($intRows);\" ";
      $strResult .= "  <td>" .getComboFromArray($arrRecruitmentID, "detailRecruitment$intRows", "", $strEmptyOption, " $strAction ")."</td>";
      $strResult .= "  <td><input type=text size=30 maxlength=100 name=detailPosition$intRows id=detailPosition$intRows value=\"\" $strDisabled></td>";
      $strResult .= "  <td><input type=text size=20 maxlength=100 name=detailNote$intRows id=detailNote$intRows value=\"\" $strDisabled></td>";
      $strAction = " onClick = \"chkDeleteChanged($intRows);\" ";
      $strResult .= "  <td align=center><input type=checkbox name='chkID$intRows' $strAction></td>\n";
      $strResult .= "</tr>\n";
  ...
  if ($db->connect()) 
  {
    ...
    $strReadonlyEmployee = "";
    $strDisabled = "";
    $strInputDate = "<input type=text size=15 maxlength=10 name=dataDate id=dataDate value=\"" .$arrData['dataDate']. "\" $strReadonlyRequest class='date'>";
    $strInputDate .= " <input name=\"btnDate\" type=button id=\"btnDate\" value='..' $strDisabled>";
    $strInputDueDate  = "<input type=text size=15 maxlength=10 name=dataDueDate id=dataDueDate value=\"" .$arrData['dataDueDate']. "\" class='date-empty'>";
    $strInputDueDate .= " <input name=\"btnDueDate\" type=button id=\"btnDueDate\" value='..' $strDisabled>";
    $strInputReference = getReferenceList($db,"dataReference", $arrData['dataReference'], "", "", " style=\"width:$strDefaultWidthPx \" ");
