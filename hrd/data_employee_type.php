<?php
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("employee type code"), "dataCode", "", array("size" => 50), "string", true, true, true);  
    $f->addInput(getWords("employee type name"), "dataName", "", array("size" => 50), "string", true, true, true);  
    $f->addCheckBox(getWords("is employee?"), "dataIsEmployee", false, array());  
    $f->addCheckBox(getWords("has leave?"), "dataIsLeave", false, array());
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("employee type code"), "code", array('width' => '120'), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("employee type name"), "name", array('width' => ''), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("is employee"), "is_employee", array('width' => '80'), array('align' => 'center'), true, false, "", "printIsEmployee()"));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("has leave"), "is_leave", array('width' => '80'), array('align' => 'center'), true, false, "", "printHasLeave()"));
  if ($bolCanEdit)
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    {
      $bolSuccess = $dataHrdEmployeeType->update(/*pk*/"code='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    if ($bolSuccess) $f->setValue('dataID', $f->getValue('dataID'));
