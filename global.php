<?php
  ...
  include_once('includes/tbs-3.7.0/tbs_class.php');
  ...
  function getDataPrivilegesFromDatabase($idGroup)
  {
    ...
    if ($db->connect())
    {
      ...
      {
       $strSQL = "
        SELECT c.view, c.edit, c.delete , c.approve, c.acknowledge, c.check, c.id_adm_menu, c.approve2,
        ...
        FROM (SELECT gm.view, gm.edit, gm.delete , gm.approve, gm.approve2, gm.acknowledge, gm.check, m.id_adm_menu, m.page_name, m.menu_level, 
  ...
  function getDataPrivileges($strPage, &$bolView, &$bolEdit,&$bolDelete, &$bolApprove, &$bolCheck = false, &$bolAcknowledge = false, &$bolApprove2)
  {   
    $bolView = false;
    $bolEdit = false;
    $bolDelete = false;
    ...
    $result = null;
    if (count($_SESSION['sessionPrivileges']) > 0)
      foreach($_SESSION['sessionPrivileges'] as $value)
      {
        if (strtolower($value["php_file"]) == $strPage)
        {
          $result = $value;
          $bolView = $value["view"]=='t';
          $bolEdit = $value["edit"]=='t';
          $bolDelete = $value["delete"]=='t';   
          $bolApprove = $value["approve"]=='t';   
          $bolApprove2 = $value["approve2"]=='t';   
          $bolAcknowledge = $value["acknowledge"]=='t';   
          $bolCheck = $value["check"]=='t';   
          break;
        }
      }
