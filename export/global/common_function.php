<?php
  ...
  function generateRoleButtons($bolCanEdit, $bolCanDelete, $bolCanCheck, $bolCanApprove, ,$bolCanApprove2 = false, $bolPaid = false, $bolDatagridClass = false, &$objDatagrid = null)
  {
    global $words;
    if ($bolDatagridClass)
    {
      ...
      if ($bolCanDelete)
        $objDatagrid->addSpecialButton("btnDelete", "btnDelete", "submit", $words['delete'], "onClick=\"javascript:return myClient.confirmDelete();\"", "deleteData()");
      if ($bolCanCheck)
      {
      ...
      if ($bolCanApprove)
      {
        ...
        $objDatagrid->addSpecialButton("btnApproved", "btnApproved", "submit", $words['approved'], "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");
        $objDatagrid->addSpecialButton("btnDenied", "btnDenied", "submit", $words['denied'], "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");
        if ($bolPaid) 
          $objDatagrid->addSpecialButton("btnPaid", "btnPaid", "submit", $words['paid'], "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");

      }
      if ($bolCanApprove2)
      {

        $objDatagrid->addSpecialButton("btnApproved2", "btnApproved2", "submit", $words['approved 2'], "onClick=\"javascript:return myClient.confirmChangeStatus();\"", "callChangeStatus()");
      }
    }
    else
    {
     ...
     /* if ($bolCanEdit) 
      ...
      {
        $strButtons .= "&nbsp;";
        $strButtons .= generateSubmit("btnChecked", $words['check'], "", " onClick=\"return confirmStatusChanges(false)\"");
        $strButtons .= "&nbsp;";
      ...
      {
        $strButtons .= "&nbsp;";
        $strButtons .= generateSubmit("btnApproved", $words['approved'], "", " onClick=\"return confirmStatusChanges(false)\"");
        $strButtons .= "&nbsp;";
        $strButtons .= generateSubmit("btnDenied", $words['denied'], "", " onClick=\"return confirmStatusChanges(false)\"");  
        $strButtons .= "&nbsp;";
        if ($bolPaid) $strButtons .= generateSubmit("btnPaid", $words['paid'], "", " onClick=\"return confirmStatusChanges(false)\"");
      }
      return $strButtons;
    }
  }
...
function getCompanyCode()
{
  ...
  function getInitialValue($strPostSufix, $defaultValue = "", $priorityValue = "", $strSessionSufix = null, $bolSetSession = true, $strPostPrefix = "data", $strSessionPrefix = "sessiondata") 
  {
    global $_SESSION;
    global $_POST;
    ...
    else
    {
      ...
      if (isset($_POST[$strPostPrefix.$strPostSufix])) $value = $_REQUEST[$strPostPrefix.$strPostSufix];
    }
