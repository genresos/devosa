<?php
  ...
  */
  class clsCommonDate
  {
    var $strDateDelimiter = "-"; // pemisah antara tanggal, default -, untuk format standard tampilan ke user entry
    var $strTimeDelimiter = ":"; // pemisah antara jam, default :
    /*
    ...
    */
    ...
    var $intDayPos    = 2; // posisi tahun
    var $bolSQLFormat = true; // apakah format tanggal yang digunakan adalah standard seperti format SQL (YYYY-MM-DD)
    ...
    */
    function getTotalDayOfMonth($intMonth, $intYear = "")
    {
      ...
      {
        if ($intYear == "" || !is_numeric($intYear)) $intYear = date("Y"); // default tahun sekarang
        $intResult = ($this->isKabisat($intYear)) ? 29 : 28;
      }
      else if (isset($arrMaxDay[$intMonth])) $intResult = $arrMaxDay[$intMonth];
    ...
    */
    function isKabisat($intYear)
    {
      ...
      else
      {
        if (($intYear % 100) == 0) // bisa dibagi seratus
          return (($intYear % 400) == 0); // bisa dibagi 400 = kabisat, kalau gak bisa,bukan
        else
          return true;
      }
    }
    ...
    {
      $arrFullName  = array(1 => "january", "february", "march", "april", "may", "june",
        "july", "august", "september", "october", "november", "december");
      $arrShortName = array(1 => "jan", "feb", "mar", "apr", "may", "jun",
        "jul", "aug", "sep", "oct", "nov", "dec");
        
      $strResult = "";
      if ($bolFull) $strResult = (isset($arrFullName[$intMonth])) ? $arrFullName[$intMonth] : "";
      else $strResult = (isset($arrShortName[$intMonth])) ? $arrShortName[$intMonth] : "";
      return $strResult;
    }
    /* getDayName : fungsi untuk mengambil kode hari (nama hari, dalam bahasa inggris) */
    function getDayName($intMonth, $bolFull = true)
    {
      $arrFullName  = array(0 => "sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday");
      $arrShortName = array(0 => "sun", "mon", "tue", "wed", "thu", "fri", "sat");
    ...
    */
    ...
    {
      $bolResult = false;
      if ($strDate != "" && preg_match('/(\d+)'.$this->strDateDelimiter.'(\d+)'.$this->strDateDelimiter.'(\d+)/',$strDate)) 
      {
    ...
    */
    ...
    {
      if ($this->bolSQLFormat) return $strDate; // tidak perlu diubah, sudah sama
      else
      {
    ...
    */
    function getPeriodOfMonth($intMonth = "", $intYear = "")
    {
      ...
      if ($intMonth == "") $intMonth = date("m");
      
      $arrResult = array("start" => "", "finish" => "");
