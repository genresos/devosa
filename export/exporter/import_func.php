<?php
  ...
  */
  ...
  function getAttendanceData($db, $bolSilent = true) {
    ...
    if ($strFileName == "") {
      if (!$bolSilent) $strResult = "No file setting!";
    } else {
  ...
  function processAttendance($db, &$prb, $strDateFrom, $strDateTo, $bolSilent = true, $strDataCompany) 
  {
    ...
    $arrBarcode = array();
    $strSQLYesterdaySet = $strSQLTodaySet = "";

    $strSQL  = "SELECT id, note FROM hrd_company WHERE id = $strDataCompany ";
    $resDb = $db->execute($strSQL);
    if ($rowDb = $db->fetchrow($resDb))
      $strCompanyNote = $rowDb['note'];


    $strSQL  = "
      SELECT id, barcode FROM hrd_employee WHERE id_company = $strDataCompany  
      AND ((join_date, resign_date) OVERLAPS (DATE '".$strDateFrom."', DATE '".$strDateTo."') 
          OR (join_date <= DATE '".$strDateFrom."' AND resign_date is null))";
    /*if ($strIDEmployee != "")
    ...
    // mulai proses
    $strResult = "";
    //$db1 = odbc_connect("etcom", "", "eagle@#$%02");
    ...
    $strSQL  = "SELECT USERID, CHECKTYPE, CHECKTIME FROM CHECKINOUT WHERE (CDate([CHECKTIME]) BETWEEN #$strDateFrom# AND #".$strDateTo1."#) ";    
    /*if ($strIDEmployee != "")
      $strSQL  .= "AND Per_Code = '$strFingerID'";*/
    ...
    { 
      ...
      if (count($arrTmp) > 1) 
      {
        ...
        {
          $strIO = ($row['CHECKTYPE'] == 'O') ? 0 : 1;
          $arrAttInfo[$strDate][$arrBarcode[$strID]][] = array($strTime /*timestamp*/, $strIO /*in/out  1 = in, 0 = out*/, $strKEY /*in/out  3 = in Shift3, 4 = out shift 3*/);
        }
      }
    }
    ...
    $objAttendanceClass->setFilter(getNextDate($strFirstdate, -1), $strLastdate, "", " AND id_company = $strDataCompany " );
    ...
    while (dateCompare($strCurrdate,$strLastdate) <= 0)
    {
      if (isset($arrAttInfo[$strCurrdate]))
      {
        ...
        foreach ($arrAttInfo[$strCurrdate] AS $strIDEmployee => $arrTimeIO)
        {
          ...
          $arrPrev = array();
          $intArray = 0;
          $strInYesterday = $objYesterday->strAttendanceStart;
          $strOutYesterday = $objYesterday->strAttendanceFinish;
          $strInToday = $objToday->strAttendanceStart;
          $strOutToday = $objToday->strAttendanceFinish;
          ...
          foreach ($arrTimeIO AS $x => $arrTime)
          {
            ...
            if($x == 0)
            {
              ...
              if ($arrTime[1] == 0)
              {
                ...
                //else 
                if ($objYesterday->strAttendanceFinish == "" && timeCompare($arrTime[0], "12:00") <= 0)
                  $strOutYesterday = $arrTime[0];
            ...
            { //jika duplikasi dari punch sejenis sebelumnya, abaikan
              if ( getIntervalHour($arrPrev[0], $arrTime[0]) < 60 && $arrPrev[1] == $arrTime[1] && $arrPrev[2] == $arrTime[2])
          ...
          if ($strInYesterday != "" && $strOutYesterday != "") // kemarin shift malam
          {
            ...
            $objYesterday->CreateCurrentAttendance($objAttendanceClass);
          }
          elseif($strInYesterday != "") // kemarin shift malam
          {
            $objYesterday->strAttendanceStart = $strInYesterday;
            $objYesterday->bolYesterday = true;
            $objYesterday->calculateDuration();
          ...
          elseif ($strOutYesterday != "") // kemarin shift malam
          {
            $objYesterday->strAttendanceFinish = $strOutYesterday;
            $objYesterday->bolYesterday = true;
            $objYesterday->calculateDuration();
          ...
          $objToday->createCurrentAttendance($objAttendanceClass);
        }
      }
      // next date
    ...
    $objAttendanceClass->saveAttendance();
      ...
      if ($handle != FALSE) {
        ...
        while (!feof($handle)) {
          ...
          if (strlen($buffer) == 23) {
            ...
            if ($strTanggal < $strMinTanggal) $strMinTanggal = $strTanggal;
            if ($strTanggal > $strMaxTanggal) $strMaxTanggal = $strTanggal;
        ...
        while ($rowDb = $db->fetchrow($resDb)) {
            ...
            $varTemp = "";
            if(substr($arrShift['start_time'],0,5) == "24:00" && (int)substr($arrShift['start_time'], 0,2) > (int)substr($strStart, 0,2) && (int)substr($strStart, 0,2) < (int)substr($strNormalFinish, 0,2))
                {
        ...
        if (!$bolSilent) $strResult = "Done! $strMinTanggal - $strMaxTanggal";

      } else  {
        if (!$bolSilent) $strResult = "ERROR File $strFileName";
      }
    }
    return $strResult;
  }//processAttendance
