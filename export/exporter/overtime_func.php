<?php
  ...
  function isShift($db,$strIDEmployee,$strDate,&$strStartTime,&$strFinishTime) {
    ...
    if ($rowDb = $db->fetchrow($resDb)) {
      ...
      $strSQL  = "SELECT t1.* FROM hrd_shift_schedule_group AS t1, ";
      $strSQL .= "hrd_shift_group AS t2, hrd_shift_group_member AS t3 ";
      $strSQL .= "WHERE t1.id_group = t2.id AND t3.id_group = t2.id ";
  ...
  function getTotalWorkingHour($db, $strDate, $strStart, $strFinish, $intBreakFlag = "", $intBreakLinkID = "",  $intTipe = 0) {
    $intResult = 0;
  ...
  function updateOvertime($db, $strIDEmployee, $strDate, $strStart, $strFinish, $bolHoliday = false) {
    ...
    $strSQL .= "AND t1.id_employee = '$strIDEmployee' AND t2.overtime_date = '$strDate' ";
    $resTmp = $db->execute($strSQL);
    if ($rowTmp = $db->fetchrow($resTmp)) {
      // cek dulu , hari libur atau bukan
      if ($bolHoliday) {
        $arrLemburApp = calculateOvertime($db, $strDate, $rowTmp['start_plan'], $rowTmp['finish_plan'], $strStart, $strFinish);
  ...
  function getOvertimeFinish($strNormalStart, $strNormalFinish, $strAttendanceStart, $strAttendanceFinish, $strOvertimeStart, $strOvertimeFinish)
  {
     $bolOT = false;
  ...
  function calculateOvertime($db, $strDate, $strNormalStart, $strNormalFinish, $strAttendanceStart, $strAttendanceFinish, $strOvertimeStart = "", $strOvertimeFinish = "", $bolHoliday = false, $bolBreak = true, $breakFlag = 0, $breakLinkID = "")
  {
     $arrResult = array("total" => 0, "l1" => 0, "l2" => 0, "l3" => 0, "l4" =>0, "morning" => 0, "late" => 0, "early" => 0);

     $intWDay = getWDay($strDate);
     
     if ($bolHoliday) $strNormalStart = $strNormalFinish = $strAttendanceStart;

     if ($strOvertimeStart == "") $strOvertimeStart = $strNormalFinish;
     if ($strOvertimeFinish == "") $strOvertimeFinish = $strAttendanceFinish;
    ...
    if (validTime($strAttendanceStart)) {
      if ($strNormalStart == $strAttendanceStart) {
        ...
        if ($strNormalStart <= $strNormalFinish) { // masuk siang
          // anggap lembur pagi
          $arrResult['morning'] = getTotalWorkingHour($db,$strDate,$strAttendanceStart, $strNormalStart, $breakFlag, $breakLinkID, $intTipe);
        } else { // masuk malam
          if ($strAttendanceStart < $strNormalFinish) { // terlambat
            $arrResult['late'] = getTotalWorkingHour($db,$strDate,$strNormalStart, $strAttendanceStart, $breakFlag, $breakLinkID, $intTipe);
          } else { // awal
            $arrResult['morning'] = getTotalWorkingHour($db,$strDate,$strAttendanceStart, $strNormalStart, $breakFlag, $breakLinkID, $intTipe);
          }
        }
    ...
    if (validTime($strAttendanceFinish)) 
    {
      if ($strAttendanceFinish < $strNormalFinish)
      {
         ...
         if ($strNormalStart <= $strNormalFinish) 
         {
            ...
            { 
               // pulang awal
               $arrResult['early'] = getTotalWorkingHour($db,$strDate,$strAttendanceFinish, $strNormalFinish, $breakFlag, $breakLinkID, $intTipe);
            }
         }
         else
         {
      ...
      if ($bolBreak) 
      {
         $intTotal = getTotalWorkingHour($db,$strDate,$strOvertimeStart,$strOvertimeFinish, $breakFlag, $breakLinkID, $intTipe);
      }
      else 
      {
      ...
      //if($intResidu >= 30) $intTotal = $intMain + 30;
...
function calculateOvertimeByOTSchedule($db, $strDate, $strOvertimeStart, $strOvertimeFinish, $intEarlyOvertime = 0, $strOvertimeType = "", $intTipe = "", $bolBreak = true, $breakFlag = 0, $breakLinkID = "")
{
   //  echo "start = $strOvertimeStart, finish = $strOvertimeFinish";
   $arrResult = array("total" => 0, "l1" => 0, "l2" => 0, "l3" => 0, "l4" =>0);
   $bol5WorkDay = false;
   $intWDay = getWDay($strDate);
   
   if ($strOvertimeType == 1) $bolHoliday = true;
   else $bolHoliday = false;
   ...
   if ($intResidu > 45) $intTotal = $intMain + 60;
   else if($intResidu >= 30) $intTotal = $intMain + 30;
   else $intTotal = $intMain;*/

   //pembulatan 30 menit ke atas
   $intTotal = (floor($intTotal / 30)) * 30;
   ...
   if ($bolHoliday) 
   {
      ...
      $intMax = (60 * 8); // maksimal L2 adalah 7
      if ($intTotal > $intMax)
      {
         $arrResult['l2'] = $intMax;
         $intTotal -= $intMax;
  ...
  function reCalculateOvertimeData($db, $strDate, $strIDEmployee = "") {
    ...
    if ($rowDb = $db->fetchrow($resDb)) {
      ...
      if ($bolOK) {
        $arrLembur = calculateOvertime($db, $strDate, $rowDb['normal_start'], $rowDb['normal_finish'], $rowDb['attendance_start'], $rowDb['attendance_finish']);
  ...
  {
    $arrResult  = array("meal" => 0, "transport" => 0);

    if ($strDate == "" || $strIDEmployee == "" || $strStart == "" || $strFinish == "") return $arrResult;
    ...
    if ($rowDb = $db->fetchrow($resDb)) {
      ...
      if (!is_numeric($fltTransport)) $fltTransport = 0;
      if (!is_numeric($intLimitMeal1)) $intLimitMeal1 = 0;
      if (!is_numeric($intLimitMeal2)) $intLimitMeal2 = 0;
      if (!is_numeric($intLimitMeal3)) $intLimitMeal3 = 0;
      ...
      if ($bolHoliday) {
        ...
        if ($strFinish >= $strLimitTransport) $arrResult['transport'] = $fltTransport;
      }
       ...
       {
          if ($rowS['subsection_meal'] == 'f') $bolGetMeal = false;
          else if ($rowS['subsection_meal'] == 't') $bolGetMeal = true;
          else if ($rowS['section_meal'] == 'f') $bolGetMeal = false;
          else if ($rowS['section_meal'] == 't') $bolGetMeal = true;
          else if ($rowS['department_meal'] == 'f') $bolGetMeal = false;
          else if ($rowS['department_meal'] == 't') $bolGetMeal = true;
          else if ($rowS['division_meal'] == 'f') $bolGetMeal = false;
          else if ($rowS['division_meal'] == 't') $bolGetMeal = true;
          else $bolGetMeal = false;
       }
       else $bolGetMeal = false;
       //--------------------------------------------------------------------------
//echo $fltMeal;
  ...
  {
      $arrOTType = array(1 => 1, 2 => 7, 3 => 1, 4 => 15);
  ...
  function syncOvertimeApplication($db, $strDateFrom, $strDateThru, $strIDEmployee = "", $strKriteria = "")
  {
     ...
     $strSQL .= "WHERE t1.overtime_date BETWEEN '$strDateFrom' AND '$strDateThru' ";
     if ($strIDEmployee != "")
     {
     ...
     if ($strKriteria != "")
     {
        $strSQL .= "AND id_employee IN (SELECT id FROM hrd_employee where 1=1 $strKriteria)";
     }
     ...
     $strSQL .= "normal_finish, normal_start, holiday FROM hrd_attendance ";
   ...
   //  $strSQL .= "WHERE active = 1 ";
     ...
     if ($strKriteria != "")
     {
        $strSQL .= "AND id IN (SELECT id FROM hrd_employee where 1=1 $strKriteria)";
     }	 
     $resS = $db->execute($strSQL);
     while ($rowS = $db->fetchrow($resS))
     {
        ...
        if($strIDEmployee == "") //jika id employee tidak didefinisikan, loop
        {
           foreach($arrAttEmp as $strID => $arrDetail)
           {
              if (!isset($arrOvertime[$strCurDate][$strID]))
                ...
                $strSQL .= generateOTAttendanceSQL($strID, $strCurDate, $arrDetail, $arrOvertime[$strCurDate][$strID]);
           }
        }
        else //jika didefinisikan tidak usah loop
        {
          if (!isset($arrOvertime[$strCurDate][$strIDEmployee]))
             $strSQL .= generateNonOTAttendanceSQL($strIDEmployee, $strCurDate, $arrAttEmp[$strIDEmployee], $arrAutoOT[$strIDEmployee]);
          else
            $strSQL .= generateOTAttendanceSQL($strIDEmployee, $strCurDate, $arrAttEmp[$strIDEmployee], $arrOvertime[$strCurDate][$strIDEmployee]);
        }
     }
     $strSQL = str_replace("''", "null", $strSQL); 
    ...
    */
    ...
    else
    {
      if (timeCompare($arrOTRow['start_plan'], $arrOTRow['finish_plan']) < 0)
      {
         ...
         else if (timeCompare($arrAttRow['attendance_start'], $arrOTRow['attendance_finish']) > 0) 
           $strOvertimeStart = $arrOTRow['start_plan'];
		 else
      ...
      {
         if (timeCompare($arrAttRow['attendance_start'], $arrOTRow['start_plan']) <= 0 && timeCompare($arrAttRow['attendance_start'], $arrOTRow['finish_plan']) > 0)
         {
           if (timeCompare($arrAttRow['attendance_finish'], $arrOTRow['start_plan']) <= 0 && timeCompare($arrAttRow['attendance_finish'], $arrOTRow['finish_plan']) > 0) 
		   {
			 if (timeCompare($arrAttRow['attendance_finish'], $arrAttRow['attendance_start']) > 0) 
			   $strOvertimeStart = $strOvertimeFinish = "";
			 else
			   $strOvertimeStart = $arrOTRow['start_plan'];
		   }   
    ...
    */
    ...
    if ($arrOTRow['finish_plan'] != "")
    {
      if (timeCompare($arrOTRow['start_plan'], $arrOTRow['finish_plan']) <= 0)
      {
        ...
        {
          if (timeCompare($arrAttRow['attendance_finish'], $arrAttRow['normal_start']) > 0 || timeCompare($arrAttRow['attendance_finish'], $arrOTRow['start_plan']) < 0)
            $strOvertimeStart = $strOvertimeFinish = "";
        ...
        {
          if (timeCompare($arrAttRow['attendance_finish'], $arrAttRow['normal_start']) < 0 || timeCompare($arrAttRow['attendance_finish'], $arrOTRow['finish_plan']) > 0)
            $strOvertimeFinish = $arrOTRow['finish_plan'];
    ...
    if ($arrOTRow['include_early_ot'] == "t")
    {
      ...
      */
      if (timeCompare($arrOTRow['start_early_plan'], $arrOTRow['finish_early_plan']) < 0)
      {
        if (timeCompare($arrAttRow['normal_start'], $arrAttRow['normal_finish']) > 0)
        {
          if (timeCompare($arrAttRow['attendance_start'], $arrAttRow['normal_start']) > 0 || timeCompare($arrAttRow['attendance_start'], $arrAttRow['normal_finish']) < 0)
        ...
        {
          if (timeCompare($arrAttRow['attendance_start'], $arrAttRow['normal_finish']) > 0 || timeCompare($arrAttRow['attendance_start'], $arrOTRow['start_early_plan']) < 0)
            $strOvertimeStartEarly = $arrOTRow['start_early_plan'];
      ...
      {
        if (timeCompare($arrAttRow['attendance_start'], $arrOTRow['start_early_plan']) > 0 || timeCompare($arrAttRow['attendance_start'], $arrOTRow['finish_early_plan']) < 0)
          $strOvertimeStartEarly = $arrAttRow['attendance_start'];
    ...
    {
      $arrBreak = (isset($arrOTRow['id_application'])) ? getOTBreak($arrOTRow['id_application']) : array('intBreakFlag' => 0, 'intBreakType' => "", 'intBreakLinkID' => "");
      foreach ($arrBreak as $strKey => $strVal)
       $$strKey = $strVal;

      $arrOT = calculateOvertimeByOTSchedule($db, $strDate, $strOvertimeStart, $strOvertimeFinish, $intEarlyDuration, $bolHoliday, $intBreakType, true, $intBreakFlag, $intBreakLinkID);
      $strIsOvertime         = 't';
    }
    else if ($arrOTRow['include_early_ot'] == "t")
    {
      $arrOT = calculateOvertimeByOTSchedule($db, $strDate, "01:00:00", "01:00:00", $intEarlyDuration, $bolHoliday, false);
      $strIsOvertime         = 't';
    }
    else
    {
      $arrOT = array('l1' => 0, 'l2' => 0, 'l3' => 0, 'l4' => 0, 'total' => 0, 'is_overtime' => 'f'); 
  ...
  function generateNonOTAttendanceSQL($strIDEmployee, $strDataDate, $arrAttendance, $bolAutoOT = true)
  {
    ...
    if ($bolAutoOT)
    {
      ...
      {
        if(timeCompare(getNextMinute($arrAttendance['attendance_start'], AUTO_OT_MINIMUM_DURATION), $arrAttendance['normal_start']) <= 0)

        {
          if (getTotalHour($arrAttendance['attendance_start'], $arrAttendance['normal_start']) > AUTO_OT_MAXIMUM_DURATION)
          {
            $strOvertimeStartEarlyAuto  =
            $strOvertimeStartEarly      = getNextMinute($arrAttendance['normal_start'], (AUTO_OT_MAXIMUM_DURATION * -1));
        ...
        if(timeCompare($arrAttendance['attendance_finish'], getNextMinute($arrAttendance['normal_finish'], AUTO_OT_MINIMUM_DURATION)) >= 0)
        {
          $strOvertimeStartAuto  = $strOvertimeStart      = $arrAttendance['normal_finish'];
          if (getTotalHour($arrAttendance['normal_finish'], $arrAttendance['attendance_finish']) > AUTO_OT_MAXIMUM_DURATION)
          {
            $strOvertimeFinishAuto = $strOvertimeFinish     = getNextMinute($arrAttendance['normal_finish'], AUTO_OT_MAXIMUM_DURATION);
            $intTotalOTMin += AUTO_OT_MAXIMUM_DURATION;
          }
          else
          {
            $strOvertimeFinishAuto = $strOvertimeFinish     = $arrAttendance['attendance_finish'];
            $intTotalOTMin += getIntervalHour($arrAttendance['normal_finish'], $arrAttendance['attendance_finish']);
          }
        }
      }
    }
    ...
    else
    {
      ...
      return  
        " UPDATE hrd_attendance SET 
          ...
          l1 = 0, l2 = 0, l3 = 0, l4 = 0, overtime = 0, early_overtime = 0, is_overtime = 'f' 
  ...
  function getSpvOvertime ($db, $intInitialOT, $strDateFrom, $strDateThru, $strIDEmployee, /*$strOutdated = "FALSE", */$strKriteria = "", $fltBasicPerHour)
  {
     ...
     $arrOTGroup1 = array(); // array untuk OT  30 jam pertama
     $arrOTGroup2 = array(); // array untuk OT  selebihnya
     while (dateCompare($strDateTemp, $strDateThru) <=0)
     {
        ...
        {
           $arrTodayOT = array($strIDEmployee => array ('total1' => 0, 'total2' => 0, 'total3' => 0, 'total4' => 0));
        }
