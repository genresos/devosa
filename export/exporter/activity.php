...
*/
  ...
  function getOutOfficeInfo($db, $strDateFrom, $strDateThru, $strIDEmployee = "", $bolAbsOnly = false) {
    $arrResult = array();
    $arrPartialCode = array("LATE START", "MID LEAVE", "EARLY FINISH");
    ...
    $strSQL  = "SELECT t2.*, t1.institution, t1.status as status1, t1.date_from, t1.date_thru ";
    $strSQL .= "FROM hrd_training_request AS t1, hrd_training_request_participant AS t2 ";
    $strSQL .= "WHERE t1.id = t2.id_request ";
    $strSQL .= "AND ((t1.date_from, t1.date_thru) ";
    $strSQL .= "    OVERLAPS (DATE '$strDateFrom', DATE '$strDateThru') ";
    $strSQL .= "    OR (t1.date_thru = DATE '$strDateFrom') ";
    $strSQL .= "    OR (t1.date_thru = DATE '$strDateThru')) ";
    ...
    $strSQL .= "WHERE (partial_absence_date BETWEEN '$strDateFrom' AND '$strDateThru' OR partial_absence_date = '$strDateFrom') ";
    $strSQL .= "AND status = " .REQUEST_STATUS_APPROVED." ";
    if ($strIDEmployee != "") $strSQL .= "AND id_employee = '$strIDEmployee' ";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb))
    {
        $arrResult[$rowDb['partial_absence_date']][$rowDb['id_employee']]['type'] = 3; // cuti
        $arrResult[$rowDb['partial_absence_date']][$rowDb['id_employee']]['code'] = $arrPartialCode[$rowDb['partial_absence_type']]; 
        $arrResult[$rowDb['partial_absence_date']][$rowDb['id_employee']]['status'] = $rowDb['status'];
    }
  ...
  function getWorkSchedule($db, $strDate = "", $strIDEmployee = "", $strCriteria = "")
  {
    ...
    $arrEmployee = getEmployeeInfoByID($db, $strIDEmployee, "t0.id, " . $strScheduleLevel);
    $intDay = -1;

    //ambil workschedule berdasarkan jadwal per susunan organisasi
    $strSQL  = "SELECT * FROM hrd_work_schedule WHERE 1=1 $strCriteria ";
    if ($strIDEmployee != "") 
      $strSQL .= "AND table_name = 'Employee' and link_code = '".$arrEmployee['employee_id']."' ";
    if ($strDate != "") 
    {
  ...
  function getAutoShiftOT($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "WHERE t3.status = '" .OVERTIME_STATUS_APPROVED."' ";
  ...
  function getEmployeeTraining($db, $strFrom, $strThru, $strSection = "", $strEmployee = "") {
    ...
    $strSQL .= "AND ((t1.date_from, t1.date_thru) ";
    $strSQL .= "    OVERLAPS (DATE '$strFrom', DATE '$strThru') ";
    $strSQL .= "    OR (t1.date_thru = DATE '$strFrom') ";
    $strSQL .= "    OR (t1.date_thru = DATE '$strThru')) ";
    $resDb = $db->execute($strSQL);
    while ($rowDb = $db->fetchrow($resDb)) {
  ...
  function syncShiftAttendance($db, $strDateFrom, $strDateThru, $strKriteria)
  {
     ...
     // cari info Shift 
     $arrOvertime = array();
     $strSQL  = "SELECT id_employee, shift_date, t1.start_time, t1.finish_time, shift_code, shift_off ";
     $strSQL .= "FROM hrd_shift_schedule_employee AS t1 ";
     $strSQL .= "LEFT JOIN hrd_shift_type AS t2 ON t1.shift_code = t2.code ";
     $strSQL .= "WHERE shift_date BETWEEN '$strDateFrom' AND '$strDateThru' ";
     ...
     if (count($arrAttendance) > 0)
     {
        foreach($arrAttendance as $strDate => $arrAttDate)
        {
           ...
           foreach($arrAttDate as $strID => $arrAttRow)
           {
			  ...
			  if ($strDate == '2011-02-26') echo $strNormalStart;
              }
