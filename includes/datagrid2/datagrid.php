...
- \$CLASSDATAGRIDPATH");
  ...
  function addColumnNumbering($column)
  {
    if (is_a($column, 'datagrid_column'))
    {
      ...
      if ($column->grouped) $this->resetNumbering = true;
      else $this->resetNumbering = false;
  ...
  function addButtonExportExcel($value = "Export to Excel", $strFileName = "exportData.xls", 
                              $strTitle1 = "", $strTitle2 = "", $strTitle3 = "", $strTitle4 = ""
							   , $strTitle5 = "", $strTitle6 = "", $strTitle7 = "", $strTitle8 = ""
							   , $strTitle9 = "", $strTitle10 = "", $strTitle11 = "",$strTitle12 = "",$strTitle13 = "",$strTitle14 = "",$strTitle15 = "")
  {
    $this->buttons[] = array("special" => false, "id"=>"btnExportXLS" , "name"=> "btnExportXLS", 
                             ...
                             "clientAction"=> "", 
                             "serverAction"=> "");
    $this->strFileNameXLS = str_replace(" ","_",$strFileName);
  ...
  function addSpecialButton($id, $name, $type, $value, $clientAction="", $serverAction="", $addedClass = "")
  {
    $this->buttons[] = array("special" => true, "id"=>$id, "name"=>$name, "type"=>$type, "value"=>$value, "class"=> "buttonSpecial ".$addedClass." ", "clientAction"=>$clientAction, "serverAction"=>$serverAction);
  }
  ...
  function _printOpeningTableDataGrid()
  {
    ...
    return "
      <tr>
        <td valign=\"top\">
          <table id=\"" . $this->gridName . "\" width=\"100%\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" class=\"" . $this->gridClass . "\">";
  }
...
<!-- END OF DATAGRID: ".$this->gridName.", generated by Datagrid Class, by Dedy Sukandar -->";
      ...
      if ($this->hasGroupBy)
      {
        ...
        if (!$isFirst)
        {
          if ($lastGroupBy != $currGroupBy)
          {
            ...
            if ($this->resetNumbering) $intNomor = $startNumber;
          }
        }
        else $isFirst = false;
      }
  ...
  function _formatFieldName($fieldName)
  {
    ...
    foreach ($arrOrder as $strOrder)
    {
    	$arrSingleOrder = explode(" ", trim($strOrder));
      if (strpos($arrSingleOrder[0], ".") === false){
        $strResult = $escapeChar.str_replace($escapeChar, "", $arrSingleOrder[0]).$escapeChar;
      }else{
        //jika ada $escapeChar
        $arrSingleOrderFields = explode(".", $arrSingleOrder[0]);
        if (count($arrSingleOrderFields) == 2)
  ...
  function getData($db, &$strSQL)
  {
    ...
    if ($db->connect())
    {
      ...
      if ($strSort != "") $strSQL .= " ORDER BY ".$strSort;
      //handle page limit
        ...
        if (is_numeric($strValue) && $value->item_formatter != "")
        {
          switch ($value->item_formatter)
          {
            ...
            case "_printConditionalCheckbox" :
              break;
            default:
              $strValue = $this->_formatter($value->item_formatter, $this->summaryData[$groupBy], null, $value->groupField);
              break;
          }
        }        
  ...
  function _renderGroupByRowToExcel($groupBy)
  {
    ...
    foreach($this->columnSet as $idx => $value)
    {
      ...
      else
      {
        if ($value->isTotalInformation)
        {
        	if (isset($this->summaryData[$groupBy]) && isset($this->summaryData[$groupBy][$value->groupField]) && (!empty($this->summaryData[$groupBy][$value->groupField]) && trim($this->summaryData[$groupBy][$value->groupField]) != "Sub Total")){
          	$strValue = $this->summaryData[$groupBy][$value->groupField];
          }else{
          	$strValue = "Sub Total ".str_replace("_","-",$groupBy);//$this->summaryData[$groupBy][$value->groupField];
          }
          if ($intSpanned > 0)
          {
            //merge cell
...
function _printDatagridCaptionExcelHTML()
  ...
  function _createExcelFormat($color = null, $bgcolor = null, $font_size = null, $is_bold = false, $border = null, $h_align = null, $v_align = null, $isWrap = false)
  {
    ...
    if ($isWrap) $format->set_text_wrap(1);
  ...
  { 
    if ($col->showInExcel) 
    {
      ...
      if ($col->intRowSpan > 1 || $col->intColSpan > 1)
      {
        ...
        $this->sheet->merge_cells($intRowPos, $intColPosition, ($intRowPos + $col->intRowSpan - 1) , ($intColPosition + $col->intColSpan -1) );
      }
    }
    //merged cell if any...
  }
...
{ 
  ...
  function _printMessageRow()
  {
    ...
    else
    {
      ...
      if (substr_count(strtolower($strMessage), "fail") > 0) $strClass = "class=\"dataGridErrorMessage\"";
    }
    return "
    <tr>
  ...
  function _writeContextMenu()
  {
    ...
    foreach ($this->columnSet as $val)
      if ($val->isDatabaseField && $val->fieldName != "" && $val->intColSpan<=0) $jml++;
...
<div class='contextMenu' id=\"divContext".$this->gridName."\">
  ...
  <div class=l>
    <ul>";
      ...
      if ($val->intColSpan<=0) $colCounter++;
    }
    $strResult .= "
    </ul>
  </div>
