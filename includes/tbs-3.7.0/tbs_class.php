...
// Render flags
if(!defined('TBS_NOTHING')) define('TBS_NOTHING', 0);
if(!defined('TBS_OUTPUT')) define('TBS_OUTPUT', 1);
if(!defined('TBS_EXIT')) define('TBS_EXIT', 2);

// Plug-ins actions
if(!defined('TBS_INSTALL')) define('TBS_INSTALL', -1);
if(!defined('TBS_ISINSTALLED')) define('TBS_ISINSTALLED', -3);

// *********************************************

class clsTbsLocator370 {
	public $PosBeg = false;
	public $PosEnd = false;
...
function __construct($Chrs='',$VarPrefix='',$FctPrefix='') {
	if ($Chrs!=='') {
		...
		if ($Ok) {
			...
			$this->meth_Misc_Alert('with clsTinyButStrong370() function','value \''.$Chrs.'\' is a bad tag delimitor definition.');
		}
	}
...
// Find a TBS Locator
	...
	do {
		...
		if ($Pos===false) {
			...
			$Loc->PosBeg = $Pos;
			if ($ReadPrm) {
				clsTinyButStrong370::f_Loc_PrmRead($Txt,$PosX,false,'\'',$this->_ChrOpen,$this->_ChrClose,$Loc,$PosEnd);
				if ($PosEnd===false) {
	...
	if ($ReadPrm and isset($Loc->PrmLst['comm'])) {
		...
		if (($comm===true) or ($comm==='')) {
			$Loc->Enlarged = clsTinyButStrong370::f_Loc_EnlargeToStr($Txt,$Loc,'<!--' ,'-->');
		} else {
			$Loc->Enlarged = clsTinyButStrong370::f_Loc_EnlargeToTag($Txt,$Loc,$comm,false);
		}
	}
...
// This position can be useful because we don't know in advance how $Value will be replaced.
	...
	if ($Loc->Ope) {
		foreach ($Loc->OpeAct as $i=>$ope) {
			...
			case 14: $CurrVal = clsTinyButStrong370::f_Loc_AttBoolean($CurrVal, $Loc->PrmLst['atttrue'], $Loc->AttName); break;
			}
		}
	}
	...
	if (isset($Loc->PrmLst['att'])) {
		$this->f_Xml_AttFind($Txt,$Loc,true,$this->AttDelim);
		if (isset($Loc->PrmLst['atttrue'])) {
			$CurrVal = clsTinyButStrong370::f_Loc_AttBoolean($CurrVal, $Loc->PrmLst['atttrue'], $Loc->AttName);
			$Loc->PrmLst['magnet'] = '#';
		}
	}
...
// Check if an expression like "exrp1=expr2" is true or false.
	...
	if ($Esc) {
		$Nude1 = clsTinyButStrong370::f_Misc_DelDelimiter($Val1,'\'');
		$Nude2 = clsTinyButStrong370::f_Misc_DelDelimiter($Val2,'\'');
	} else {
		$Nude1 = $Nude2 = false;
	}
			...
			if ($BegCnt===0) {
				...
				if (($Chr===' ') or ($Chr==="\r") or ($Chr==="\n")) {
					if ($Status===1) {
						...
						clsTinyButStrong370::f_Loc_PrmCompute($Txt,$Loc,$SubName,$Status,$XmlTag,$DelimChr,$DelimCnt,$PosName,$PosNend,$PosVal,$Pos,$WithPos);
						$Status = 0;
					}
...
static function f_Loc_PrmCompute(&$Txt,&$Loc,&$SubName,$Status,$XmlTag,$DelimChr,$DelimCnt,$PosName,$PosNend,$PosVal,$Pos,$WithPos) {
	...
	if ($Status===0) {
		...
		if ($SubName) {
			...
			if ($x==='if') {
				clsTinyButStrong370::f_Loc_PrmIfThen($Loc,true,$v);
			} elseif ($x==='then') {
				clsTinyButStrong370::f_Loc_PrmIfThen($Loc,false,$v);
			} else {
				$Loc->PrmLst[$x] = $v;
...
//Modify $Loc, return false if tags not found, returns the inner source of tag if $RetInnerSrc=true
  ...
  for ($i=0;$i<=$TagMax;$i++) {
	...
	$TagO = clsTinyButStrong370::f_Xml_FindTag($Txt,$TagLst[$Ref],true,$Loc->PosBeg-1,false,$LevelStop,false);
	...
	if ($TagIsSgl[$Ref]) {
		...
		if ($LevelStop>0) {
			$TagC = clsTinyButStrong370::f_Xml_FindTag($Txt,$TagLst[$Ref],false,$Loc->PosEnd+1,true,-$LevelStop,false);
	...
	for ($i=$Ref+1;$i<=$TagMax;$i++) {
		...
		if (($x!=='') and ($TagC!==false)) {
			...
			$TagC = clsTinyButStrong370::f_Xml_FindTag($Txt,$x,$TagIsSgl[$i],$PosEnd+1,true,$level,false);
	...
	for ($i=$Ref-1;$i>=0;$i--) {
		...
		if (($x!=='') and ($TagO!==false)) {
			$level = ($TagIsSgl[$i]) ? false : 0;
			$TagO = clsTinyButStrong370::f_Xml_FindTag($Txt,$x,true,$PosBeg-1,false,$level,false);
...
// att=div#class ; att=((div))#class ; att=+((div))#class
	...
	foreach ($TagLst as $i=>$Tag) {
		...
		unset($LocO);
		$LocO = clsTinyButStrong370::f_Xml_FindTag($Txt,$Tag,true,$Pos,$Forward,$LevelStop,$WithPrm,$WithPrm);
	...
	if ($Move) return clsTinyButStrong370::f_Xml_AttMove($Txt,$Loc,$AttDelim);
...
// Returns parts of the XML/HTML content, default is BODY.
	...
	do {
		...
		foreach ($TagLst as $i=>$Tag) {
			if ($Tag['b']<$Pos) {
				$Loc = clsTinyButStrong370::f_Xml_FindTag($Txt,$Tag['t'],true,$Pos,true,false,false);
				if ($Loc===false) {
		...
		if ($TagMin!==false) {
			...
			if ($Tag['s']) {
				...
				$Loc = clsTinyButStrong370::f_Xml_FindTag($Txt,$Tag['t'],false,$Pos,true,false,false);
				if ($Loc===false) {
...
*/
	...
	if ($Tag==='_') { // New line
		$p = clsTinyButStrong370::f_Xml_FindNewLine($Txt,$PosBeg,$Forward,($LevelStop!==0));
		$Loc = new clsTbsLocator370;
	...
	if ($TagOk) {
		$Loc = new clsTbsLocator370;
		if ($WithPrm) {
			clsTinyButStrong370::f_Loc_PrmRead($Txt,$PosX,true,'\'"','<','>',$Loc,$PosEnd,$WithPos);
		} elseif ($PosEnd===false) {
			$PosEnd = strpos($Txt,'>',$PosX);
			if ($PosEnd===false) {
				$TagOk = false;
