...
class cDataGrid
{
  ...
  var $buttons;
  var $buttonExcel = array();
  var $buttonExcelHTML = array();
  var $buttonPrint = array();
...
- \$CLASSDATAGRIDPATH");
  ...
  function addButtonExportExcel($id = "btnExportXLS", $value = "Export to Excel", $strFileName = "exportData.xls", 
                              $strTitle1 = "", $strTitle2 = "", $strTitle3 = "")
  {
    $this->buttonExcel[] = $id;
		$this->buttons[] = array("special" => false, "id" => $id, "name"=> $id,
  ...
  function addButtonExportExcelHTML($id = "btnExportXLSHTML", $value = "Export to Excel", $strFileName = "exportData.xls", 
                              $strTitle1 = "", $strTitle2 = "", $strTitle3 = "")
  {
...
<!-- END OF DATAGRID: ".$this->gridName.", generated by Datagrid Class, by Dedy Sukandar -->";
      ...
      if (DB_TYPE == "mssql")
      {
        ...
        foreach($arrField as $field)
        {
          ...
          if (strpos($field, ".") === false)
            $arrResult[] = "[".str_replace(array("[", "]"), "", $field)."]";
          else
          {
            $arrFields = explode(".", $field);
            if (count($arrFields) == 2)
              $arrResult[] = $arrFields[0]."."."[".str_replace(array("[", "]"), "", $arrFields[1])."]";
            else
              $arrResult[] = $field;
          }
        }
  ...
  function _createInputHidden()
  {
    ...
    if ($this->DATAGRID_RENDER_OUTPUT == DATAGRID_RENDER_EXCEL_HTML || $this->DATAGRID_RENDER_OUTPUT == DATAGRID_RENDER_EXCEL_BIFF || $this->DATAGRID_RENDER_OUTPUT == DATAGRID_RENDER_PRINT_HTML) return "";
  ...
  function _setHeaderDeepLevel()
  {
    ...
    foreach($this->columnSet as $key => $col)
    {
      ...
      if ($col->intColSpan >= 1)
      {
        ...
        if ($col->intDeepLevel == 1) $colCounter += ($col->intColSpan - 1);
      }
    }
    return $maxDeepLevel;
  }
