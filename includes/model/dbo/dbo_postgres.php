...
class CDboModel 
{
  ...
  {
		$fields = false;
		$cols = $this->getRecordSet("SELECT DISTINCT column_name AS name, data_type AS type, is_nullable AS null, column_default AS default, ordinal_position AS position, character_maximum_length AS char_length, character_octet_length AS oct_length FROM information_schema.columns WHERE table_name = '" . $tableName . "' ORDER BY position");
		foreach ($cols as $c) 
    {
 ...
 */
  ...
  function fetchAll($strTableName, $strCondition, $strFields, $strOrder, $intLimit, $page, $autoIndexField/*set with field name */, &$strSQL)
  {
    $strLimit = "";
    $strOffset = "";
    ...
    $strSQL = "SELECT ".$strFields." FROM ".$this->formatTableName($strTableName)." ".$strCondition." ".$strOrder." ".$strLimit." ".$strOffset;
