...
class CDboModel 
{
  ...
  var $isShowError = true;
	var $columns = array(
		...
		'boolean'	=> array('name' => 'bit', 'formatter' => 'intval')
	);
 ...
 */
  ...
  function fetchAll($strTableName, $strCondition, $strFields, $strOrder, $intLimit, $page, $autoIndexField/*set with field name */, &$strSQL)
  {
    ...
    $strSQL = "SELECT ".$strLimit." ".$strFields." FROM ".$this->formatTableName($strTableName)." ".$strCondition." ".$strOrder;
		...
		if ($page > 1 && $intLimit != null) 
    {
      //jump to offset 
			$offset = ($page - 1) * $limit;
      if ($this->numrows($res) > 0) $db->dataSeek($offset);
		}
    $arrResult = array();
    $counter = 0;
