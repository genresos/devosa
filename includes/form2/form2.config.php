 ...
 */
  ...
  if (!DEFINED("DATATYPE_TIME")) DEFINE("DATATYPE_TIME", "time");    
  if (!DEFINED("DATATYPE_DATE")) DEFINE("DATATYPE_DATE", "date");
  if (!DEFINED("DATATYPE_NUMERIC")) DEFINE("DATATYPE_NUMERIC", "numeric");
  if (!DEFINED("DATATYPE_STRING")) DEFINE("DATATYPE_STRING", "string");
  if (!DEFINED("DATATYPE_EMAIL")) DEFINE("DATATYPE_EMAIL", "email");
  if (!DEFINED("DATATYPE_INTEGER")) DEFINE("DATATYPE_INTEGER", "integer");
  if (!DEFINED("DATATYPE_UNDEFINED")) DEFINE("DATATYPE_UNDEFINED", "");

  //path of datagrid class please end with / (slash)
  $GLOBALS['CLASSFORMPATH'] = "../includes/form2/";
  ...
  if ($GLOBALS['globalLanguage'] == "id")
  {
    ...
    $GLOBALS['validatorErrorMessage'] = array(
      DATATYPE_UNDEFINED => "",
      DATATYPE_DATE => "Masukkan input tanggal yang valid dalam format (YYYY-MM-DD) untuk ", 
      DATATYPE_TIME => "Masukkan input waktu yang valid dalam format (hh:mm) untuk ", 
  ...
  else
  {
    ...
    $GLOBALS['validatorErrorMessage'] = array(
      ...
      DATATYPE_TIME => "Please enter a valid date (hh:mm) input for ", 
  ...
  $GLOBALS['validatorDataType'] = array(
    DATATYPE_UNDEFINED => "",
    DATATYPE_DATE => "validateDate", 
    DATATYPE_TIME => "validateTime", 
  ...
  $GLOBALS['AUTOCOMPLETE_LOADED'] = false;

  //$FORM_DATE_FORMAT = "%Y-%m-%d";
  $GLOBALS['FORM_DATE_FORMAT'] = "%Y-%m-%d";
?>