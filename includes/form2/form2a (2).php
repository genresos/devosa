...
class clsForm
{
  ...
  function addFormObject($type, $title, $name, $value, $arrAttribute, $dataType, $bolRequired, $bolEnabled, $bolVisible, $htmlBefore, $htmlAfter, $renderLabel = true, $arrLabelAttribute, $serverAction="", $targetFolder = "" , $bolVertical = null, $calendarUpdate = null)
  {
    $strAllCanons = "";
    $strNickTokens = "";
  ...
  function addInputAutoComplete($title, $name, $value, $arrAttribute = array(), $dataType="string", $bolRequired = true, $bolEnabled = true, $bolVisible = true, $htmlBefore="", $htmlAfter="", $renderLabel = true, $arrLabelAttribute = null)
  {
    if ($bolRequired) $this->hasRequiredControl = true;
    if (!is_array($value))
    {
    ...
    $this->addFormObject('autocomplete', $title, $name, $value, $arrAttribute, $dataType, $bolRequired, $bolEnabled, $bolVisible, $htmlBefore, $htmlAfter, $renderLabel, $arrLabelAttribute);
  }
...
//      print_r($this->objects);die();
  ...
  <link href=\"".$this->CSSFileName."\" rel=\"stylesheet\" type=\"text/css\">";
    ...
    if ($this->hasAutoComplete)
      $strResult .= "
  <link href=\"".ereg_replace('/+', '/', $GLOBALS['CLASSFORMPATH']."/css/ac.css")."\" rel=\"stylesheet\" type=\"text/css\">";
...
<!-- END OF FORMENTRY, generated by FormEntry Class, by Dedy Sukandar -->";
  ...
  function _drawHidden($obj)
  {
    $obj['id'] = $obj['name'];
      if (strpos($obj['name'],'[]') !== false) {
          $obj['id'] = str_ireplace("[]","",$obj['id']);
    }
    $strResult = "<input type=\"hidden\" name=\"".$obj['name']."\" id=\"".$obj['id']."\" value=\"".$obj['value']."\"";
  ...
  function _printHidden($name, $value)
  {
    ...
    $strResult = "<input type=\"hidden\" name=\"".$name."\" id=\"".$id."\" value=\"".$value."\" />";
    return $strResult;
  }
  ...
  function _printSpan($name, $text, $arrAttribute, $htmlBefore, $htmlAfter)
  {
    ...
    if ($name == "")
      ...
      return $htmlBefore."<span id=\"".$id."\" name=\"".$name."\" $att>".nl2br($text)."</span>".$htmlAfter;
  }
    ...
    switch ($obj['type'])
    {
      case 'select' :
        if ($obj['visible'])
        {
          ...
          else
          {
            ...
            $strResult = $obj['htmlBefore']."<select id=\"".$obj['id']."\" name=\"".$obj['name']."\" ".$obj['attribute'];
            if (!$obj['enabled']) $strResult .= " disabled";
            $strResult .= ">";
            $isSelected = false;
      ...
      case 'checkbox' :
        if ($obj['visible'])
        {
          ...
          {
            if ($obj['value'])
              $strResult = $obj['htmlBefore']."<input class=c type=\"checkbox\" id=\"".$obj['name']."\" name=\"".$obj['name']."\" checked disabled>";//.$obj['htmlAfter'];
            else
              $strResult = $obj['htmlBefore']."<input class=c type=\"checkbox\" id=\"".$obj['name']."\" name=\"".$obj['name']."\" disabled>";//.$obj['htmlAfter'];
          }
          else
          {
      ...
      for($j=$i*$jmlBaris; ($j<($i+1)*$jmlBaris && $j<count($this->formObject[$tabPageNumber][$groupNumber])); $j++)
      {
        ...
        else
        {
          if (count($this->formObject[$tabPageNumber][$groupNumber][$j]['labelAttribute']) > 0)
            $strResult .= "
                    <td ".$this->_serializeAttribute($this->formObject[$tabPageNumber][$groupNumber][$j]['labelAttribute']).">".$this->_printObjectCaption($this->formObject[$tabPageNumber][$groupNumber][$j])."&nbsp;</td>";
          else
            $strResult .= "
                    <td nowrap>".$this->_printObjectCaption($this->formObject[$tabPageNumber][$groupNumber][$j])."</td>";
