<?php
  ...
  $f = new clsForm("form1", /*2 column view*/2, "900","400");
  ...
  $arrAutoComplete[] = array("value" => 1, "text" => "Dedy");
  $arrAutoComplete[] = array("value" => 2, "text" => "Agus");
  $arrAutoComplete[] = array("value" => 3, "text" => "Linda");
  $arrAutoComplete[] = array("value" => 4, "text" => "Test");
  $arrAutoComplete[] = array("value" => 5, "text" => "AAAA");
  
  $f->addInputAutoComplete("ID", "dataCodeAC", $arrAutoComplete, array("title" => "Vendor Code", "size" => 20), "string", true, true, true);
  $f->addLabelAutoComplete("Name", "dataCodeAC", /*AC name*/"");
  $f->addInput("Vendor Number", "dataCode", $vendorData['code'], array("title" => "Vendor code", "size" => 50), "string", true, true, true);  
  $f->addCheckBox("Company Name", "dataName2", $vendorData['companyName'], array());
  $f->addRadio("Company Name", "dataName3", $vendorData['companyName'], array());
  $f->addInput("Company Name", "dataName", $vendorData['companyName'], array("title" => "Company Name", "size" => 50), "string", false, true, true);
  
  $f->addTextArea("Office Address", "dataAddress", $vendorData['officeAddress'], array("title" => "Address", "cols" => 50, "rows" => 3), "string", false, true, true);
  $f->addInput("Office Postcode", "dataZip", $vendorData['zipCode'], array("title" => "Office's Postcode", "size" => 5), "string", false, true, true);
  $f->addInput("Office Phone", "dataPhoneNumber", $vendorData['phoneNumber'], array("title" => "Office's Phone", "size" => 5), "string", false, true, true);

  $f->addTextArea("Factory Address", "dataFactoryAddress", $vendorData['factoryAddress'], array("title" => "Factory Address", "cols" => 50, "rows" => 3), "string", false, true, true);
  $f->addFieldSet("Additional Information", 1);
  $f->addInput("Established Date", "dataEstablishDate", $vendorData['establishDate'], array("title" => "Established Date"), "date", false, true, true);
  $f->addInput("NPWP", "dataNPWP", $vendorData['npwp'], array("title" => "Vendor NPWP"), "string", false, true, true);
  $f->addSelect("Group", "dataGroup", array(array("value"=>1,"text"=>"Dedy")), array("title" => "Group of User"));
  $f->addFile("Data", "dataFile", "", array("title" => "Vendor NPWP"), "string", false, true, true);

  $f->addTabPage("Page 2");
  $f->addInput("Coba aja", "dataCobaAja", "", array("size" => 5), "string", false, true, true);
  $f->addInput("Coba lagi", "dataCobaLagi", "", array("size" => 5), "string", false, true, true);

  //this save button will hide after save <toggle>
  $f->addSubmit("btnSave", "Save", array("onClick" => "\"return confirm('Do you want to save this entry?');\"", "title" => "Save this change"), true, true, "", "", "saveData()");
