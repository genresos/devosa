<?php
  ...
  $f = new clsForm("form1", /*2 column view*/2, "100%","100%");
  ...
  $f->addTextBox("Employee ID", "dataEmployeeID", "", "Employee ID", false, true, true, true, false, "string", null,"","","",50,20);
  $f->addTextBox("Name", "dataName", "", "Employee Name", false, true, true, true, false, "string", null,"","","",50,50);
  $f->addTextArea("Address", "dataAddress", "", "Primary Address", false, true, true, true, false, "string", null,"","","",46,2);
  ...
  $f->addButton("btnAdd", "Add New","Add New Data", true,true, null, "", "", "", "onClick='javascript:myClient.editData(0)'");
  ...
  $myDataGrid->addColumn(new DataGrid_Column("Employee ID", "employeeID", array('width' => '100'), array('align' => 'center', 'nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column("Name", "employeeName", array('width' => '250'), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column("Address", "primaryAddress", null, array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column("Gender", "gender", array('width' => '50'), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column("Department", "departmentCode", array('width' => '150'), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column("", "editLink", array('width' => '40'), array('nowrap' => ''), false));

  $myDataGrid->addSpecialButton("btnDelete","btnDelete","submit","Delete",
    "onClick=\"javascript:return confirm('Delete this selected data?');\"", "deleteData()");

  $myDataGrid->setPermission(/*view*/true, /*edit*/true, /*delete*/true); 
  ...
  function getMasterDepartment($db, $default = "")
  {
    ...
    {    
      ...
      while ($rowDb = $db->fetchrow($resDb))
        $arrData[] = array("value" => $rowDb['department_code'],
  ...
  function getTotalData($db)
  {
    ...
    if ($db->connect())
    {
      ...
      if ($rowDb = $db->fetchrow($resDb)) $totalData = $rowDb[0]; else $totalData = 0;
    }
      ...
      if ($myDataGrid->searchKriteria!="")
        if ($myDataGrid->pageSearchBy == "")
        {
          ...
          foreach($myDataGrid->columnSet as $col)
            ...
            {
              if ($strResult == "")
                $strResult = " lower(\"" . $col->fieldName . "\") LIKE '%" . strtolower($myDataGrid->searchKriteria) . "%'";
              else
                $strResult .= " OR lower(\"" . $col->fieldName . "\") LIKE '%" . strtolower($myDataGrid->searchKriteria) . "%'";
            }
          $strResult = "( ".$strResult. " )";
        ...
        else
        {
          ...
          $strResult = " lower(\"" . $myDataGrid->pageSearchBy . "\") LIKE '%" . strtolower($myDataGrid->searchKriteria) . "%'";
        }
  ...
  function deleteData() 
  {
    ...
    if (!$myDataGrid->bolCanDelete) echo "No delete privilege";
    else
    {
      $strSQL = "";
      ...
      return executeDeleteSQL($strSQL, /*entity name*/"Employee", /*returned message*/$myDataGrid->message);
    }
    return false;        
  } //deleteData
  ...
  function saveData()
  {
    ...
    //return true if succesfully execute Query, false otherwise
    $isSaved = executeSaveSQL($strSQL, /*entity name*/ "yourTable", /*returned message*/ $f->message);
    ...
    if ($isSaved)
    {
        ...
        $f->objects['btnAdd']['clientAction'] = "onClick=\"javascript:location.href='".basename($_SERVER['PHP_SELF'])."'\"";
