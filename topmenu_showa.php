<?php
  ...
  $menu->addMenuItem(new MenuItem('home', 1, 0, "Home", $GLOBALS['globalRelativeFolder']."main.php", "home.png", "Back to Home"));
  $menu->addMenuItem(new MenuItem('pwd', 1, 0, "Change Password", $GLOBALS['globalRelativeFolder']."changepwd.php", "password.png", "Ubah Password"));
  ...
  else
  {
    ...
    $menu->addMenuItem(new MenuItem('lang2', 2, 'lang', "Indonesian&nbsp;&nbsp;&nbsp;<img src='".$GLOBALS['globalRelativeFolder']."images/ok.png' border=0 />", $_SERVER['PHP_SELF']."?changeLanguageTo=id", "indonesia.png", "Back to Home"));
    $menu->addMenuItem(new MenuItem('lang3', 2, 'lang', "Japan", $_SERVER['PHP_SELF']."?changeLanguageTo=jp".$strGetQueryString, "japan.png", "Back to Home"));
  }
  ...
  $menu->addMenuItem(new MenuItem('exit', 1, 0, "Exit", "javascript:goMenu('".$GLOBALS['globalRelativeFolder']."logout.php', '', '".getWords("Leave the system?")."')", "exit.png", "Logout dan keluar dari aplikasi"));
  ...
  //pastikan bahwa data ini adalah array
  {
    foreach ($_SESSION['sessionPrivileges'] as $rowDb)
      if ($_SESSION['sessionModuleID'] == $rowDb['id_adm_module'])
        $menu->addMenuItem(new MenuItem($rowDb['id_adm_menu'], $rowDb['menu_level'], $rowDb['parent_id_adm_menu'], $rowDb['menu_name'], $GLOBALS['globalRelativeFolder'].$rowDb['folder'].'/'.$rowDb['php_file'], $rowDb['icon_file'], $rowDb['note']));
  }
  ...
  <table width="100%" border="0" cellspacing="0" cellpadding="0" class='logoTop'>
      ...
      <td valign="top" class="logoTop2">
        <?php
          ...
          foreach ($_SESSION['sessionModuleList'] as $module)
          {
            ...
            {
              $strModuleSelected = $module['name'];
              $strResult .= "\n            <td class=\"moduleSelected\">".$module['name']."</td>";
            }
            else
              if ($GLOBALS['globalLanguage'] == "id")
                $strResult .= "\n            <td class=\"moduleNormal\" onMouseOver=\"this.className='moduleNormalHover'\" onMouseOut=\"this.className='moduleNormal'\" onClick=\"goMenu('".$GLOBALS['globalRelativeFolder']."main.php','".$module['id_adm_module']."')\">".$module['name']."</td>";
              else
                $strResult .= "\n            <td class=\"moduleNormal\" onMouseOver=\"this.className='moduleNormalHover'\" onMouseOut=\"this.className='moduleNormal'\" onClick=\"goMenu('".$GLOBALS['globalRelativeFolder']."main.php','".$module['id_adm_module']."')\">".$module['name']."</td>";
          }
          if ($strResult != "")
    ...
    <tr>
      <td colspan="2" style="height : 21px">
        <noscript><font style="color:white">Sorry, you must use javascript capability browser to run this ERP application.</font></noscript>
