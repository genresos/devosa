<?php
  ...
  function getData($db)
  {
   ...
   //-------------------------------------- BEGIN Data Grid---------------------------------------------------------------------------------//
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("stock opname Date"), "stock_opname_date", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("item amount"), "item_amount", array('width' => '150'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("item old amount"), "item_old_amount", array('width' => '150'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("remark"), "remark", array('width' => '40'),array('nowrap' => '')));
  ...
  if ($db->connect()) 
  {
    ...
    $f->addSelect(getWords("item category"), "dataIdCategory", getDataListItem($arrData['dataIdItem']),array("style" => "width:$strDefaultWidthPx"),"",false);
    $f->addInput(getWords("stock opname date"), "dataStockOpnameDate",  null ,array("style" => "width:$strDateWidth"), "date", false, true, true);
    $f->addInput(getWords("item amount"), "dataItemAmount",  null ,array("style" => "width:$strDateWidth"), "numeric", false, true, true);
    $f->addInput(getWords("item old amount"), "dataItemOldAmount",  null ,array("style" => "width:$strDateWidth"), "numeric", false, true, true);
    $f->addInput(getWords("remark"), "dataRemark",  null ,array("style" => "width:$strDateWidth"), "string", false, true, true);
