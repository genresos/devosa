<?php
  ...
  function getData($db)
  {
   ...
   //-------------------------------------- BEGIN Data Grid---------------------------------------------------------------------------------//
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("purchase request number"), "purchase_no", array('width' => '100'), array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("employee name"), "employee_name", array('width' => '100'), array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("request Date"), "request_date", array('width' => '100'),array('nowrap' => '')));
	$myDataGrid->addColumn(new DataGrid_Column(getWords("Item category"), "category_name", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("remark"), "remark", array('width' => '150'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("attachment file"), "attachment_file", array('width' => '60'), array('align' => 'center', 'nowrap' => ''), false, false, "","printDownloadLink()"));
  ...
  //----------------BEGIN JIKA Punya Hak Edit---------------------------------//
     if ($bolCanEdit)
     $myDataGrid->addSpecialButton("btnOrder","btnOrder","submit","Order Now",
    "onClick=\"javascript:return confirm('Do you want Order?');\"","requestOrder()");
     //---------------- END Jika Punya Hak Akses Tambah-------------------------//
      ...
      generateRoleButtons($bolCanEdit, $bolCanDelete, $bolCanCheck, $bolCanApprove, $bolCanApprove2, true, true, $myDataGrid);
  ...
  function printDownloadLink($params)
  {
	...
	return "
      ...
      <a href=\"/innogene/ga/prdoc/".$record['attachment_file']."\">" .getWords($record['attachment_file'])."</a>";
...
//    print_r($_REQUEST);
  ...
  if ($db->connect()) 
  {
   ...
   $f->addSelect(getWords("Purchase Request No#"), "dataPurchaseNo", getDataListPurchaseNo(""),array("style" => "width:$strDefaultWidthPx"),"",false);
    $f->addInputAutoComplete(getWords("employee ID"), "dataEmployee", getDataEmployee(getInitialValue("Employee", null, $strDataEmployee)), 
	    "style=width:$strDefaultWidthPx ".$strReadonly, "string", false);
	$f->addLabelAutoComplete("", "dataEmployee", "");
    $f->addSelect(getWords("item category"), "dataIdCategory", getDataListCategoryItem(""),array("style" => "width:$strDefaultWidthPx"),"",false);
    $f->addInput(getWords("request date"), "dataRequestDate",  null ,array("style" => "width:$strDateWidth"), "date", false, true, true);
