...
// Variabel get ID item yang akan di request order
  ...
  // --------------------------------------------------------------------------------------------------------------------------------
    ...
    $f->addInput(getWords("Request Order No"), "dataRequestNo", $arrData['dataRequestNo'],array("style" => "width:$strDateWidth"), "string", 
	  true, true, true);
	if($isNew){
    $f->addSelect(getWords("purchase request"), "dataPurchaseNo",getDataListPurchaseNo($arrData['dataPurchaseNo'], true, array("value" => "",
     "text" => "", "selected" => true),"AND active='YES'"), "", true, true, true);
     }else{
    $f->addSelect(getWords("purchase request"), "dataPurchaseNo",getDataListPurchaseNo($arrData['dataPurchaseNo'], true, array("value" => "",
     "text" => "", "selected" => true),""), "", true, true, false);
	}
    $f->addSelect(getWords("item category"), "dataIdCategory",  getDataListCategoryItem($arrData['dataIdCategory']) ,"style='width:250px' ", "string",
	  true, true, true);
    ...
    $f->addTextArea(getWords("remark"), "dataRemark", $arrData['dataRemark'], array("cols" => 42, "rows" => 4, "maxlength" => 255), "string", true, true, true);  
  ...
  function saveData() 
  {
    ...
    if ($db->connect())
    {
     /// Cek apakah ada nomer reuest ada yang sama
      ...
      $data = array("id_purchase_request" => $f->getValue('dataPurchaseNo'),
