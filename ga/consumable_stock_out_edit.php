<?php
  ...
  // --------------------------------------------------------------------------------------------------------------------------------
	...
	$f->addSelect(getWords("item"), "dataIdItem", getDataListItem($arrData['dataIdItem'], true, array("value" => "", "text" => "", "selected" => true)),  "style='width:250px' ", "", true, true, true);
    $f->addSelect(getWords("departement code"), "dataDepartmentCode", getDataListDepartment($arrData['dataDepartmentCode'], true,
	   array("value" => "", "text" => "", "selected" => true)), "style='width:250px' ", "", true, true, true);
    $f->addInput(getWords("transaction date"), "dataTransactionDate", $arrData['dataTransactionDate'],array("style" => "width:$strDateWidth"), "date", true, true, true);
    $f->addInput(getWords("item amount"), "dataItemAmount", $arrData['dataItemAmount'],array("style" => "width:$strDateWidth"), "numeric", true, true, true);
    $f->addTextArea(getWords("remark"), "dataRemark", $arrData['dataRemark'], array("cols" => 40, "rows" => 4, "maxlength" => 255), "string", true, true, true); 
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    if ($db->connect())
    {
      ...
      $dataToUpdate=array("item_stock" => $amountStock,"current_stock" => $lastStock);
      ...
      if ($isNew)
      {
        ...
        $bolSuccess = $dataUpdateItem->update(/*pk*/"id='".$f->getValue('dataIdItem')."'", /*data to update*/ $dataToUpdate);
      } 
      else 
      {
