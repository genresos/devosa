  ...
  if ($db->connect())
  {
   ...
   // --------------------------------------------------------------------------------------------------------------------------------
    ...
    if ($bolCanEdit)
    {
      ...
      $f->addHidden("dataIdEmployeeFrom",$arrData['dataIdEmployeeFrom'], array("size" => 20 ,"readonly"=>true), "string",false, true, true);
	  ...
	  $f->addInput(getWords("employee ID  from "), "dataEmployeeNameFrom",$arrData['dataEmployeeNameFrom'], array("size" => 45, "readonly"=>true),
      ...
      $f->addSelect(getWords("Location Room From"), "dataIdRoomFrom",getDataListRoom($arrData['dataIdRoomFrom'], true, array("value" => "",
	     "text" => "", "selected" => true)),  array ("style" => "width:200"), "", true, false, true);
	  $f->addSelect(getWords("department From"), "dataDepartmentFrom",getDataListDepartment($arrData['dataDepartmentFrom'], true, array("value" => "",
	     "text" => "", "selected" => true,)),  array ("style" => "width:200"), "", true, false, true);
	  ///------ EMPLOYEE TO
	  $f->addLabel("<hr>","","<hr>");
      ...
      $f->addSelect(getWords("Locatoin Room To "), "dataIdRoomTo",getDataListRoom($arrData['dataIdRoomTo'], true, array("value" => "",
	     "text" => "", "selected" => true)),  array ("style" => "width:200"), "", true, true, true);
	 // Jika St ReadOnly kosong-------------------------------------------------------------------------------------------------------------
	   if ($strReadonly==""){
		  $f->addSelect(getWords("departement To"), "dataDepartmentTo",getDataListDepartment($arrData['dataDepartmentTo'], true, array("value" => "",
	      "text" => "", "selected" => true)),  array ("style" => "width:200"), "", true, true, true);
		}else{
	  ...
	  //--------------------------------------------------------------------------------------------------------------------------------------
	  $f->addInput(getWords("moving date"), "dataMovingDate", $arrData['dataMovingDate'], array("style" => "width:$strDateWidth"), "date", false, true, true);
      ...
      $f->addButton("btnAdd", getWords("Back"), array("onClick" => "self.history.back();"));
      $formInput = $f->render();
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    if ($db->connect())
    {
      ...
      $data = array("id_item" => $f->getValue('dataIdItem'),
