    ...
    if ($strFileName != "") 
    {
      ...
      if ($handle != FALSE) {
        ...
        while (!feof($handle)) 
        {
          ...
          //cek tanggal
          if ((!validStandardDate($strCurrDate)) || dateCompare($strCurrDate, $strDateFrom) < 0 || dateCompare($strCurrDate, $strDateThru) > 0) 
            continue;  
    ...
    while (dateCompare($strCurrdate,$strLastdate) <= 0)
    {
      if (isset($arrAttInfo[$strCurrdate]))
      {
            ...
            foreach ($arrAttInfo[$strCurrdate] AS $strIDEmployee => $arrTime)
            {
                ...
                $arrAM = $arrPM = array(); // untuk nampun jam masuk-pulang
                foreach ($arrTime AS $strRemark => $strTime)
                {
                  if ($strRemark[0] == "M" && $strRemark[4] != "I" ) $strAttendanceStart = $strTime;
                  else if ($strRemark[0] == "I") $strAttendanceStart = getNextMinute($objToday->strNormalStart, 30);
                  else if ($strRemark[0] == "P") $strAttendanceFinish = $strTime;
                }
...
*/
                ...
                if ($objToday->strAttendanceStart == "" || dateCompare($strAttendanceStart, $objToday->strAttendanceStart) < 0) $objToday->strAttendanceStart = $strAttendanceStart ;
                if ($objToday->strAttendanceFinish == "" || dateCompare($strAttendanceFinish, $objToday->strAttendanceFinish) < 0) $objToday->strAttendanceFinish = $strAttendanceFinish;
  ...
  function processAttendanceAutomatic($db, $strFilePath, $strFileType, $strDateFrom, $strDateThru, $strDataCompany = '', $bolSilent = true)
  {
    $strResult = "";
    if ($strFilePath != "" && $strFileType != "" && validStandardDate($strDateFrom) && validStandardDate($strDateThru)) 
    {
      $strFileDate = $strDateFrom;
      While (dateCompare($strFileDate, getNextDate($strDateThru, 10)) < 0)
      {//echo $strFilePath.str_replace("-", "", $strFileDate).".".$strFileType;
        ...
        {echo $strFileName;
          ...
          $strResult .= "<br>". processAttendance($db, $strFileName, getNextDate($strFileDate, -1), $strFileDate, $strDataCompany, false);
          if (dateCompare($strFileDate, $strDateThru) >= 0) break;
        }
