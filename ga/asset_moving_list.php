<?php
   ...
   function getData($db)
  {
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Emply. From"), "employee_name_from", array('width' => '70'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("room From"), "room_name_from", array('width' => '70'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Dept. From"), "department_code_from", array('width' => '50'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column("Emply.To", "", array("width" => "60"), array('align' => 'center', 'nowrap' => ''), false, false,
	  "","changeNameEmployeeTo()", "", false /*show in excel*/));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("room to"), "room_name_to", array('width' => '80'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("dept. to"), "department_code_to", array('width' => '70'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("date"), "moving_date", array('width' => '70'),array('nowrap' => '')));
   ...
   {
    extract($params);
    return "<a href=\"asset_moving_edit.php?dataIDFrom=".$record['employee_id_from']."&dataID=".$record['id']."\">" .getWords('edit'). "</a>";
   }
  ...
  */
  ...
  if ($db->connect()) 
  {
	...
	$f->addInputAutoComplete(getWords("employee ID From "), "dataEmployeeFrom", getDataEmployee($arrData['dataEmployee']), "style='width:250px' ". $strReadonly, 
	     "string", false);
    $f->addLabelAutoComplete("", "dataEmployee", "");
    $f->addInputAutoComplete(getWords("employee ID To"), "dataEmployee", getDataEmployee($arrData['dataEmployee']), "style='width:250px' ". $strReadonly, 
	     "string", false);
    $f->addLabelAutoComplete("", "dataEmployee", "");
    $f->addSelect(getWords("Location Room From"), "dataIdRoomFrom",getDataListRoom($arrData['dataIdRoomFrom'], true, array("value" => "",
	     "text" => "", "selected" => true)),  array ("style" => "width:200"), "", false, true, true);
	$f->addSelect(getWords("Location Room To"), "dataIdRoomTo",getDataListRoom($arrData['dataIdRoomTo'], true, array("value" => "",
	     "text" => "", "selected" => true)),  array ("style" => "width:200"), "", false, true, true);
	
     // Jika St ReadOnly kosong-------------------------------------------------------------------------------------------------------------
	   if ($strReadonly==""){
		  $f->addSelect(getWords("department From"), "dataDepartmentFrom",getDataListDepartment($arrData['dataDepartementFrom'], true, array("value" => "",
	      "text" => "", "selected" => true,)),  array ("style" => "width:200"), "", false, true, true);
		}else{
	      $f->addSelect(getWords("department from"), "dataDepartmentFrom", getDataListDepartment(getInitialValue("Department", "", $strDataDepartment), true),
	 ...
	 // Jika St ReadOnly kosong-------------------------------------------------------------------------------------------------------------
	   if ($strReadonly==""){
		  $f->addSelect(getWords("department To"), "dataDepartmentTo",getDataListDepartment($arrData['dataDepartementTo'], true, array("value" => "",
	      "text" => "", "selected" => true,)),  array ("style" => "width:200"), "", false, true, true);
		}else{
	      $f->addSelect(getWords("department To"), "dataDepartmentTo", getDataListDepartment(getInitialValue("Department", "", $strDataDepartment), true),
	      array("style" => "width:$strDefaultWidthPx"), "", false, ($ARRAY_DISABLE_GROUP['department'] == ""));
	    }
	    //--------------------------------------------------------------------------------------------------------------------------------------  
	$f->addInput(getWords("Moving date From"), "dataMovingDateFrom","", array("style" => "width:$strDateWidth"), "date", false, true, true);
	$f->addInput(getWords("moving date Thru"), "dataMovingDateThru","", array("style" => "width:$strDateWidth"), "date", false, true, true);  
