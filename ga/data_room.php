<?php
  ...
  */
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("Room Code"), "room_code", "", array("size" => 10), "string", true, true, true); 
    $f->addInput(getWords("Room Name"), "room_name", "", array("size" => 40), "string", true, true, true); 
	$f->addInput(getWords("Room Type"), "room_type", "", array("size" => 35), "string", true, true, true);
	$f->addInput(getWords("Capacity"), "capacity", "", array("size" => 7), "string", true, true, true,"", "Person");
  ...
  //-------------------------------------- BEGIN Data Grid---------------------------------------------------------------------------------//
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Location Name"), "location_name", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Room Code"), "room_code", array('width' => '40'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Room Name"), "room_name", array('width' => '160'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Room Type"), "room_type", array('width' => '60'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Capacity"), "capacity", array('width' => '45'),array('nowrap' => '')));
  ...
  function getDataLocation()
  {
    ...
    if ($db->connect())
    {
      ...
      while ($rowDb = $db->fetchrow($res)) 
        $result[] = array("value"=>$rowDb['id'], "text"=>$rowDb['location_name'], "selected" => false);
    }
    return $result;
  }
  ...
  function saveData() 
  {
    ...
    $data = array("id_location" => $f->getValue('id_location'),
    ...
    {
      $bolSuccess = $dataGaLocation->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    //---------- END Jika Data Baru---------------------------------------------//
