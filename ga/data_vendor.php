<?php
  ...
  */
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("vendor name"), "dataVendorName", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);
    $f->addInput(getWords("address"), "dataVendorAddress", "", array("size" => 30, "maxlength" => 31), "string", true, true, true);  
    $f->addInput(getWords("phone"), "dataVendorPhone", "", array("size" => 30, "maxlength" => 127), "string", true, true, true);
	$f->addInput(getWords("contact person"), "dataVendorContactPerson", "", array("size" => 30, "maxlength" => 31), "string", true, true,  true);  
    $f->addInput(getWords("email"), "dataVendorEmail", "", array("size" => 30, "maxlength" => 127), "string", true, true, true);    
    $f->addInput(getWords("remark"), "dataRemark", "", array("size" => 30, "maxlength" => 127), "string", true, true, true);  
  ...
  $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id", array('width' => '50'), array('align'=>'center', 'nowrap' => '')));
  $myDataGrid->addColumnNumbering(new DataGrid_Column("No", "", array('width'=>'50'), array('nowrap'=>'')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("vendor name"), "vendor_name", array('width' => '150'),array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("address"), "vendor_address", array('width' => '250'),array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("phone"), "vendor_phone", array('width' => '150'),array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("contact person"), "vendor_contactPerson", array('width' => '150'),array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("email"), "vendor_email", array('width' => '150'),array('nowrap' => '')));
...
//--------------------------------------------------------------------------------
  ...
  function printEditLink($params)
  {
    ...
    return "
      ...
      <input type=hidden name='detailVendorContactPerson$counter' id='detailVendorContactPerson$counter' value='".$record[		
  ...
  function saveData() 
  {
    ...
    $data = array("vendor_name" => $f->getValue('dataVendorName'),
    ...
    {
      $bolSuccess = $dataGaVendor->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    if ($bolSuccess)
    {
      if ($isNew)
