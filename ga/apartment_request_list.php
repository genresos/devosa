<?php
   ...
   function getData($db)
  {
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Req.No"), "apartment_req_no", array('width' => '100'),array('nowrap' => ''))); 
    $myDataGrid->addColumn(new DataGrid_Column(getWords("room"), "room_name", array('width' => '100'),array('nowrap' => '')));   
    ...
    generateRoleButtons($bolCanEdit, $bolCanDelete, $bolCanCheck, $bolCanApprove, $bolCanApprove2, false, true, $myDataGrid);
      ...
      if (substr($strIndex,0,15) == 'DataGrid1_chkID') 
      {
        ...
        {  
          ...
          if ($rowDb['status'] < $intStatus && $rowDb['status'] != REQUEST_STATUS_DENIED )
          {
            ...
            writeLog(ACTIVITY_EDIT, MODULE_PAYROLL, $rowDb['employee_name']." - ". $rowDb['request_by'] ." - ". $rowDb['request_date'], $intStatus);
          }
        }
      }
  ...
  if ($db->connect()) 
  {
    ...
    $f->addInputAutoComplete(getWords("request by"), "dataEmployeeID", getDataEmployee(getInitialValue("Employee", null, $strDataEmployee)), 
	    "style=width:$strDefaultWidthPx ".$strReadonly, "string", false);
    $f->addLabelAutoComplete("", "dataEmployeeID", "");
    $f->addSelect(getWords("Locatoin Room"), "dataIdRoom",getDataListRoom($arrData['dataIdRoom'], true, array("value" => "",
