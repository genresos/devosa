<?php
  ...
  */
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("budget code"), "dataBudgetCode", "", array("size" => 30), "string", true, true, true);
    $f->addSelect(getWords("item category"), "dataIdCategory", getDataItemCategory());
    $f->addInput(getWords("budget year"), "dataBudgetYear", "", array("size" => 30), "numeric", true, true, true); 
    $f->addInput(getWords("budget amount (Harga Barang)"), "dataBudgetAmount", "", array("size" => 30), "numeric", true, true, true);
    $f->addInput(getWords("budget count (Jumlah Barang)"), "dataBudgetCount", "", array("size" => 30), "numeric", true, true, true); 
  ...
  //-------------------------------------- BEGIN Data Grid---------------------------------------------------------------------------------//
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Budget code"), "budget_code", array('width' => '100'), array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Item category"), "category_name", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Budget Year"), "budget_year", array('width' => '40'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Budget Amount"), "budget_amount", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Budget Count"), "budget_count", array('width' => '40'),array('nowrap' => '')));
  ...
  function getDataItemCategory()
  {
    ...
    if ($db->connect())
    {
      ...
      while ($rowDb = $db->fetchrow($res)) 
        $result[] = array("value"=>$rowDb['id'], "text"=>$rowDb['category_name'], "selected" => true);
    }
    return $result;
  }
 ...
 /* function printFormat($params)
  ...
  function saveData() 
  {
    ...
    $data = array("budget_code" => $f->getValue('dataBudgetCode'),
    ...
    {
      $bolSuccess = $dataGaAssetBudget->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    //---------- END Jika Data Baru---------------------------------------------//
