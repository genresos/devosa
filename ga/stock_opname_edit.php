    ...
    if ($bolCanEdit)
    {
      ...
      //$f->addSelect(getWords("item"), "dataIdItem", getDataListItem($arrData['dataIdItem']),array("onChange" => "onItemChanged();"));
       $f->addSelect(getWords("item"), "dataIdItem", getDataListItemCriteria($db,$arrData['dataIdItem'], true, array("value" => "",
	     "text" => "", "selected" => true,),"Consumable"),  array ("style" => "width:200","size" =>10,"onChange" => "onItemChanged();"), "", true, true, true);
	  $f->addInput(getWords("stock opname date"), "dataStockOpnameDate", $arrData['dataStockOpnameDate'],array("style" => "width:$strDateWidth"), "date", true, true, true);
      ...
      $f->addInput(getWords("item old amount"), "dataItemOldAmount", $arrData['dataItemOldAmount'], array("readonly" =>true), "string", false, true, true);
