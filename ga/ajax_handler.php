<?php
  ...
  function getOutOfficeInfo($db, $strDateFrom, $strDateThru, $strIDEmployee = "", $bolAbsOnly = false) {
    ...
    {
      // cari awal dan akhirnya
      $strStart = (pgDateFormat($strDateFrom, "Ymd") < pgDateFormat($rowDb['training_date'], "Ymd")) ? pgDateFormat($rowDb['training_date'], "Y-m-d") : pgDateFormat($strDateFrom, "Y-m-d");
      $strFinish = (pgDateFormat($strDateThru, "Ymd") < pgDateFormat($rowDb['training_date_thru'], "Ymd")) ? pgDateFormat($strDateThru, "Y-m-d") : pgDateFormat($rowDb['training_date_thru'], "Y-m-d");
  ...
  function getEmployeeOvertimeApplication($db, $strFrom, $strThru, $intTipe = 0, $strSection = "", $strEmployee = "") {
    ...
    // cek validasi
    for ($i = 1; $i <= 4; $i++) {
      if (!is_numeric($arrOTType[$i])) $arrOTType[$i] = 1; // default
    }
