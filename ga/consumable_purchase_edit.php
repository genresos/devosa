<?php
  ...
  // --------------------------------------------------------------------------------------------------------------------------------
    ...
    if ($bolCanEdit)
    {
      ...
      $f->addInput(getWords("Consumable Purchase No"), "dataConsPurchaseNo", $arrData['dataConsPurchaseNo'], "style='width:250px' ", "string", true, true, true);
	  
	  // Jika Baru maka enable nomer cons req no 
     if ($isNew){
	  $f->addSelect(getWords("Consumable Request No"), "dataConsReqNo",getDataListConsumableRequestNo($arrData['dataConsReqNo'], true, array("value" => "",
     "text" => "", "selected" => true),"AND active='YES'"), "", true, true, true);
      }
    else{
	  $f->addSelect(getWords("Consumable Request No"), "dataConsReqNo",getDataListConsumableRequestNo($arrData['dataConsReqNo'], true, array("value" => "",
      "text" => "", "selected" => true),""), "", true, true, false);	
	 }
	 ///------------
	 
	 $f->addSelect(getWords("item"), "dataIdItem", getDataListItemCriteria($db,$arrData['dataIdItem'], true, array("value" => "",
	     "text" => "", "selected" => true),"Consumable"),  "style='width:250px' ", "", true, true, true);
     $f->addInput(getWords("request date"), "dataRequestDate", $arrData['dataRequestDate'],array("style" => "width:$strDateWidth"), "date", true, true, true);
