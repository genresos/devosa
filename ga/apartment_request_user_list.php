<?php
   ...
   function getData($db)
  {
    ...
    if (validStandardDate($arrData['dataApartmentRequestDateFrom']) && validStandardDate($arrData['dataApartmentRequestDateThru'])) {
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Req. No"), "apartment_req_no", array('width' => '90'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Room"), "room_name", array('width' => '90'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Date Request"), "request_date", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Employee"), "employee_name", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("User Name"), "user_name", array('width' => '100'),array('nowrap' => '')));
  ...
  if ($db->connect()) 
  {
    ...
    $f->addSelect(getWords("Apartment Request"), "dataIdApartmentRequest", getDataLisApartmentRequest($arrData['dataIdApartmentRequest'],true, 
	  array("value" => "","text" => "", "selected" => true)),  array ("size"=>10), "", false, true, true);
    ...
    $f->addInput(getWords("User Name"), "dataUserName", "", array("size" => 45), "string", false, true, true);
    $f->addInput(getWords("request date From"), "dataApartmentRequestDateFrom","", array("style" => "width:$strDateWidth"), "date", false, true, true);
	$f->addInput(getWords("request date Thru"), "dataApartmentRequestDateThru","", array("style" => "width:$strDateWidth"), "date", false, true, true);  
