 ...
 // BEGIN hak akses =========================================================================================================
   ...
   // --------------------------------------------------------------------------------------------------------------------------------
    ...
    if ($bolCanEdit)
    {
      ...
      if ($isNew){
	     ...
	     "text" => "", "selected" => true),"AND active='YES'"),  array ("size"=>10), "", true, true, true);
	      $f->addSelect(getWords("Item"), "dataIdItem",getDataListItemCriteria($db,$arrData['dataIdItem'], true, array("value" => "",
	     "text" => "", "selected" => true),"Vehicle"),  array ("style" => "width:200","size" =>10), "", true, true, true);
	  }else{
	  	 $f->addSelect(getWords("Vehicle request"), "dataIdVehicleRequest",getDataLisVehicleRequest($arrData['dataIdVehicleRequest'], true, array("value" => "",
	     "text" => "", "selected" => true)),  array ("size"=>""), "", true, false, true);
	     $f->addSelect(getWords("Item"), "dataIdItem",getDataListItem($arrData['dataIdItem'], true, array("value" => "",
	     "text" => "", "selected" => true)),  array ("style" => "width:200","size" =>10), "", true, true, true);
	  }
      
      $f->addSelect(getWords("Driver"), "dataIdDriver",getDataListDriver($arrData['dataIdDriver'], true, array("value" => "",
  ...
  function saveData() 
  {
    ...
    //------------ CLASS GA  asset maintenance -------------//
     ...
     $data = array("id_vehicle_request" => $f->getValue('dataIdVehicleRequest'),
