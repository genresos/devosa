<?php
  ...
  function getData($db)
  {
   ...
   //-------------------------------------- BEGIN Data Grid---------------------------------------------------------------------------------//
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Request Order number"), "request_order_no",array('width' => '100'), array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("purchase request number"), "purchase_no",array('width' => '100'), array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("requester"), "employee_name", array('width' => '100'), array('nowrap' => '')));
...
//    print_r($_REQUEST);
    ...
    function getDataPurchaseRequest()
  {
    ...
    if ($db->connect())
    {
      ...
      while ($rowDb = $db->fetchrow($res)) 
        $result[] = array("value"=>$rowDb['id'], "text"=>$rowDb['purchase_no'], "selected" => false);
    }
    return $result;
  }
  ...
  if ($db->connect()) 
  {
	...
	$f->addSelect(getWords("purchase request"), "dataPurchaseRequest", getDataPurchaseRequest($arrData['dataPurchaseRequest']),"style='width:250px' ", "string", false, true, true);
    $f->addSelect(getWords("item category"), "dataIdCategory",  getDataListCategoryItem($arrData['dataIdCategory']) ,"style='width:250px' ", "string", false, true, true); 
    $f->addInput(getWords("request date"), "dataRequestDate",$arrData['dataRequestDate'],array("style" => "width:$strDateWidth"), "date", false, true, true);
