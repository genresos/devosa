<?php
   ...
   function getData($db)
  {
   ...
   $myDataGrid->addColumnNumbering(new DataGrid_Column("No", "", array('width'=>'30','rowspan'=>'2'), array('nowrap'=>'')));
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("date"), "disposal_date", array('width' => '100'),array('nowrap' => '')));
  ...
  if ($db->connect()) 
  {
    ...
    $f->addSelect(getWords("Item"), "dataIdItem",getDataListItem($arrData['dataIdItem'], true, array("value" => "",
      ...
      $f->addInput(getWords("disposal date From"), "dataDisposalDateFrom","", array("style" => "width:$strDateWidth"), "date", false, true, true);
	  $f->addInput(getWords("disposal date Thru"), "dataDisposalDateThru","", array("style" => "width:$strDateWidth"), "date", false, true, true);  
