  ...
  if ($db->connect())
  {
   ...
   // --------------------------------------------------------------------------------------------------------------------------------
    ...
    if ($bolCanEdit)
    {
      ...
      //Jika data baru
	  if ($isNew){
	  $f->addSelect(getWords("Apartment Request"), "dataIdApartmentRequest", getDataLisApartmentRequest($arrData['dataIdApartmentRequest'], 
	     true, array("value" => "","text" => "", "selected" => true),"AND active='YES'"),  array ("size"=>10), "", true, true, true);
	  }else{
	   $f->addSelect(getWords("Apartment Request"), "dataIdApartmentRequest", getDataLisApartmentRequest($arrData['dataIdApartmentRequest'], 
	     true, array("value" => "","text" => "", "selected" => true),""),  array ("size"=>""), "", true, false, true);  	
	  }
	  //---
      ...
      $f->addInput(getWords("User Name"), "dataUserName", $arrData['dataUserName'], array("size" => 45), "string", true, true, true);
...
//--------------------------------------------------------------------------------
  ...
  function getDataApartmentRequest()
  {
    ...
    if ($db->connect())
    {
      ...
      while ($rowDb = $db->fetchrow($res)) 
        $result[] = array("value"=>$rowDb['id_ap'], "text"=>$rowDb['request_date']." / ".$rowDb['room_name'], "selected" => false);
    }
    return $result;
  }
   //****************************** END Get Data Room ***********************************************//
  ...
  function saveData() 
  {
    ...
    //------------ CLASS GA  asset maintenance -------------//
     ...
     $data = array("id_ga_apartment_request" => $f->getValue('dataIdApartmentRequest'),
