<?php
  ...
  */
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("Category name"), "dataCategoryName", "", array("size" => 40), "string", true, true, true);
    $f->addSelect(getWords("Category type"), "dataCategoryType",getDataListCategoryType(),  array ("style" => "width:200"), "", true, true, true);
    //$f->addInput(getWords("Category Type"), "category_type", "", array("size" => 30), "string", true, true, true);
    $f->addInput(getWords("Maintenance Action"), "dataMaintenanceAction", "", array("size" => 40), "string", true, true, true); 
    $f->addTextArea(getWords("Remarks"), "dataRemark", "", array("cols" => 60, "rows" => 4, "maxlength" => 255), "string", false, true, true); 
  ...
  //-------------------------------------- BEGIN Data Grid---------------------------------------------------------------------------------//
    ...
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Name"), "category_name", array('width' => '100'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Type"), "category_type", array('width' => '40'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Maintenance Action"), "maintenance_action", array('width' => '160'),array('nowrap' => '')));
    $myDataGrid->addColumn(new DataGrid_Column(getWords("Remark"), "remark", array('width' => '160'),array('nowrap' => '')));
  ...
  //----------------BEGIN JIKA Punya Hak Edit---------------------------------//
     if ($bolCanEdit)
	 $myDataGrid->addSpecialButton("btnPurchaseRequest","btnPurchaseRequest","submit","Purchase Request",
    "onClick=\"javascript:return confirm('Do you want to purchase request this item?');\"","purchaseRequest()");
     //---------------- END Jika Punya Hak Akses Tambah-------------------------//
  
  //----------------BEGIN JIKA Punya Hak Edit---------------------------------//
     if ($bolCanEdit)
	 $myDataGrid->addSpecialButton("btnRequestOrder","btnRequestOrder","submit","Request Order",
    "onClick=\"javascript:return confirm('Do you want to request order this item?');\"","requestOrder()");
     //---------------- END Jika Punya Hak Akses Tambah-------------------------//
  ...
  function saveData() 
  {
    ...
    $data = array("category_name" => $f->getValue('dataCategoryName'),
    ...
    {
      $bolSuccess = $dataGaItemCategory->update(/*pk*/"id='".$f->getValue('dataID')."'", /*data to update*/ $data);
    }
    //---------- END Jika Data Baru---------------------------------------------//
