...
include_once("../includes/krumo/class.krumo.php");
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("group code"), "dataCode", "",  array("size"=>30), "string", true, true, true);
    $f->addInput(getWords("group name"), "dataName", "", array("cols" => 48, "rows" => 2), "string", false, true, true);
    $f->addSelect(getWords("group level"), "dataGroupRole", $GLOBALS['ARRAY_GROUP_ROLE'], "");
    $f->addCheckBox(getWords("active"), "dataActive", false, array(), false);
  ...
  if ($bolCanDelete)
    $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id_adm_group", array('width' => '30'), array('align'=>'center', 'nowrap' => '')));
  $myDataGrid->addColumnNumbering(new DataGrid_Column(getWords("no."), "", array('width'=>'30'), array('nowrap'=>'')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("group code"), "code", array('width' => '130'),array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("group name"), "name", array('width' => ''), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("group level"), "group_role", array('width' => '120'), array('nowrap' => ''), true, false));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("status"), "active", array('width' => '100'), array('align' => 'center', 'nowrap' => ''), true, false));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("total users"), "totalmember", array('width' => '110'), array('align'=>'center','nowrap' => ''), false, false));
  $myDataGrid->addColumn(new DataGrid_Column("", "", null, array('align' => 'center', 'nowrap' => ''), false, false, "","printViewMember()"));
  if ($bolCanEdit)
  {
    $myDataGrid->addColumn(new DataGrid_Column("", "", null, array('align' => 'center', 'nowrap' => ''), false, false, "","printEditLink()"));
    $myDataGrid->addColumn(new DataGrid_Column("", "", null, array('align' => 'center', 'nowrap' => ''), false, false, "","printPermission()"));
...
//      $rowDb['get_email'] = "<input type=\"hidden\" name=\"detailGetEmail$counter\" id=\"detailGetEmail$counter\" value=\"f\" disabled>".getWords("not eligible");
  ...
  {
    extract($params);
    return "<a href=\"javascript:myClient.showDetail(".$record['id_adm_group'].", $counter, true);\" id=\"hrefViewMember" . $counter . "\">" . "<span id=\"spanViewMember" . $counter . "\">". getWords('view members') . "</span></a>";
  }
  ...
  {
    extract($params);
    return "<a href=\"master_group_permission.php?dataID=" .$record['id_adm_group']. "\">" .getWords('permission'). "</a>";
  }

  function printEmailSetting($params)
  {
    extract($params);
    return "<a href=\"master_group_email_setting.php?dataID=" .$record['id_adm_group']. "\">" .getWords('email setting'). "</a>";
  }
  ...
  function saveData()
  {
    ...
    {
      $dataGroup->update(array("id_adm_group" => $f->getValue('dataID')), $data);
    }
