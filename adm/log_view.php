<?php
  ...
  $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id_adm_userlog", array('width' => '30'), array('align'=>'center', 'nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("date / time"), "action_date", array("width" => 145), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("login"), "login_name", array("width" => 75), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("employee name"), "employee_name", array("width" => 75), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("user group"), "id_adm_group", array("width" => 75), array('nowrap' => ''), "", "", "", "printUserGroup()"));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("activity"), "action_type", array("width" => 75), array('nowrap' => ''), true, false));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("status"), "status", array("width" => 75), array('nowrap' => ''), "", "", "", "printRequestStatus()"));
  //$myDataGrid->addColumn(new DataGrid_Column(getWords("page"), "php_file", array("width" => 120), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("menu name"), "menu_name", array("width" => 150), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("ip address"), "ip_address", array("width" => 75), array('nowrap' => '')));
  ...
  { 
    ...
    if (isset($arrGroup[$value]['name'])) return $arrGroup[$value]['name'];
    else return "";
  } //printUserGroup  
  ...
  { 
    ...
    if ($record['action_type'] != $ARRAY_ACTIVITY_TYPE[ACTIVITY_EDIT] || !isset($ARRAY_REQUEST_STATUS[$value])) return "";
    else return $ARRAY_REQUEST_STATUS[$value];
  } //peintRequestStatus
