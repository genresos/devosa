<?php
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("table name"), "tablename", null, array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("record #"), "records", array('width' => '120'), array('align' => 'right')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("sequence name"), "sequencename", null, array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("last sequence #"), "lastSeq", array('width' => '120'), array('align' => 'right')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("actual sequence #"), "actualSeq", array('width' => '120'), array('align' => 'right')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("duplicate #"), "duplicateSeq", array('width' => '120'), array('align' => 'right')));

  if ($bolCanDelete)
    $myDataGrid->addSpecialButton("btnEmpty","btnEmpty","submit",getWords("empty record"),"onClick=\"javascript:if (confirm('Are you sure to emptying record of the selected table?')) return true\"","emptyData()");
  
  if ($bolCanEdit)
    $myDataGrid->addButton("btnFix","btnFix","submit",getWords("fix sequence #"), "", "fixSequence()");
  ...
  foreach ($dataset as &$rowDb)
  {
    ...
    if ($rowDb['attname'] != '')
    {
      ...
      while ($rowTmp = $db->fetchrow($resTmp)) {
        if ($strDuplicateID != "") $strDuplicateID .= ", ";
