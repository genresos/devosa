<?php
  ...
  if ($strDataModule == "") $strDataModule = getDataFromCookie("dataModule");
  else setDataToCookie("dataModule", $strDataModule);

  $arrModule = getModuleList($strDataModule);
  
  $db = new CdbClass;

  if($strAction=="desc") goSortOrder($db, $strDataModule, false);
  else if ($strAction=="asc") goSortOrder($db, $strDataModule, true);
  ...
  if ($bolCanEdit)
  {
    ...
    $f->addSelect(getWords("module"), "dataModule", $arrModule, array("onChange" => "javascript:myClient.doRefreshMenu()"), "string", true, true, true);
    ...
    $f->addSelect(getWords("Parent Menu"), "dataParentID", $arrParentMenu, array(), "string", false, true, true);
    
    if (isset($_POST['dataIcon'])) $icon = $_POST['dataIcon']; else $icon = "";
    $f->addFile(getWords("icon file"), "dataIconFile", $icon, array(), "string", false, true, true, "","<div id=\"divIcon\" style=\"display:none\"><img id=\"imgIcon\" border=0 /><br /><a href=\"javascript:myClient.modifyIcon()\">".getWords("edit")."</a></div>", true, null, "../images/icons/");
    
    $f->addTextArea(getWords("note"), "dataNote", "", array("cols"=>50, "rows"=>3), "string", false, true, true);
    ...
    $f->addButton("btnAdd", getWords("add new menu"), array("onClick" => "javascript:myClient.editData(0);"));
  ...
  if ($bolCanDelete)
    $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id_adm_menu", array('width' => '30'), array('align'=>'center', 'nowrap' => '')));
  ...
  $myDataGrid->addColumn(new DataGrid_Column(getWords("menu name"), "name", array('width' => ''), array('nowrap' => 'nowrap'), true, true, "", "printEditLink()"));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("icon file"), "image_icon", array('width' => '32'), array('align' => 'center')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("menu level"), "menu_level", array('width' => '80'), array('nowrap' => 'nowrap','align' => 'center'), true, false));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("order"), "sequence_no", array('width' => '40'), array('nowrap' => 'nowrap','align' => 'center'), true, false));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("published"), "visible", array('width' => '75'), array('align' => 'center'), false, false));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("order"), "order", array('width' => '50'), array('align' => 'center', 'nowrap' => ''), false, false));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("page name"), "page_name", array('width' => '140'), array('nowrap' => 'nowrap')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("php file"), "php_file", array('width' => '150'), array('nowrap' => 'nowrap')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("parent menu"), "parent_menu_name", array('width' => ''), array('nowrap' => 'nowrap')));
  ...
  $counter = 0;
  if (count($dataset) == 0) $dataset = array();
      ...
      while ($rowDb = $db->fetchrow($res)) 
      {
        if ($default == "" || $default == "0")
        {
          if ($isFirst)
          {
            ...
            $arrModule[] = array("value"=>$rowDb['id_adm_module'], "text"=>$rowDb['name'], "selected" => true);
          }
          else
            $arrModule[] = array("value"=>$rowDb['id_adm_module'], "text"=>$rowDb['name'], "selected" => false);
        }
    ...
    foreach($arrMenu as $key => $value)
    {
      if ($value['menu_level'] == $menu_level && $value['parent_id_adm_menu'] == $id_menu)
      {
        ...
        for($i = 0; $i < $value['menu_level']; $i++)
          if ($i == 0)  $dashes = "&#9492;&#9472;&nbsp;"; else $dashes = "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;".$dashes; 

        if ($menu_level == 0)
          $arrResult[] = array("value"=>$value['id_adm_menu'], "text"=>$value['name'], "selected" => false);
        else
          $arrResult[] = array("value"=>$value['id_adm_menu'], "text"=>$dashes.$value['name'], "selected" => false);
  ...
  function getParentMenu($strDataModule, $default = "") 
  {
    $arrResult = array();
    if ($default == "")
      $arrResult[] = array("value"=>"0", "text"=>"[Main Menu - No Parent]", "selected" => true);
    else
      $arrResult[] = array("value"=>"0", "text"=>"[Main Menu - No Parent]", "selected" => false);
