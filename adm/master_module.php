<?php
  ...
  if($strAction=="desc") goSortOrder($intDataID, false);
  else if ($strAction=="asc") goSortOrder($intDataID, true);
  if ($bolCanEdit)
  {
    ...
    $f->addInput(getWords("module name"), "dataName", "", array("size" => 50), "string", true, true, true);
    $f->addInput(getWords("folder name"), "dataFolder", "", array("size" => 50), "string", true, true, true, "","&nbsp;".getWords("Note: Relative to application folder"));
  ...
  if ($bolCanDelete)
    $myDataGrid->addColumnCheckbox(new DataGrid_Column("chkID", "id_adm_module", array('width' => '30'), array('align'=>'center', 'nowrap' => '')));
  //$myDataGrid->addColumnNumbering(new DataGrid_Column("No", "", array('width'=>'30'), array('nowrap'=>'')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("name"), "name", null,array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("folder"), "folder", array('width' => '150'), array('nowrap' => '')));
  $myDataGrid->addColumn(new DataGrid_Column(getWords("status"), "visible", array('width' => '100'), array('align' => 'center'), true, false));
  if ($bolCanEdit)
    $myDataGrid->addColumn(new DataGrid_Column(getWords("order"), "order", array('width' => '40'), array('align' => 'center'), false, false));
...
//--------------------------------------------------------------------------------
  ...
  function saveData() 
  {
    ...
    {      
      $dataModule->update(array("id_adm_module" => $f->getValue('dataID')), $data);
    }
  ...
  function goSortOrder($intDataID, $modeAsc) 
  {
    ...
    if ($prevData != null && $currData != null )
    {
      ...
      if ($dataModule->update(
        array("id_adm_module" => $prevData['id_adm_module']),
        array("sequence_no" => $currData['sequence_no'])
      ))
        if ($dataModule->update(
          array("id_adm_module" => $currData['id_adm_module']),
          array("sequence_no" => $prevData['sequence_no'])
        ))
        {
          $dataModule->commit();
        }
